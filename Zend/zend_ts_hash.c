...
ZEND_API int _zend_ts_hash_add_or_update(TsHashTable *ht, char *arKey, uint nKeyLength, EncodingPtr enc, void *pData, uint nDataSize, void **pDest, int flag ZEND_FILE_LINE_DC)
{
	int retval;

	begin_write(ht);
	retval = _zend_hash_add_or_update(TS_HASH(ht), arKey, nKeyLength, enc, pData, nDataSize, pDest, flag ZEND_FILE_LINE_RELAY_CC);
	end_write(ht);

	return retval;
}

ZEND_API int _zend_ts_hash_quick_add_or_update(TsHashTable *ht, char *arKey, uint nKeyLength, EncodingPtr enc, ulong h, void *pData, uint nDataSize, void **pDest, int flag ZEND_FILE_LINE_DC)
{
	int retval;

	begin_write(ht);
	retval = _zend_hash_quick_add_or_update(TS_HASH(ht), arKey, nKeyLength, enc, h, pData, nDataSize, pDest, flag ZEND_FILE_LINE_RELAY_CC);
	end_write(ht);
...
ZEND_API int zend_ts_hash_add_empty_element(TsHashTable *ht, char *arKey, uint nKeyLength, EncodingPtr enc)
{
	int retval;

	begin_write(ht);
	retval = zend_hash_add_empty_element_enc(TS_HASH(ht), arKey, nKeyLength, enc);
	end_write(ht);
...
{
	begin_write(ht);
	zend_hash_apply(TS_HASH(ht), apply_func, TSRMLS_C);
	end_write(ht);
}

ZEND_API void zend_ts_hash_apply_with_argument(TsHashTable *ht, apply_func_arg_t apply_func, void *argument, TSRMLS_D)
{
	begin_write(ht);
	zend_hash_apply_with_argument(TS_HASH(ht), apply_func, argument, TSRMLS_C);
	end_write(ht);
}

ZEND_API void zend_ts_hash_apply_with_arguments(TsHashTable *ht, TSRMLS_D, apply_func_args_t apply_func, int num_args, ...)
{
	va_list args;

	va_start(args, num_args);
	begin_write(ht);
	zend_hash_apply_with_arguments(TS_HASH(ht), TSRMLS_C, apply_func, num_args, args);
	end_write(ht);
	va_end(args);
}
...
{
	begin_write(ht);
	zend_hash_reverse_apply(TS_HASH(ht), apply_func, TSRMLS_C);
	end_write(ht);
}
...
ZEND_API int zend_ts_hash_sort(TsHashTable *ht, sort_func_t sort_func, compare_func_t compare_func, int renumber, TSRMLS_D)
{
	...
	begin_write(ht);
	retval = zend_hash_sort(TS_HASH(ht), sort_func, compare_func, renumber, TSRMLS_C);
	end_write(ht);
...
ZEND_API int zend_ts_hash_compare(TsHashTable *ht1, TsHashTable *ht2, compare_func_t compar, zend_bool ordered, TSRMLS_D)
{
	...
	begin_read(ht1);
	begin_read(ht2);
	retval = zend_hash_compare(TS_HASH(ht1), TS_HASH(ht2), compar, ordered, TSRMLS_C);
	end_read(ht2);
	end_read(ht1);
...
ZEND_API int zend_ts_hash_minmax(TsHashTable *ht, compare_func_t compar, int flag, void **pData, TSRMLS_D)
{
	...
	begin_read(ht);
	retval = zend_hash_minmax(TS_HASH(ht), compar, flag, pData, TSRMLS_C);
	end_read(ht);
