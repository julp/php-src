...
ZEND_API void zend_qsort_r(void *base, size_t nmemb, size_t siz, compare_r_func_t compare, void *arg, TSRMLS_D)
{
	...
	for (loop = 0; loop >= 0; --loop) {
		...
		while (begin < end) {
			...
			while (1) {
				for (; seg1 < seg2 && compare(begin, seg1, TSRMLS_C, arg) > 0;
				     seg1 += siz);

				for (; seg2 >= seg1 && compare(seg2, begin, TSRMLS_C, arg) > 0;
				     seg2 -= siz);
...
ZEND_API void zend_qsort(void *base, size_t nmemb, size_t siz, compare_func_t compare, TSRMLS_D)
{
	zend_qsort_r(base, nmemb, siz, (compare_r_func_t)compare, NULL, TSRMLS_C);
}
