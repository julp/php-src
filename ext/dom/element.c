...
PHP_METHOD(domelement, __construct)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Os|s!s", &id, dom_element_class_entry, &name, &name_len, &value, &value_len, &uri, &uri_len) == FAILURE) {
	...
	if (uri_len > 0) {
		...
		if (errorcode != 0) {
			...
			php_dom_throw_error(errorcode, 1, TSRMLS_C);
			RETURN_FALSE;
		}
	} else {
	    ...
	    if (prefix != NULL) {
			xmlFree(localname);
			xmlFree(prefix);
	        php_dom_throw_error(NAMESPACE_ERR, 1, TSRMLS_C);
	        RETURN_FALSE;
	    }
...
PHP_FUNCTION(dom_element_get_attribute)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Os", &id, dom_element_class_entry, &name, &name_len) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(dom_element_set_attribute)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Oss", &id, dom_element_class_entry, &name, &name_len, &value, &value_len) == FAILURE) {
		return;
	}
	...
	if (dom_node_is_read_only(nodep) == SUCCESS) {
		php_dom_throw_error(NO_MODIFICATION_ALLOWED_ERR, dom_get_strict_error(intern->document), TSRMLS_C);
		RETURN_FALSE;
	}
	...
	if (attr != NULL) {
		switch (attr->type) {
			case XML_ATTRIBUTE_NODE:
				node_list_unlink(attr->children, TSRMLS_C);
				break;
			case XML_NAMESPACE_DECL:
				RETURN_FALSE;
			default:
	...
	if (!attr) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "No such attribute '%s'", name);
		RETURN_FALSE;
	}
...
PHP_FUNCTION(dom_element_remove_attribute)
{
	...
	switch (attrp->type) {
		case XML_ATTRIBUTE_NODE:
			...
			node_list_unlink(attrp->children, TSRMLS_C);
				xmlUnlinkNode(attrp);
...
PHP_FUNCTION(dom_element_set_attribute_node)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "OO", &id, dom_element_class_entry, &node, dom_attr_class_entry) == FAILURE) {
		return;
	}
	...
	if (attrp->type != XML_ATTRIBUTE_NODE) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Attribute node is required");
		RETURN_FALSE;
	}
	...
	if (attrp->doc == NULL && nodep->doc != NULL) {
		...
		php_libxml_increment_doc_ref((php_libxml_node_object *)attrobj, NULL, TSRMLS_C);
	}
...
PHP_FUNCTION(dom_element_get_attribute_ns)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Os!s", &id, dom_element_class_entry, &uri, &uri_len, &name, &name_len) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(dom_element_set_attribute_ns)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Os!ss", &id, dom_element_class_entry, &uri, &uri_len, &name, &name_len, &value, &value_len) == FAILURE) {
		return;
	}
	...
	if (errorcode != 0) {
		php_dom_throw_error(errorcode, stricterror, TSRMLS_C);
	}
...
PHP_FUNCTION(dom_element_get_elements_by_tag_name_ns)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Oss", &id, dom_element_class_entry, &uri, &uri_len, &name, &name_len) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(dom_element_set_id_attribute)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Osb", &id, dom_element_class_entry, &name, &name_len, &is_id) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(dom_element_set_id_attribute_ns)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Ossb", &id, dom_element_class_entry, &uri, &uri_len, &name, &name_len, &is_id) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(dom_element_set_id_attribute_node)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "OOb", &id, dom_element_class_entry, &node, dom_attr_class_entry, &is_id) == FAILURE) {
		return;
	}
