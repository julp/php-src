...
{
	if (strict_error == 1) {
		zend_throw_exception(dom_domexception_class_entry, error_message, error_code, TSRMLS_C);
	} else {
		php_libxml_issue_error(E_WARNING, error_message, TSRMLS_C);
	}
}
/* }}} */
...
void php_dom_throw_error(int error_code, int strict_error, TSRMLS_D)
{
	...
	php_dom_throw_error_with_message(error_code, error_message, strict_error, TSRMLS_C);
}
