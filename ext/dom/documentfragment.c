...
PHP_METHOD(domdocumentfragment, __construct)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "O", &id, dom_documentfragment_class_entry) == FAILURE) {
	...
	if (intern != NULL) {
		...
		/* php_dom_set_object(intern, nodep, TSRMLS_C); */
...
PHP_METHOD(domdocumentfragment, appendXML) {
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Os", &id, dom_documentfragment_class_entry, &data, &data_len) == FAILURE) {
		return;
	}
