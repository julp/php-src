...
int dom_node_node_name_read(dom_object *obj, zval **retval, TSRMLS_D)
{
	...
	switch (nodep->type) {
		...
		case XML_TEXT_NODE:
			...
			break;
		default:
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid Node Type");
	}
...
int dom_node_node_value_write(dom_object *obj, zval *newval, TSRMLS_D)
{
	...
	switch (nodep->type) {
		...
		case XML_ATTRIBUTE_NODE:
			if (nodep->children) {
				node_list_unlink(nodep->children, TSRMLS_C);
			}
		case XML_TEXT_NODE:
		case XML_COMMENT_NODE:
		case XML_CDATA_SECTION_NODE:
...
int dom_node_parent_node_read(dom_object *obj, zval **retval, TSRMLS_D)
{
	...
	if (NULL == (*retval = php_dom_create_object(nodeparent, &ret, *retval, obj, TSRMLS_C))) {
...
int dom_node_child_nodes_read(dom_object *obj, zval **retval, TSRMLS_D)
{
	...
	if (dom_node_children_valid(nodep) == FAILURE) {
		...
		php_dom_create_interator(*retval, DOM_NODELIST, TSRMLS_C);
		intern = (dom_object *)zend_objects_get_address(*retval, TSRMLS_C);
		dom_namednode_iter(obj, XML_ELEMENT_NODE, intern, NULL, NULL, NULL, TSRMLS_C);
	}
...
int dom_node_first_child_read(dom_object *obj, zval **retval, TSRMLS_D)
{
	...
	if (NULL == (*retval = php_dom_create_object(first, &ret, *retval, obj, TSRMLS_C))) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Cannot create required DOM object");
		return FAILURE;
	}
	return SUCCESS;
...
int dom_node_last_child_read(dom_object *obj, zval **retval, TSRMLS_D)
{
	...
	if (NULL == (*retval = php_dom_create_object(last, &ret, *retval, obj, TSRMLS_C))) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Cannot create required DOM object");
		return FAILURE;
	}
	return SUCCESS;
...
int dom_node_previous_sibling_read(dom_object *obj, zval **retval, TSRMLS_D)
{
	...
	if (NULL == (*retval = php_dom_create_object(prevsib, &ret, *retval, obj, TSRMLS_C))) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Cannot create required DOM object");
		return FAILURE;
	}
	return SUCCESS;
...
int dom_node_next_sibling_read(dom_object *obj, zval **retval, TSRMLS_D)
{
	...
	if (NULL == (*retval = php_dom_create_object(nextsib, &ret, *retval, obj, TSRMLS_C))) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Cannot create required DOM object");
		return FAILURE;
	}
	return SUCCESS;
...
int dom_node_attributes_read(dom_object *obj, zval **retval, TSRMLS_D)
{
	...
	if (nodep->type == XML_ELEMENT_NODE) {
		php_dom_create_interator(*retval, DOM_NAMEDNODEMAP, TSRMLS_C);
		intern = (dom_object *)zend_objects_get_address(*retval, TSRMLS_C);
		dom_namednode_iter(obj, XML_ATTRIBUTE_NODE, intern, NULL, NULL, NULL, TSRMLS_C);
	} else {
		ZVAL_NULL(*retval);
	}
...
int dom_node_owner_document_read(dom_object *obj, zval **retval, TSRMLS_D)
{
	...
	if (NULL == (*retval = php_dom_create_object((xmlNodePtr) docp, &ret, *retval, obj, TSRMLS_C))) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Cannot create required DOM object");
		return FAILURE;
	}
	return SUCCESS;
		...
		case XML_ATTRIBUTE_NODE:
			...
			if (nsnode && nodep->ns != NULL && !xmlStrEqual(nodep->ns->prefix, (xmlChar *)prefix)) {
				...
				if (ns == NULL) {
					...
					php_dom_throw_error(NAMESPACE_ERR, dom_get_strict_error(obj->document), TSRMLS_C);
					return FAILURE;
				}
...
static xmlNodePtr _php_dom_insert_fragment(xmlNodePtr nodep, xmlNodePtr prevsib, xmlNodePtr nextsib, xmlNodePtr fragment, dom_object *intern, dom_object *childobj, TSRMLS_D) /* {{{ */
{
		...
		while (node != NULL) {
			...
			if (node->doc != nodep->doc) {
				...
				if (node->_private != NULL) {
					...
					php_libxml_increment_doc_ref((php_libxml_node_object *)childobj, NULL, TSRMLS_C);
				}
			}
...
PHP_FUNCTION(dom_node_insert_before)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "OO|O!", &id, dom_node_class_entry, &node, dom_node_class_entry, &ref, dom_node_class_entry) == FAILURE) {
		return;
	}
	...
	if (dom_node_is_read_only(parentp) == SUCCESS ||
		...
		php_dom_throw_error(NO_MODIFICATION_ALLOWED_ERR, stricterror, TSRMLS_C);
		RETURN_FALSE;
	}

	if (dom_hierarchy(parentp, child) == FAILURE) {
		php_dom_throw_error(HIERARCHY_REQUEST_ERR, stricterror, TSRMLS_C);
		RETURN_FALSE;
	}

	if (child->doc != parentp->doc && child->doc != NULL) {
		php_dom_throw_error(WRONG_DOCUMENT_ERR, stricterror, TSRMLS_C);
		RETURN_FALSE;
	}

	if (child->type == XML_DOCUMENT_FRAG_NODE && child->children == NULL) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Document Fragment is empty");
		RETURN_FALSE;
	}
	...
	if (ref != NULL) {
		...
		if (refp->parent != parentp) {
			php_dom_throw_error(NOT_FOUND_ERR, stricterror, TSRMLS_C);
			RETURN_FALSE;
		}
		...
		if (child->type == XML_TEXT_NODE && (refp->type == XML_TEXT_NODE || 
			...
			if (lastattr != NULL && lastattr->type != XML_ATTRIBUTE_DECL) {
				if (lastattr != (xmlAttrPtr) child) {
					...
					php_libxml_node_free_resource((xmlNodePtr) lastattr, TSRMLS_C);
				} else {
			...
			new_child = _php_dom_insert_fragment(parentp, refp->prev, refp, child, intern, childobj, TSRMLS_C);
		}
		...
		if (child->type == XML_TEXT_NODE && parentp->last != NULL && parentp->last->type == XML_TEXT_NODE) {
			...
			new_child = _php_dom_insert_fragment(parentp, parentp->last, NULL, child, intern, childobj, TSRMLS_C);
		}
		if (new_child == NULL) {
	...
	if (NULL == new_child) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Couldn't add newnode as the previous sibling of refnode");
		RETURN_FALSE;
	}
...
PHP_FUNCTION(dom_node_replace_child)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "OOO", &id, dom_node_class_entry, &newnode, dom_node_class_entry, &oldnode, dom_node_class_entry) == FAILURE) {
		return;
	}
	...
	if (foundoldchild) {
		if (newchild->type == XML_DOCUMENT_FRAG_NODE) {
			...
			newchild = _php_dom_insert_fragment(nodep, prevsib, nextsib, newchild, intern, newchildobj, TSRMLS_C);
			if (newchild) {
			...
			if (newchild->doc == NULL && nodep->doc != NULL) {
				...
				php_libxml_increment_doc_ref((php_libxml_node_object *)newchildobj, NULL, TSRMLS_C);
			}
		...
		php_dom_throw_error(NOT_FOUND_ERR, dom_get_strict_error(intern->document), TSRMLS_C);
		RETURN_FALSE;
	}
}
...
PHP_FUNCTION(dom_node_remove_child)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "OO", &id, dom_node_class_entry, &node, dom_node_class_entry) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(dom_node_append_child)
{
	...
	if (child->type == XML_TEXT_NODE && nodep->last != NULL && nodep->last->type == XML_TEXT_NODE) {
		...
		new_child = _php_dom_insert_fragment(nodep, nodep->last, NULL, child, intern, childobj, TSRMLS_C);
	}

	if (new_child == NULL) {
		new_child = xmlAddChild(nodep, child);
		if (new_child == NULL) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Couldn't append node");
			RETURN_FALSE;
		}
	}
...
PHP_FUNCTION(dom_node_has_child_nodes)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "O", &id, dom_node_class_entry) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(dom_node_clone_node)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "O|l", &id, dom_node_class_entry, &recursive) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(dom_node_is_supported)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Oss", &id, dom_node_class_entry, &feature, &feature_len, &version, &version_len) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(dom_node_lookup_prefix)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Os", &id, dom_node_class_entry, &uri, &uri_len) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(dom_node_lookup_namespace_uri)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Os!", &id, dom_node_class_entry, &prefix, &prefix_len) == FAILURE) {
		return;
	}
...
static void dom_canonicalization(INTERNAL_FUNCTION_PARAMETERS, int mode) /* {{{ */
{
	...
	if (! docp) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Node must be associated with a document");
		RETURN_FALSE;
	}
	...
	if (xpath_array == NULL) {
		if (nodep->type != XML_DOCUMENT_NODE) {
			...
			if (xpathobjp && xpathobjp->type == XPATH_NODESET) {
				...
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "XPath query did not return a nodeset.");
				RETURN_FALSE;
			}
		}
	} else {
		...
		if (zend_hash_find(ht, "query", sizeof("query"), (void**)&tmp) == SUCCESS &&
			...
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "'query' missing from xpath array or is not a string");
			RETURN_FALSE;
		}
	...
	if (ns_prefixes != NULL) {
		if (exclusive) {
			...
			php_error_docref(NULL, TSRMLS_C, E_NOTICE, 
