...
PHP_FUNCTION(dom_node_lookup_prefix)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Ou", &id, dom_node_class_entry, &uri, &uri_len) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(dom_node_lookup_namespace_uri)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Ou!", &id, dom_node_class_entry, &prefix, &prefix_len) == FAILURE) {
		return;
	}
