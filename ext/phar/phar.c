...
char *(*phar_save_resolve_path)(const char *filename, int filename_len, TSRMLS_D);
#endif

/**
 * set's phar->is_writeable based on the current INI value
 */
static int phar_set_writeable_bit(void *pDest, void *argument, TSRMLS_D) /* {{{ */
{
...
ZEND_INI_MH(phar_ini_modify_handler) /* {{{ */
{
	...
	if (entry->name_length == 14) {
		...
		if (PHAR_GLOBALS->request_init && PHAR_GLOBALS->phar_fname_map.arBuckets) {
			zend_hash_apply_with_argument(&(PHAR_GLOBALS->phar_fname_map), phar_set_writeable_bit, (void *)&ini, TSRMLS_C);
		}
	} else {
...
static void phar_split_cache_list(TSRMLS_D) /* {{{ */
{
	...
	for (key = php_strtok_r(tmp, ds, &lasts);
		...
		if (end) {
			if (SUCCESS == phar_open_from_filename(key, end - key, NULL, 0, 0, &phar, NULL, TSRMLS_C)) {
finish_up:
				phar->phar_pos = i++;
...
finish_error:
			...
			if (SUCCESS == phar_open_from_filename(key, strlen(key), NULL, 0, 0, &phar, NULL, TSRMLS_C)) {
				goto finish_up;
			} else {
				goto finish_error;
			}
 ...
 * and the phar slot
 */
void phar_destroy_phar_data(phar_archive_data *phar, TSRMLS_D) /* {{{ */
{
 ...
 */
int phar_archive_delref(phar_archive_data *phar, TSRMLS_D) /* {{{ */
{
	if (phar->is_persistent) {
		return 0;
	}
...
static void destroy_phar_data_only(void *pDest) /* {{{ */
{
	...
	if (EG(exception) || --phar_data->refcount < 0) {
		phar_destroy_phar_data(phar_data, TSRMLS_C);
	}
}
/* }}}*/
 ...
 */
static int phar_unalias_apply(void *pDest, void *argument, TSRMLS_D) /* {{{ */
{
 ...
 * Delete aliases to phar's that got kicked out of the global table
 */
static int phar_tmpclose_apply(void *pDest, TSRMLS_D) /* {{{ */
{
...
static void destroy_phar_data(void *pDest) /* {{{ */
{
	...
	if (PHAR_GLOBALS->request_ends) {
		...
		zend_hash_apply(&(phar_data->manifest), phar_tmpclose_apply, TSRMLS_C);
		destroy_phar_data_only(pDest);
		return;
	}

	zend_hash_apply_with_argument(&(PHAR_GLOBALS->phar_alias_map), phar_unalias_apply, phar_data, TSRMLS_C);
...
int phar_entry_delref(phar_entry_data *idata, TSRMLS_D) /* {{{ */
{
	int ret = 0;
	...
	phar_archive_delref(idata->phar, TSRMLS_C);
	efree(idata);
	return ret;
}
/* }}} */
 ...
 */
void phar_entry_remove(phar_entry_data *idata, char **error, TSRMLS_D) /* {{{ */
{
	phar_archive_data *phar;
	...
	if (idata->internal_file->fp_refcount < 2) {
		...
		phar_entry_delref(idata, TSRMLS_C);
	}

	if (!phar->donotflush) {
		phar_flush(phar, 0, 0, 0, error, TSRMLS_C);
	}
}
/* }}} */
...
#define MAPPHAR_FAIL(msg) \
	efree(savebuf);\
	if (mydata) {\
		phar_destroy_phar_data(mydata, TSRMLS_C);\
	}\
	if (signature) {\
 ...
 */
int phar_open_parsed_phar(char *fname, int fname_len, char *alias, int alias_len, int is_data, int options, phar_archive_data** pphar, char **error, TSRMLS_D) /* {{{ */
{
	phar_archive_data *phar;
#ifdef PHP_WIN32
	char *unixfname;
	...
	if (SUCCESS == phar_get_archive(&phar, unixfname, fname_len, alias, alias_len, error, TSRMLS_C)
		...
		efree(unixfname);
#else
	if (SUCCESS == phar_get_archive(&phar, fname, fname_len, alias, alias_len, error, TSRMLS_C)
		&& ((alias && fname_len == phar->fname_len
 ...
 */
int phar_parse_metadata(char **buffer, zval **metadata, int zip_metadata_len, TSRMLS_D) /* {{{ */
{
	...
	if (buf_len) {
		...
		if (!php_var_unserialize(metadata, &p, p + buf_len, &var_hash, TSRMLS_C)) {
 ...
 * directly.
 */
static int phar_parse_pharfile(php_stream *fp, char *fname, int fname_len, char *alias, int alias_len, long halt_offset, phar_archive_data** pphar, php_uint32 compression, char **error, TSRMLS_D) /* {{{ */
{
	...
	if (manifest_flags & PHAR_HDR_SIGNATURE) {
		...
		switch(sig_flags) {
			case PHAR_SIG_OPENSSL: {
				...
				if (FAILURE == phar_verify_signature(fp, end_of_phar, PHAR_SIG_OPENSSL, sig, signature_len, fname, &signature, &sig_len, error, TSRMLS_C)) {
					efree(savebuf);
					efree(sig);
					php_stream_close(fp);
					if (error) {
			...
			case PHAR_SIG_SHA512: {
				...
				if (FAILURE == phar_verify_signature(fp, read_len, PHAR_SIG_SHA512, (char *)digest, 64, fname, &signature, &sig_len, error, TSRMLS_C)) {
					efree(savebuf);
					php_stream_close(fp);
					if (error) {
						char *save = *error;
			...
			case PHAR_SIG_SHA256: {
				...
				if (FAILURE == phar_verify_signature(fp, read_len, PHAR_SIG_SHA256, (char *)digest, 32, fname, &signature, &sig_len, error, TSRMLS_C)) {
					efree(savebuf);
					php_stream_close(fp);
					if (error) {
						char *save = *error;
			...
			case PHAR_SIG_SHA1: {
				...
				if (FAILURE == phar_verify_signature(fp, read_len, PHAR_SIG_SHA1, (char *)digest, 20, fname, &signature, &sig_len, error, TSRMLS_C)) {
					efree(savebuf);
					php_stream_close(fp);
					if (error) {
						char *save = *error;
			...
			case PHAR_SIG_MD5: {
				...
				if (FAILURE == phar_verify_signature(fp, read_len, PHAR_SIG_MD5, (char *)digest, 16, fname, &signature, &sig_len, error, TSRMLS_C)) {
					efree(savebuf);
					php_stream_close(fp);
					if (error) {
						char *save = *error;
	...
	if (mydata->is_persistent) {
		...
		if (phar_parse_metadata(&buffer, &mydata->metadata, mydata->metadata_len, TSRMLS_C) == FAILURE) {
			MAPPHAR_FAIL("unable to read phar metadata in .phar file \"%s\"");
		}
	} else {
		if (phar_parse_metadata(&buffer, &mydata->metadata, 0, TSRMLS_C) == FAILURE) {
			MAPPHAR_FAIL("unable to read phar metadata in .phar file \"%s\"");
		}
	}
	...
	for (manifest_index = 0; manifest_index < manifest_count; ++manifest_index) {
		...
		phar_add_virtual_dirs(mydata, buffer, entry.filename_len, TSRMLS_C);
		...
		if (entry.is_persistent) {
			...
			if (phar_parse_metadata(&buffer, &entry.metadata, entry.metadata_len, TSRMLS_C) == FAILURE) {
				pefree(entry.filename, entry.is_persistent);
				MAPPHAR_FAIL("unable to read file metadata in .phar file \"%s\"");
			}
		} else {
			if (phar_parse_metadata(&buffer, &entry.metadata, 0, TSRMLS_C) == FAILURE) {
				pefree(entry.filename, entry.is_persistent);
				MAPPHAR_FAIL("unable to read file metadata in .phar file \"%s\"");
			}
		}
 ...
 */
int phar_open_or_create_filename(char *fname, int fname_len, char *alias, int alias_len, int is_data, int options, phar_archive_data** pphar, char **error, TSRMLS_D) /* {{{ */
{
	...
	if (phar_detect_phar_fname_ext(fname, fname_len, &ext_str, &ext_len, !is_data, 0, 1, TSRMLS_C) == SUCCESS) {
		goto check_file;
	}

	/* next try to create a new file */
	if (FAILURE == phar_detect_phar_fname_ext(fname, fname_len, &ext_str, &ext_len, !is_data, 1, 1, TSRMLS_C)) {
		if (error) {
			if (ext_len == -2) {
...
check_file:
	if (phar_open_parsed_phar(fname, fname_len, alias, alias_len, is_data, options, test, &my_error, TSRMLS_C) == SUCCESS) {
		if (pphar) {
			*pphar = *test;
		}
	...
	if (ext_len > 3 && (z = memchr(ext_str, 'z', ext_len)) && ((ext_str + ext_len) - z >= 2) && !memcmp(z + 1, "ip", 2)) {
		...
		return phar_open_or_create_zip(fname, fname_len, alias, alias_len, is_data, options, pphar, error, TSRMLS_C);
	}

	if (ext_len > 3 && (z = memchr(ext_str, 't', ext_len)) && ((ext_str + ext_len) - z >= 2) && !memcmp(z + 1, "ar", 2)) {
		/* assume tar-based phar */
		return phar_open_or_create_tar(fname, fname_len, alias, alias_len, is_data, options, pphar, error, TSRMLS_C);
	}

	return phar_create_or_parse_filename(fname, fname_len, alias, alias_len, is_data, options, pphar, error, TSRMLS_C);
}
/* }}} */

int phar_create_or_parse_filename(char *fname, int fname_len, char *alias, int alias_len, int is_data, int options, phar_archive_data** pphar, char **error, TSRMLS_D) /* {{{ */
{
	...
	if (fp) {
		if (phar_open_from_fp(fp, fname, fname_len, alias, alias_len, options, pphar, is_data, error, TSRMLS_C) == SUCCESS) {
	...
	mydata->fname = expand_filepath(fname, NULL, TSRMLS_C);
 ...
 */
int phar_open_from_filename(char *fname, int fname_len, char *alias, int alias_len, int options, phar_archive_data** pphar, char **error, TSRMLS_D) /* {{{ */
{
	php_stream *fp;
	char *actual;
	int ret, is_data = 0;
	...
	if (phar_open_parsed_phar(fname, fname_len, alias, alias_len, is_data, options, pphar, error, TSRMLS_C) == SUCCESS) {
		return SUCCESS;
	} else if (error && *error) {
		return FAILURE;
	}
	...
	ret =  phar_open_from_fp(fp, fname, fname_len, alias, alias_len, options, pphar, is_data, error, TSRMLS_C);
 ...
 * or FAILURE is returned and pphar is set to a pointer to the phar's manifest
 */
static int phar_open_from_fp(php_stream* fp, char *fname, int fname_len, char *alias, int alias_len, int options, phar_archive_data** pphar, int is_data, char **error, TSRMLS_D) /* {{{ */
{
...
#define MAX_WBITS 15
				...
				filter = php_stream_filter_create("zlib.inflate", &filterparams, php_stream_is_persistent(fp), TSRMLS_C);
			...
			if (!memcmp(pos, zip_magic, 4)) {
				...
				return phar_parse_zipfile(fp, fname, fname_len, alias, alias_len, pphar, error, TSRMLS_C);
			}

			if (got > 512) {
				if (phar_is_tar(pos, fname)) {
					php_stream_rewind(fp);
					return phar_parse_tarfile(fp, fname, fname_len, alias, alias_len, pphar, is_data, compression, error, TSRMLS_C);
				}
			}
		}
		...
		if (got > 0 && (pos = phar_strnstr(buffer, got + sizeof(token), token, sizeof(token)-1)) != NULL) {
			...
			return phar_parse_pharfile(fp, fname, fname_len, alias, alias_len, halt_offset, pphar, compression, error, TSRMLS_C);
		}
 ...
 */
static int phar_analyze_path(const char *fname, const char *ext, int ext_len, int for_create, TSRMLS_D) /* {{{ */
{
	php_stream_statbuf ssb;
	char *realpath;
	char *filename = estrndup(fname, (ext - fname) + ext_len);

	if ((realpath = expand_filepath(filename, NULL, TSRMLS_C))) {
#ifdef PHP_WIN32
...
static int phar_check_str(const char *fname, const char *ext_str, int ext_len, int executable, int for_create, TSRMLS_D) /* {{{ */
{
	char test[51];
	const char *pos;
	...
	if (executable == 1) {
		...
		if (pos && (*(pos - 1) != '/')
				&& (pos += 5) && (*pos == '\0' || *pos == '/' || *pos == '.')) {
			return phar_analyze_path(fname, ext_str, ext_len, for_create, TSRMLS_C);
		} else {
			return FAILURE;
		}
	}
 ...
 */
int phar_detect_phar_fname_ext(const char *filename, int filename_len, const char **ext_str, int *ext_len, int executable, int for_create, int is_complete, TSRMLS_D) /* {{{ */
{
...
next_extension:
	...
	if (!slash) {
		...
		*ext_len = strlen(pos);
		...
		switch (phar_check_str(filename, *ext_str, *ext_len, executable, for_create, TSRMLS_C)) {
			case SUCCESS:
				return SUCCESS;
			case FAILURE:
 ...
 */
char *phar_fix_filepath(char *path, int *new_len, int use_cwd, TSRMLS_D) /* {{{ */
{
 ...
 */
int phar_split_fname(char *filename, int filename_len, char **arch, int *arch_len, char **entry, int *entry_len, int executable, int for_create, TSRMLS_D) /* {{{ */
{
	const char *ext_str;
#ifdef PHP_WIN32
	char *save;
...
#endif
	if (phar_detect_phar_fname_ext(filename, filename_len, &ext_str, &ext_len, executable, for_create, 0, TSRMLS_C) == FAILURE) {
		if (ext_len != -1) {
			if (!ext_str) {
...
#endif
		*entry = phar_fix_filepath(*entry, entry_len, 0, TSRMLS_C);
	} else {
		*entry_len = 1;
		*entry = estrndup("/", 1);
	}
 ...
 */
int phar_open_executed_filename(char *alias, int alias_len, char **error, TSRMLS_D) /* {{{ */
{
	char *fname;
	zval *halt_constant;
	php_stream *fp;
	...
	if (phar_open_parsed_phar(fname, fname_len, alias, alias_len, 0, REPORT_ERRORS, NULL, 0, TSRMLS_C) == SUCCESS) {
		return SUCCESS;
	}
	...
	if (0 == zend_get_constant("__COMPILER_HALT_OFFSET__", 24, halt_constant, TSRMLS_C)) {
	...
	ret = phar_open_from_fp(fp, fname, fname_len, alias, alias_len, REPORT_ERRORS, NULL, 0, error, TSRMLS_C);
 ...
 */
int phar_postprocess_file(phar_entry_data *idata, php_uint32 crc32, char **error, int process_zip, TSRMLS_D) /* {{{ */
{
	php_uint32 crc = ~0;
	...
	if (entry->is_zip && process_zip > 0) {
		...
		if (SUCCESS != phar_open_archive_fp(idata->phar, TSRMLS_C)) {
		...
		php_stream_seek(phar_get_entrypfp(idata->internal_file, TSRMLS_C), entry->header_offset, SEEK_SET);

		if (sizeof(local) != php_stream_read(phar_get_entrypfp(idata->internal_file, TSRMLS_C), (char *) &local, sizeof(local))) {
		...
		if (((PHAR_ZIP_16(local.flags)) & 0x8) == 0x8) {
			php_stream_seek(phar_get_entrypfp(idata->internal_file, TSRMLS_C),
			...
			if (sizeof(desc) != php_stream_read(phar_get_entrypfp(idata->internal_file, TSRMLS_C),
...
static int phar_flush_clean_deleted_apply(void *data, TSRMLS_D) /* {{{ */
{
...
char *phar_create_default_stub(const char *index_php, const char *web_index, size_t *len, char **error, TSRMLS_D) /* {{{ */
{
	char *stub = NULL;
	...
	phar_get_stub(index_php, web_index, len, &stub, index_len+1, web_len+1, TSRMLS_C);
	return stub;
}
/* }}} */
 ...
 */
int phar_flush(phar_archive_data *phar, char *user_stub, long len, int convert, char **error, TSRMLS_D) /* {{{ */
{
	...
	if (phar->is_zip) {
		return phar_zip_flush(phar, user_stub, len, convert, error, TSRMLS_C);
	}

	if (phar->is_tar) {
		return phar_tar_flush(phar, user_stub, len, convert, error, TSRMLS_C);
	}
	...
	if (user_stub) {
		...
		if (!user_stub && phar->halt_offset && oldfile && !phar->is_brandnew) {
			...
			newstub = phar_create_default_stub(NULL, NULL, &(phar->halt_offset), NULL, TSRMLS_C);
	...
	zend_hash_apply(&phar->manifest, phar_flush_clean_deleted_apply, TSRMLS_C);
	...
	for (zend_hash_internal_pointer_reset(&phar->manifest);
		...
		++new_manifest_count;
		phar_add_virtual_dirs(phar, entry->filename, entry->filename_len, TSRMLS_C);
		...
		if (!phar_get_efp(entry, 0, TSRMLS_C)) {
			...
			newentry = phar_open_jit(phar, entry, error, TSRMLS_C);
			if (!newentry) {
		...
		file = phar_get_efp(entry, 0, TSRMLS_C);
		if (-1 == phar_seek_efp(entry, 0, SEEK_SET, 0, 1, TSRMLS_C)) {
			if (closeoldfile) {
				php_stream_close(oldfile);
			}
			php_stream_close(newfile);
	...
	if (global_flags & PHAR_HDR_SIGNATURE) {
		...
		switch(phar->sig_flags) {
			...
			default: {
				...
				if (FAILURE == phar_create_signature(phar, newfile, &digest, &digest_len, error, TSRMLS_C)) {
					if (error) {
						char *save = *error;
	...
	*/
	...
	if (phar->donotflush) {
		...
		if (phar->flags & PHAR_FILE_COMPRESSED_GZ) {
			...
			filter = php_stream_filter_create("zlib.deflate", &filterparams, php_stream_is_persistent(phar->fp), TSRMLS_C);
			zval_dtor(&filterparams);
			...
			filter = php_stream_filter_create("bzip2.compress", NULL, php_stream_is_persistent(phar->fp), TSRMLS_C);
			php_stream_filter_append(&phar->fp->writefilters, filter);
			phar_stream_copy_to_stream(newfile, phar->fp, PHP_STREAM_COPY_ALL, NULL);
			php_stream_filter_flush(filter, 1);
			php_stream_filter_remove(filter, 1, TSRMLS_C);
...
static size_t phar_zend_stream_reader(void *handle, char *buf, size_t len, TSRMLS_D) /* {{{ */
{
	return php_stream_read(phar_get_pharfp((phar_archive_data*)handle, TSRMLS_C), buf, len);
}
/* }}} */

#if PHP_VERSION_ID >= 50300
static size_t phar_zend_stream_fsizer(void *handle, TSRMLS_D) /* {{{ */
{
...
static long phar_stream_fteller_for_zend(void *handle, TSRMLS_D) /* {{{ */
{
	return (long)php_stream_tell(phar_get_pharfp((phar_archive_data*)handle, TSRMLS_C));
}
/* }}} */
#endif

zend_op_array *(*phar_orig_compile_file)(zend_file_handle *file_handle, int type, TSRMLS_D);
#if PHP_VERSION_ID >= 50300
#define phar_orig_zend_open zend_stream_open_function
static char *phar_resolve_path(const char *filename, int filename_len, TSRMLS_D)
{
	return phar_find_in_include_path((char *) filename, filename_len, NULL, TSRMLS_C);
}
#else
int (*phar_orig_zend_open)(const char *filename, zend_file_handle *handle, TSRMLS_D);
#endif

static zend_op_array *phar_compile_file(zend_file_handle *file_handle, int type, TSRMLS_D) /* {{{ */
{
	zend_op_array *res;
	char *name = NULL;
	int failed;
	...
	if (!file_handle || !file_handle->filename) {
		return phar_orig_compile_file(file_handle, type, TSRMLS_C);
	}
	if (strstr(file_handle->filename, ".phar") && !strstr(file_handle->filename, "://")) {
		if (SUCCESS == phar_open_from_filename((char*)file_handle->filename, strlen(file_handle->filename), NULL, 0, 0, &phar, NULL, TSRMLS_C)) {
			if (phar->is_zip || phar->is_tar) {
				zend_file_handle f = *file_handle;

				/* zip or tar-based phar */
				spprintf(&name, 4096, "phar://%s/%s", file_handle->filename, ".phar/stub.php");
				if (SUCCESS == phar_orig_zend_open((const char *)name, file_handle, TSRMLS_C)) {
					efree(name);
					name = NULL;
	...
	zend_try {
		failed = 0;
		res = phar_orig_compile_file(file_handle, type, TSRMLS_C);
	} zend_catch {
		failed = 1;
		res = NULL;
	} zend_end_try();
...
#if PHP_VERSION_ID < 50300
int phar_zend_open(const char *filename, zend_file_handle *handle, TSRMLS_D) /* {{{ */
{
	char *arch, *entry;
	...
	if (!IS_ABSOLUTE_PATH(entry, strlen(entry)) && !strstr(entry, "://")) {
		...
		if (fname_len > 7 && !strncasecmp(fname, "phar://", 7)) {
			if (SUCCESS == phar_split_fname(fname, fname_len, &arch, &arch_len, &entry, &entry_len, 1, 0, TSRMLS_C)) {
		...
		if (!(entry = phar_find_in_include_path((char *) filename, strlen(filename), NULL, TSRMLS_C))) {
			/* this file is not in the phar, use the original path */
			goto skip_phar;
		}

		if (SUCCESS == phar_orig_zend_open(entry, handle, TSRMLS_C)) {
...
skip_phar:
	return phar_orig_zend_open(filename, handle, TSRMLS_C);
}
/* }}} */
#endif
typedef zend_op_array* (zend_compile_t)(zend_file_handle*, int, TSRMLS_D);
...
PHP_MINIT_FUNCTION(phar) /* {{{ */
{
	...
	return php_register_url_stream_wrapper("phar", &php_stream_phar_wrapper, TSRMLS_C);
}
/* }}} */

PHP_MSHUTDOWN_FUNCTION(phar) /* {{{ */
{
	php_unregister_url_stream_wrapper("phar", TSRMLS_C);
