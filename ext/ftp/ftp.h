 ...
 */
ftpbuf_t*	ftp_open(const char *host, short port, long timeout_sec, TSRMLS_D);
...
int		ftp_login(ftpbuf_t *ftp, const char *user, const char *pass, TSRMLS_D);
 ...
 */
char**		ftp_nlist(ftpbuf_t *ftp, const char *path, TSRMLS_D);
 ...
 * free the array elements)
 */
char**		ftp_list(ftpbuf_t *ftp, const char *path, int recursive, TSRMLS_D);
 ...
 * returns true on success, false on error
 */
int		ftp_get(ftpbuf_t *ftp, php_stream *outstream, const char *path, ftptype_t type, int resumepos, TSRMLS_D);

/* stores the data from a file, socket, or process as a file on the remote server
 * returns true on success, false on error
 */
int		ftp_put(ftpbuf_t *ftp, const char *path, php_stream *instream, ftptype_t type, int startpos, TSRMLS_D);
 ...
 * returns true on success, false on error
 */
int		ftp_nb_get(ftpbuf_t *ftp, php_stream *outstream, const char *path, ftptype_t type, int resumepos, TSRMLS_D);

/* stores the data from a file, socket, or process as a file on the remote server
 * returns true on success, false on error
 */
int		ftp_nb_put(ftpbuf_t *ftp, const char *path, php_stream *instream, ftptype_t type, int startpos, TSRMLS_D);

/* continues a previous nb_(f)get command
 */
int		ftp_nb_continue_read(ftpbuf_t *ftp, TSRMLS_D);

/* continues a previous nb_(f)put command
 */
int		ftp_nb_continue_write(ftpbuf_t *ftp, TSRMLS_D);
