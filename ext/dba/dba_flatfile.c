...
DBA_FETCH_FUNC(flatfile)
{
	...
	gval = flatfile_fetch(dba, gkey, TSRMLS_C);
	if (gval.dptr) {
		if (newlen) {
			*newlen = gval.dsize;
		}
...
DBA_UPDATE_FUNC(flatfile)
{
	...
	switch(flatfile_store(dba, gkey, gval, mode==1 ? FLATFILE_INSERT : FLATFILE_REPLACE, TSRMLS_C)) {
		case 0:
			return SUCCESS;
		case 1:
			return FAILURE;
		case -1:
			...
			return FAILURE;
		default:
			php_error_docref2(NULL, TSRMLS_C, key, val, E_WARNING, "Unknown return value");
			return FAILURE;
	}
}
...
DBA_DELETE_FUNC(flatfile)
{
	FLATFILE_DATA;
	FLATFILE_GKEY;
	return(flatfile_delete(dba, gkey, TSRMLS_C) == -1 ? FAILURE : SUCCESS);
}
...
DBA_FIRSTKEY_FUNC(flatfile)
{
	...
	dba->nextkey = flatfile_firstkey(dba, TSRMLS_C);
	if (dba->nextkey.dptr) {
		if (newlen)  {
...
DBA_NEXTKEY_FUNC(flatfile)
{
	...
	dba->nextkey = flatfile_nextkey(dba, TSRMLS_C);
	if (dba->nextkey.dptr) {
		if (newlen) {
			*newlen = dba->nextkey.dsize;
		}
