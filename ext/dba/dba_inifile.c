...
#define INIFILE_GKEY \
	key_type ini_key; \
	if (!key) { \
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "No key specified"); \
		return 0; \
	} \
...
{
	info->dbf = inifile_alloc(info->fp, info->mode == DBA_READER, info->flags&DBA_PERSISTENT, TSRMLS_C);
...
DBA_FETCH_FUNC(inifile)
{
	...
	ini_val = inifile_fetch(dba, &ini_key, skip, TSRMLS_C);
...
DBA_UPDATE_FUNC(inifile)
{
	...
	if (mode == 1) {
		res = inifile_append(dba, &ini_key, &ini_val, TSRMLS_C);
	} else {
		res = inifile_replace(dba, &ini_key, &ini_val, TSRMLS_C);
	}
	INIFILE_DONE;
	switch(res) {
	case -1:
		php_error_docref1(NULL, TSRMLS_C, key, E_WARNING, "Operation not possible");
		return FAILURE;
	default:
	case 0:
		return SUCCESS;
...
DBA_EXISTS_FUNC(inifile)
{
	...
	ini_val = inifile_fetch(dba, &ini_key, 0, TSRMLS_C);
	INIFILE_DONE;
	if (ini_val.value) {
...
DBA_DELETE_FUNC(inifile)
{
	...
	res =  inifile_delete(dba, &ini_key, TSRMLS_C);
...
DBA_FIRSTKEY_FUNC(inifile)
{
	...
	if (inifile_firstkey(dba, TSRMLS_C)) {
...
DBA_NEXTKEY_FUNC(inifile)
{
	...
	if (inifile_nextkey(dba, TSRMLS_C)) {
		char *result = inifile_key_string(&dba->curr.key);
		*newlen = strlen(result);
		return result;
	} else {
