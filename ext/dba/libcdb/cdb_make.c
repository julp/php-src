...
/* {{{ cdb_make_write */
static int cdb_make_write(struct cdb_make *c, char *buf, uint32 sz, TSRMLS_D) {
...
/* {{{ cdb_make_start */
int cdb_make_start(struct cdb_make *c, php_stream * f, TSRMLS_D)
{
	c->head = 0;
	c->split = 0;
	c->hash = 0;
	...
	if (php_stream_seek(f, c->pos, SEEK_SET) == -1) {
		php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Fseek failed");
		return -1;
	}
...
/* {{{ cdb_make_addend */
int cdb_make_addend(struct cdb_make *c, unsigned int keylen, unsigned int datalen, uint32 h, TSRMLS_D)
{
...
/* {{{ cdb_make_addbegin */
int cdb_make_addbegin(struct cdb_make *c, unsigned int keylen, unsigned int datalen, TSRMLS_D)
{
	char buf[8];
	...
	if (cdb_make_write(c, buf, 8, TSRMLS_C) != 0)
		return -1;
	return 0;
}

/* {{{ cdb_make_add */
int cdb_make_add(struct cdb_make *c,char *key,unsigned int keylen,char *data,unsigned int datalen, TSRMLS_D)
{
	if (cdb_make_addbegin(c, keylen, datalen, TSRMLS_C) == -1)
		return -1;
	if (cdb_make_write(c, key, keylen, TSRMLS_C) != 0)
		return -1;
	if (cdb_make_write(c, data, datalen, TSRMLS_C) != 0)
		return -1;
	return cdb_make_addend(c, keylen, datalen, cdb_hash(key, keylen), TSRMLS_C);
}
/* }}} */

/* {{{ cdb_make_finish */
int cdb_make_finish(struct cdb_make *c, TSRMLS_D)
{
	char buf[8];
	int i;
	uint32 len;
	...
	if (php_stream_tell(c->fp) != 0)
		return -1;
	if (cdb_make_write(c, c->final, sizeof(c->final), TSRMLS_C) != 0)
		return -1;
