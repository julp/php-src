...
#if DBA_CDB_BUILTIN
	if (make) {
		cdb_make_start(&cdb->m, file, TSRMLS_C);
	} else {
		cdb_init(&cdb->c, file, TSRMLS_C);
	}
	cdb->make = make;
#else
	cdb_init(&cdb->c, file);
...
#if DBA_CDB_BUILTIN
	if (cdb->make) {
		cdb_make_finish(&cdb->m, TSRMLS_C);
	} else {
		cdb_free(&cdb->c, TSRMLS_C);
	}
#else
	cdb_free(&cdb->c);
	close(cdb->file);
...
#if DBA_CDB_BUILTIN
# define php_cdb_read(cdb, buf, len, pos) cdb_read(cdb, buf, len, pos, TSRMLS_C)
# define php_cdb_findnext(cdb, key, len) cdb_findnext(cdb, key, len, TSRMLS_C)
# define php_cdb_find(cdb, key, len) cdb_find(cdb, key, len, TSRMLS_C)
#else
...
DBA_UPDATE_FUNC(cdb)
{
	...
	if (cdb_make_add(&cdb->m, key, keylen, val, vallen, TSRMLS_C) != -1)
		return SUCCESS;
#endif
	return FAILURE;
}
...
#if DBA_CDB_BUILTIN
int cdb_file_lseek(php_stream *fp, off_t offset, int whence, TSRMLS_D) {
	php_stream_seek(fp, offset, whence);
	return php_stream_tell(fp);
}
#else
int cdb_file_lseek(int fd, off_t offset, int whence, TSRMLS_D) {
...
#define CSEEK(n) do { \
	...
	if (cdb_file_lseek(cdb->file, (off_t)n, SEEK_SET, TSRMLS_C) != (off_t) n) return NULL; \
} while (0)
