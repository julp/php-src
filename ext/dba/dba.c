...
/* {{{ php_dba_myke_key */
static size_t php_dba_make_key(zval *key, char **key_str, char **key_free, TSRMLS_D)
{
	if (Z_TYPE_P(key) == IS_ARRAY) {
		...
		if (zend_hash_num_elements(Z_ARRVAL_P(key)) != 2) {
			php_error_docref(NULL, TSRMLS_C, E_RECOVERABLE_ERROR, "Key does not have exactly two elements: (key, name)");
			return -1;
		}
...
#define DBA_GET2 												\
	...
	size_t key_len; 											\
	if (zend_parse_parameters(ac, TSRMLS_C, "zr", &key, &id) == FAILURE) { 	\
		return; 												\
	} 															\
	if ((key_len = php_dba_make_key(key, &key_str, &key_free, TSRMLS_C)) == 0) {\
		RETURN_FALSE;											\
	}
...
#define DBA_GET2_3												\
	...
	case 2: 													\
		...
		break;  												\
	case 3: 													\
		if (zend_parse_parameters(ac, TSRMLS_C, "zlr", &key, &skip, &id) == FAILURE) { \
			return;												\
		} 														\
		break;  												\
	default:													\
...
#define DBA_WRITE_CHECK \
	if(info->mode != DBA_WRITER && info->mode != DBA_TRUNC && info->mode != DBA_CREAT) { \
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "You cannot perform a modification to a database without proper access"); \
		RETURN_FALSE; \
	}
...
/* {{{ dba_fetch_resource
PHPAPI void dba_fetch_resource(dba_info **pinfo, zval **id, TSRMLS_D)
{
	dba_info *info;
	DBA_ID_FETCH
	*pinfo = info;
...
/* {{{ dba_close 
 */ 
static void dba_close(dba_info *info, TSRMLS_D)
{
	if (info->hnd) {
		info->hnd->close(info, TSRMLS_C);
	}
	if (info->path) {
...
/* {{{ dba_close_rsrc
 */
static void dba_close_rsrc(zend_rsrc_list_entry *rsrc, TSRMLS_D)
{
	dba_info *info = (dba_info *)rsrc->ptr; 

	dba_close(info, TSRMLS_C);
}
/* }}} */

/* {{{ dba_close_pe_rsrc_deleter */
int dba_close_pe_rsrc_deleter(zend_rsrc_list_entry *le, void *pDba, TSRMLS_D)
{
	return le->ptr == pDba;
}
/* }}} */
...
/* {{{ dba_close_pe_rsrc */
static void dba_close_pe_rsrc(zend_rsrc_list_entry *rsrc, TSRMLS_D)
{
	dba_info *info = (dba_info *)rsrc->ptr; 

	/* closes the resource by calling dba_close_rsrc() */
	zend_hash_apply_with_argument(&EG(persistent_list), (apply_func_arg_t) dba_close_pe_rsrc_deleter, info, TSRMLS_C);
}
/* }}} */
...
ZEND_INI_MH(OnUpdateDefaultHandler)
{
	...
	if (!hptr->name) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "No such handler: %s", new_value);
		return FAILURE;
	}
...
static void php_dba_update(INTERNAL_FUNCTION_PARAMETERS, int mode)
{
	...
	if (zend_parse_parameters(ac, TSRMLS_C, "zsr", &key, &val, &val_len, &id) == FAILURE) {
		return;
	}
	...
	if (info->hnd->update(info, key_str, key_len, val, val_len, mode, TSRMLS_C) == SUCCESS) {
		DBA_ID_DONE;
		RETURN_TRUE;
	}
...
/* {{{ php_find_dbm
 */
dba_info *php_dba_find(const char* path, TSRMLS_D)
{
...
static void php_dba_open(INTERNAL_FUNCTION_PARAMETERS, int persistent)
{
	...
	if (ac==2) {
		...
		if (!hptr) {
			php_error_docref2(NULL, TSRMLS_C, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_WARNING, "No default handler selected");
			FREENOW;
			RETURN_FALSE;
		}
	} else {
	...
	if (!hptr->name) {
		php_error_docref2(NULL, TSRMLS_C, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_WARNING, "No such handler: %s", Z_STRVAL_PP(args[2]));
		FREENOW;
		RETURN_FALSE;
	}
	...
	if (pmode[0] && (pmode[1]=='d' || pmode[1]=='l' || pmode[1]=='-')) { /* force lock on db file or lck file or disable locking */
		...
		case 'l':
			...
			if ((hptr->flags & DBA_LOCK_ALL) == 0) {
				php_error_docref2(NULL, TSRMLS_C, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_NOTICE, "Handler %s does locking internally", hptr->name);
			}
			break;
		default:
		case '-':
			if ((hptr->flags & DBA_LOCK_ALL) == 0) {
				php_error_docref2(NULL, TSRMLS_C, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_WARNING, "Locking cannot be disabled for handler %s", hptr->name);
				FREENOW;
				RETURN_FALSE;
			}
			lock_flag = 0;
	...
	switch (*pmode++) {
		...
		case 'n':
			...
			break;
		default:
			php_error_docref2(NULL, TSRMLS_C, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_WARNING, "Illegal DBA mode");
			FREENOW;
			RETURN_FALSE;
	}
	if (!lock_file_mode) {
	...
	if (*pmode=='t') {
		pmode++;
		if (!lock_flag) {
			php_error_docref2(NULL, TSRMLS_C, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_WARNING, "You cannot combine modifiers - (no lock) and t (test lock)");
			FREENOW;
			RETURN_FALSE;
		}
		if (!lock_mode) {
			if ((hptr->flags & DBA_LOCK_ALL) == 0) {
				php_error_docref2(NULL, TSRMLS_C, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_WARNING, "Handler %s uses its own locking which doesn't support mode modifier t (test lock)", hptr->name);
				FREENOW;
				RETURN_FALSE;
			} else {
				php_error_docref2(NULL, TSRMLS_C, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_WARNING, "Handler %s doesn't uses locking for this mode which makes modifier t (test lock) obsolete", hptr->name);
				FREENOW;
				RETURN_FALSE;
			}
		} else {
	...
	if (hptr->flags & DBA_LOCK_ALL) {
		if ((other = php_dba_find(info->path, TSRMLS_C)) != NULL) {
	...
	if (!error && (hptr->flags&DBA_STREAM_OPEN)==DBA_STREAM_OPEN) {
		...
		if (hptr->flags & (DBA_NO_APPEND|DBA_CAST_AS_FD)) {
			...
			if (SUCCESS != php_stream_cast(info->fp, PHP_STREAM_AS_FD, (void*)&info->fd, 1)) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Could not cast stream");
				dba_close(info, TSRMLS_C);
				FREENOW;
				RETURN_FALSE;
#ifdef F_SETFL
	...
	if (error || hptr->open(info, &error, TSRMLS_C) != SUCCESS) {
		dba_close(info, TSRMLS_C);
		php_error_docref2(NULL, TSRMLS_C, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_WARNING, "Driver initialization failed for handler: %s%s%s", hptr->name, error?": ":"", error?error:"");
		FREENOW;
		RETURN_FALSE;
	}
	...
	if (persistent) {
		...
		if (zend_hash_update(&EG(persistent_list), key, keylen+1, &new_le, sizeof(zend_rsrc_list_entry), NULL) == FAILURE) {
			dba_close(info, TSRMLS_C);
			php_error_docref2(NULL, TSRMLS_C, Z_STRVAL_PP(args[0]), Z_STRVAL_PP(args[1]), E_WARNING, "Could not register persistent resource");
			FREENOW;
			RETURN_FALSE;
		}
	}
...
PHP_FUNCTION(dba_close)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r", &id) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(dba_exists)
{
	...
	if(info->hnd->exists(info, key_str, key_len, TSRMLS_C) == SUCCESS) {
		DBA_ID_DONE;
		RETURN_TRUE;
	}
	DBA_ID_DONE;
...
PHP_FUNCTION(dba_fetch)
{
	...
	if (ac==3) {
		if (!strcmp(info->hnd->name, "cdb")) {
			if (skip < 0) {
				php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Handler %s accepts only skip values greater than or equal to zero, using skip=0", info->hnd->name);
				skip = 0;
			}
			 ...
			 */
			if (skip < -1) {
				php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Handler %s accepts only skip value -1 and greater, using skip=0", info->hnd->name);
				skip = 0;
			}
		} else {
			php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Handler %s does not support optional skip parameter, the value will be ignored", info->hnd->name);
			skip = 0;			
		}
	} else {
		skip = 0; 
	}
	if((val = info->hnd->fetch(info, key_str, key_len, skip, &len, TSRMLS_C)) != NULL) {
		DBA_ID_DONE;
...
PHP_FUNCTION(dba_key_split)
{
	...
	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS(), TSRMLS_C, "z", &zkey) == SUCCESS) {
		if (Z_TYPE_P(zkey) == IS_NULL || (Z_TYPE_P(zkey) == IS_BOOL && !Z_LVAL_P(zkey))) {
			RETURN_BOOL(0);
		}
	}
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s", &key, &key_len) == FAILURE) {
		RETURN_BOOL(0);
	}
...
PHP_FUNCTION(dba_firstkey)
{
	...
	fkey = info->hnd->firstkey(info, &len, TSRMLS_C);
...
PHP_FUNCTION(dba_nextkey)
{
	...
	nkey = info->hnd->nextkey(info, &len, TSRMLS_C);
...
PHP_FUNCTION(dba_delete)
{
	...
	if(info->hnd->delete(info, key_str, key_len, TSRMLS_C) == SUCCESS)
	{
		DBA_ID_DONE;
		RETURN_TRUE;
	}
...
PHP_FUNCTION(dba_optimize)
{
	...
	if (info->hnd->optimize(info, TSRMLS_C) == SUCCESS) {
		RETURN_TRUE;
	}
...
PHP_FUNCTION(dba_sync)
{
	...
	if (info->hnd->sync(info, TSRMLS_C) == SUCCESS) {
		RETURN_TRUE;
	}
...
PHP_FUNCTION(dba_handlers)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "|b", &full_info) == FAILURE) {
		RETURN_FALSE;
	}
	...
	for(hptr = handler; hptr->name; hptr++) {
		if (full_info) {
			add_assoc_string(return_value, hptr->name, hptr->info(hptr, NULL, TSRMLS_C), 0);
		} else {
