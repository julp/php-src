...
/* {{{ inifile_findkey
 */
static int inifile_read(inifile *dba, line_type *ln, TSRMLS_D) {
	char *fline;
	char *pos;
 ...
 * 2 = DIFFERENT
 */
static int inifile_key_cmp(const key_type *k1, const key_type *k2, TSRMLS_D)
{
...
val_type inifile_fetch(inifile *dba, const key_type *key, int skip, TSRMLS_D) {
	...
	if (skip == -1 && dba->next.key.group && dba->next.key.name && !inifile_key_cmp(&dba->next.key, key, TSRMLS_C)) {
	...
	while(inifile_read(dba, &ln, TSRMLS_C)) {
		if (!(res=inifile_key_cmp(&ln.key, key, TSRMLS_C))) {
			if (!skip) {
...
int inifile_firstkey(inifile *dba, TSRMLS_D) {
	inifile_line_free(&dba->curr);
	dba->curr.pos = 0;
	return inifile_nextkey(dba, TSRMLS_C);
}
/* }}} */
...
/* {{{ inifile_truncate
 */
static int inifile_truncate(inifile *dba, size_t size, TSRMLS_D)
{
	int res;

	if ((res=php_stream_truncate_set_size(dba->fp, size)) != 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Error in ftruncate: %d", res);
		return FAILURE;
	}
 ...
 */
static int inifile_find_group(inifile *dba, const key_type *key, size_t *pos_grp_start, TSRMLS_D) 
{
	int ret = FAILURE;
	...
	if (key->group && strlen(key->group)) {
		...
		res = 1;
		while(inifile_read(dba, &ln, TSRMLS_C)) {
			if ((res=inifile_key_cmp(&ln.key, key, TSRMLS_C)) < 2) {
				ret = SUCCESS;
				break;
			}
 ...
 */
static int inifile_next_group(inifile *dba, const key_type *key, size_t *pos_grp_start, TSRMLS_D) 
{
	int ret = FAILURE;
	line_type ln = {{NULL,NULL},{NULL}};
	...
	*pos_grp_start = php_stream_tell(dba->fp);
	...
	while(inifile_read(dba, &ln, TSRMLS_C)) {
		if (inifile_key_cmp(&ln.key, key, TSRMLS_C) == 2) {
			ret = SUCCESS;
			break;
		}
		*pos_grp_start = php_stream_tell(dba->fp);
...
/* {{{ inifile_copy_to
 */
static int inifile_copy_to(inifile *dba, size_t pos_start, size_t pos_end, inifile **ini_copy, TSRMLS_D)
{
	php_stream *fp;
	...
	if ((fp = php_stream_temp_create(0, 64 * 1024)) == NULL) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Could not create temporary stream");
		*ini_copy = NULL;
		return FAILURE;
	}

	if ((*ini_copy = inifile_alloc(fp, 1, 0, TSRMLS_C)) == NULL) {
		/* writes error */
		return FAILURE;
	}
	php_stream_seek(dba->fp, pos_start, SEEK_SET);
	if (!php_stream_copy_to_stream_ex(dba->fp, fp, pos_end - pos_start, NULL)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Could not copy group [%zu - %zu] to temporary stream", pos_start, pos_end);
		return FAILURE;
	} 
	return SUCCESS;
}
 ...
 */
static int inifile_filter(inifile *dba, inifile *from, const key_type *key, TSRMLS_D) 
{
	...
	while(inifile_read(from, &ln, TSRMLS_C)) {
		switch(inifile_key_cmp(&ln.key, key, TSRMLS_C)) {
		case 0:
			...
			if (pos_start != pos_next) {
				...
				if (!php_stream_copy_to_stream_ex(from->fp, dba->fp, pos_next - pos_start, NULL)) {
					php_error_docref(NULL, TSRMLS_C, E_WARNING, "Could not copy [%zu - %zu] from temporary stream", pos_next, pos_start);
					ret = FAILURE;
				}
 ...
 */
static int inifile_delete_replace_append(inifile *dba, const key_type *key, const val_type *value, int append, TSRMLS_D) 
{
	...
	/* 1 - 3 */
	inifile_find_group(dba, key, &pos_grp_start, TSRMLS_C);
	inifile_next_group(dba, key, &pos_grp_next, TSRMLS_C);
	if (append) {
		ret = SUCCESS;
	} else {
		ret = inifile_copy_to(dba, pos_grp_start, pos_grp_next, &ini_tmp, TSRMLS_C);
	}
		...
		if (!fp_tmp) {
			...
			if (pos_grp_next != (size_t)php_stream_tell(dba->fp)) {
				...
				if (!php_stream_copy_to_stream_ex(dba->fp, fp_tmp, PHP_STREAM_COPY_ALL, NULL)) {
					php_error_docref(NULL, TSRMLS_C, E_WARNING, "Could not copy remainder to temporary stream");
					ret = FAILURE;
				}
			}
		}
	...
	if (ret == SUCCESS) {
		if (!value || (key->name && strlen(key->name))) {
			ret = inifile_truncate(dba, append ? pos_grp_next : pos_grp_start, TSRMLS_C); /* writes error on fail */
		}
	}
	...
	if (ret == SUCCESS) {
		if (key->name && strlen(key->name)) {
			/* 6 */
			if (!append && ini_tmp) {
				ret = inifile_filter(dba, ini_tmp, key, TSRMLS_C);
			}
			...
			if (value) {
				if (pos_grp_start == pos_grp_next && key->group && strlen(key->group)) {
					php_stream_printf(dba->fp, TSRMLS_C, "[%s]\n", key->group);
				}
				php_stream_printf(dba->fp, TSRMLS_C, "%s=%s\n", key->name, value->value ? value->value : "");
			}
		}
		...
		if (fp_tmp && php_stream_tell(fp_tmp)) {
			...
			if (!php_stream_copy_to_stream_ex(fp_tmp, dba->fp, PHP_STREAM_COPY_ALL, NULL)) {
				php_error_docref(NULL, TSRMLS_C, E_RECOVERABLE_ERROR, "Could not copy from temporary stream - ini file truncated");
				ret = FAILURE;
			}
		}
	}
...
{
	return inifile_delete_replace_append(dba, key, NULL, 0, TSRMLS_C);
}	
/* }}} */

/* {{{ inifile_relace
 */
int inifile_replace(inifile *dba, const key_type *key, const val_type *value, TSRMLS_D) 
{
	return inifile_delete_replace_append(dba, key, value, 0, TSRMLS_C);
}
/* }}} */

/* {{{ inifile_append
 */
int inifile_append(inifile *dba, const key_type *key, const val_type *value, TSRMLS_D) 
{
	return inifile_delete_replace_append(dba, key, value, 1, TSRMLS_C);
}
/* }}} */
