...
static int _php_mcast_join_leave(php_socket *sock, int level, struct sockaddr *group, socklen_t group_len, unsigned int if_index, int join, TSRMLS_D);
#ifdef HAS_MCAST_EXT
static int _php_mcast_source_op(php_socket *sock, int level, struct sockaddr *group, socklen_t group_len, struct sockaddr *source, socklen_t source_len, unsigned int if_index, enum source_op sop, TSRMLS_D);
#endif
...
static int php_get_if_index_from_zval(zval *val, unsigned *out, TSRMLS_D)
{
	int ret;
...
static int php_get_if_index_from_array(const HashTable *ht, const char *key,
	php_socket *sock, unsigned int *if_index, TSRMLS_D)
{
	zval **val;
	...
	return php_get_if_index_from_zval(*val, if_index, TSRMLS_C);
}

static int php_get_address_from_array(const HashTable *ht, const char *key,
	php_socket *sock, php_sockaddr_storage *ss, socklen_t *ss_len, TSRMLS_D)
{
	zval **val,
		 *valcp;

	if (zend_hash_find(ht, key, strlen(key) + 1, (void **)&val) == FAILURE) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "no key \"%s\" passed in optval", key);
		return FAILURE;
	}
	valcp = *val;
	zval_add_ref(&valcp);
	convert_to_string_ex(val);
	if (!php_set_inet46_addr(ss, ss_len, Z_STRVAL_P(valcp), sock, TSRMLS_C)) {
		zval_ptr_dtor(&valcp);
		return FAILURE;
	}
	zval_ptr_dtor(&valcp);
...
static int php_do_mcast_opt(php_socket *php_sock, int level, int optname, zval **arg4, TSRMLS_D)
{
	HashTable		 		*opt_ht;
	...
	int (*mcast_sreq_fun)(php_socket *, int, struct sockaddr *, socklen_t,
		struct sockaddr *, socklen_t, unsigned, TSRMLS_D);
#endif
	...
	case PHP_MCAST_LEAVE_SOURCE_GROUP:
		...
		mcast_sreq_fun:
			...
			retval = mcast_sreq_fun(php_sock, level, (struct sockaddr*)&group,
					glen, (struct sockaddr*)&source, slen, if_index, TSRMLS_C);
			break;
		}
#endif
	default:
							   ...
							   zval **arg4, TSRMLS_D)
{
	...
	case PHP_MCAST_LEAVE_SOURCE_GROUP:
#endif
		if (php_do_mcast_opt(php_sock, level, optname, arg4, TSRMLS_C) == FAILURE) {
			return FAILURE;
		} else {
			return SUCCESS;
		}
	...
	case IP_MULTICAST_IF:
		if (php_get_if_index_from_zval(*arg4, &if_index, TSRMLS_C) == FAILURE) {
			return FAILURE;
		}

		if (php_if_index_to_addr4(if_index, php_sock, &if_addr, TSRMLS_C) == FAILURE) {
			return FAILURE;
		}
		opt_ptr = &if_addr;
...
int php_mcast_join(
	...
	socklen_t group_len,
	unsigned int if_index, TSRMLS_D)
{
	return _php_mcast_join_leave(sock, level, group, group_len, if_index, 1, TSRMLS_C);
}
...
int php_mcast_leave(
	...
	unsigned int if_index, TSRMLS_D)
{
	return _php_mcast_join_leave(sock, level, group, group_len, if_index, 0, TSRMLS_C);
}
...
int php_mcast_join_source(
	...
	unsigned int if_index, TSRMLS_D)
{
	return _php_mcast_source_op(sock, level, group, group_len, source, source_len, if_index, JOIN_SOURCE, TSRMLS_C);
}
...
int php_mcast_leave_source(
	...
	unsigned int if_index, TSRMLS_D)
{
	return _php_mcast_source_op(sock, level, group, group_len, source, source_len, if_index, LEAVE_SOURCE, TSRMLS_C);
}
...
int php_mcast_block_source(
	...
	unsigned int if_index, TSRMLS_D)
{
	return _php_mcast_source_op(sock, level, group, group_len, source, source_len, if_index, BLOCK_SOURCE, TSRMLS_C);
}
...
int php_mcast_unblock_source(
	...
	unsigned int if_index, TSRMLS_D)
{
	return _php_mcast_source_op(sock, level, group, group_len, source, source_len, if_index, UNBLOCK_SOURCE, TSRMLS_C);
}
#endif /* HAS_MCAST_EXT */
	...
	int join, TSRMLS_D)
{
	...
	if (sock->type == AF_INET) {
		...
		if (if_index != 0) {
			if (php_if_index_to_addr4(if_index, sock, &addr, TSRMLS_C) ==
					FAILURE)
...
#if PHP_WIN32
int php_if_index_to_addr4(unsigned if_index, php_socket *php_sock, struct in_addr *out_addr, TSRMLS_D)
{
...
int php_add4_to_if_index(struct in_addr *addr, php_socket *php_sock, unsigned *if_index, TSRMLS_D)
{
	MIB_IPADDRTABLE *addr_table;
    ULONG size;
    DWORD retval;
