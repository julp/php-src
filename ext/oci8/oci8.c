...
/* static protos {{{ */
static void php_oci_connection_list_dtor (zend_rsrc_list_entry *, TSRMLS_D);
static void php_oci_pconnection_list_dtor (zend_rsrc_list_entry *, TSRMLS_D);
static void php_oci_pconnection_list_np_dtor (zend_rsrc_list_entry *, TSRMLS_D);
static void php_oci_statement_list_dtor (zend_rsrc_list_entry *, TSRMLS_D);
static void php_oci_descriptor_list_dtor (zend_rsrc_list_entry *, TSRMLS_D);
static void php_oci_spool_list_dtor(zend_rsrc_list_entry *entry, TSRMLS_D);
static void php_oci_collection_list_dtor (zend_rsrc_list_entry *, TSRMLS_D);

static int php_oci_persistent_helper(zend_rsrc_list_entry *le, TSRMLS_D);
static int php_oci_connection_ping(php_oci_connection *, TSRMLS_D);
static int php_oci_connection_status(php_oci_connection *, TSRMLS_D);
static int php_oci_connection_close(php_oci_connection *, TSRMLS_D);
static void php_oci_spool_close(php_oci_spool *session_pool, TSRMLS_D);

static OCIEnv *php_oci_create_env(ub2 charsetid, TSRMLS_D);
static int php_oci_create_session(php_oci_connection *connection, php_oci_spool *session_pool, char *dbname, int dbname_len, char *username, int username_len, char *password, int password_len, char *new_password, int new_password_len, int session_mode, TSRMLS_D);
static int php_oci_old_create_session(php_oci_connection *connection, char *dbname, int dbname_len, char *username, int username_len, char *password, int password_len, char *new_password, int new_password_len, int session_mode, TSRMLS_D);
static php_oci_spool *php_oci_get_spool(char *username, int username_len, char *password, int password_len, char *dbname, int dbname_len, int charsetid, TSRMLS_D);
static php_oci_spool *php_oci_create_spool(char *username, int username_len, char *password, int password_len, char *dbname, int dbname_len, char *hash_key, int hash_key_len, int charsetid, TSRMLS_D);
static sword php_oci_ping_init(php_oci_connection *connection, OCIError *errh, TSRMLS_D);
/* }}} */
...
static void php_oci_init_global_handles(TSRMLS_D)
{
	...
	if (errstatus == OCI_ERROR) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "OCIEnvNlsCreate() failed. There is something wrong with your system - please check that " PHP_OCI8_LIB_PATH_MSG " includes the directory with Oracle Instant Client libraries");
#else
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "OCIEnvNlsCreate() failed. There is something wrong with your system - please check that ORACLE_HOME and " PHP_OCI8_LIB_PATH_MSG " are set and point to the right directories");
#endif
		if (OCI_G(env)
			&& OCIErrorGet(OCI_G(env), (ub4)1, NULL, &ora_error_code, tmp_buf, (ub4)OCI_ERROR_MAXMSG_SIZE, (ub4)OCI_HTYPE_ENV) == OCI_SUCCESS
			&& *tmp_buf) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "%s", tmp_buf);
		}
	...
	if (errstatus == OCI_SUCCESS) {
		...
		if (ora_error_code) {
			...
			if (errstatus == OCI_SUCCESS_WITH_INFO) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Initialization error: OCI_SUCCESS_WITH_INFO: %s", tmp_buf);
			} else {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Initialization error: OCI_ERROR: %s", tmp_buf);
...
static PHP_GINIT_FUNCTION(oci)
#else
static void php_oci_init_globals(zend_oci_globals *oci_globals, TSRMLS_D)
#endif
{
...
static PHP_GSHUTDOWN_FUNCTION(oci)
#else
static void php_oci_shutdown_globals(zend_oci_globals *oci_globals, TSRMLS_D)
#endif
{
...
PHP_MINIT_FUNCTION(oci)
{
	...
	oci_lob_class_entry_ptr = zend_register_internal_class(&oci_lob_class_entry, TSRMLS_C);
	oci_coll_class_entry_ptr = zend_register_internal_class(&oci_coll_class_entry, TSRMLS_C);
...
PHP_RSHUTDOWN_FUNCTION(oci)
{
	 ...
	 */
	zend_hash_apply(&EG(persistent_list), (apply_func_t) php_oci_persistent_helper, TSRMLS_C);
...
PHP_MINFO_FUNCTION(oci)
{
	...
	php_oci_client_get_version(&ver, TSRMLS_C);
 ...
 */
static void php_oci_connection_list_dtor(zend_rsrc_list_entry *entry, TSRMLS_D)
{
	php_oci_connection *connection = (php_oci_connection *)entry->ptr;

	if (connection) {
		php_oci_connection_close(connection, TSRMLS_C);
		OCI_G(num_links)--;
	}
} /* }}} */
 ...
 */
static void php_oci_pconnection_list_dtor(zend_rsrc_list_entry *entry, TSRMLS_D)
{
	php_oci_connection *connection = (php_oci_connection *)entry->ptr;
 ...
 */
static void php_oci_pconnection_list_np_dtor(zend_rsrc_list_entry *entry, TSRMLS_D)
{
	php_oci_connection *connection = (php_oci_connection *)entry->ptr;
	...
	if (!connection->is_open ||
		 ...
		 */
		php_oci_connection_release(connection, TSRMLS_C);
 ...
 */
static void php_oci_statement_list_dtor(zend_rsrc_list_entry *entry, TSRMLS_D)
{
	php_oci_statement *statement = (php_oci_statement *)entry->ptr;
	php_oci_statement_free(statement, TSRMLS_C);
} /* }}} */
 ...
 */
static void php_oci_descriptor_list_dtor(zend_rsrc_list_entry *entry, TSRMLS_D)
{
	php_oci_descriptor *descriptor = (php_oci_descriptor *)entry->ptr;
	php_oci_lob_free(descriptor, TSRMLS_C);
} /* }}} */
 ...
 */
static void php_oci_collection_list_dtor(zend_rsrc_list_entry *entry, TSRMLS_D)
{
	php_oci_collection *collection = (php_oci_collection *)entry->ptr;
	php_oci_collection_close(collection, TSRMLS_C);
} /* }}} */
...
void php_oci_descriptor_flush_hash_dtor(void *data)
{
	...
	if (descriptor && descriptor->buffering == PHP_OCI_LOB_BUFFER_USED && (descriptor->type == OCI_DTYPE_LOB || descriptor->type == OCI_DTYPE_FILE)) {
		php_oci_lob_flush(descriptor, OCI_LOB_BUFFER_FREE, TSRMLS_C);
 ...
 */
void php_oci_connection_descriptors_free(php_oci_connection *connection, TSRMLS_D)
{
 ...
 */
sb4 php_oci_error(OCIError *err_p, sword status, TSRMLS_D)
{
	...
	switch (status) {
		...
		case OCI_SUCCESS_WITH_INFO:
			errcode = php_oci_fetch_errmsg(err_p, &errbuf, TSRMLS_C);
			if (errbuf) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "OCI_SUCCESS_WITH_INFO: %s", errbuf);
				efree(errbuf);
			} else {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "OCI_SUCCESS_WITH_INFO: failed to fetch error message");
			}
			break;
		case OCI_NEED_DATA:
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "OCI_NEED_DATA");
			break;
		case OCI_NO_DATA:
			errcode = php_oci_fetch_errmsg(err_p, &errbuf, TSRMLS_C);
			if (errbuf) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "%s", errbuf);
				efree(errbuf);
			} else {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "OCI_NO_DATA: failed to fetch error message");
			}
			break;
		case OCI_ERROR:
			errcode = php_oci_fetch_errmsg(err_p, &errbuf, TSRMLS_C);
			if (errbuf) {
				...
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "failed to fetch error message");
			}
			break;
		case OCI_INVALID_HANDLE:
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "OCI_INVALID_HANDLE");
			break;
		case OCI_STILL_EXECUTING:
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "OCI_STILL_EXECUTING");
			break;
		case OCI_CONTINUE:
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "OCI_CONTINUE");
			break;
		default:
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unknown OCI error code: %d", status);
			break;
	}
	return errcode;
}
 ...
 */
sb4 php_oci_fetch_errmsg(OCIError *error_handle, text **error_buf, TSRMLS_D)
{
	sb4 error_code = 0;
 ...
 */
int php_oci_fetch_sqltext_offset(php_oci_statement *statement, text **sqltext, ub2 *error_offset, TSRMLS_D)
{
	sword errstatus;
	...
	*error_offset = 0;
	...
	if (errstatus != OCI_SUCCESS) {
		statement->errcode = php_oci_error(statement->err, errstatus, TSRMLS_C);
...
void php_oci_do_connect(INTERNAL_FUNCTION_PARAMETERS, int persistent, int exclusive)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ss|ssl", &username, &username_len, &password, &password_len, &dbname, &dbname_len, &charset, &charset_len, &session_mode) == FAILURE) {
		return;
	}
	...
	connection = php_oci_do_connect_ex(username, username_len, password, password_len, NULL, 0, dbname, dbname_len, charset, session_mode, persistent, exclusive, TSRMLS_C);
 ...
 */
php_oci_connection *php_oci_do_connect_ex(char *username, int username_len, char *password, int password_len, char *new_password, int new_password_len, char *dbname, int dbname_len, char *charset, long session_mode, int persistent, int exclusive, TSRMLS_D)
{
	zend_rsrc_list_entry *le;
	...
	if (session_mode & ~(OCI_SYSOPER | OCI_SYSDBA | PHP_OCI_CRED_EXT)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid session mode specified (%ld)", session_mode);
		return NULL;
	}
	if (session_mode & (OCI_SYSOPER | OCI_SYSDBA | PHP_OCI_CRED_EXT)) {
		if ((session_mode & OCI_SYSOPER) && (session_mode & OCI_SYSDBA)) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "OCI_SYSDBA and OCI_SYSOPER cannot be used together");
			return NULL;
		}
		if (session_mode & PHP_OCI_CRED_EXT) {
			 ...
			 */
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "External Authentication is not supported on Windows");
			return NULL;
#endif
			if (username_len != 1 || username[0] != '/' || password_len != 0) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "OCI_CRED_EXT can only be used with a username of \"/\" and a NULL password");
				return NULL;
			}
		}
		if (session_mode & (OCI_SYSOPER | OCI_SYSDBA)) {
			...
			if (!OCI_G(privileged_connect)) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Privileged connect is disabled. Enable oci8.privileged_connect to be able to connect as SYSOPER or SYSDBA");
				return NULL;
			}
#if (PHP_MAJOR_VERSION == 5 && PHP_MINOR_VERSION < 4) || (PHP_MAJOR_VERSION < 5)
			/* Safe mode has been removed in PHP 5.4 */
			if (PG(safe_mode)) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Privileged connect is disabled in Safe Mode");
				return NULL;
			}
#endif
		}
	...
	if (charset && *charset) {
		...
		if (!charsetid) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid character set name: %s", charset);
		} else {
	...
	if (!exclusive && !new_password) {
		...
		if (connection && connection->is_persistent && connection->is_stub) {
			if (php_oci_create_session(connection, NULL, dbname, dbname_len, username, username_len, password, password_len, new_password, new_password_len, session_mode, TSRMLS_C)) {
		...
		if (connection) {
			if (connection->is_open) {
				...
				if (connection->is_persistent) {
					 ...
					 * 3) ping it if needed
					 */
					if (php_oci_connection_status(connection, TSRMLS_C)) {
						/* Only ping if:
						 *
						 ...
						 * still alive"
						 */
						if (!ping_done && (*(connection->next_pingp) > 0) && (timestamp >= *(connection->next_pingp)) && !php_oci_connection_ping(connection, TSRMLS_C)) {
							/* server died */
						} else {
							...
							if (tmp != NULL && rsrc_type == le_pconnection && strlen(tmp->hash_key) == hashed_details.len &&
								...
								connection->rsrc_id = zend_list_insert(connection, le_pconnection, TSRMLS_C);
#else
	...
	if (persistent) {
		...
		if (OCI_G(max_persistent) != -1 && OCI_G(num_persistent) >= OCI_G(max_persistent)) {
			...
			if (OCI_G(max_persistent) != -1 && OCI_G(num_persistent) >= OCI_G(max_persistent)) {
				...
				php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Too many open persistent connections (%ld)", OCI_G(num_persistent));
	...
	if (use_spool && !connection->is_persistent) {
		if ((session_pool = php_oci_get_spool(username, username_len, password, password_len, dbname, dbname_len, charsetid ? charsetid:charsetid_nls_lang, TSRMLS_C))==NULL)
		{
			php_oci_connection_close(connection, TSRMLS_C);
			smart_str_free_ex(&hashed_details, 0);
			return NULL;
	 ...
	 */
	if (!use_spool) {
		if (php_oci_old_create_session(connection, dbname, dbname_len, username, username_len, password, password_len, new_password, new_password_len, session_mode, TSRMLS_C)) {
			php_oci_connection_close(connection, TSRMLS_C);
			return NULL;
		}
	} else {
		/* create using the client-side session pool */
		if (php_oci_create_session(connection, session_pool, dbname, dbname_len, username, username_len, password, password_len, new_password, new_password_len, session_mode, TSRMLS_C)) {
			php_oci_connection_close(connection, TSRMLS_C);
			return NULL;
		}
	}
...
#if (PHP_MAJOR_VERSION == 5 && PHP_MINOR_VERSION > 3) || (PHP_MAJOR_VERSION > 5)
		connection->rsrc_id = zend_list_insert(connection, le_connection, TSRMLS_C);
#else
 ...
 */
static int php_oci_connection_ping(php_oci_connection *connection, TSRMLS_D)
{
 ...
 */
static int php_oci_connection_status(php_oci_connection *connection, TSRMLS_D)
{
	ub4 ss = 0;
 ...
 * Rollback connection
 */
int php_oci_connection_rollback(php_oci_connection *connection, TSRMLS_D)
{
	...
	if (connection->errcode != OCI_SUCCESS) {
		connection->errcode = php_oci_error(connection->err, connection->errcode, TSRMLS_C);
 ...
 * Commit connection
 */
int php_oci_connection_commit(php_oci_connection *connection, TSRMLS_D)
{
 ...
 */
static int php_oci_connection_close(php_oci_connection *connection, TSRMLS_D)
{
	int result = 0;
	 ...
	 */
		php_oci_spool_close(connection->private_spool, TSRMLS_C);
	}
 ...
 */
int php_oci_connection_release(php_oci_connection *connection, TSRMLS_D)
{
	int result = 0;
	zend_bool in_call_save = OCI_G(in_call);
 ...
 */
int php_oci_password_change(php_oci_connection *connection, char *user, int user_len, char *pass_old, int pass_old_len, char *pass_new, int pass_new_len, TSRMLS_D)
{
 ...
 */
void php_oci_client_get_version(char **version, TSRMLS_D)
{
 ...
 */
int php_oci_server_get_version(php_oci_connection *connection, char **version, TSRMLS_D)
{
	char version_buff[256];
 ...
 */
int php_oci_column_to_zval(php_oci_out_column *column, zval *value, int mode, TSRMLS_D)
{
	...
	if (column->is_cursor) { /* REFCURSOR -> simply return the statement id */
		...
		if (column->data_type != SQLT_RDD) {
			...
			if (!descriptor || rsrc_type != le_descriptor) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to find LOB descriptor #%d", column->descid);
				return 1;
			}
		...
		if (column->data_type != SQLT_RDD && (mode & PHP_OCI_RETURN_LOBS)) {
			...
			lob_fetch_status = php_oci_lob_read(descriptor, -1, 0, &lob_buffer, &lob_length, TSRMLS_C);
			php_oci_temp_lob_close(descriptor, TSRMLS_C);
			if (lob_fetch_status) {
				ZVAL_FALSE(value);
				return 1;
			} else {
...
void php_oci_fetch_row (INTERNAL_FUNCTION_PARAMETERS, int mode, int expected_args)
{
	...
	if (expected_args > 2) {
		...
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rz|l", &z_statement, &array, &fetch_mode) == FAILURE) {
			return;
		}
		...
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r|l", &z_statement, &fetch_mode) == FAILURE) {
			return;
		}
	...
	for (i = 0; i < statement->ncolumns; i++) {
		...
		column = php_oci_statement_get_column(statement, i + 1, NULL, 0, TSRMLS_C);
		...
		if (!(column->indicator == -1)) {
			...
			php_oci_column_to_zval(column, element, fetch_mode, TSRMLS_C);
...
static php_oci_spool *php_oci_create_spool(char *username, int username_len, char *password, int password_len, char *dbname, int dbname_len, char *hash_key, int hash_key_len, int charsetid, TSRMLS_D)
{
	...
	if (!(session_pool->env = php_oci_create_env(charsetid, TSRMLS_C))) {
		iserror = 1;
		goto exit_create_spool;
	}
...
exit_create_spool:
	if (iserror && session_pool) {
		php_oci_spool_close(session_pool, TSRMLS_C);
		session_pool = NULL;
	}
...
static php_oci_spool *php_oci_get_spool(char *username, int username_len, char *password, int password_len, char *dbname, int dbname_len, int charsetid, TSRMLS_D)
{
	...
	if (zend_hash_find(&EG(persistent_list),spool_hashed_details.c, spool_hashed_details.len+1, (void **)&spool_out_le) == FAILURE) {
		...
		session_pool = php_oci_create_spool(username, username_len, password, password_len, dbname, dbname_len, spool_hashed_details.c, spool_hashed_details.len, charsetid, TSRMLS_C);
...
#if (PHP_MAJOR_VERSION == 5 && PHP_MINOR_VERSION > 3) || (PHP_MAJOR_VERSION > 5)
		zend_list_insert(session_pool, le_psessionpool, TSRMLS_C);
#else
...
static OCIEnv *php_oci_create_env(ub2 charsetid, TSRMLS_D)
{
	...
	if (OCI_G(errcode) != OCI_SUCCESS) {
		...
		if (retenv
			...
			&& *ora_msg_buf) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "%s", ora_msg_buf);
		}
...
static int php_oci_old_create_session(php_oci_connection *connection, char *dbname, int dbname_len, char *username, int username_len, char *password, int password_len, char *new_password, int new_password_len, int session_mode, TSRMLS_D)
{
	...
	if (!(connection->env = php_oci_create_env(connection->charset, TSRMLS_C))) {
		return 1;
	}
	...
	if (php_oci_ping_init(connection, OCI_G(err), TSRMLS_C) != OCI_SUCCESS) {
		php_oci_error(OCI_G(err), OCI_G(errcode), TSRMLS_C);
		return 1;
	}
...
static int php_oci_create_session(php_oci_connection *connection, php_oci_spool *session_pool, char *dbname, int dbname_len, char *username, int username_len, char *password, int password_len, char *new_password, int new_password_len, int session_mode, TSRMLS_D)
{
	...
	if (connection->is_persistent && !connection->private_spool &&
		!(connection->private_spool = php_oci_create_spool(username, username_len, password, password_len, dbname, dbname_len, NULL, 0, connection->charset, TSRMLS_C))) {
			return 1;
	}
	...
	do {
		...
		if (!(connection->next_pingp)){
			...
			if (php_oci_connection_ping(connection, TSRMLS_C)) {
