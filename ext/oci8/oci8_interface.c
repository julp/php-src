...
PHP_FUNCTION(oci_define_by_name)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rsz/|l", &stmt, &name, &name_len, &var, &type) == FAILURE) {
		return;
	}

	if (!name_len) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Column name cannot be empty");
		RETURN_FALSE;
	}
...
PHP_FUNCTION(oci_bind_by_name)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rsz/|ll", &z_statement, &name, &name_len, &bind_var, &maxlen, &type) == FAILURE) {
		return;
	}
	...
	if (php_oci_bind_by_name(statement, name, name_len, bind_var, maxlen, bind_type, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(oci_bind_array_by_name)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rsz/l|ll", &z_statement, &name, &name_len, &bind_var, &max_array_len, &max_item_len, &type) == FAILURE) {
		return;
	}
	...
	if (max_array_len <= 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Maximum array length must be greater than zero");
		RETURN_FALSE;
	}
	
	if (php_oci_bind_array_by_name(statement, name, name_len, bind_var, max_array_len, max_item_len, type, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(oci_free_descriptor)
{
	...
	if (!getThis()) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "O", &z_descriptor, oci_lob_class_entry_ptr) == FAILURE) {
			return;
		}
	}
	
	if (zend_hash_find(Z_OBJPROP_P(z_descriptor), "descriptor", sizeof("descriptor"), (void **)&tmp) == FAILURE) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to find descriptor property");
		RETURN_FALSE;
	}
...
PHP_FUNCTION(oci_lob_save)
{
	...
	if (getThis()) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|l", &data, &data_len, &offset) == FAILURE) {
			return;
		}
	}
	else {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Os|l", &z_descriptor, oci_lob_class_entry_ptr, &data, &data_len, &offset) == FAILURE) {
			return;
		}
	}
	...
	if (offset < 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Offset parameter must be greater than or equal to 0");
		RETURN_FALSE;
	}
	
	if (php_oci_lob_write(descriptor, offset, data, data_len, &bytes_written, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(oci_lob_import)
{
...
#else
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s", &filename, &filename_len) == FAILURE) {
#endif
			return;
		}
	}
...
#if (PHP_MAJOR_VERSION == 5 && PHP_MINOR_VERSION > 3) || (PHP_MAJOR_VERSION > 5)
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Op", &z_descriptor, oci_lob_class_entry_ptr, &filename, &filename_len) == FAILURE) {
#else
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Os", &z_descriptor, oci_lob_class_entry_ptr, &filename, &filename_len) == FAILURE) {
#endif
			return;
		}	
	}
	...
	if (strlen(filename) != filename_len) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Filename cannot contain null bytes");
		RETURN_FALSE;  
	}
#endif
	...
	if (php_oci_lob_import(descriptor, filename, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(oci_lob_load)
{
	...
	if (php_oci_lob_read(descriptor, -1, 0, &buffer, &buffer_len, TSRMLS_C)) {
		RETURN_FALSE;
	}
	if (buffer_len > 0) {
...
PHP_FUNCTION(oci_lob_read)
{
	...
	if (getThis()) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l", &length) == FAILURE) {
			return;
		}
	}
	else {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Ol", &z_descriptor, oci_lob_class_entry_ptr, &length) == FAILURE) {
			return;
		}	
	}
	...
	if (length <= 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Length parameter must be greater than 0");
		RETURN_FALSE;
	}
	
	if (php_oci_lob_read(descriptor, length, descriptor->lob_current_position, &buffer, &buffer_len, TSRMLS_C)) {
		RETURN_FALSE;
	}	
	if (buffer_len > 0) {
		RETURN_STRINGL(buffer, buffer_len, 0);
...
PHP_FUNCTION(oci_lob_eof)
{
	...
	if (!php_oci_lob_get_length(descriptor, &lob_length, TSRMLS_C) && lob_length >= 0) {
...
PHP_FUNCTION(oci_lob_seek)
{
	...
	if (getThis()) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l|l", &offset, &whence) == FAILURE) {
			return;
		}
	}
	else {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Ol|l", &z_descriptor, oci_lob_class_entry_ptr, &offset, &whence) == FAILURE) {
			return;
		}	
	}
	...
	if (php_oci_lob_get_length(descriptor, &lob_length, TSRMLS_C)) {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(oci_lob_write)
{
	...
	if (getThis()) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|l", &data, &data_len, &write_len) == FAILURE) {
			return;
		}
		...
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Os|l", &z_descriptor, oci_lob_class_entry_ptr, &data, &data_len, &write_len) == FAILURE) {
			return;
		}
	...
	if (php_oci_lob_write(descriptor, descriptor->lob_current_position, data, data_len, &bytes_written, TSRMLS_C)) {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(oci_lob_append)
{
	...
	if (getThis()) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "O", &z_descriptor_from, oci_lob_class_entry_ptr) == FAILURE) {
			return;
		}
	}
	else {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "OO", &z_descriptor_dest, oci_lob_class_entry_ptr, &z_descriptor_from, oci_lob_class_entry_ptr) == FAILURE) {
			return;
		}	
	}
	
	if (zend_hash_find(Z_OBJPROP_P(z_descriptor_dest), "descriptor", sizeof("descriptor"), (void **)&tmp_dest) == FAILURE) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to find descriptor property. The first argument should be valid descriptor object");
		RETURN_FALSE;
	}
	
	if (zend_hash_find(Z_OBJPROP_P(z_descriptor_from), "descriptor", sizeof("descriptor"), (void **)&tmp_from) == FAILURE) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to find descriptor property. The second argument should be valid descriptor object");
		RETURN_FALSE;
	}
	...
	if (php_oci_lob_append(descriptor_dest, descriptor_from, TSRMLS_C)) {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(oci_lob_truncate)
{
	...
	if (getThis()) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "|l", &trim_length) == FAILURE) {
			return;
		}
	}
	else {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "O|l", &z_descriptor, oci_lob_class_entry_ptr, &trim_length) == FAILURE) {
			return;
		}	
	}
	...
	if (trim_length < 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Length must be greater than or equal to zero");
		RETURN_FALSE;
	}
	...
	if (php_oci_lob_truncate(descriptor, ub_trim_length, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(oci_lob_erase)
{
	...
	if (getThis()) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "|ll", &offset, &length) == FAILURE) {
			return;
		}

		if (ZEND_NUM_ARGS() > 0 && offset < 0) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Offset must be greater than or equal to 0");
			RETURN_FALSE;
		}

		if (ZEND_NUM_ARGS() > 1 && length < 0) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Length must be greater than or equal to 0");
			RETURN_FALSE;
		}
	}
	else {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "O|ll", &z_descriptor, oci_lob_class_entry_ptr, &offset, &length) == FAILURE) {
			return;
		}
	...
	if (php_oci_lob_erase(descriptor, offset, length, &bytes_erased, TSRMLS_C)) {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(oci_lob_flush)
{
	...
	if (getThis()) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "|l", &flush_flag) == FAILURE) {
			return;
		}
	}
	else {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "O|l", &z_descriptor, oci_lob_class_entry_ptr, &flush_flag) == FAILURE) {
			return;
		}
	}
	...
	if (php_oci_lob_flush(descriptor, flush_flag, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(ocisetbufferinglob)
{
	...
	if (getThis()) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "b", &flag) == FAILURE) {
			return;
		}
	}
	else {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Ob", &z_descriptor, oci_lob_class_entry_ptr, &flag) == FAILURE) {
			return;
		}	
	}
	...
	if (php_oci_lob_set_buffering(descriptor, flag, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(oci_lob_copy)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "OO|l", &z_descriptor_dest, oci_lob_class_entry_ptr, &z_descriptor_from, oci_lob_class_entry_ptr, &length) == FAILURE) {
		return;
	}
	...
	if (php_oci_lob_copy(descriptor_dest, descriptor_from, length, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(oci_lob_is_equal)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "OO", &z_descriptor_first, oci_lob_class_entry_ptr, &z_descriptor_second, oci_lob_class_entry_ptr) == FAILURE) {
		return;
	}
	...
	if (php_oci_lob_is_equal(descriptor_first, descriptor_second, &is_equal, TSRMLS_C)) {
		RETURN_FALSE;
	}
...
{	
	...
	if (getThis()) {
#if (PHP_MAJOR_VERSION == 5 && PHP_MINOR_VERSION > 3) || (PHP_MAJOR_VERSION > 5)
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "p|ll", &filename, &filename_len, &start, &length) == FAILURE) {
#else
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|ll", &filename, &filename_len, &start, &length) == FAILURE) {
#endif
			return;
		}
	
		if (ZEND_NUM_ARGS() > 1 && start < 0) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Start parameter must be greater than or equal to 0");
			RETURN_FALSE;
		}
		if (ZEND_NUM_ARGS() > 2 && length < 0) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Length parameter must be greater than or equal to 0");
			RETURN_FALSE;
		}
	}
	else {
#if (PHP_MAJOR_VERSION == 5 && PHP_MINOR_VERSION > 3) || (PHP_MAJOR_VERSION > 5)
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Op|ll", &z_descriptor, oci_lob_class_entry_ptr, &filename, &filename_len, &start, &length) == FAILURE) {
#else
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Os|ll", &z_descriptor, oci_lob_class_entry_ptr, &filename, &filename_len, &start, &length) == FAILURE) {
#endif
			return;
		}
	...
	while(length > 0) {
		...
		if (php_oci_lob_read(descriptor, block_length, start, &buffer, &tmp_bytes_read, TSRMLS_C)) {
...
PHP_FUNCTION(oci_lob_write_temporary)
{
	...
	if (getThis()) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|l", &data, &data_len, &type) == FAILURE) {
			return;
		}
	}
	else {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Os|l", &z_descriptor, oci_lob_class_entry_ptr, &data, &data_len, &type) == FAILURE) {
			return;
		}	
	}
	...
	if (php_oci_lob_write_tmp(descriptor, type, data, data_len, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(oci_lob_close)
{
	...
	if (php_oci_lob_close(descriptor, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(oci_new_descriptor)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r|l", &z_connection, &type) == FAILURE) {
		return;
	}
	...
	descriptor = php_oci_lob_create(connection, type, TSRMLS_C);	
...
PHP_FUNCTION(oci_rollback)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r", &z_connection) == FAILURE) {
		return;
	}
	...
	if (connection->descriptors) {
		php_oci_connection_descriptors_free(connection, TSRMLS_C);
	}

	if (php_oci_connection_rollback(connection, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(oci_commit)
{
	...
	if (php_oci_connection_commit(connection, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(oci_internal_debug)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "b", &on_off) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(oci_execute)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r|l", &z_statement, &mode) == FAILURE) {
		return;
	}

	PHP_OCI_ZVAL_TO_STATEMENT(z_statement, statement);

	if (php_oci_statement_execute(statement, mode, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(oci_cancel)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r", &z_statement) == FAILURE) {
		return;
	}

	PHP_OCI_ZVAL_TO_STATEMENT(z_statement, statement);

	if (php_oci_statement_cancel(statement, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(oci_fetch)
{
	...
	if (php_oci_statement_fetch(statement, nrows, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(oci_fetch_all)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rz/|lll", &z_statement, &array, &skip, &maxrows, &flags) == FAILURE) {
		return;
	}
	...
	if (flags & PHP_OCI_FETCHSTATEMENT_BY_ROW) {
		...
		for (i = 0; i < statement->ncolumns; i++) {
			columns[ i ] = php_oci_statement_get_column(statement, i + 1, NULL, 0, TSRMLS_C);
		}

		while (!php_oci_statement_fetch(statement, nrows, TSRMLS_C)) {
			zval *row;
			...
			for (i = 0; i < statement->ncolumns; i++) {
				...
				php_oci_column_to_zval(columns[ i ], element, PHP_OCI_RETURN_LOBS, TSRMLS_C);
			...
			if (maxrows != -1 && rows == maxrows) {
				php_oci_statement_cancel(statement, TSRMLS_C);
				break;
			}
		}
		efree(columns);
...
PHP_FUNCTION(oci_error)
{
	...
	if (ZEND_NUM_ARGS() > 0) {
		statement = (php_oci_statement *) zend_fetch_resource(&arg, TSRMLS_C, -1, NULL, NULL, 1, le_statement);
	
		if (statement) {
			errh = statement->err;
			error = statement->errcode;

			if (php_oci_fetch_sqltext_offset(statement, &sqltext, &error_offset, TSRMLS_C)) {
				RETURN_FALSE;
			}
			goto go_out;
		}

		connection = (php_oci_connection *) zend_fetch_resource(&arg, TSRMLS_C, -1, NULL, NULL, 1, le_connection);
		if (connection) {
			errh = connection->err;
		...
		connection = (php_oci_connection *) zend_fetch_resource(&arg, TSRMLS_C, -1, NULL, NULL, 1, le_pconnection);
		if (connection) {
			errh = connection->err;
			error = connection->errcode;
			goto go_out;
...
go_out:
	...
	if (!errh) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Oci_error: unable to find error handle");
		RETURN_FALSE;
	}

	errcode = php_oci_fetch_errmsg(errh, &errbuf, TSRMLS_C);
...
PHP_FUNCTION(oci_parse)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rs", &z_connection, &query, &query_len) == FAILURE) {
		return;
	}

	PHP_OCI_ZVAL_TO_CONNECTION(z_connection, connection);

	statement = php_oci_statement_create(connection, query, query_len, TSRMLS_C);
...
PHP_FUNCTION(oci_set_prefetch)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rl", &z_statement, &size) == FAILURE) {
		return;
	}

	PHP_OCI_ZVAL_TO_STATEMENT(z_statement, statement);

	if (php_oci_statement_set_prefetch(statement, size, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(oci_set_client_identifier)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rs", &z_connection, &client_id, &client_id_len) == FAILURE) {
		return;
	}
	...
	if (OCI_G(errcode) != OCI_SUCCESS) {
		php_oci_error(OCI_G(err), OCI_G(errcode), TSRMLS_C);
		RETURN_FALSE;
	}
...
PHP_FUNCTION(oci_set_edition)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s", &edition, &edition_len) == FAILURE) {
		return;
	}
	...
	RETURN_TRUE;
#else
	php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Unsupported attribute type");
	RETURN_FALSE;
#endif
}
/* }}} */
...
PHP_FUNCTION(oci_set_module_name)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rs", &z_connection, &module, &module_len) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(oci_set_action)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rs", &z_connection, &action, &action_len) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(oci_set_client_info)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rs", &z_connection, &client_info, &client_info_len) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(oci_password_change)
{
	...
	/* Safe mode has been removed in PHP 5.4 */
	if (PG(safe_mode)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "is disabled in Safe Mode");
		RETURN_FALSE;
	}
#endif

	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS(), TSRMLS_C, "rsss", &z_connection, &user, &user_len, &pass_old, &pass_old_len, &pass_new, &pass_new_len) == SUCCESS) {
		PHP_OCI_ZVAL_TO_CONNECTION(z_connection, connection);

		if (!user_len) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "username cannot be empty");
			RETURN_FALSE;
		}
		if (!pass_old_len) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "old password cannot be empty");
			RETURN_FALSE;
		}
		if (!pass_new_len) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "new password cannot be empty");
			RETURN_FALSE;
		}

		if (php_oci_password_change(connection, user, user_len, pass_old, pass_old_len, pass_new, pass_new_len, TSRMLS_C)) {
			RETURN_FALSE;
		}
		RETURN_TRUE;
	} else if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS(), TSRMLS_C, "ssss", &dbname, &dbname_len, &user, &user_len, &pass_old, &pass_old_len, &pass_new, &pass_new_len) == SUCCESS) {
		...
		connection = php_oci_do_connect_ex(user, user_len, pass_old, pass_old_len, pass_new, pass_new_len, dbname, dbname_len, NULL, OCI_DEFAULT, 0, 0, TSRMLS_C);
		if (!connection) {
			RETURN_FALSE;
		}
...
PHP_FUNCTION(oci_new_cursor)
{
	...
	statement = php_oci_statement_create(connection, NULL, 0, TSRMLS_C);
...
PHP_FUNCTION(oci_result)
{
	...
	if(column) {
		php_oci_column_to_zval(column, return_value, 0, TSRMLS_C);
	}
	else {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(oci_client_version)
{
	...
	php_oci_client_get_version(&version, TSRMLS_C);
...
PHP_FUNCTION(oci_server_version)
{
	...
	if (php_oci_server_get_version(connection, &version, TSRMLS_C)) {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(oci_statement_type)
{
	...
	if (php_oci_statement_get_type(statement, &type, TSRMLS_C)) {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(oci_num_rows)
{
	...
	if (php_oci_statement_get_numrows(statement, &rowcount, TSRMLS_C)) {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(oci_free_collection)
{
	...
	if (!getThis()) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "O", &z_collection, oci_coll_class_entry_ptr) == FAILURE) {
			return;
		}	
	}
	
	if (zend_hash_find(Z_OBJPROP_P(z_collection), "collection", sizeof("collection"), (void **)&tmp) == FAILURE) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to find collection property");
		RETURN_FALSE;
	}
...
PHP_FUNCTION(oci_collection_append)
{
	...
	if (getThis()) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s", &value, &value_len) == FAILURE) {
			return;
		}
	}
	else {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Os", &z_collection, oci_coll_class_entry_ptr, &value, &value_len) == FAILURE) {
			return;
		}	
	}
	...
	if (php_oci_collection_append(collection, value, value_len, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(oci_collection_element_get)
{
	...
	if (getThis()) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l", &element_index) == FAILURE) {
			return;
		}
	}
	else {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Ol", &z_collection, oci_coll_class_entry_ptr, &element_index) == FAILURE) {
			return;
		}	
	}
	...
	if (php_oci_collection_element_get(collection, element_index, &value, TSRMLS_C)) {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(oci_collection_assign)
{
	...
	if (getThis()) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "O", &z_collection_from, oci_coll_class_entry_ptr) == FAILURE) {
			return;
		}
	}
	else {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "OO", &z_collection_dest, oci_coll_class_entry_ptr, &z_collection_from, oci_coll_class_entry_ptr) == FAILURE) {
			return;
		}	
	}
	
	if (zend_hash_find(Z_OBJPROP_P(z_collection_dest), "collection", sizeof("collection"), (void **)&tmp_dest) == FAILURE) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to find collection property. The first argument should be valid collection object");
		RETURN_FALSE;
	}

	if (zend_hash_find(Z_OBJPROP_P(z_collection_from), "collection", sizeof("collection"), (void **)&tmp_from) == FAILURE) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to find collection property. The second argument should be valid collection object");
		RETURN_FALSE;
	}
	...
	if (php_oci_collection_assign(collection_dest, collection_from, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(oci_collection_element_assign)
{
	...
	if (getThis()) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ls", &element_index, &value, &value_len) == FAILURE) {
			return;
		}
	}
	else {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Ols", &z_collection, oci_coll_class_entry_ptr, &element_index, &value, &value_len) == FAILURE) {
			return;
		}	
	}
	...
	if (php_oci_collection_element_set(collection, element_index, value, value_len, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(oci_collection_size)
{
	...
	if (php_oci_collection_size(collection, &size, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_LONG(size);
}
...
PHP_FUNCTION(oci_collection_max)
{
	...
	if (php_oci_collection_max(collection, &max, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_LONG(max);
}
...
PHP_FUNCTION(oci_collection_trim)
{
	...
	if (getThis()) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l", &trim_size) == FAILURE) {
			return;
		}
	}
	else {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Ol", &z_collection, oci_coll_class_entry_ptr, &trim_size) == FAILURE) {
			return;
		}	
	}
	...
	if (php_oci_collection_trim(collection, trim_size, TSRMLS_C)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;	
}
...
PHP_FUNCTION(oci_new_collection)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rs|s", &z_connection, &tdo, &tdo_len, &schema, &schema_len) == FAILURE) {
		return;
	}
	
	PHP_OCI_ZVAL_TO_CONNECTION(z_connection, connection);
	
	if ( (collection = php_oci_collection_create(connection, tdo, tdo_len, schema, schema_len, TSRMLS_C)) ) {
