...
PHP_FUNCTION( normalizer_normalize )
{
	...
	/* Parse parameters. */
	if( zend_parse_method_parameters( ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "s|l",
				&input, &input_len, &form ) == FAILURE )
	{
		intl_error_set( NULL, U_ILLEGAL_ARGUMENT_ERROR,
						 "normalizer_normalize: unable to parse input params", 0, TSRMLS_C );
	...
	switch(form) {
		...
		default:
			intl_error_set( NULL, U_ILLEGAL_ARGUMENT_ERROR,
						"normalizer_normalize: illegal normalization form", 0, TSRMLS_C );
			RETURN_FALSE;
	}
	...
	if ( size_needed > uret_len ) {
		...
		if( U_FAILURE(status)  ) {
			/* Set error messages. */
			intl_error_set_custom_msg( NULL,"Error normalizing string", 0, TSRMLS_C );
			efree( uret_buf );
			efree( uinput );
			RETURN_FALSE;
		}
	...
	if( U_FAILURE( status ) )
	{
		intl_error_set( NULL, status,
				"normalizer_normalize: error converting normalized text UTF-8", 0, TSRMLS_C );
		RETURN_FALSE;
	}
...
PHP_FUNCTION( normalizer_is_normalized )
{
	...
	{
		intl_error_set( NULL, U_ILLEGAL_ARGUMENT_ERROR,
				"normalizer_is_normalized: unable to parse input params", 0, TSRMLS_C );
	...
	if( U_FAILURE( status ) )
	{
		...
		/* Set error messages. */
		intl_error_set_custom_msg( NULL, "Error converting string to UTF-16.", 0, TSRMLS_C );
		if (uinput) {
			efree( uinput );
		}
		RETURN_FALSE;
	...
	if( U_FAILURE(status)  ) {
		/* Set error messages. */
		intl_error_set_custom_msg( NULL,"Error testing if string is the given normalization form.", 0, TSRMLS_C );
		RETURN_FALSE;
	}
