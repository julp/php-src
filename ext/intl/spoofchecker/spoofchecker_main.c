...
PHP_METHOD(Spoofchecker, isSuspicious)
{
	...
	if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|z", &text, &text_len, &error_code)) {
		return;
	}
	...
	if (U_FAILURE(SPOOFCHECKER_ERROR_CODE(co))) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "(%d) %s", SPOOFCHECKER_ERROR_CODE(co), u_errorName(SPOOFCHECKER_ERROR_CODE(co)));
		RETURN_TRUE;
	}
...
PHP_METHOD(Spoofchecker, areConfusable)
{
	...
	if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ss|z", &s1, &s1_len,
...
PHP_METHOD(Spoofchecker, setAllowedLocales)
{
	...
	if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s", &locales, &locales_len)) {
		return;
	}
...
PHP_METHOD(Spoofchecker, setChecks)
{
	...
	if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l", &checks)) {
		return;
	}
