...
U_CFUNC int intl_datetime_decompose(zval *z, double *millis, TimeZone **tz,
		intl_error *err, const char *func, TSRMLS_D)
{
	zval	retval;
	zval	*zfuncname;
	char	*message;
	...
	if (millis) {
		...
		if (call_user_function(NULL, &(z), zfuncname, &retval, 0, NULL, TSRMLS_C)
	...
	if (tz) {
		...
		datetime = (php_date_obj*)zend_object_store_get_object(z, TSRMLS_C);
		if (!datetime->time) {
...
U_CFUNC double intl_zval_to_millis(zval *z, intl_error *err, const char *func, TSRMLS_D)
{
	double	rv = NAN;
	long	lv;
	int		type;
	...
	case IS_DOUBLE:
		...
		break;
	case IS_OBJECT:
		if (instanceof_function(Z_OBJCE_P(z), php_date_get_date_ce(), TSRMLS_C)) {
			intl_datetime_decompose(z, &rv, NULL, err, func, TSRMLS_C);
		} else if (instanceof_function(Z_OBJCE_P(z), Calendar_ce_ptr, TSRMLS_C)) {
			Calendar_object *co = (Calendar_object *)
				zend_object_store_get_object(z, TSRMLS_C );
			if (co->ucal == NULL) {
				...
				if (U_FAILURE(status)) {
					...
					intl_errors_set(err, status, message, 1, TSRMLS_C);
					efree(message);
				}
			}
		} else {
