...
#define INTLITERATOR_METHOD_FETCH_OBJECT\
	...
	if (ii->iterator == NULL) { \
		intl_errors_set(&ii->err, U_ILLEGAL_ARGUMENT_ERROR, "Found unconstructed IntlIterator", 0, TSRMLS_C); \
		RETURN_FALSE; \
	}
...
typedef struct {
	...
	zval					*current;
	zval					*wrapping_obj;
	void					(*destroy_it)(zend_object_iterator	*iterator, TSRMLS_D);
} zoi_with_current;

extern zend_class_entry *IntlIterator_ce_ptr;
extern zend_object_handlers IntlIterator_handlers;

U_CFUNC void zoi_with_current_dtor(zend_object_iterator *iter, TSRMLS_D);
