...
*/
static void internal_format(IntlDateFormatter_object *dfo, UDate timestamp, zval *return_value, TSRMLS_D)
{
...
static int32_t internal_get_arr_ele(IntlDateFormatter_object *dfo,
		HashTable* hash_arr, char* key_name, intl_error *err, TSRMLS_D)
{
	zval	**ele_value	= NULL;
	int32_t	result		= 0;
	char	*message;
	...
	if (zend_hash_find(hash_arr, key_name, strlen(key_name) + 1,
		...
		if(Z_TYPE_PP(ele_value) != IS_LONG) {
			...
			intl_errors_set(err, U_ILLEGAL_ARGUMENT_ERROR, message, 1, TSRMLS_C);
			efree(message);
		} else {
...
#define INTL_GET_ELEM(elem) \
	internal_get_arr_ele(dfo, hash_arr, (elem), err, TSRMLS_C)
...
#undef INTL_GET_ELEM
	...
	if (INTL_DATA_ERROR_CODE(dfo) != U_ZERO_ERROR) {
		intl_errors_set(err, INTL_DATA_ERROR_CODE(dfo), "datefmt_format: "
				"error cloning calendar", 0, TSRMLS_C);
		return 0;
	}
...
PHP_FUNCTION(datefmt_format) 
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Oz",
		...
		intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR, "datefmt_format: unable "
				"to parse input params", 0, TSRMLS_C );
		RETURN_FALSE;
	}
	...
	if (Z_TYPE_P(zarg) == IS_ARRAY) {
		...
		timestamp = internal_get_timestamp(dfo, hash_arr, TSRMLS_C);
		...
		timestamp = intl_zval_to_millis(zarg, INTL_DATA_ERROR_P(dfo),
				"datefmt_format", TSRMLS_C);
		if (U_FAILURE(INTL_DATA_ERROR_CODE(dfo))) {
			RETURN_FALSE;
		}
	}
	
	internal_format( dfo, timestamp, return_value, TSRMLS_C);
}
