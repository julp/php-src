...
U_CFUNC PHP_FUNCTION(datefmt_get_timezone_id)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "O",
		...
		intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR,	"datefmt_get_timezone_"
				"id: unable to parse input params", 0, TSRMLS_C);
		RETURN_FALSE;
	}
...
U_CFUNC PHP_FUNCTION(datefmt_get_timezone)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "O",
		...
		intl_error_set( NULL, U_ILLEGAL_ARGUMENT_ERROR,
			"datefmt_get_timezone: unable to parse input params", 0, TSRMLS_C );
		RETURN_FALSE;
	}
	...
	timezone_object_construct(tz_clone, return_value, 1, TSRMLS_C);
}

U_CFUNC PHP_FUNCTION(datefmt_set_timezone_id)
{
	php_error_docref0(NULL, TSRMLS_C, E_DEPRECATED,
...
U_CFUNC PHP_FUNCTION(datefmt_set_timezone)
{
	...
	if ( zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(),
		...
		intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR, "datefmt_set_timezone: "
				"unable to parse input params", 0, TSRMLS_C);
		RETURN_FALSE;
	}
	...
	timezone = timezone_process_timezone_argument(timezone_zv,
			INTL_DATA_ERROR_P(dfo), "datefmt_set_timezone", TSRMLS_C);
	if (timezone == NULL) {
		RETURN_FALSE;
	}
...
U_CFUNC PHP_FUNCTION(datefmt_get_calendar)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "O",
			&object, IntlDateFormatter_ce_ptr ) == FAILURE) {
		intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR,
			"datefmt_get_calendar: unable to parse input params", 0, TSRMLS_C);
		RETURN_FALSE;
	}
...
U_CFUNC PHP_FUNCTION(datefmt_get_calendar_object)
{
	...
	calendar_object_create(return_value, cal_clone, TSRMLS_C);
}
/* }}} */
...
U_CFUNC PHP_FUNCTION(datefmt_set_calendar)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Oz",
			&object, IntlDateFormatter_ce_ptr, &calendar_zv) == FAILURE) {
		intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR,
			"datefmt_set_calendar: unable to parse input params", 0, TSRMLS_C);
		RETURN_FALSE;
	}
