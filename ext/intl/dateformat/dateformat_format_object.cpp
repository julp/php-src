...
U_CFUNC PHP_FUNCTION(datefmt_format_object)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "o|Zs!",
	...
	if (format == NULL || Z_TYPE_PP(format) == IS_NULL) {
		...
		if (!valid_format(z)) {
			intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR,
					...
					"element of the array) is not valid", 0, TSRMLS_C);
			RETURN_FALSE;
		}
		...
		if (!valid_format(z)) {
			intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR,
					...
					"second element of the array) is not valid", 0, TSRMLS_C);
			RETURN_FALSE;
		}
		...
		if (Z_STRLEN_PP(format) == 0) {
			intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR,
					"datefmt_format_object: the format is empty", 0, TSRMLS_C);
			RETURN_FALSE;
		}
		pattern = true;
	}
	...
	if (instanceof_function(instance_ce, Calendar_ce_ptr, TSRMLS_C)) {
		Calendar *obj_cal = calendar_fetch_native_calendar(object, TSRMLS_C);
		if (obj_cal == NULL) {
			intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR,
					"datefmt_format_object: bad IntlCalendar instance: "
					"not initialized properly", 0, TSRMLS_C);
			RETURN_FALSE;
		}
		...
		if (U_FAILURE(status)) {
			intl_error_set(NULL, status,
					...
					"IntlCalendar", 0, TSRMLS_C);
			RETVAL_FALSE;
			goto cleanup;
		}
		cal = obj_cal->clone();
	} else if (instanceof_function(instance_ce, php_date_get_date_ce(), TSRMLS_C)) {
		if (intl_datetime_decompose(object, &date, &timeZone, NULL,
				"datefmt_format_object", TSRMLS_C) == FAILURE) {
			RETURN_FALSE;
		}
