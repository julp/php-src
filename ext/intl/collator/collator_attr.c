...
PHP_FUNCTION( collator_get_attribute )
{
		...
		&object, Collator_ce_ptr, &attribute ) == FAILURE )
	{
		intl_error_set( NULL, U_ILLEGAL_ARGUMENT_ERROR,
			"collator_get_attribute: unable to parse input params", 0, TSRMLS_C );
...
PHP_FUNCTION( collator_set_attribute )
{
	...
	/* Parse parameters. */
	if( zend_parse_method_parameters( ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Oll",
		&object, Collator_ce_ptr, &attribute, &value ) == FAILURE)
	{
		intl_error_set( NULL, U_ILLEGAL_ARGUMENT_ERROR,
			 "collator_set_attribute: unable to parse input params", 0, TSRMLS_C );
...
PHP_FUNCTION( collator_get_strength )
{
	...
	{
		intl_error_set( NULL, U_ILLEGAL_ARGUMENT_ERROR,
			 "collator_get_strength: unable to parse input params", 0, TSRMLS_C );
...
PHP_FUNCTION( collator_set_strength )
{
	...
	{
		intl_error_set( NULL, U_ILLEGAL_ARGUMENT_ERROR,
			 "collator_set_strength: unable to parse input params", 0, TSRMLS_C );
