...
PHP_FUNCTION( collator_compare )
{
	...
	/* Parse parameters. */
	if( zend_parse_method_parameters( ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Oss",
		&object, Collator_ce_ptr, &str1, &str1_len, &str2, &str2_len ) == FAILURE )
	{
		intl_error_set( NULL, U_ILLEGAL_ARGUMENT_ERROR,
			 "collator_compare: unable to parse input params", 0, TSRMLS_C );
	...
	if( U_FAILURE( COLLATOR_ERROR_CODE( co ) ) )
	{
		...
		/* Set error messages. */
		intl_errors_set_custom_msg( COLLATOR_ERROR_P( co ),
			"Error converting second argument to UTF-16", 0, TSRMLS_C );
		if (ustr1) {
			efree( ustr1 );
		}
		if (ustr2) {
