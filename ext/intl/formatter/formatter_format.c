...
PHP_FUNCTION( numfmt_format )
{
	...
	/* Parse parameters. */
	if( zend_parse_method_parameters( ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "OZ|l",
		&object, NumberFormatter_ce_ptr,  &number, &type ) == FAILURE )
	{
		intl_error_set( NULL, U_ILLEGAL_ARGUMENT_ERROR,
			"numfmt_format: unable to parse input params", 0, TSRMLS_C );
	...
	if(Z_TYPE_PP(number) != IS_DOUBLE && Z_TYPE_PP(number) != IS_LONG) {
		...
		convert_scalar_to_number( *number, TSRMLS_C );
	}
	...
	switch(type) {
		case FORMAT_TYPE_INT32:
			...
			if (INTL_DATA_ERROR_CODE(nfo) == U_BUFFER_OVERFLOW_ERROR) {
				intl_error_reset(INTL_DATA_ERROR_P(nfo), TSRMLS_C); 
...
PHP_FUNCTION( numfmt_format_currency )
{
	...
	/* Parse parameters. */
	if( zend_parse_method_parameters( ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Ods",
		&object, NumberFormatter_ce_ptr,  &number, &currency, &currency_len ) == FAILURE )
	{
		intl_error_set( NULL, U_ILLEGAL_ARGUMENT_ERROR,
			"numfmt_format_currency: unable to parse input params", 0, TSRMLS_C );
	...
	if( U_FAILURE( INTL_DATA_ERROR_CODE((nfo)) ) ) {
		intl_error_set_code( NULL, INTL_DATA_ERROR_CODE((nfo)), TSRMLS_C );
		intl_errors_set_custom_msg( INTL_DATA_ERROR_P(nfo), "Number formatting failed", 0, TSRMLS_C );
		RETVAL_FALSE;
