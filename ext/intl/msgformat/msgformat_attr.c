...
PHP_FUNCTION( msgfmt_get_pattern )
{
	...
	/* Parse parameters. */
	if( zend_parse_method_parameters( ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "O", &object, MessageFormatter_ce_ptr ) == FAILURE )
	{
		intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR,	
			"msgfmt_get_pattern: unable to parse input params", 0, TSRMLS_C );
		RETURN_FALSE;
	}
...
PHP_FUNCTION( msgfmt_set_pattern )
{
	...
	{
		intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR,	
			"msgfmt_set_pattern: unable to parse input params", 0, TSRMLS_C);
		RETURN_FALSE;
	}
	...
	if(msgformat_fix_quotes(&spattern, &spattern_len, &INTL_DATA_ERROR_CODE(mfo)) != SUCCESS) {
		intl_error_set( NULL, U_INVALID_FORMAT_ERROR,
			"msgfmt_set_pattern: error converting pattern to quote-friendly format", 0, TSRMLS_C );
		RETURN_FALSE;
	}
#endif
...
PHP_FUNCTION( msgfmt_get_locale )
{
	...
	{
		intl_error_set( NULL, U_ILLEGAL_ARGUMENT_ERROR,
			"msgfmt_get_locale: unable to parse input params", 0, TSRMLS_C );
