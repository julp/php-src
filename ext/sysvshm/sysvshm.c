...
/* {{{ php_release_sysvshm
 */
static void php_release_sysvshm(zend_rsrc_list_entry *rsrc, TSRMLS_D)
{
...
PHP_FUNCTION(shm_attach)
{
	...
	if (SUCCESS != zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l|ll", &shm_key, &shm_size, &shm_flag)) {
		return;
	}

	if (shm_size < 1) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Segment size must be greater than zero");
		RETURN_FALSE;
  	}
	...
	if ((shm_id = shmget(shm_key, 0, 0)) < 0) {
		if (shm_size < sizeof(sysvshm_chunk_head)) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "failed for key 0x%lx: memorysize too small", shm_key);
			efree(shm_list_ptr);
			RETURN_FALSE;
		}
		if ((shm_id = shmget(shm_key, shm_size, shm_flag | IPC_CREAT | IPC_EXCL)) < 0) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "failed for key 0x%lx: %s", shm_key, strerror(errno));
			efree(shm_list_ptr);
			RETURN_FALSE;
		}
	}
...
PHP_FUNCTION(shm_detach)
{
	...
	if (SUCCESS != zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r", &shm_id)) {
		return;
	}
...
PHP_FUNCTION(shm_remove)
{
	...
	if (shmctl(shm_list_ptr->id, IPC_RMID, NULL) < 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "failed for key 0x%x, id %ld: %s", shm_list_ptr->key, Z_LVAL_P(shm_id), strerror(errno));
		RETURN_FALSE;
	}
...
PHP_FUNCTION(shm_put_var)
{
	...
	if (SUCCESS != zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rlz", &shm_id, &shm_key, &arg_var)) {
		return;
	}
	
	/* setup string-variable and serialize */
	PHP_VAR_SERIALIZE_INIT(var_hash);
	php_var_serialize(&shm_var, &arg_var, &var_hash, TSRMLS_C);
	PHP_VAR_SERIALIZE_DESTROY(var_hash);
	
	shm_list_ptr = zend_fetch_resource(&shm_id, TSRMLS_C, -1, PHP_SHM_RSRC_NAME, NULL, 1, php_sysvshm.le_shm);
	if (!shm_list_ptr) {
	...
	if (ret == -1) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "not enough shared memory left");
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(shm_get_var)
{
	...
	if (SUCCESS != zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rl", &shm_id, &shm_key)) {
		return;
	}
	SHM_FETCH_RESOURCE(shm_list_ptr, shm_id);
	...
	if (shm_varpos < 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "variable key %ld doesn't exist", shm_key);
		RETURN_FALSE;
	}
	...
	if (php_var_unserialize(&return_value, (const unsigned char **) &shm_data, (unsigned char *) shm_data + shm_var->length, &var_hash, TSRMLS_C) != 1) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "variable data in shared memory is corrupted");
		RETVAL_FALSE;
	}
