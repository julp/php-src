...
static void
MYSQLND_METHOD(mysqlnd_conn_data, free_options)(MYSQLND_CONN_DATA * conn, TSRMLS_D)
{
...
static void
MYSQLND_METHOD(mysqlnd_conn_data, free_contents)(MYSQLND_CONN_DATA * conn, TSRMLS_D)
{
	zend_bool pers = conn->persistent;
	...
	if (conn->current_result) {
		conn->current_result->m.free_result(conn->current_result, TRUE, TSRMLS_C);
		conn->current_result = NULL;
	}

	if (conn->net) {
		conn->net->data->m.free_contents(conn->net, TSRMLS_C);
	}
...
static void
MYSQLND_METHOD_PRIVATE(mysqlnd_conn_data, dtor)(MYSQLND_CONN_DATA * conn, TSRMLS_D)
{
	DBG_ENTER("mysqlnd_conn_data::dtor");
	DBG_INF_FMT("conn=%llu", conn->thread_id);

	conn->m->free_contents(conn, TSRMLS_C);
	conn->m->free_options(conn, TSRMLS_C);

	if (conn->net) {
		mysqlnd_net_free(conn->net, conn->stats, conn->error_info, TSRMLS_C);
		conn->net = NULL;
	}

	if (conn->protocol) {
		mysqlnd_protocol_free(conn->protocol, TSRMLS_C);
		conn->protocol = NULL;
	}
...
MYSQLND_METHOD(mysqlnd_conn_data, simple_command_handle_response)(MYSQLND_CONN_DATA * conn, enum mysqlnd_packet_type ok_packet,
															 ...
															 zend_bool ignore_upsert_status, TSRMLS_D)
{
	...
	switch (ok_packet) {
		case PROT_OK_PACKET:{
			MYSQLND_PACKET_OK * ok_response = conn->protocol->m.get_ok_packet(conn->protocol, FALSE, TSRMLS_C);
			if (!ok_response) {
			...
			if (FAIL == (ret = PACKET_READ(ok_response, conn))) {
				if (!silent) {
					...
					php_error_docref(NULL, TSRMLS_C, E_WARNING, "Error while reading %s's OK packet. PID=%u",
		...
		case PROT_EOF_PACKET:{
			MYSQLND_PACKET_EOF * ok_response = conn->protocol->m.get_eof_packet(conn->protocol, FALSE, TSRMLS_C);
			if (!ok_response) {
				SET_OOM_ERROR(*conn->error_info);
				break;
			}
			if (FAIL == (ret = PACKET_READ(ok_response, conn))) {
				...
				if (!silent) {
					...
					php_error_docref(NULL, TSRMLS_C, E_WARNING, "Error while reading %s's EOF packet. PID=%d",
									 mysqlnd_command_to_text[command], getpid());
				}
				...
				if (!silent) {
					...
					php_error_docref(NULL, TSRMLS_C, E_WARNING, "EOF packet expected, field count wasn't 0xFE but 0x%2X",
		...
		default:
			SET_CLIENT_ERROR(*conn->error_info, CR_MALFORMED_PACKET, UNKNOWN_SQLSTATE, "Malformed packet");
			php_error_docref(NULL, TSRMLS_C, E_ERROR, "Wrong response packet %u passed to the function", ok_packet);
			break;
	}
...
MYSQLND_METHOD(mysqlnd_conn_data, simple_command_send_request)(MYSQLND_CONN_DATA * conn, enum php_mysqlnd_server_command command,
			   const zend_uchar * const arg, size_t arg_len, zend_bool silent, zend_bool ignore_upsert_status, TSRMLS_D)
{
	...
	cmd_packet = conn->protocol->m.get_command_packet(conn->protocol, FALSE, TSRMLS_C);
	if (!cmd_packet) {
		SET_OOM_ERROR(*conn->error_info);
		DBG_RETURN(FAIL);
	}
	...
	if (! PACKET_WRITE(cmd_packet, conn)) {
		...
		conn->m->send_close(conn, TSRMLS_C);
		DBG_ERR("Server is gone");
		ret = FAIL;
	}
			   ...
			   zend_bool ignore_upsert_status, TSRMLS_D)
{
	...
	ret = conn->m->simple_command_send_request(conn, command, arg, arg_len, silent, ignore_upsert_status, TSRMLS_C);
	if (PASS == ret && ok_packet != PROT_LAST) {
		ret = conn->m->simple_command_handle_response(conn, ok_packet, silent, command, ignore_upsert_status, TSRMLS_C);
	}
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, set_server_option)(MYSQLND_CONN_DATA * const conn, enum_mysqlnd_server_option option, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {

		int2store(buffer, (unsigned int) option);
		ret = conn->m->simple_command(conn, COM_SET_OPTION, buffer, sizeof(buffer), PROT_EOF_PACKET, FALSE, TRUE, TSRMLS_C);
	
		conn->m->local_tx_end(conn, this_func, ret, TSRMLS_C);
	}
	DBG_RETURN(ret);
}
/* }}} */
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, restart_psession)(MYSQLND_CONN_DATA * conn, TSRMLS_D)
{
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, end_psession)(MYSQLND_CONN_DATA * conn, TSRMLS_D)
{
		...
		)
{
	...
	auth_packet = conn->protocol->m.get_auth_packet(conn->protocol, FALSE, TSRMLS_C);
	if (!auth_packet) {
		SET_OOM_ERROR(*conn->error_info);
		goto end;
	}
	...
	if ((greet_packet->server_capabilities & CLIENT_SSL) && (mysql_flags & CLIENT_SSL)) {
		...
		conn->net->data->m.set_client_option(conn->net, MYSQL_OPT_SSL_VERIFY_SERVER_CERT, (const char *) &verify, TSRMLS_C);

		if (FAIL == conn->net->data->m.enable_ssl(conn->net, TSRMLS_C)) {
			goto end;
		}
	}
#endif
...
MYSQLND_METHOD(mysqlnd_conn_data, fetch_auth_plugin_by_name)(const char * const requested_protocol, TSRMLS_D)
{
			...
			,TSRMLS_D)
{
	...
	do {
		struct st_mysqlnd_authentication_plugin * auth_plugin = conn->m->fetch_auth_plugin_by_name(requested_protocol, TSRMLS_C);

		if (!auth_plugin) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "The server requested authentication method unknown to the client [%s]", requested_protocol);
...

		{
			...
			scrambled_data =
				auth_plugin->methods.get_auth_data(NULL, &scrambled_data_len, conn, user, passwd, passwd_len,
												   plugin_data, plugin_data_len, options, &conn->net->data->options, mysql_flags, TSRMLS_C);
	...
	if (ret == PASS) {
		...
		conn->m->set_client_option(conn, MYSQLND_OPT_AUTH_PROTOCOL, requested_protocol, TSRMLS_C);
	}
end:
	if (plugin_data) {
		mnd_efree(plugin_data);
			...
			,TSRMLS_D)
{
	...
	ret = mysqlnd_switch_to_ssl_if_needed(conn, greet_packet, options, mysql_flags, TSRMLS_C);
	if (PASS == ret) {
		ret = mysqlnd_run_authentication(conn, user, passwd, passwd_len, db, db_len,
										 ...
										 greet_packet->charset_no, options, mysql_flags, FALSE /*silent*/, FALSE/*is_change*/, TSRMLS_C);
	}
	DBG_RETURN(ret);
}
/* }}} */
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, execute_init_commands)(MYSQLND_CONN_DATA * conn, TSRMLS_D)
{
	enum_func_status ret = PASS;
	...
	if (conn->options->init_commands) {
		...
		for (; current_command < conn->options->num_commands; ++current_command) {
			...
			if (command) {
				...
				if (PASS != conn->m->query(conn, command, strlen(command), TSRMLS_C)) {
					MYSQLND_INC_CONN_STATISTIC(conn->stats, STAT_INIT_COMMAND_FAILED_COUNT);
					ret = FAIL;
					break;
				}
				if (conn->last_query_type == QUERY_SELECT) {
					MYSQLND_RES * result = conn->m->use_result(conn, TSRMLS_C);
					if (result) {
						result->m.free_result(result, TRUE, TSRMLS_C);
					}
				}
			}
		}
...
static unsigned int
MYSQLND_METHOD(mysqlnd_conn_data, get_updated_connect_flags)(MYSQLND_CONN_DATA * conn, unsigned int mysql_flags, TSRMLS_D)
{
...
MYSQLND_METHOD(mysqlnd_conn_data, connect_handshake)(MYSQLND_CONN_DATA * conn,
						...
						const unsigned int mysql_flags, TSRMLS_D)
{
	...
	greet_packet = conn->protocol->m.get_greet_packet(conn->protocol, FALSE, TSRMLS_C);
	if (!greet_packet) {
		SET_OOM_ERROR(*conn->error_info);
		DBG_RETURN(FAIL); /* OOM */
	}
	...
	if (FAIL == net->data->m.connect_ex(conn->net, conn->scheme, conn->scheme_len, conn->persistent,
										conn->stats, conn->error_info, TSRMLS_C))
	{
		goto err;
	}

	DBG_INF_FMT("stream=%p", net->data->m.get_stream(net, TSRMLS_C));

	if (FAIL == PACKET_READ(greet_packet, conn)) {
		DBG_ERR("Error while reading greeting packet");
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Error while reading greeting packet. PID=%d", getpid());
		goto err;
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Connecting to 3.22, 3.23 & 4.0 "
	...
	if (FAIL == mysqlnd_connect_run_authentication(conn, user, passwd, db, db_len, (size_t) passwd_len,
												   greet_packet, conn->options, mysql_flags, TSRMLS_C))
	{
		goto err;
	}
						...
						, TSRMLS_D)
{
	...
	if (CONN_GET_STATE(conn) > CONN_ALLOCED && CONN_GET_STATE(conn) ) {
		...
		conn->m->set_client_option(conn, MYSQLND_OPT_MAX_ALLOWED_PACKET, (char *)&max_allowed_size, TSRMLS_C);
	}
	...
	mysql_flags = conn->m->get_updated_connect_flags(conn, mysql_flags, TSRMLS_C);

	if (FAIL == conn->m->connect_handshake(conn, host, user, passwd, passwd_len, db, db_len, mysql_flags, TSRMLS_C)) {
		goto err;
	}
		...
		*/
		...
		if (!unix_socket && !named_pipe) {
			...
			if (unix_socket) {
				...
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Impossible. Should be either socket or a pipe. Report a bug!");
			}
		...
		if (FAIL == conn->m->execute_init_commands(conn, TSRMLS_C)) {
			goto err;
		}
		...
		conn->m->local_tx_end(conn, this_func, PASS, TSRMLS_C);
		DBG_RETURN(PASS);
	}
err:
	...
	if (!conn->error_info->error_no) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "[%u] %.128s (trying to connect via %s)",
	...
	if (TRUE == local_tx_started) {
		conn->m->local_tx_end(conn, this_func, FAIL, TSRMLS_C);
	}
						...
						, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		...
		ret = conn->m->connect(conn, host, user, passwd, passwd_len, db, db_len, port, socket_or_pipe, mysql_flags, TSRMLS_C);
						...
						, TSRMLS_D)
{
	...
	ret = conn_handle->m->connect(conn_handle, host, user, passwd, passwd_len, db, db_len, port, socket_or_pipe, mysql_flags, TSRMLS_C);
	...
	if (ret == FAIL) {
		if (self_alloced) {
			...
			*/
			conn_handle->m->dtor(conn_handle, TSRMLS_C);
		}
		DBG_RETURN(NULL);
	}
...
*/
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, query)(MYSQLND_CONN_DATA * conn, const char * query, unsigned int query_len, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		if (PASS == conn->m->send_query(conn, query, query_len, TSRMLS_C) &&
			PASS == conn->m->reap_query(conn, TSRMLS_C))
		{
			ret = PASS;
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, send_query)(MYSQLND_CONN_DATA * conn, const char * query, unsigned int query_len, TSRMLS_D)
{
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, reap_query)(MYSQLND_CONN_DATA * conn, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		if (state <= CONN_READY || state == CONN_QUIT_SENT) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Connection not opened, clear or has been closed");
			DBG_ERR_FMT("Connection not opened, clear or has been closed. State=%u", state);
			DBG_RETURN(ret);
		}
		ret = conn->m->query_read_result_set_header(conn, NULL, TSRMLS_C);
...
MYSQLND ** mysqlnd_stream_array_check_for_readiness(MYSQLND ** conn_array, TSRMLS_D)
{
	int cnt = 0;
...
static int mysqlnd_stream_array_to_fd_set(MYSQLND ** conn_array, fd_set * fds, php_socket_t * max_fd, TSRMLS_D)
{
	...
	while (*p) {
		 ...
		 * is not displayed.
		 * */
		if (SUCCESS == php_stream_cast((*p)->data->net->data->m.get_stream((*p)->data->net, TSRMLS_C), PHP_STREAM_AS_FD_FOR_SELECT | PHP_STREAM_CAST_INTERNAL,
...
static int mysqlnd_stream_array_from_fd_set(MYSQLND ** conn_array, fd_set * fds, TSRMLS_D)
{
	php_socket_t this_fd;
	int ret = 0;
	...
	while (*fwd) {
		if (SUCCESS == php_stream_cast((*fwd)->data->net->data->m.get_stream((*fwd)->data->net, TSRMLS_C), PHP_STREAM_AS_FD_FOR_SELECT | PHP_STREAM_CAST_INTERNAL,
										(void*)&this_fd, 1) && this_fd >= 0) {
...
_mysqlnd_poll(MYSQLND **r_array, MYSQLND **e_array, MYSQLND ***dont_poll, long sec, long usec, uint * desc_num, TSRMLS_D)
{
	struct timeval	tv;
	...
	if (sec < 0 || usec < 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Negative values passed for sec and/or usec");
		DBG_RETURN(FAIL);
	}
	...
	if (r_array != NULL) {
		*dont_poll = mysqlnd_stream_array_check_for_readiness(r_array, TSRMLS_C);
		set_count = mysqlnd_stream_array_to_fd_set(r_array, &rfds, &max_fd, TSRMLS_C);
	...
	if (e_array != NULL) {
		set_count = mysqlnd_stream_array_to_fd_set(e_array, &efds, &max_fd, TSRMLS_C);
		if (set_count > max_set_count) {
			max_set_count = set_count;
		}
		sets += set_count;
	...
	if (!sets) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, *dont_poll ? "All arrays passed are clear":"No stream arrays were passed");
	...
	if (retval == -1) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "unable to select [%d]: %s (max_fd=%d)",
						errno, strerror(errno), max_fd);
		DBG_RETURN(FAIL);
	}

	if (r_array != NULL) {
		mysqlnd_stream_array_from_fd_set(r_array, &rfds, TSRMLS_C);
	}
	if (e_array != NULL) {
		mysqlnd_stream_array_from_fd_set(e_array, &efds, TSRMLS_C);
	}
...
MYSQLND_RES *
MYSQLND_METHOD(mysqlnd_conn_data, list_fields)(MYSQLND_CONN_DATA * conn, const char *table, const char *achtung_wild, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		do {
			...
			if (PASS != conn->m->simple_command(conn, COM_FIELD_LIST, buff, p - buff,
											   PROT_LAST /* we will handle the OK packet*/,
											   FALSE, TRUE, TSRMLS_C)) {
				conn->m->local_tx_end(conn, 0, FAIL, TSRMLS_C);
				break;
			}
			...
			*/
			result = conn->m->result_init(5000, conn->persistent, TSRMLS_C);
			if (!result) {
				break;
			}

			if (FAIL == result->m.read_result_metadata(result, conn, TSRMLS_C)) {
		...
		conn->m->local_tx_end(conn, this_func, result == NULL? FAIL:PASS, TSRMLS_C);
	}
...
MYSQLND_RES *
MYSQLND_METHOD(mysqlnd_conn_data, list_method)(MYSQLND_CONN_DATA * conn, const char * query, const char *achtung_wild, char *par1, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		...
		if (PASS == conn->m->query(conn, show_query, show_query_len, TSRMLS_C)) {
			result = conn->m->store_result(conn, TSRMLS_C);
		}
		if (show_query != query) {
...
static unsigned int
MYSQLND_METHOD(mysqlnd_conn_data, errno)(const MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
...
static const char *
MYSQLND_METHOD(mysqlnd_conn_data, error)(const MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
...
static const char *
MYSQLND_METHOD(mysqlnd_conn_data, sqlstate)(const MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
...
PHPAPI ulong 
mysqlnd_old_escape_string(char * newstr, const char * escapestr, size_t escapestr_len, TSRMLS_D)
{
	DBG_ENTER("mysqlnd_old_escape_string");
	DBG_RETURN(mysqlnd_cset_escape_slashes(mysqlnd_find_charset_name("latin1"), newstr, escapestr, escapestr_len, TSRMLS_C));
}
/* }}} */
...
MYSQLND_METHOD(mysqlnd_conn_data, ssl_set)(MYSQLND_CONN_DATA * const conn, const char * key, const char * const cert,
									  const char * const ca, const char * const capath, const char * const cipher, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		ret = (PASS == net->data->m.set_client_option(net, MYSQLND_OPT_SSL_KEY, key, TSRMLS_C) &&
			PASS == net->data->m.set_client_option(net, MYSQLND_OPT_SSL_CERT, cert, TSRMLS_C) &&
			PASS == net->data->m.set_client_option(net, MYSQLND_OPT_SSL_CA, ca, TSRMLS_C) &&
			PASS == net->data->m.set_client_option(net, MYSQLND_OPT_SSL_CAPATH, capath, TSRMLS_C) &&
			PASS == net->data->m.set_client_option(net, MYSQLND_OPT_SSL_CIPHER, cipher, TSRMLS_C)) ? PASS : FAIL;
...
static ulong
MYSQLND_METHOD(mysqlnd_conn_data, escape_string)(MYSQLND_CONN_DATA * const conn, char * newstr, const char * escapestr, size_t escapestr_len, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		if (conn->upsert_status->server_status & SERVER_STATUS_NO_BACKSLASH_ESCAPES) {
			ret = mysqlnd_cset_escape_quotes(conn->charset, newstr, escapestr, escapestr_len, TSRMLS_C);
		} else {
			ret = mysqlnd_cset_escape_slashes(conn->charset, newstr, escapestr, escapestr_len, TSRMLS_C);
		}
		conn->m->local_tx_end(conn, this_func, PASS, TSRMLS_C);
	}
	DBG_RETURN(ret);
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, dump_debug_info)(MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
	...
	DBG_INF_FMT("conn=%llu", conn->thread_id);
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		ret = conn->m->simple_command(conn, COM_DEBUG, NULL, 0, PROT_EOF_PACKET, FALSE, TRUE, TSRMLS_C);
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, select_db)(MYSQLND_CONN_DATA * const conn, const char * const db, unsigned int db_len, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		ret = conn->m->simple_command(conn, COM_INIT_DB, (zend_uchar*) db, db_len, PROT_OK_PACKET, FALSE, TRUE, TSRMLS_C);
		/*
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, ping)(MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		ret = conn->m->simple_command(conn, COM_PING, NULL, 0, PROT_OK_PACKET, TRUE, TRUE, TSRMLS_C);
		/*
		  The server sends 0 but libmysql doesn't read it and has established
		  a protocol of giving back -1. Thus we have to follow it :(
		*/
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, statistic)(MYSQLND_CONN_DATA * conn, char **message, unsigned int * message_len, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		do {
			ret = conn->m->simple_command(conn, COM_STATISTICS, NULL, 0, PROT_LAST, FALSE, TRUE, TSRMLS_C);
			if (FAIL == ret) {
				break;
			}
			stats_header = conn->protocol->m.get_stats_packet(conn->protocol, FALSE, TSRMLS_C);
			if (!stats_header) {
				SET_OOM_ERROR(*conn->error_info);
				break;
			}
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, kill)(MYSQLND_CONN_DATA * conn, unsigned int pid, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		...
		if (pid != conn->thread_id) {
			ret = conn->m->simple_command(conn, COM_PROCESS_KILL, buff, 4, PROT_OK_PACKET, FALSE, TRUE, TSRMLS_C);
			/*
			  The server sends 0 but libmysql doesn't read it and has established
			  a protocol of giving back -1. Thus we have to follow it :(
			*/
			SET_ERROR_AFF_ROWS(conn);
		} else if (PASS == (ret = conn->m->simple_command(conn, COM_PROCESS_KILL, buff, 4, PROT_LAST, FALSE, TRUE, TSRMLS_C))) {
			CONN_SET_STATE(conn, CONN_QUIT_SENT);
			conn->m->send_close(conn, TSRMLS_C);
		}
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, set_charset)(MYSQLND_CONN_DATA * const conn, const char * const csname, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		...
		if (FAIL == (ret = conn->m->query(conn, query, query_len, TSRMLS_C))) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Error executing query");
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, refresh)(MYSQLND_CONN_DATA * const conn, uint8_t options, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		...
		ret = conn->m->simple_command(conn, COM_REFRESH, bits, 1, PROT_OK_PACKET, FALSE, TRUE, TSRMLS_C);
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, shutdown)(MYSQLND_CONN_DATA * const conn, uint8_t level, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		...
		ret = conn->m->simple_command(conn, COM_SHUTDOWN, bits, 1, PROT_OK_PACKET, FALSE, TRUE, TSRMLS_C);
...
/* {{{ mysqlnd_send_close */
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, send_close)(MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
	enum_func_status ret = PASS;
	MYSQLND_NET * net = conn->net;
	php_stream * net_stream = net->data->m.get_stream(net, TSRMLS_C);
	...
	switch (CONN_GET_STATE(conn)) {
		case CONN_READY:
			...
			if (net_stream) {
				ret = conn->m->simple_command(conn, COM_QUIT, NULL, 0, PROT_LAST, TRUE, TRUE, TSRMLS_C);
				net->data->m.close_stream(net, conn->stats, conn->error_info, TSRMLS_C);
			}
			CONN_SET_STATE(conn, CONN_QUIT_SENT);
			break;
...
MYSQLND_METHOD_PRIVATE(mysqlnd_conn_data, get_reference)(MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
...
static enum_func_status
MYSQLND_METHOD_PRIVATE(mysqlnd_conn_data, free_reference)(MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
	enum_func_status ret = PASS;
	...
	if (!(--conn->refcount)) {
		/*
		  ...
		  reached zero.
		*/
		ret = conn->m->send_close(conn, TSRMLS_C);
		conn->m->dtor(conn, TSRMLS_C);
	}
	DBG_RETURN(ret);
}
/* }}} */
...
MYSQLND_METHOD_PRIVATE(mysqlnd_conn_data, get_state)(const MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
...
static void
MYSQLND_METHOD_PRIVATE(mysqlnd_conn_data, set_state)(MYSQLND_CONN_DATA * const conn, enum mysqlnd_connection_state new_state, TSRMLS_D)
{
...
static unsigned int
MYSQLND_METHOD(mysqlnd_conn_data, field_count)(const MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
...
static unsigned int
MYSQLND_METHOD(mysqlnd_conn_data, server_status)(const MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
...
static uint64_t
MYSQLND_METHOD(mysqlnd_conn_data, insert_id)(const MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
...
static uint64_t
MYSQLND_METHOD(mysqlnd_conn_data, affected_rows)(const MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
...
static unsigned int
MYSQLND_METHOD(mysqlnd_conn_data, warning_count)(const MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
...
static const char *
MYSQLND_METHOD(mysqlnd_conn_data, info)(const MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
...
static const char *
MYSQLND_METHOD(mysqlnd_conn_data, get_server_info)(const MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
...
static const char *
MYSQLND_METHOD(mysqlnd_conn_data, get_host_info)(const MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
...
static unsigned int
MYSQLND_METHOD(mysqlnd_conn_data, get_proto_info)(const MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
...
static const char *
MYSQLND_METHOD(mysqlnd_conn_data, charset_name)(const MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
...
static uint64_t
MYSQLND_METHOD(mysqlnd_conn_data, thread_id)(const MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
...
MYSQLND_METHOD(mysqlnd_conn_data, get_server_version)(const MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
...
static zend_bool
MYSQLND_METHOD(mysqlnd_conn_data, more_results)(const MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, next_result)(MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		do {
			...
			*/
			if (FAIL == (ret = conn->m->query_read_result_set_header(conn, NULL, TSRMLS_C))) {
				/*
				...
				*/
				if (!conn->error_info->error_no) {
					...
					php_error_docref(NULL, TSRMLS_C, E_WARNING, "Serious error. PID=%d", getpid());
					CONN_SET_STATE(conn, CONN_QUIT_SENT);
					conn->m->send_close(conn, TSRMLS_C);
				} else {
										...
										,  TSRMLS_D)
{
	...
	ret = mysqlnd_run_authentication(conn, user, passwd, strlen(passwd), db, strlen(db),
									...
									0 /*charset not used*/, conn->options, conn->server_capabilities, silent, TRUE/*is_change*/, TSRMLS_C);
												...
												,TSRMLS_D)
{
	...
	switch (option) {
		...
		case MYSQL_SERVER_PUBLIC_KEY:
			ret = conn->net->data->m.set_client_option(conn->net, option, value, TSRMLS_C);
			break;
...
static MYSQLND_RES *
MYSQLND_METHOD(mysqlnd_conn_data, use_result)(MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		do {
			...
			conn->current_result->conn = conn->m->get_reference(conn, TSRMLS_C);
			result = conn->current_result->m.use_result(conn->current_result, FALSE, TSRMLS_C);
...
static MYSQLND_RES *
MYSQLND_METHOD(mysqlnd_conn_data, store_result)(MYSQLND_CONN_DATA * const conn, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		do {
			...
			result = conn->current_result->m.store_result(conn->current_result, conn, FALSE, TSRMLS_C);
			if (!result) {
				conn->current_result->m.free_result(conn->current_result, TRUE, TSRMLS_C);
			}
			conn->current_result = NULL;
...
MYSQLND_METHOD(mysqlnd_conn_data, get_connection_stats)(const MYSQLND_CONN_DATA * const conn,
												   zval * return_value, TSRMLS_D ZEND_FILE_LINE_DC)
{
	DBG_ENTER("mysqlnd_conn_data::get_connection_stats");
	mysqlnd_fill_stats_hash(conn->stats, mysqlnd_stats_values_names, return_value, TSRMLS_C ZEND_FILE_LINE_CC);
	DBG_VOID_RETURN;
}
/* }}} */
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, set_autocommit)(MYSQLND_CONN_DATA * conn, unsigned int mode, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		ret = conn->m->query(conn, (mode) ? "SET AUTOCOMMIT=1":"SET AUTOCOMMIT=0", sizeof("SET AUTOCOMMIT=1") - 1, TSRMLS_C);
		conn->m->local_tx_end(conn, this_func, ret, TSRMLS_C);	
	}
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, tx_commit)(MYSQLND_CONN_DATA * conn, TSRMLS_D)
{
	return conn->m->tx_commit_or_rollback(conn, TRUE, TRANS_COR_NO_OPT, NULL, TSRMLS_C);
}
/* }}} */
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, tx_rollback)(MYSQLND_CONN_DATA * conn, TSRMLS_D)
{
	return conn->m->tx_commit_or_rollback(conn, FALSE, TRANS_COR_NO_OPT, NULL, TSRMLS_C);
}
/* }}} */
...
static void
MYSQLND_METHOD(mysqlnd_conn_data, tx_cor_options_to_string)(const MYSQLND_CONN_DATA * const conn, smart_str * str, const unsigned int mode, TSRMLS_D)
{
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, tx_commit_or_rollback)(MYSQLND_CONN_DATA * conn, const zend_bool commit, const unsigned int flags, const char * const name, TSRMLS_D)
{
	...
	if (PASS == conn->m->local_tx_start(conn, this_func, TSRMLS_C)) {
		do {
			...
			conn->m->tx_cor_options_to_string(conn, &tmp_str, flags, TSRMLS_C);
			smart_str_0(&tmp_str);

			{
				...
				ret = conn->m->query(conn, query, query_len, TSRMLS_C);
				mnd_sprintf_free(query);
				if (commented_name) {
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, tx_begin)(MYSQLND_CONN_DATA * conn, const unsigned int mode, const char * const name, TSRMLS_D)
{
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, tx_savepoint)(MYSQLND_CONN_DATA * conn, const char * const name, TSRMLS_D)
{
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, tx_savepoint_release)(MYSQLND_CONN_DATA * conn, const char * const name, TSRMLS_D)
{
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, local_tx_start)(MYSQLND_CONN_DATA * conn, size_t this_func, TSRMLS_D)
{
	enum_func_status ret = PASS;
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, local_tx_end)(MYSQLND_CONN_DATA * conn, size_t this_func, enum_func_status status, TSRMLS_D)
{
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, init)(MYSQLND_CONN_DATA * conn, TSRMLS_D)
{
	DBG_ENTER("mysqlnd_conn_data::init");
	mysqlnd_stats_init(&conn->stats, STAT_LAST);
	SET_ERROR_AFF_ROWS(conn);

	conn->net = mysqlnd_net_init(conn->persistent, conn->stats, conn->error_info, TSRMLS_C);
	conn->protocol = mysqlnd_protocol_init(conn->persistent, TSRMLS_C);
...
MYSQLND_STMT * _mysqlnd_stmt_init(MYSQLND_CONN_DATA * const conn, TSRMLS_D);
...
static MYSQLND *
MYSQLND_METHOD(mysqlnd_conn, clone_object)(MYSQLND * const conn, TSRMLS_D)
{
	MYSQLND * ret;
	DBG_ENTER("mysqlnd_conn::get_reference");
	ret = MYSQLND_CLASS_METHOD_TABLE_NAME(mysqlnd_object_factory).clone_connection_object(conn, TSRMLS_C);
	DBG_RETURN(ret);
}
/* }}} */
...
/* {{{ mysqlnd_conn_data::dtor */
static void
MYSQLND_METHOD_PRIVATE(mysqlnd_conn, dtor)(MYSQLND * conn, TSRMLS_D)
{
	DBG_ENTER("mysqlnd_conn::dtor");
	DBG_INF_FMT("conn=%llu", conn->data->thread_id);

	conn->data->m->free_reference(conn->data, TSRMLS_C);
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn, close)(MYSQLND * conn_handle, enum_connection_close_type close_type, TSRMLS_D)
{
...
/* {{{ _mysqlnd_init */
PHPAPI MYSQLND *
_mysqlnd_init(zend_bool persistent, TSRMLS_D)
{
	MYSQLND * ret;
	DBG_ENTER("mysqlnd_init");
	ret = MYSQLND_CLASS_METHOD_TABLE_NAME(mysqlnd_object_factory).get_connection(persistent, TSRMLS_C);
	DBG_RETURN(ret);
}
/* }}} */
