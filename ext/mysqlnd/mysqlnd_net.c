...
static int
mysqlnd_set_sock_no_delay(php_stream * stream, TSRMLS_D)
{
...
MYSQLND_METHOD(mysqlnd_net, network_read_ex)(MYSQLND_NET * const net, zend_uchar * const buffer, const size_t count,
											 MYSQLND_STATS * const stats, MYSQLND_ERROR_INFO * const error_info, TSRMLS_D)
{
											  ...
											  MYSQLND_STATS * const stats, MYSQLND_ERROR_INFO * const error_info, TSRMLS_D)
{
	...
	ret = php_stream_write(net->data->m.get_stream(net, TSRMLS_C), (char *)buffer, count);
	DBG_RETURN(ret);
}
/* }}} */
...
MYSQLND_METHOD(mysqlnd_net, open_pipe)(MYSQLND_NET * const net, const char * const scheme, const size_t scheme_len,
									   ...
									   MYSQLND_STATS * const conn_stats, MYSQLND_ERROR_INFO * const error_info, TSRMLS_D)
{
												  ...
												  MYSQLND_STATS * const conn_stats, MYSQLND_ERROR_INFO * const error_info, TSRMLS_D)
{
	...
	if (net_stream) {
		...
		if (!memcmp(scheme, "tcp://", sizeof("tcp://") - 1)) {
			/* TCP -> Set TCP_NODELAY */
			mysqlnd_set_sock_no_delay(net_stream, TSRMLS_C);
		}
	}
...
MYSQLND_METHOD(mysqlnd_net, get_open_stream)(MYSQLND_NET * const net, const char * const scheme, const size_t scheme_len,
											 MYSQLND_ERROR_INFO * const error_info, TSRMLS_D)
{
										...
										MYSQLND_STATS * const conn_stats, MYSQLND_ERROR_INFO * const error_info, TSRMLS_D)
{
	...
	net->data->m.close_stream(net, conn_stats, error_info, TSRMLS_C);

	open_stream = net->data->m.get_open_stream(net, scheme, scheme_len, error_info, TSRMLS_C);
	if (open_stream) {
		php_stream * net_stream = open_stream(net, scheme, scheme_len, persistent, conn_stats, error_info, TSRMLS_C);
		if (net_stream) {
			(void) net->data->m.set_stream(net, net_stream, TSRMLS_C);
			net->data->m.post_connect_set_opt(net, scheme, scheme_len, conn_stats, error_info, TSRMLS_C);
			ret = PASS;
		}
	}
	...
	do {
		...
		if (net->data->compressed == TRUE) {
			...
			if (PASS == net->data->m.encode((compress_buf + COMPRESSED_HEADER_SIZE + MYSQLND_HEADER_SIZE), &tmp_complen,
									   uncompressed_payload, to_be_sent + MYSQLND_HEADER_SIZE, TSRMLS_C))
			{
			...
			bytes_sent = net->data->m.network_write_ex(net, compress_buf, payload_size + MYSQLND_HEADER_SIZE + COMPRESSED_HEADER_SIZE,
												 conn_stats, error_info, TSRMLS_C);
...
#endif /* MYSQLND_COMPRESSION_ENABLED */
		{
			...
			bytes_sent = net->data->m.network_write_ex(net, p, to_be_sent + MYSQLND_HEADER_SIZE, conn_stats, error_info, TSRMLS_C);
...
static void
php_mysqlnd_read_buffer_free(MYSQLND_READ_BUFFER ** buffer, TSRMLS_D)
{
...
mysqlnd_create_read_buffer(size_t count, TSRMLS_D)
{
...
MYSQLND_METHOD(mysqlnd_net, read_compressed_packet_from_stream_and_fill_read_buffer)
		(MYSQLND_NET * net, size_t net_payload_size, MYSQLND_STATS * conn_stats, MYSQLND_ERROR_INFO * error_info, TSRMLS_D)
{
	...
	if (FAIL == net->data->m.network_read_ex(net, comp_header, COMPRESSED_HEADER_SIZE, conn_stats, error_info, TSRMLS_C)) {
		DBG_RETURN(FAIL);
	}
	...
	if (decompressed_size) {
		...
		if (FAIL == net->data->m.network_read_ex(net, compressed_data, net_payload_size, conn_stats, error_info, TSRMLS_C)) {
			retval = FAIL;
			goto end;
		}
		net->uncompressed_data = mysqlnd_create_read_buffer(decompressed_size, TSRMLS_C);
		retval = net->data->m.decode(net->uncompressed_data->data, decompressed_size, compressed_data, net_payload_size, TSRMLS_C);
		if (FAIL == retval) {
			goto end;
		}
	} else {
		DBG_INF_FMT("The server decided not to compress the data. Our job is easy. Copying %u bytes", net_payload_size);
		net->uncompressed_data = mysqlnd_create_read_buffer(net_payload_size, TSRMLS_C);
		if (FAIL == net->data->m.network_read_ex(net, net->uncompressed_data->data, net_payload_size, conn_stats, error_info, TSRMLS_C)) {
			retval = FAIL;
			goto end;
		}
	}
...
MYSQLND_METHOD(mysqlnd_net, decode)(zend_uchar * uncompressed_data, const size_t uncompressed_data_len,
									const zend_uchar * const compressed_data, const size_t compressed_data_len, TSRMLS_D)
{
#ifdef MYSQLND_COMPRESSION_ENABLED
	int error;
...
MYSQLND_METHOD(mysqlnd_net, encode)(zend_uchar * compress_buffer, size_t * compress_buffer_len,
									const zend_uchar * const uncompressed_data, const size_t uncompressed_data_len, TSRMLS_D)
{
#ifdef MYSQLND_COMPRESSION_ENABLED
	int error;
	...
	if (net->data->compressed) {
		if (net->uncompressed_data) {
			...
			if (TRUE == net->uncompressed_data->is_empty(net->uncompressed_data)) {
				...
				net->uncompressed_data->free_buffer(&net->uncompressed_data, TSRMLS_C);
			}
		}
		if (to_read) {
			...
			if (FAIL == net->data->m.network_read_ex(net, net_header, MYSQLND_HEADER_SIZE, conn_stats, error_info, TSRMLS_C)) {
				DBG_RETURN(FAIL);
			}
			...
			net->data->m.read_compressed_packet_from_stream_and_fill_read_buffer(net, net_payload_size, conn_stats, error_info, TSRMLS_C);
			...
			*/
			DBG_RETURN(net->data->m.receive_ex(net, p, to_read, conn_stats, error_info, TSRMLS_C));
		}
		DBG_RETURN(PASS);
	}
#endif /* MYSQLND_COMPRESSION_ENABLED */
	DBG_RETURN(net->data->m.network_read_ex(net, p, to_read, conn_stats, error_info, TSRMLS_C));
}
/* }}} */
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_net, set_client_option)(MYSQLND_NET * const net, enum mysqlnd_option option, const char * const value, TSRMLS_D)
{
...
size_t 
MYSQLND_METHOD(mysqlnd_net, consume_uneaten_data)(MYSQLND_NET * const net, enum php_mysqlnd_server_command cmd, TSRMLS_D)
{
	...
	*/
	...
	php_stream * net_stream = net->data->get_stream(net, TSRMLS_C);
	int was_blocked = net_stream->ops->set_option(net_stream, opt, 0, NULL, TSRMLS_C);
	...
	if (PHP_STREAM_OPTION_RETURN_ERR != was_blocked) {
		...
		if (was_blocked) {
			net_stream->ops->set_option(net_stream, opt, 1, NULL, TSRMLS_C);
		}
		...
		if (bytes_consumed) {
			...
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Skipped %u bytes. Last command %s hasn't "
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_net, enable_ssl)(MYSQLND_NET * const net, TSRMLS_D)
{
	...
	if (php_stream_xport_crypto_setup(net_stream, STREAM_CRYPTO_METHOD_TLS_CLIENT, NULL, TSRMLS_C) < 0 ||
	    php_stream_xport_crypto_enable(net_stream, 1, TSRMLS_C) < 0)
	{
		DBG_ERR("Cannot connect to MySQL by using SSL");
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Cannot connect to MySQL by using SSL");
		DBG_RETURN(FAIL);
	}
	net->data->ssl = TRUE;
	/*
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_net, disable_ssl)(MYSQLND_NET * const net, TSRMLS_D)
{
...
static void
MYSQLND_METHOD(mysqlnd_net, free_contents)(MYSQLND_NET * net, TSRMLS_D)
{
	zend_bool pers = net->persistent;
...
static void
MYSQLND_METHOD(mysqlnd_net, close_stream)(MYSQLND_NET * const net, MYSQLND_STATS * const stats, MYSQLND_ERROR_INFO * const error_info, TSRMLS_D)
{
	php_stream * net_stream;
	DBG_ENTER("mysqlnd_net::close_stream");
	if (net && (net_stream = net->data->m.get_stream(net, TSRMLS_C))) {
		zend_bool pers = net->persistent;
		...
		(void) net->data->m.set_stream(net, NULL, TSRMLS_C);
	}
...
/* {{{ mysqlnd_net::init */
static enum_func_status
MYSQLND_METHOD(mysqlnd_net, init)(MYSQLND_NET * const net, MYSQLND_STATS * const stats, MYSQLND_ERROR_INFO * const error_info, TSRMLS_D)
{
	unsigned int buf_size;
	DBG_ENTER("mysqlnd_net::init");

	buf_size = MYSQLND_G(net_cmd_buffer_size); /* this is long, cast to unsigned int*/
	net->data->m.set_client_option(net, MYSQLND_OPT_NET_CMD_BUFFER_SIZE, (char *) &buf_size, TSRMLS_C);

	buf_size = MYSQLND_G(net_read_buffer_size); /* this is long, cast to unsigned int*/
	net->data->m.set_client_option(net, MYSQLND_OPT_NET_READ_BUFFER_SIZE, (char *)&buf_size, TSRMLS_C);

	buf_size = MYSQLND_G(net_read_timeout); /* this is long, cast to unsigned int*/
	net->data->m.set_client_option(net, MYSQL_OPT_READ_TIMEOUT, (char *)&buf_size, TSRMLS_C);
...
/* {{{ mysqlnd_net::dtor */
static void
MYSQLND_METHOD(mysqlnd_net, dtor)(MYSQLND_NET * const net, MYSQLND_STATS * const stats, MYSQLND_ERROR_INFO * const error_info, TSRMLS_D)
{
	DBG_ENTER("mysqlnd_net::dtor");
	if (net) {
		net->data->m.free_contents(net, TSRMLS_C);
		net->data->m.close_stream(net, stats, error_info, TSRMLS_C);
...
static php_stream *
MYSQLND_METHOD(mysqlnd_net, get_stream)(const MYSQLND_NET * const net, TSRMLS_D)
{
...
static php_stream *
MYSQLND_METHOD(mysqlnd_net, set_stream)(MYSQLND_NET * const net, php_stream * net_stream, TSRMLS_D)
{
	php_stream * ret = NULL;
...
/* {{{ mysqlnd_net_init */
PHPAPI MYSQLND_NET *
mysqlnd_net_init(zend_bool persistent, MYSQLND_STATS * stats, MYSQLND_ERROR_INFO * error_info, TSRMLS_D)
{
	MYSQLND_NET * net;
	DBG_ENTER("mysqlnd_net_init");
	net = MYSQLND_CLASS_METHOD_TABLE_NAME(mysqlnd_object_factory).get_io_channel(persistent, stats, error_info, TSRMLS_C);
	DBG_RETURN(net);
}
/* }}} */
...
/* {{{ mysqlnd_net_free */
PHPAPI void
mysqlnd_net_free(MYSQLND_NET * const net, MYSQLND_STATS * stats, MYSQLND_ERROR_INFO * error_info, TSRMLS_D)
{
