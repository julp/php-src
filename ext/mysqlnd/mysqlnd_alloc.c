...
void * _mysqlnd_ecalloc(unsigned int nmemb, size_t size MYSQLND_MEM_D)
{
...
#endif
	TRACE_ALLOC_INF_FMT("before: %lu", zend_memory_usage(FALSE, TSRMLS_C));
	...
	TRACE_ALLOC_INF_FMT("after : %lu", zend_memory_usage(FALSE, TSRMLS_C));
	TRACE_ALLOC_INF_FMT("size=%lu ptr=%p", size, ret);
	if (ret && collect_memory_statistics) {
		*(size_t *) ret = size;
...
void * _mysqlnd_realloc(void *ptr, size_t new_size MYSQLND_MEM_D)
{
	...
	TRACE_ALLOC_INF_FMT("before: %lu", zend_memory_usage(TRUE, TSRMLS_C));
