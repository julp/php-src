...
/* Packet handling */
#define PACKET_WRITE(packet, conn)	((packet)->header.m->write_to_net((packet), (conn), TSRMLS_C))
#define PACKET_READ(packet, conn)	((packet)->header.m->read_from_net((packet), (conn), TSRMLS_C))
#define PACKET_FREE(packet) \
	do { \
		DBG_INF_FMT("PACKET_FREE(%p)", packet); \
		if ((packet)) { \
			((packet)->header.m->free_mem((packet), FALSE, TSRMLS_C)); \
		} \
	} while (0);
...
typedef struct st_mysqlnd_packet_methods {
	size_t				struct_size;
	enum_func_status	(*read_from_net)(void * packet, MYSQLND_CONN_DATA * conn, TSRMLS_D);
	size_t				(*write_to_net)(void * packet, MYSQLND_CONN_DATA * conn, TSRMLS_D);
	void				(*free_mem)(void *packet, zend_bool stack_allocation, TSRMLS_D);
...
PHPAPI MYSQLND_PROTOCOL * mysqlnd_protocol_init(zend_bool persistent, TSRMLS_D);
PHPAPI void mysqlnd_protocol_free(MYSQLND_PROTOCOL * const protocol, TSRMLS_D);
