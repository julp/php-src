							...
							,  TSRMLS_D)
{
	...
	auth_resp_packet = conn->protocol->m.get_auth_response_packet(conn->protocol, FALSE, TSRMLS_C);
	...
	if (use_full_blown_auth_packet != TRUE) {
		change_auth_resp_packet = conn->protocol->m.get_change_auth_response_packet(conn->protocol, FALSE, TSRMLS_C);
								...
								,TSRMLS_D)
{
	...
	chg_user_resp = conn->protocol->m.get_change_user_response_packet(conn->protocol, FALSE, TSRMLS_C);
	...
	if (use_full_blown_auth_packet != TRUE) {
		...
		if (conn->m->get_server_version(conn, TSRMLS_C) >= 50123) {
	...
	if (conn->error_info->error_no) {
		...
		*/
		if (conn->m->get_server_version(conn, TSRMLS_C) > 50113L &&conn->m->get_server_version(conn, TSRMLS_C) < 50118L) {
			MYSQLND_PACKET_OK * redundant_error_packet = conn->protocol->m.get_ok_packet(conn->protocol, FALSE, TSRMLS_C);
			if (redundant_error_packet) {
				PACKET_READ(redundant_error_packet, conn);
				PACKET_FREE(redundant_error_packet);
				DBG_INF_FMT("Server is %u, buggy, sends two ERR messages", conn->m->get_server_version(conn, TSRMLS_C));
			} else {
				SET_OOM_ERROR(*conn->error_info);
			}
		}
	...
	if (ret == PASS) {
		...
		if (conn->m->get_server_version(conn, TSRMLS_C) < 50123) {
			ret = conn->m->set_charset(conn, old_cs->name, TSRMLS_C);
		}
						...
						,   TSRMLS_D)
{
	...
	if (!fname || fname[0] == '\0') {
		...
		do {
			...
			pk_req_packet = conn->protocol->m.get_sha256_pk_request_packet(conn->protocol, FALSE, TSRMLS_C);
			if (!pk_req_packet) {
				SET_OOM_ERROR(*conn->error_info);
				break;
			}
			pk_resp_packet = conn->protocol->m.get_sha256_pk_request_response_packet(conn->protocol, FALSE, TSRMLS_C);
			if (!pk_resp_packet) {
				SET_OOM_ERROR(*conn->error_info);
								...
								,  TSRMLS_D)
{
	...
	if (conn->net->data->ssl) {
		...
		*auth_data_len = 0;
		server_public_key = mysqlnd_sha256_get_rsa_key(conn, options, net_options, TSRMLS_C);
...
{
	mysqlnd_plugin_register_ex((struct st_mysqlnd_plugin_header *) &mysqlnd_native_auth_plugin, TSRMLS_C);
	mysqlnd_plugin_register_ex((struct st_mysqlnd_plugin_header *) &mysqlnd_pam_authentication_plugin, TSRMLS_C);
#ifdef MYSQLND_HAVE_SSL
	mysqlnd_plugin_register_ex((struct st_mysqlnd_plugin_header *) &mysqlnd_sha256_authentication_plugin, TSRMLS_C);
#endif
}
/* }}} */
