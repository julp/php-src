...
static void
mysqlnd_mempool_free_chunk(MYSQLND_MEMORY_POOL_CHUNK * chunk, TSRMLS_D)
{
...
mysqlnd_mempool_resize_chunk(MYSQLND_MEMORY_POOL_CHUNK * chunk, unsigned int size, TSRMLS_D)
{
...
static
MYSQLND_MEMORY_POOL_CHUNK * mysqlnd_mempool_get_chunk(MYSQLND_MEMORY_POOL * pool, unsigned int size, TSRMLS_D)
{
	...
	if (chunk) {
		...
		*/
		...
		if (size > pool->free_size) {
			...
			if (!chunk->ptr) {
				chunk->free_chunk(chunk, TSRMLS_C);
				chunk = NULL;
			}
		} else {
...
mysqlnd_mempool_create(size_t arena_size, TSRMLS_D)
{
	...
	if (ret) {
		...
		if (!ret->arena) {
			mysqlnd_mempool_destroy(ret, TSRMLS_C);
			ret = NULL;
		}
	}
	DBG_RETURN(ret);
...
PHPAPI void
mysqlnd_mempool_destroy(MYSQLND_MEMORY_POOL * pool, TSRMLS_D)
{
