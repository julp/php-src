...
static char *php_xsl_xslt_string_to_xpathexpr(const char *str, TSRMLS_D)
{
	...
	if (xmlStrchr(string, '"')) {
		if (xmlStrchr(string, '\'')) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Cannot create XPath expression (string contains both quote and double-quotes)");
			return NULL;
		}
...
static char **php_xsl_xslt_make_params(HashTable *parht, int xpath_params, TSRMLS_D)
{
	...
	for (zend_hash_internal_pointer_reset(parht);
		...
		if (zend_hash_get_current_key(parht, &string_key, &num_key, 1) != HASH_KEY_IS_STRING) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid argument or parameter array");
			efree(params);
			return NULL;
		} else {
			...
			if (!xpath_params) {
				xpath_expr = php_xsl_xslt_string_to_xpathexpr(Z_STRVAL_PP(value), TSRMLS_C);
			} else {
...
static void xsl_ext_function_php(xmlXPathParserContextPtr ctxt, int nargs, int type) /* {{{ */
{
				...
				if (type == 1) {
					...
					if (obj->nodesetval && obj->nodesetval->nodeNr > 0) {
						for (j = 0; j < obj->nodesetval->nodeNr; j++) {
							...
							child = php_dom_create_object(node, &ret, child, domintern, TSRMLS_C);
	...
	if (obj->stringval == NULL) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Handler name must be a string");
		xmlXPathFreeObject(obj);
	...
	if (!zend_make_callable(&handler, &callable, TSRMLS_C)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to call handler %s()", callable);
		valuePush(ctxt, xmlXPathNewString(""));
	} else if ( intern->registerPhpFunctions == 2 && zend_hash_exists(intern->registered_phpfunctions, callable, strlen(callable) + 1) == 0) { 
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Not allowed to call handler '%s()'", callable);
		...
		if (result == FAILURE) {
			if (Z_TYPE(handler) == IS_STRING) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to call handler %s()", Z_STRVAL_P(&handler));
				valuePush(ctxt, xmlXPathNewString(""));
			}
		/* retval is == NULL, when an exception occurred, don't report anything, because PHP itself will handle that */
		} else if (retval == NULL) {
		} else {
			if (retval->type == IS_OBJECT && instanceof_function( Z_OBJCE_P(retval), dom_node_class_entry, TSRMLS_C)) {
				xmlNode *nodep;
				dom_object *obj;
				...
				obj = (dom_object *)zend_object_store_get_object(retval, TSRMLS_C);
				nodep = dom_object_get_node(obj);
				valuePush(ctxt, xmlXPathNewNodeSet(nodep));
			} else if (retval->type == IS_BOOL) {
				valuePush(ctxt, xmlXPathNewBoolean(retval->value.lval));
			} else if (retval->type == IS_OBJECT) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "A PHP Object cannot be converted to a XPath-string");
				valuePush(ctxt, xmlXPathNewString(""));
			} else {
...
PHP_FUNCTION(xsl_xsltprocessor_import_stylesheet)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Oo", &id, xsl_xsltprocessor_class_entry, &docp) == FAILURE) {
		RETURN_FALSE;
	}

	nodep = php_libxml_import_node(docp, TSRMLS_C);
	...
	intern = (xsl_object *)zend_object_store_get_object(id, TSRMLS_C); 

	std_hnd = zend_get_std_object_handlers();
	MAKE_STD_ZVAL(member);
	ZVAL_STRING(member, "cloneDocument", 0);
	cloneDocu = std_hnd->read_property(id, member, BP_VAR_IS, NULL, TSRMLS_C);
	...
	php_xsl_set_object(id, sheetp, TSRMLS_C);
	RETVAL_TRUE;
}
/* }}} end xsl_xsltprocessor_import_stylesheet */

static xmlDocPtr php_xsl_apply_stylesheet(zval *id, xsl_object *intern, xsltStylesheetPtr style, zval *docp, TSRMLS_D) /* {{{ */
{
	xmlDocPtr newdocp;
	xmlDocPtr doc = NULL;
	...
	node = php_libxml_import_node(docp, TSRMLS_C);
	
	if (node) {
		doc = node->doc;
	}
	if (doc == NULL) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid Document");
		return NULL;
	}

	if (style == NULL) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "No stylesheet associated to this object");
		return NULL;
	}
	
	if (intern->profiling) {
		if (php_check_open_basedir(intern->profiling, TSRMLS_C)) {
			f = NULL;
		} else {
	...
	if (intern->parameter) {
		params = php_xsl_xslt_make_params(intern->parameter, 0, TSRMLS_C);
	}
	...
	if (intern->hasKeys == 1) {
		...
		object = (php_libxml_node_object *)zend_object_store_get_object(docp, TSRMLS_C);
		intern->doc->document = object->document;
	}

	php_libxml_increment_doc_ref(intern->doc, doc, TSRMLS_C);
	...
	doXInclude = std_hnd->read_property(id, member, BP_VAR_IS, NULL, TSRMLS_C);
	...
	if (secPrefsIni != secPrefsValue) {
		if (secPrefsIni != XSL_SECPREF_DEFAULT) {
			...
			php_error_docref(NULL, TSRMLS_C, E_DEPRECATED, "The xsl.security_prefs php.ini option is deprecated; use XsltProcessor->setSecurityPrefs() instead");
			if (intern->securityPrefsSet == 0) {
				...
				php_error_docref(NULL, TSRMLS_C, E_NOTICE, "The xsl.security_prefs php.ini was not used, since the  XsltProcessor->setSecurityPrefs() method was used");
			}
		}
	}
	...
	if (secPrefsError == 1) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Can't set libxslt security properties, not doing transformation for security reasons");
	} else {
...
PHP_FUNCTION(xsl_xsltprocessor_transform_to_doc)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "o|s!", &docp, &ret_class, &ret_class_len) == FAILURE) {
		RETURN_FALSE;
	}

	newdocp = php_xsl_apply_stylesheet(id, intern, sheetp, docp, TSRMLS_C);
	...
	if (newdocp) {
		if (ret_class) {
			...
			found = zend_lookup_class(ret_class, ret_class_len, &ce, TSRMLS_C);
			if ((found != SUCCESS) || !instanceof_function(*ce, curce, TSRMLS_C)) {
				xmlFreeDoc(newdocp);
			...
			interndoc = (php_libxml_node_object *)zend_objects_get_address(return_value, TSRMLS_C);
			php_libxml_increment_doc_ref(interndoc, newdocp, TSRMLS_C);
			php_libxml_increment_node_ptr(interndoc, (xmlNodePtr)newdocp, (void *)interndoc, TSRMLS_C);
		} else {
...
PHP_FUNCTION(xsl_xsltprocessor_transform_to_uri)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "op", &docp, &uri, &uri_len) == FAILURE) {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(xsl_xsltprocessor_transform_to_xml)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "o", &docp) == FAILURE) {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(xsl_xsltprocessor_set_parameter)
{
	...
	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS(), TSRMLS_C, "sa", &namespace, &namespace_len, &array_value) == SUCCESS) {
		intern = (xsl_object *)zend_object_store_get_object(id, TSRMLS_C);
		...
		while (zend_hash_get_current_data(Z_ARRVAL_P(array_value), (void **)&entry) == SUCCESS) {
			...
			if (zend_hash_get_current_key_ex(Z_ARRVAL_P(array_value), &string_key, &string_key_len, &idx, 0, NULL) != HASH_KEY_IS_STRING) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid parameter array");
				RETURN_FALSE;
			}
	...
	} else if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS(), TSRMLS_C, "sss", &namespace, &namespace_len, &name, &name_len, &value, &value_len) == SUCCESS) {
...
PHP_FUNCTION(xsl_xsltprocessor_get_parameter)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ss", &namespace, &namespace_len, &name, &name_len) == FAILURE) {
		RETURN_FALSE;
	}
	intern = (xsl_object *)zend_object_store_get_object(id, TSRMLS_C);
...
PHP_FUNCTION(xsl_xsltprocessor_register_php_functions)
{
	...
	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS(), TSRMLS_C, "a",  &array_value) == SUCCESS) {
		intern = (xsl_object *)zend_object_store_get_object(id, TSRMLS_C);
		zend_hash_internal_pointer_reset(Z_ARRVAL_P(array_value));
	...
	} else if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS(), TSRMLS_C, "s",  &name, &name_len) == SUCCESS) {
		intern = (xsl_object *)zend_object_store_get_object(id, TSRMLS_C);
...
PHP_FUNCTION(xsl_xsltprocessor_set_profiling)
{
	...
	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS(), TSRMLS_C, "p!", &filename, &filename_len) == SUCCESS) {
		intern = (xsl_object *)zend_object_store_get_object(id, TSRMLS_C);
		if (intern->profiling) {
...
PHP_FUNCTION(xsl_xsltprocessor_set_security_prefs)
{
	...
	DOM_GET_THIS(id);
 	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l", &securityPrefs) == FAILURE) {
		return;
	}
	intern = (xsl_object *)zend_object_store_get_object(id, TSRMLS_C);
...
PHP_FUNCTION(xsl_xsltprocessor_get_security_prefs)
{
	...
	DOM_GET_THIS(id);
	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS(), TSRMLS_C, "") == SUCCESS) {
		intern = (xsl_object *)zend_object_store_get_object(id, TSRMLS_C);
