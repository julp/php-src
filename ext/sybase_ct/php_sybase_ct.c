...
#define CHECK_LINK(link) { if (link==-1) { php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  A link to the server could not be established"); RETURN_FALSE; } }


static int _clean_invalid_results(zend_rsrc_list_entry *le, TSRMLS_D)
{
...
/* Forward declaration */
static int php_sybase_finish_results (sybase_result *result, TSRMLS_D);

static void php_free_sybase_result(zend_rsrc_list_entry *rsrc, TSRMLS_D)
{
	...
	if (result->sybase_ptr && result->sybase_ptr->active_result_index) {
		...
		php_sybase_finish_results(result, TSRMLS_C);
	}

	FREE_SYBASE_RESULT(result);
}

static void _close_sybase_link(zend_rsrc_list_entry *rsrc, TSRMLS_D)
{
	...
	zend_hash_apply(&EG(regular_list), (apply_func_t) _clean_invalid_results, TSRMLS_C);
	...
	if (ct_con_props(sybase_ptr->connection, CS_GET, CS_CON_STATUS,
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Unable to get connection status on close");
		/* Assume the worst. */
...
static void _close_sybase_plink(zend_rsrc_list_entry *rsrc, TSRMLS_D)
{
	sybase_link *sybase_ptr = (sybase_link *)rsrc->ptr;
	CS_INT con_status;
...
static CS_RETCODE CS_PUBLIC _client_message_handler(CS_CONTEXT *context, CS_CONNECTION *connection, CS_CLIENTMSG *errmsg)
{
	...
	if (CS_SEVERITY(errmsg->msgnumber) >= SybCtG(min_client_severity)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Client message:  %s (severity %ld)", errmsg->msgstring, (long)CS_SEVERITY(errmsg->msgnumber));
	}
...
static int _call_message_handler(zval *callback_name, CS_SERVERMSG *srvmsg, TSRMLS_D)
{
	int handled = 0;
	...
	if (call_user_function_ex(EG(function_table), NULL, callback_name, &retval, 5, args, 0, NULL, TSRMLS_C) == FAILURE) {
		zval expr_copy;
		int use_copy;

		zend_make_printable_zval(callback_name, &expr_copy, &use_copy);
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Cannot call the messagehandler %s", Z_STRVAL(expr_copy));
		zval_dtor(&expr_copy);
	}
...
static CS_RETCODE CS_PUBLIC _server_message_handler(CS_CONTEXT *context, CS_CONNECTION *connection, CS_SERVERMSG *srvmsg)
{
	...
	handled = handled | _call_message_handler(SybCtG(callback_name), srvmsg, TSRMLS_C);

	/* Call link specific message handler */
	if (sybase) {
		handled = handled | _call_message_handler(sybase->callback_name, srvmsg, TSRMLS_C);
	}

	/* Spit out a warning if neither of them has handled this message */
	if (!handled) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Server message:  %s (severity %ld, procedure %s)",
...
static PHP_GINIT_FUNCTION(sybase)
{
	...
	if (ct_callback(sybase_globals->context, NULL, CS_SET, CS_SERVERMSG_CB, (CS_VOID *)_server_message_handler)!=CS_SUCCEED) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Unable to set server message handler");
	}

	if (ct_callback(sybase_globals->context, NULL, CS_SET, CS_CLIENTMSG_CB, (CS_VOID *)_client_message_handler)!=CS_SUCCEED) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Unable to set client message handler");
	}
	 ...
	 */
	{
		...
		if (cs_dt_info(sybase_globals->context, CS_SET, NULL, CS_DT_CONVFMT, CS_UNUSED, &dt_convfmt, sizeof(dt_convfmt), NULL)!=CS_SUCCEED) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Unable to set datetime conversion format");
		}
	}
	...
	if (cfg_get_long("sybct.timeout", &opt)==SUCCESS) {
		...
		if (ct_config(sybase_globals->context, CS_SET, CS_TIMEOUT, &cs_timeout, CS_UNUSED, NULL)!=CS_SUCCEED) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Unable to update the timeout");
		}
	}
...
static int php_sybase_do_connect_internal(sybase_link *sybase, char *host, char *user, char *passwd, char *charset, char *appname, TSRMLS_D)
{
	...
	if (ct_con_alloc(SybCtG(context), &sybase->connection)!=CS_SUCCEED) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Unable to allocate connection record");
		return 0;
	}
	...
	if (ct_con_props(sybase->connection, CS_SET, CS_USERDATA, &sybase, CS_SIZEOF(sybase), NULL)!=CS_SUCCEED) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Unable to set userdata");
	...
	if (charset) {
		if (cs_loc_alloc(SybCtG(context), &tmp_locale)!=CS_SUCCEED) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase: Unable to allocate locale information");
		} else {
			if (cs_locale(SybCtG(context), CS_SET, tmp_locale, CS_LC_ALL, NULL, CS_NULLTERM, NULL)!=CS_SUCCEED) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase: Unable to load default locale data");
			} else {
				if (cs_locale(SybCtG(context), CS_SET, tmp_locale, CS_SYB_CHARSET, charset, CS_NULLTERM, NULL)!=CS_SUCCEED) {
					php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase: Unable to update character set");
				} else {
					if (ct_con_props(sybase->connection, CS_SET, CS_LOC_PROP, tmp_locale, CS_UNUSED, NULL)!=CS_SUCCEED) {
						php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase: Unable to update connection properties");
					}
				}
			}
		}
	...
	if (cfg_get_long("sybct.packet_size", &packetsize) == SUCCESS) {
		if (ct_con_props(sybase->connection, CS_SET, CS_PACKETSIZE, (CS_VOID *)&packetsize, CS_UNUSED, NULL) != CS_SUCCEED) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase: Unable to update connection packetsize");
		}
	}
	...
	if (SybCtG(login_timeout) != -1) {
		...
		if (ct_config(SybCtG(context), CS_SET, CS_LOGIN_TIMEOUT, &cs_login_timeout, CS_UNUSED, NULL)!=CS_SUCCEED) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Unable to update the login timeout");
		}
	}
	...
	if (ct_connect(sybase->connection, host, CS_NULLTERM)!=CS_SUCCEED) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Unable to connect");
		ct_con_drop(sybase->connection);
		return 0;
	}

	if (ct_cmd_alloc(sybase->connection, &sybase->cmd)!=CS_SUCCEED) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Unable to allocate command record");
...
static void php_sybase_do_connect(INTERNAL_FUNCTION_PARAMETERS, int persistent)
{
	...
	if (persistent) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "|s!s!s!s!s!", &host, &len, &user, &len, &passwd, &len, &charset, &len, &appname, &len) == FAILURE) {
			return;
		}
	} else {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "|s!s!s!s!s!b", &host, &len, &user, &len, &passwd, &len, &charset, &len, &appname, &len, &new) == FAILURE) {
			return;
		}
	}
	...
	if (persistent) {
		...
		if (zend_hash_find(&EG(persistent_list), hashed_details, hashed_details_length+1, (void **) &le)==FAILURE) {  /* we don't */
			...
			if (SybCtG(max_links)!=-1 && SybCtG(num_links)>=SybCtG(max_links)) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Too many open links (%ld)", SybCtG(num_links));
				efree(hashed_details);
				RETURN_FALSE;
			}
			if (SybCtG(max_persistent)!=-1 && SybCtG(num_persistent)>=SybCtG(max_persistent)) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Too many open persistent links (%ld)", SybCtG(num_persistent));
				efree(hashed_details);
				RETURN_FALSE;
			}
			...
			if (!php_sybase_do_connect_internal(sybase_ptr, host, user, passwd, charset, appname, TSRMLS_C)) {
				free(sybase_ptr);
				efree(hashed_details);
				RETURN_FALSE;
			}
			...
			if (ct_con_props(sybase_ptr->connection, CS_GET, CS_CON_STATUS,
							 &con_status, CS_UNUSED, NULL)!=CS_SUCCEED) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Unable to get connection status");
				efree(hashed_details);
				RETURN_FALSE;
			}
...
inline static int php_sybase_connection_id(zval *sybase_link_index, int *id, TSRMLS_D)
{
...
PHP_FUNCTION(sybase_close)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "|r", &sybase_link_index) == FAILURE) {
		return;
	}

	if (php_sybase_connection_id(sybase_link_index, &id, TSRMLS_C) == FAILURE) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  No connection to close");
		RETURN_FALSE;
	}
...
PHP_FUNCTION(sybase_select_db)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|r", &db, &len, &sybase_link_index) == FAILURE) {
		return;
	}

	if (php_sybase_connection_id(sybase_link_index, &id, TSRMLS_C) == FAILURE) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  No connection");
		RETURN_FALSE;
	}
...
static int php_sybase_finish_results(sybase_result *result, TSRMLS_D) 
{
	...
	while ((retcode = ct_results(result->sybase_ptr->cmd, &restype))==CS_SUCCEED) {
		switch ((int) restype) {
			...
			case CS_CMD_FAIL:
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Command failed, cancelling rest");
				ct_cancel(NULL, result->sybase_ptr->cmd, CS_CANCEL_ALL);
				fail = 1;
				break;
			...
			case CS_ROW_RESULT:
				...
				php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Sybase:  Unexpected results, cancelling current");
			...
			default:
				php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Sybase:  Unexpected results, cancelling all");
				ct_cancel(NULL, result->sybase_ptr->cmd, CS_CANCEL_ALL);
				break;
		}
...
static int php_sybase_fetch_result_row(sybase_result *result, int numrows, TSRMLS_D)
{
	int i, j;
	CS_INT retcode;
	...
	if (retcode==CS_ROW_FAIL) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Error reading row %d", result->num_rows);
		return retcode;
	}
	result->last_retcode= retcode;
	switch (retcode) {
		case CS_END_DATA:
			retcode = php_sybase_finish_results(result, TSRMLS_C);
			break;
...
static sybase_result * php_sybase_fetch_result_set(sybase_link *sybase_ptr, int buffered, int store, TSRMLS_D)
{
	int num_fields;
	...
	if (buffered) {
		...
		if ((retcode = php_sybase_fetch_result_row(result, -1, TSRMLS_C)) == CS_FAIL) {
			return NULL;
		}
	}
...
static void php_sybase_query (INTERNAL_FUNCTION_PARAMETERS, int buffered)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|rb", &query, &len, &sybase_link_index, &store) == FAILURE) {
		return;
	}

	if (!store && !buffered) {
		php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Sybase:  Cannot use non-storing mode with buffered queries");
		store = 1;
	}
	...
	if (sybase_ptr->active_result_index) {
		...
		php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Sybase:  Called without first fetching all rows from a previous unbuffered query");
		if (sybase_ptr->cmd) {
			ct_cancel(NULL, sybase_ptr->cmd, CS_CANCEL_ALL);
		}
	...
	for (;;) {
		...
		if (ct_command(sybase_ptr->cmd, CS_LANG_CMD, query, CS_NULLTERM, CS_UNUSED)!=CS_SUCCEED) {
			 ...
			 */
			sybase_ptr->dead = 1;
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Connection is dead");
			RETURN_FALSE;
		}
		...
		if (ct_send(sybase_ptr->cmd)!=CS_SUCCEED) {
			ct_cancel(NULL, sybase_ptr->cmd, CS_CANCEL_ALL);
			sybase_ptr->dead = 1;
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Cannot send command");
			RETURN_FALSE;
		}
		...
		if (ct_results(sybase_ptr->cmd, &restype)!=CS_SUCCEED) {
			ct_cancel(NULL, sybase_ptr->cmd, CS_CANCEL_ALL);
			sybase_ptr->dead = 1;
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Cannot read results");
			RETURN_FALSE;
		}
		switch ((int) restype) {
			case CS_CMD_FAIL:
			...
			case CS_ROW_RESULT:
			case CS_STATUS_RESULT:
				result = php_sybase_fetch_result_set(sybase_ptr, buffered, store, TSRMLS_C);
				if (result == NULL) {
					ct_cancel(NULL, sybase_ptr->cmd, CS_CANCEL_ALL);
					RETURN_FALSE;
				}
		...
		if (sybase_ptr->deadlock && SybCtG(deadlock_retry_count) != -1 && ++deadlock_count > SybCtG(deadlock_retry_count)) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Retried deadlock %d times [max: %ld], giving up", deadlock_count- 1, SybCtG(deadlock_retry_count));
			FREE_SYBASE_RESULT(result);
			break;
		}
...
PHP_FUNCTION(sybase_free_result)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r", &sybase_result_index) == FAILURE) {
		return;
	}
	...
	if (result->last_retcode != CS_END_DATA && result->last_retcode != CS_END_RESULTS) {
		/* php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Cancelling the rest of the results"); */
		ct_cancel(NULL, result->sybase_ptr->cmd, CS_CANCEL_ALL);
		php_sybase_finish_results(result, TSRMLS_C);
	}
...
PHP_FUNCTION(sybase_fetch_row)
{
	...
	/* Unbuffered? */
	if (result->last_retcode != CS_END_DATA && result->last_retcode != CS_END_RESULTS) {
		php_sybase_fetch_result_row(result, 1, TSRMLS_C);
	}
...
PHP_FUNCTION(sybase_fetch_object)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r|z", &sybase_result_index, &object) == FAILURE) {
		return;
	}
	ZEND_FETCH_RESOURCE(result, sybase_result *, &sybase_result_index, -1, "Sybase result", le_result);
		...
		switch (Z_TYPE_P(object)) {
			...
			default: {
				...
				if (zend_lookup_class(Z_STRVAL_P(object), Z_STRLEN_P(object), &pce, TSRMLS_C) == FAILURE) {
					php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Sybase:  Class %s has not been declared", Z_STRVAL_P(object));
					/* Use default (ZEND_STANDARD_CLASS_DEF_PTR) */
				} else {
					ce = *pce;
				}
...
PHP_FUNCTION(sybase_data_seek)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rl", &sybase_result_index, &offset) == FAILURE) {
		return;
	}
	ZEND_FETCH_RESOURCE(result, sybase_result *, &sybase_result_index, -1, "Sybase result", le_result);
	...
	if (result->last_retcode != CS_END_DATA && result->last_retcode != CS_END_RESULTS && offset >= result->num_rows) {
		php_sybase_fetch_result_row(result, offset+ 1, TSRMLS_C);
	}
	
	if (offset < 0 || offset >= result->num_rows) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Bad row offset %ld, must be betweem 0 and %d", offset, result->num_rows - 1);
		RETURN_FALSE;
	}
...
PHP_FUNCTION(sybase_fetch_field)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r|l", &sybase_result_index, &field_offset) == FAILURE) {
		return;
	}
	ZEND_FETCH_RESOURCE(result, sybase_result *, &sybase_result_index, -1, "Sybase result", le_result);
	...
	if (field_offset < 0 || field_offset >= result->num_fields) {
		if (ZEND_NUM_ARGS() == 2) { /* field specified explicitly */
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Bad column offset");
		}
		RETURN_FALSE;
	}
...
PHP_FUNCTION(sybase_field_seek)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rl", &sybase_result_index, &field_offset) == FAILURE) {
		return;
	}
	ZEND_FETCH_RESOURCE(result, sybase_result *, &sybase_result_index, -1, "Sybase result", le_result);
...
PHP_FUNCTION(sybase_result)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rlz", &sybase_result_index, &row, &field) == FAILURE) {
		return;
	}
	ZEND_FETCH_RESOURCE(result, sybase_result *, &sybase_result_index, -1, "Sybase result", le_result);
	...
	if (result->last_retcode != CS_END_DATA && result->last_retcode != CS_END_RESULTS && row >= result->num_rows) {
		php_sybase_fetch_result_row(result, row, TSRMLS_C);
	}

	if (row < 0 || row >= result->num_rows) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Bad row offset (%ld)", row);
		RETURN_FALSE;
	}
	...
	switch(Z_TYPE_P(field)) {
		case IS_STRING: {
			...
			if (i >= result->num_fields) { /* no match found */
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  %s field not found in result", Z_STRVAL_P(field));
				RETURN_FALSE;
			}
			break;
		}
		default:
			...
			if (field_offset < 0 || field_offset >= result->num_fields) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sybase:  Bad column offset specified");
				RETURN_FALSE;
			}
			break;
	}
...
PHP_FUNCTION(sybase_min_client_severity)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l", &severity) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(sybase_deadlock_retry_count)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l", &retry_count) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(sybase_set_message_handler)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "f!|r", &fci, &cache, &sybase_link_index) == FAILURE) {
		return;
	}
