...
PHP_FUNCTION(bin2hex)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s", &data, &datalen) == FAILURE) {
		return;
	}
	...
	RETURN_STRINGL_ENC(result, newlen, enc_ascii, 0);
}
/* }}} */
...
PHP_FUNCTION(hex2bin)
{
	...
	if (datalen % 2 != 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Hexadecimal input string must have an even length");
		RETURN_FALSE;
	}
...
static void php_spn_common_handler(INTERNAL_FUNCTION_PARAMETERS, int behavior) /* {{{ */
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ss|ll", &s11, &len1,
...
PHP_FUNCTION(nl_langinfo)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l", &item) == FAILURE) {
		return;
	}
	...
	switch(item) { /* {{{ */
		...
		case CODESET:
			...
			break;
		default:
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Item '%ld' is not valid", item);
			RETURN_FALSE;
	}
	/* }}} */
...
PHP_FUNCTION(strcoll)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ss", &s1, &s1len, &s2, &s2len) == FAILURE) {
		return;
	}
 ...
 */
static inline int php_charmask(unsigned char *input, int len, char *mask, TSRMLS_D)
{
	unsigned char *end;
	unsigned char c;
	int result = SUCCESS;
	...
	for (end = input+len; input < end; input++) {
		...
		if ((input+3 < end) && input[1] == '.' && input[2] == '.'
			...
			if (end-len >= input) { /* there was no 'left' char */
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid '..'-range, no character to the left of '..'");
				result = FAILURE;
				continue;
			}
			if (input+2 >= end) { /* there is no 'right' char */
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid '..'-range, no character to the right of '..'");
				result = FAILURE;
				continue;
			}
			if (input[-1] > input[2]) { /* wrong order */
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid '..'-range, '..'-range needs to be incrementing");
				result = FAILURE;
				continue;
			}
			/* FIXME: better error (a..b..c is the only left possibility?) */
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid '..'-range");
			result = FAILURE;
			continue;
		} else {
			mask[c]=1;
...
PHPAPI char *php_trim(char *c, int len, char *what, int what_len, zval *return_value, int mode, TSRMLS_D)
{
	...
	if (what) {
		php_charmask((unsigned char*)what, what_len, mask, TSRMLS_C);
	} else {
		php_charmask((unsigned char*)" \n\r\t\v\0", 6, mask, TSRMLS_C);
	}
...
static void php_do_trim(INTERNAL_FUNCTION_PARAMETERS, int mode)
{
	...
	if (zend_parse_parameters_mbunaware(ZEND_NUM_ARGS(), TSRMLS_C, "E|s", &str, &str_len, &enc, &what, &what_len) == FAILURE) {
		return;
	}

	php_trim(str, str_len, what, what_len, return_value, mode, TSRMLS_C);
...
PHP_FUNCTION(wordwrap)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "E|lsb", &text, &textlen, &enc, &linelength, &breakchar, &breakcharlen, &docut) == FAILURE) {
		return;
	}
	...
	if (breakcharlen == 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Break string cannot be empty");
		RETURN_FALSE;
	}

	if (linelength == 0 && docut) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Can't force cut when width is zero");
		RETURN_FALSE;
	}
	...
	if (breakcharlen == 1 && !docut) {
		...
		RETURN_STRINGL_ENC(newtext, textlen, enc, 0);
	} else {
		...
		RETURN_STRINGL_ENC(newtext, newtextlen, enc, 0);
	}
}
/* }}} */
...
PHPAPI void php_explode(zval *delim, zval *str, zval *return_value, long limit)
{
	...
	if (p2 == NULL) {
		...
		do {
			add_next_index_stringl_enc(return_value, p1, p2 - p1, Z_STRENC_P(str), 1);
			p1 = p2 + Z_STRLEN_P(delim);
		} while ((p2 = php_memnstr(p1, Z_STRVAL_P(delim), Z_STRLEN_P(delim), endp)) != NULL &&
				 --limit > 1);

		if (p1 <= endp)
			add_next_index_stringl_enc(return_value, p1, endp-p1, Z_STRENC_P(str), 1);
	}
}
/* }}} */
	...
	if (p2 == NULL) {
		...
		*/
		...
		for (i = 0;i < to_return;i++) { /* this checks also for to_return > 0 */
			add_next_index_stringl_enc(return_value, positions[i],
...
PHP_FUNCTION(explode)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "zz|l", &zdelim, &zstr, &limit) == FAILURE) {
		return;
	}

	if (Z_STRLEN_P(zdelim) == 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Empty delimiter");
		RETURN_FALSE;
	}

	array_init(return_value);

	if (Z_STRLEN_P(zstr) == 0) {
	  	if (limit >= 0) {
			add_next_index_stringl_enc(return_value, "", sizeof("") - 1, enc_ascii, 1);
		}
		return;
	}
	...
	if (limit > 1) {
		...
		add_index_stringl_enc(return_value, 0, Z_STRVAL_P(zstr), Z_STRLEN_P(zstr), Z_STRENC_P(zstr), 1);
	}
}
/* }}} */
...
PHPAPI void php_implode(zval *delim, zval *arr, zval *return_value, TSRMLS_D)
{
	...
	// TODO: ensure all args have the same encoding
...
PHP_FUNCTION(implode)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Z|Z", &arg1, &arg2) == FAILURE) {
		return;
	}

	if (arg2 == NULL) {
		if (Z_TYPE_PP(arg1) != IS_ARRAY) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Argument must be an array");
			return;
		}
...
#define _IMPL_EMPTY ""
		...
		if (Z_TYPE_PP(arg1) == IS_ARRAY) {
			...
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid arguments passed");
			return;
		}
	}

	php_implode(delim, arr, return_value, TSRMLS_C);
...
PHP_FUNCTION(strtok)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|s", &str, &str_len, &tok, &tok_len) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(strtoupper)
{
	...
	if (zend_parse_parameters_mbunaware(ZEND_NUM_ARGS(), TSRMLS_C, "E", &arg, &arglen, &enc) == FAILURE) {
		return;
	}

	arg = estrndup(arg, arglen);
	php_strtoupper(arg, arglen);
	RETURN_STRINGL_ENC(arg, arglen, enc, 0);
}
/* }}} */
...
PHP_FUNCTION(strtolower)
{
	...
	if (zend_parse_parameters_mbunaware(ZEND_NUM_ARGS(), TSRMLS_C, "E", &str, &arglen, &enc) == FAILURE) {
		return;
	}

	str = estrndup(str, arglen);
	php_strtolower(str, arglen);
	RETURN_STRINGL_ENC(str, arglen, enc, 0);
}
/* }}} */
...
PHP_FUNCTION(basename)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "E|s", &string, &string_len, &enc, &suffix, &suffix_len) == FAILURE) {
		return;
	}

	php_basename(string, string_len, suffix, suffix_len, &ret, &ret_len, TSRMLS_C);
	RETURN_STRINGL_ENC(ret, (int)ret_len, enc, 0);
}
/* }}} */
...
PHP_FUNCTION(dirname)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "E", &str, &str_len, &enc) == FAILURE) {
		return;
	}
	...
	RETURN_STRINGL_ENC(ret, ret_len, enc, 0);
}
/* }}} */
...
PHP_FUNCTION(pathinfo)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "E|l", &path, &path_len, &enc, &opt) == FAILURE) {
		return;
	}
	...
	if ((opt & PHP_PATHINFO_DIRNAME) == PHP_PATHINFO_DIRNAME) {
		...
		ret_len = php_dirname(ret, path_len);
		if (*ret) {
			add_assoc_stringl_enc_ex(tmp, "dirname", sizeof("dirname"), ret, ret_len, enc, 1);
		}
		efree(ret);
		ret = NULL;
	}
	...
	if (have_basename) {
		php_basename(path, path_len, NULL, 0, &ret, &ret_len, TSRMLS_C);
		add_assoc_stringl_enc_ex(tmp, "basename", sizeof("basename"), ret, ret_len, enc, 0);
	}
	...
	if ((opt & PHP_PATHINFO_EXTENSION) == PHP_PATHINFO_EXTENSION) {
		...
		if (p) {
			idx = p - ret;
			add_assoc_stringl_enc_ex(tmp, "extension", sizeof("extension"), ret + idx + 1, ret_len - idx - 1, enc, 1);
		}
	}
	...
	if ((opt & PHP_PATHINFO_FILENAME) == PHP_PATHINFO_FILENAME) {
		...
		idx = p ? (p - ret) : ret_len;
		add_assoc_stringl_enc_ex(tmp, "filename", sizeof("filename"), ret, idx, enc, 1);
	}
...
/* {{{ php_needle_char
 */
static int php_needle_char(zval *needle, char *target, TSRMLS_D)
{
	switch (Z_TYPE_P(needle)) {
		...
		default: {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "needle is not a string or an integer");
			return FAILURE;
		 }
	}
}
...
PHP_FUNCTION(stristr)
{
	...
	if (zend_parse_parameters_mbunaware(ZEND_NUM_ARGS(), TSRMLS_C, "Ez|b", &haystack, &haystack_len, &enc, &needle, &part) == FAILURE) {
		return;
	}
	...
	if (Z_TYPE_P(needle) == IS_STRING) {
		...
		if (!Z_STRLEN_P(needle)) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Empty needle");
			efree(haystack_dup);
			RETURN_FALSE;
		}
		...
		if (php_needle_char(needle, needle_char, TSRMLS_C) != SUCCESS) {
			efree(haystack_dup);
			RETURN_FALSE;
		}
		needle_char[1] = 0;
	...
	if (found) {
		...
		if (part) {
			RETVAL_STRINGL_ENC(haystack, found_offset, enc, 1);
		} else {
			RETVAL_STRINGL_ENC(haystack + found_offset, haystack_len - found_offset, enc, 1);
		}
	} else {
		RETVAL_FALSE;
	}
...
PHP_FUNCTION(strstr)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Ez|b", &haystack, &haystack_len, &enc, &needle, &part) == FAILURE) {
		return;
	}
	...
	if (found) {
		...
		if (part) {
			RETURN_STRINGL_ENC(haystack, found_offset, enc, 1);
		} else {
			RETURN_STRINGL_ENC(found, haystack_len - found_offset, enc, 1);
		}
	}
	RETURN_FALSE;
}
...
PHP_FUNCTION(strpos)
{
	...
	if (offset < 0 || offset > haystack_len) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Offset not contained in string");
		RETURN_FALSE;
	}
...
PHP_FUNCTION(strrpos)
{
	...
	if (Z_TYPE_P(zneedle) == IS_STRING) {
		...
		if (php_needle_char(zneedle, ord_needle, TSRMLS_C) != SUCCESS) {
			RETURN_FALSE;
		}
		ord_needle[1] = '\0';
		needle = ord_needle;
	...
	if (offset >= 0) {
		if (offset > haystack_len) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Offset is greater than the length of haystack string");
			RETURN_FALSE;
		}
		p = haystack + offset;
...
PHP_FUNCTION(strrchr)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Ez", &haystack, &haystack_len, &enc, &needle) == FAILURE) {
		return;
	}
	...
	if (Z_TYPE_P(needle) == IS_STRING) {
		...
		char needle_chr;
		if (php_needle_char(needle, &needle_chr, TSRMLS_C) != SUCCESS) {
			RETURN_FALSE;
		}
...
PHP_FUNCTION(chunk_split)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|ls", &str, &str_len, &chunklen, &end, &endlen) == FAILURE) {
		return;
	}

	if (chunklen <= 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Chunk length should be greater than zero");
		RETURN_FALSE;
	}
...
PHP_FUNCTION(substr)
{
	...
	if (zend_parse_parameters_mbunaware(ZEND_NUM_ARGS(), TSRMLS_C, "El|l", &str, &str_len, &enc, &f, &l) == FAILURE) {
		return;
	}
	...
	RETURN_STRINGL_ENC(str + f, l, enc, 1);
}
/* }}} */
...
PHP_FUNCTION(substr_replace)
{
	...
	if (Z_TYPE_PP(str) == IS_STRING) {
		if (
			...
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "'from' and 'len' should be of same type - numerical or array ");
		...
		if (argc == 4 && Z_TYPE_PP(from) == IS_ARRAY) {
			if (zend_hash_num_elements(Z_ARRVAL_PP(from)) != zend_hash_num_elements(Z_ARRVAL_PP(len))) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "'from' and 'len' should have the same number of elements");
				RETURN_STRINGL(Z_STRVAL_PP(str), Z_STRLEN_PP(str), 1);
			}
		}
	}
	...
	if (Z_TYPE_PP(str) != IS_ARRAY) {
		if (Z_TYPE_PP(from) != IS_ARRAY) {
			...
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Functionality of 'from' and 'len' as arrays is not implemented");
			RETURN_STRINGL(Z_STRVAL_PP(str), Z_STRLEN_PP(str), 1);
		}
			...
			if (Z_TYPE_PP(repl) == IS_ARRAY) {
				if (SUCCESS == zend_hash_get_current_data_ex(Z_ARRVAL_PP(repl), (void **) &tmp_repl, &pos_repl)) {
					...
					if(Z_REFCOUNT_P(orig_str) != refcount) {
						php_error_docref(NULL, TSRMLS_C, E_WARNING, "Argument was modified while replacing");
						if(Z_TYPE_PP(tmp_repl) != IS_STRING) {
							zval_dtor(repl_str);
						}
						break;
...
PHP_FUNCTION(quotemeta)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "E", &old, &old_len, &enc) == FAILURE) {
		return;
	}
	...
	*q = 0;
	...
	RETURN_STRINGL_ENC(erealloc(str, q - str + 1), q - str, enc, 0);
}
/* }}} */
...
PHP_FUNCTION(chr)
{
	...
	if (zend_parse_parameters_ex(ZEND_PARSE_PARAMS_QUIET, ZEND_NUM_ARGS(), TSRMLS_C, "l", &c) == FAILURE) {
		c = 0;
	}
...
PHP_FUNCTION(ucfirst)
{
	...
	if (zend_parse_parameters_mbunaware(ZEND_NUM_ARGS(), TSRMLS_C, "E", &str, &str_len, &enc) == FAILURE) {
		return;
	}
	...
	ZVAL_STRINGL_ENC(return_value, str, str_len, enc, 1);
...
static int php_strtr_compare_hash_suffix(const void *a, const void *b, TSRMLS_D, void *ctx_g)
{
...
PHP_FUNCTION(strtr)
{
	...
	// TODO: check all strings are in the same encoding
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "EZ|s", &str, &str_len, &enc, &from, &to, &to_len) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(strrev)
{
	...
	*p = '\0';
	...
	RETVAL_STRINGL_ENC(n, str_len, enc, 0);
}
/* }}} */
...
PHP_FUNCTION(similar_text)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ss|Z", &t1, &t1_len, &t2, &t2_len, &percent) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(addcslashes)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Es", &str, &str_len, &enc, &what, &what_len) == FAILURE) {
		return;
	}
	...
	if (what_len == 0) {
		RETURN_STRINGL_ENC(str, str_len, enc, 1);
	}

	Z_STRVAL_P(return_value) = php_addcslashes(str, str_len, &Z_STRLEN_P(return_value), 0, what, what_len, TSRMLS_C);
	RETURN_STRINGL_ENC(Z_STRVAL_P(return_value), Z_STRLEN_P(return_value), enc, 0);
}
/* }}} */
...
PHP_FUNCTION(addslashes)
{
	...
	RETURN_STRING_ENC(php_addslashes(str,
	                             str_len,
...
PHP_FUNCTION(stripslashes)
{
	...
	ZVAL_STRINGL_ENC(return_value, str, str_len, enc, 1);
	php_stripslashes(Z_STRVAL_P(return_value), &Z_STRLEN_P(return_value), TSRMLS_C);
}
/* }}} */
...
/* {{{ php_addcslashes
 */
PHPAPI char *php_addcslashes(const char *str, int length, int *new_length, int should_free, char *what, int wlength, TSRMLS_D)
{
	char flags[256];
	...
	php_charmask((unsigned char *)what, wlength, flags, TSRMLS_C);
...
/* {{{ php_addslashes
 */
PHPAPI char *php_addslashes(char *str, int length, int *new_length, int should_free, TSRMLS_D)
{
...
static void php_hebrev(INTERNAL_FUNCTION_PARAMETERS, int convert_newlines)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|l", &str, &str_len, &max_chars) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(nl2br)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "E|b", &str, &str_len, &enc, &is_xhtml) == FAILURE) {
		return;
	}
	...
	*target = '\0';
	...
	RETURN_STRINGL_ENC(tmp, new_length, enc, 0);
}
/* }}} */
...
PHP_FUNCTION(strip_tags)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "E|Z", &str, &str_len, &enc, &allow) == FAILURE) {
		return;
	}
	...
	RETURN_STRINGL_ENC(buf, retval_len, enc, 0);
}
/* }}} */
...
PHP_FUNCTION(setlocale)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Z+", &pcategory, &args, &num_args) == FAILURE) {
		return;
	}
	...
	if (Z_TYPE_PP(pcategory) == IS_LONG) {
		...
		php_error_docref(NULL, TSRMLS_C, E_DEPRECATED, "Passing locale category name as string is deprecated. Use the LC_* -constants instead");
		...
		if (!strcasecmp("LC_ALL", category)) {
			...
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid locale category name %s, must be one of LC_ALL, LC_COLLATE, LC_CTYPE, LC_MONETARY, LC_NUMERIC, or LC_TIME", category);
	...
	while (1) {
		...
		if (!strcmp ("0", Z_STRVAL_PP(plocale))) {
			...
			if (Z_STRLEN_PP(plocale) >= 255) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Specified locale name is too long");
				break;
			}
		}
...
PHP_FUNCTION(parse_str)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|z", &arg, &arglen, &arrayArg) == FAILURE) {
		return;
	}
	...
	if (arrayArg == NULL) {
		...
		sapi_module.treat_data(PARSE_STRING, res, &tmp, TSRMLS_C);
	} else 	{
		zval ret;

		array_init(&ret);
		sapi_module.treat_data(PARSE_STRING, res, &ret, TSRMLS_C);
...
PHP_FUNCTION(str_getcsv)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|sss", &str, &str_len, &delim_str, &delim_len,
	...
	php_fgetcsv(NULL, delim, enc, esc, str_len, str, return_value, TSRMLS_C);
}
/* }}} */
...
PHP_FUNCTION(str_repeat)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "El", &input_str, &input_len, &enc, &mult) == FAILURE) {
		return;
	}

	if (mult < 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Second argument has to be greater than or equal to 0");
		return;
	}
	...
	RETURN_STRINGL_ENC(result, result_len, enc, 0);
}
/* }}} */
...
PHP_FUNCTION(count_chars)
{
	...
	if (zend_parse_parameters_mbunaware(ZEND_NUM_ARGS(), TSRMLS_C, "E|l", &input, &len, &enc, &mymode) == FAILURE) {
		return;
	}

	if (mymode < 0 || mymode > 4) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unknown mode");
		RETURN_FALSE;
	}
	...
	if (mymode >= 3 && mymode <= 4) {
		RETURN_STRINGL_ENC(retstr, retlen, enc, 1);
	}
}
/* }}} */
...
{
	return string_natural_compare_function_ex(result, op1, op2, 1, TSRMLS_C);
}
/* }}} */

PHPAPI int string_natural_compare_function(zval *result, zval *op1, zval *op2, TSRMLS_D) /* {{{ */
{
	return string_natural_compare_function_ex(result, op1, op2, 0, TSRMLS_C);
}
/* }}} */
...
PHP_FUNCTION(substr_count)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ss|ll", &haystack, &haystack_len, &needle, &needle_len, &offset, &length) == FAILURE) {
		return;
	}

	if (needle_len == 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Empty substring");
		RETURN_FALSE;
	}
	...
	if (offset < 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Offset should be greater than or equal to 0");
		RETURN_FALSE;
	}

	if (offset > haystack_len) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Offset value %ld exceeds string length", offset);
		RETURN_FALSE;
	}
	p += offset;
	...
	if (ac == 4) {
		...
		if (length <= 0) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Length should be greater than 0");
			RETURN_FALSE;
		}
		if (length > (haystack_len - offset)) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Length value %ld exceeds string length", length);
			RETURN_FALSE;
		}
		endp = p + length;
	}
...
PHP_FUNCTION(str_pad)
{
	...
	EncodingPtr input_enc, pad_enc;

	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "El|El", &input, &input_len, &input_enc, &pad_length,
		  &pad_str_val, &pad_str_len, &pad_enc, &pad_type_val) == FAILURE) {
		return;
	}
	if (ZEND_NUM_ARGS() >= 3 && enc_are_incompatible(input_enc, pad_enc)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "incompatible charsets found between input et padding string ('%s' vs '%s')", enc_name(input_enc), enc_name(pad_enc));
	}

	/* If resulting string turns out to be shorter than input string,
	   we simply copy the input and return. */
	if (pad_length <= 0 || (pad_length - input_len) <= 0) {
		RETURN_STRINGL_ENC(input, input_len, input_enc, 1);
	}

	if (pad_str_len == 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Padding string cannot be empty");
		return;
	}

	if (pad_type_val < STR_PAD_LEFT || pad_type_val > STR_PAD_BOTH) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Padding type has to be STR_PAD_LEFT, STR_PAD_RIGHT, or STR_PAD_BOTH");
		return;
	}

	num_pad_chars = pad_length - input_len;
	if (num_pad_chars >= INT_MAX) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Padding length is too long");
		return;
	}
	...
	RETURN_STRINGL_ENC(result, result_len, input_enc, 0);
}
/* }}} */
...
PHP_FUNCTION(sscanf)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ss*", &str, &str_len, &format, &format_len,
		&args, &num_args) == FAILURE) {
		return;
	}

	result = php_sscanf_internal(str, format, num_args, args, 0, &return_value, TSRMLS_C);
...
PHP_FUNCTION(str_rot13)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "E", &arg, &arglen, &enc) == FAILURE) {
		return;
	}

	RETVAL_STRINGL_ENC(arg, arglen, enc, 1);

	php_strtr(Z_STRVAL_P(return_value), Z_STRLEN_P(return_value), rot13_from, rot13_to, 52);
}
/* }}} */

static void php_string_shuffle(char *str, long len, TSRMLS_D) /* {{{ */
{
...
PHP_FUNCTION(str_shuffle)
{
	...
	if (Z_STRLEN_P(return_value) > 1) {
		php_string_shuffle(Z_STRVAL_P(return_value), (long) Z_STRLEN_P(return_value), TSRMLS_C);
	}
}
/* }}} */
...
PHP_FUNCTION(str_word_count)
{
	...
	if (zend_parse_parameters_mbunaware(ZEND_NUM_ARGS(), TSRMLS_C, "E|ls", &str, &str_len, &enc, &type, &char_list, &char_list_len) == FAILURE) {
		return;
	}
	...
	switch(type) {
		...
		case 0:
			...
			break;
		default:
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid format value %ld", type);
			RETURN_FALSE;
	}

	if (char_list) {
		php_charmask((unsigned char *)char_list, char_list_len, ch, TSRMLS_C);
	}
	...
	while (p < e) {
		...
		if (p > s) {
			switch (type)
			{
				case 1:
					buf = estrndup(s, (p-s));
					add_next_index_stringl_enc(return_value, buf, (p-s), enc, 0);
					break;
				case 2:
					buf = estrndup(s, (p-s));
					add_index_stringl_enc(return_value, (s - str), buf, p-s, enc, 0);
					break;
				default:
					word_count++;
					break;
...
PHP_FUNCTION(money_format)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "sd", &format, &format_len, &value) == FAILURE) {
		return;
	}
	...
	while ((p = memchr(p, '%', (e - p)))) {
		if (*(p + 1) == '%') {
			...
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Only a single %%i or %%n token can be used");
			RETURN_FALSE;
		}
	}
...
PHP_FUNCTION(str_split)
{
	...
	if (zend_parse_parameters_mbunaware(ZEND_NUM_ARGS(), TSRMLS_C, "E|l", &str, &str_len, &enc, &split_length) == FAILURE) {
		return;
	}

	if (split_length <= 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "The length of each segment must be greater than zero");
		RETURN_FALSE;
	}
	...
	if (split_length >= str_len) {
		add_next_index_stringl_enc(return_value, str, str_len, enc, 1);
		return;
	}
	...
	while (n_reg_segments-- > 0) {
		add_next_index_stringl_enc(return_value, p, split_length, enc, 1);
		p += split_length;
	}

	if (p != (str + str_len)) {
		add_next_index_stringl_enc(return_value, p, (str + str_len - p), enc, 1);
	}
}
/* }}} */
...
PHP_FUNCTION(strpbrk)
{
	...
	EncodingPtr haystack_enc, list_enc;

	if (zend_parse_parameters_mbunaware(ZEND_NUM_ARGS(), TSRMLS_C, "ss", &haystack, &haystack_len, &haystack_enc, &char_list, &char_list_len, &list_enc) == FAILURE) {
		RETURN_FALSE;
	}
	if (enc_are_incompatible(haystack_enc, list_enc)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "incompatible charsets found between haystack and characters list ('%s' vs '%s')", enc_name(haystack_enc), enc_name(list_enc));
	}

	if (!char_list_len) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "The character list cannot be empty");
		RETURN_FALSE;
	}
	...
	for (haystack_ptr = haystack; haystack_ptr < (haystack + haystack_len); ++haystack_ptr) {
		for (cl_ptr = char_list; cl_ptr < (char_list + char_list_len); ++cl_ptr) {
			if (*cl_ptr == *haystack_ptr) {
				RETURN_STRINGL_ENC(haystack_ptr, (haystack + haystack_len - haystack_ptr), haystack_enc, 1);
			}
		}
	}
...
PHP_FUNCTION(substr_compare)
{
	...
	zend_bool cs=0;
	uint cmp_len;
	EncodingPtr s1_enc, s2_enc;

	if (zend_parse_parameters_mbunaware(ZEND_NUM_ARGS(), TSRMLS_C, "EEl|lb", &s1, &s1_len, &s1_enc, &s2, &s2_len, &s2_enc, &offset, &len, &cs) == FAILURE) {
		RETURN_FALSE;
	}

	if (ZEND_NUM_ARGS() >= 4 && len <= 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "The length must be greater than zero");
		RETURN_FALSE;
	}

	if (enc_are_incompatible(s1_enc, s2_enc)) {
                php_error_docref(NULL, TSRMLS_C, E_WARNING, "incompatible charsets found between strings ('%s' vs '%s')", enc_name(s1_enc), enc_name(s2_enc));
        }
	...
	if (offset >= s1_len) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "The start position cannot exceed initial string length");
		RETURN_FALSE;
	}
...
PHP_FUNCTION(str_encoding)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "z", &zstr) == FAILURE) {
        RETURN_FALSE;
    }
...
PHP_FUNCTION(str_force_encoding)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "zs", &zstr, &wanted_enc, &wanted_enc_len) == FAILURE) {
        RETURN_FALSE;
    }
    if (Z_TYPE_P(zstr) == IS_STRING) {
        EncodingPtr enc;
        ...
        if (NULL == (enc = enc_by_name(wanted_enc))) {
            php_error_docref(NULL, TSRMLS_C, E_WARNING, "unknown encoding: '%s'", wanted_enc);
            RETURN_FALSE;
        } else {
            Z_STRENC_P(zstr) = enc;
            RETURN_TRUE;
