...
/* {{{ _php_array_to_envp */
static php_process_env_t _php_array_to_envp(zval *environment, int is_persistent, TSRMLS_D)
{
	zval **element;
...
/* {{{ proc_open_rsrc_dtor */
static void proc_open_rsrc_dtor(zend_rsrc_list_entry *rsrc, TSRMLS_D)
{
...
PHP_FUNCTION(proc_terminate)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r|l", &zproc, &sig_no) == FAILURE) {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(proc_close)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r", &zproc) == FAILURE) {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(proc_open)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "saz|s!a!a!", &command,
	...
	if (environment) {
		env = _php_array_to_envp(environment, is_persistent, TSRMLS_C);
	} else {
		memset(&env, 0, sizeof(env));
	}
	...
	while (zend_hash_get_current_data_ex(Z_ARRVAL_P(descriptorspec), (void **)&descitem, &pos) == SUCCESS) {
		...
		if (str_index) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "descriptor spec must be an integer indexed array");
			goto exit_fail;
		}
		...
		if (Z_TYPE_PP(descitem) == IS_RESOURCE) {
			...
			if (descriptors[ndesc].childend == NULL) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "unable to dup File-Handle for descriptor %d", nindex);
				goto exit_fail;
			}
#else
			descriptors[ndesc].childend = dup(fd);
			if (descriptors[ndesc].childend < 0) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "unable to dup File-Handle for descriptor %ld - %s", nindex, strerror(errno));
				goto exit_fail;
			}
#endif
			...
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Descriptor item must be either an array or a File-Handle");
			goto exit_fail;
		} else {
			...
			if (zend_hash_index_find(Z_ARRVAL_PP(descitem), 0, (void **)&ztype) == SUCCESS) {
				...
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Missing handle qualifier in array");
				goto exit_fail;
			}
			...
			if (strcmp(Z_STRVAL_PP(ztype), "pipe") == 0) {
				...
				if (zend_hash_index_find(Z_ARRVAL_PP(descitem), 1, (void **)&zmode) == SUCCESS) {
					...
					php_error_docref(NULL, TSRMLS_C, E_WARNING, "Missing mode parameter for 'pipe'");
					goto exit_fail;
				}
				...
				if (0 != pipe(newpipe)) {
					php_error_docref(NULL, TSRMLS_C, E_WARNING, "unable to create pipe %s", strerror(errno));
					goto exit_fail;
				}
				...
				if (zend_hash_index_find(Z_ARRVAL_PP(descitem), 1, (void **)&zfile) == SUCCESS) {
					...
					php_error_docref(NULL, TSRMLS_C, E_WARNING, "Missing file name parameter for 'file'");
					goto exit_fail;
				}
				...
				if (zend_hash_index_find(Z_ARRVAL_PP(descitem), 2, (void **)&zmode) == SUCCESS) {
					...
					php_error_docref(NULL, TSRMLS_C, E_WARNING, "Missing mode parameter for 'file'");
					goto exit_fail;
				}
				...
				if (dev_ptmx == -1) {
					...
					if (dev_ptmx == -1) {
						php_error_docref(NULL, TSRMLS_C, E_WARNING, "failed to open /dev/ptmx, errno %d", errno);
						goto exit_fail;
					}
					grantpt(dev_ptmx);
					unlockpt(dev_ptmx);
					...
					if (slave_pty == -1) {
						php_error_docref(NULL, TSRMLS_C, E_WARNING, "failed to open slave pty, errno %d", errno);
						goto exit_fail;
					}
				}
				descriptors[ndesc].mode = DESC_PIPE;
...
#else
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "pty pseudo terminal not supported on this system");
				goto exit_fail;
#endif
			} else {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "%s is not a valid descriptor spec/mode", Z_STRVAL_PP(ztype));
				goto exit_fail;
			}
		}
	...
	if (cwd == NULL) {
		...
		if (!getcwd_result) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Cannot get current directory");
			goto exit_fail;
		}
		cwd = cur_cwd;
	}
	...
	if (FALSE == newprocok) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "CreateProcess failed, error code - %u", dw);
		goto exit_fail;
	}
	...
	if (child < 0) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "procve failed - %s", strerror(errno));
		goto exit_fail;
	}
#elif HAVE_FORK
	/* the unix way */
	...
	if (child == 0) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "fork failed - %s", strerror(errno));
