...
static void basic_globals_ctor(php_basic_globals *basic_globals_p, TSRMLS_D) /* {{{ */
{
	BG(rand_is_seeded) = 0;
...
static void basic_globals_dtor(php_basic_globals *basic_globals_p, TSRMLS_D) /* {{{ */
{
...
#endif
#else
	basic_globals_ctor(&basic_globals, TSRMLS_C);
#ifdef PHP_WIN32
	php_win32_core_globals_ctor(&the_php_win32_core_globals, TSRMLS_C);
#endif
#endif
...
#define REGISTER_MATH_CONSTANT(x)  REGISTER_DOUBLE_CONSTANT(#x, x, CONST_CS | CONST_PERSISTENT)
	...
	php_register_url_stream_wrapper("php", &php_stream_php_wrapper, TSRMLS_C);
	php_register_url_stream_wrapper("file", &php_plain_files_wrapper, TSRMLS_C);
#ifdef HAVE_GLOB
	php_register_url_stream_wrapper("glob", &php_glob_stream_wrapper, TSRMLS_C);
#endif
	php_register_url_stream_wrapper("data", &php_stream_rfc2397_wrapper, TSRMLS_C);
	php_register_url_stream_wrapper("http", &php_stream_http_wrapper, TSRMLS_C);
	php_register_url_stream_wrapper("ftp", &php_stream_ftp_wrapper, TSRMLS_C);
...
#endif
#else
	basic_globals_dtor(&basic_globals, TSRMLS_C);
#ifdef PHP_WIN32
	php_win32_core_globals_dtor(&the_php_win32_core_globals, TSRMLS_C);
#endif
#endif

	php_unregister_url_stream_wrapper("php", TSRMLS_C);
	php_unregister_url_stream_wrapper("http", TSRMLS_C);
	php_unregister_url_stream_wrapper("ftp", TSRMLS_C);
...
PHP_FUNCTION(constant)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s", &const_name, &const_name_len) == FAILURE) {
		return;
	}

	if (!zend_get_constant_ex(const_name, const_name_len, return_value, NULL, ZEND_FETCH_CLASS_SILENT, TSRMLS_C)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Couldn't find constant %s", const_name);
		RETURN_NULL();
	}
}
/* }}} */
...
PHP_NAMED_FUNCTION(php_inet_ntop)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s", &address, &address_len) == FAILURE) {
		RETURN_FALSE;
	}
...
#endif
	if (address_len != 4) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid in_addr value");
		RETURN_FALSE;
	}

	if (!inet_ntop(af, address, buffer, sizeof(buffer))) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "An unknown error occurred");
		RETURN_FALSE;
	}
...
#endif
	if (!strchr(address, '.')) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unrecognized address %s", address);
		RETURN_FALSE;
	}
...
PHP_FUNCTION(ip2long)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s", &addr, &addr_len) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(long2ip)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s", &ip, &ip_len) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(getenv)
{
	...
	ptr = sapi_getenv(str, str_len, TSRMLS_C);
	if (ptr) {
		RETURN_STRING(ptr, 0);
	}
#ifdef PHP_WIN32
...
PHP_FUNCTION(putenv)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s", &setting, &setting_len) == FAILURE) {
		return;
	}
...
# endif
	...
	php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid parameter syntax");
	RETURN_FALSE;
}
/* }}} */
#endif
...
PHP_FUNCTION(getopt)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|a", &options, &options_len, &p_longopts) == FAILURE) {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(sleep)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l", &num) == FAILURE) {
		RETURN_FALSE;
	}
	if (num < 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Number of seconds must be greater than or equal to 0");
		RETURN_FALSE;
	}
...
PHP_FUNCTION(usleep)
{
	...
	if (num < 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Number of microseconds must be greater than or equal to 0");
		RETURN_FALSE;
	}
	usleep(num);
#endif
...
PHP_FUNCTION(time_nanosleep)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ll", &tv_sec, &tv_nsec) == FAILURE) {
		return;
	}

	if (tv_sec < 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "The seconds value must be greater than 0");
		RETURN_FALSE;
	}
	if (tv_nsec < 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "The nanoseconds value must be greater than 0");
		RETURN_FALSE;
	}
	...
	if (!nanosleep(&php_req, &php_rem)) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "nanoseconds was not in the range 0 to 999 999 999 or seconds was negative");
	}
...
PHP_FUNCTION(time_sleep_until)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "d", &d_ts) == FAILURE) {
		return;
	}
	...
	if (c_ts < 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Sleep until to time is less than current time");
		RETURN_FALSE;
	}
...
/* {{{ add_config_entry_cb
 */
static int add_config_entry_cb(zval *entry, TSRMLS_D, int num_args, va_list args, zend_hash_key *hash_key)
{
	...
	if (Z_TYPE_P(entry) == IS_STRING) {
		...
		MAKE_STD_ZVAL(tmp);
		array_init(tmp);
		zend_hash_apply_with_arguments(Z_ARRVAL_P(entry), TSRMLS_C, (apply_func_args_t) add_config_entry_cb, 1, tmp);
...
PHP_FUNCTION(get_cfg_var)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s", &varname, &varname_len) == FAILURE) {
		return;
	}
	...
	if (retval) {
		if (Z_TYPE_P(retval) == IS_ARRAY) {
			array_init(return_value);
			zend_hash_apply_with_arguments(Z_ARRVAL_P(retval), TSRMLS_C, (apply_func_args_t) add_config_entry_cb, 1, return_value);
			return;
		} else {
...
PHP_FUNCTION(set_magic_quotes_runtime)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "b", &new_setting) == FAILURE) {
		return;
	}
	
	if (new_setting) {
		php_error_docref(NULL, TSRMLS_C, E_CORE_ERROR, "magic_quotes_runtime is not supported anymore");
	}
	RETURN_FALSE;
}
/* }}} */
...
PHP_FUNCTION(error_log)
{
	...
	if (zend_parse_parameters(argc, TSRMLS_C, "s|lps", &message, &message_len, &erropt, &opt, &opt_len, &headers, &headers_len) == FAILURE) {
		return;
	}
	...
	if (_php_error_log_ex(opt_err, message, message_len, opt, headers, TSRMLS_C) == FAILURE) {
		RETURN_FALSE;
	}
...
{
	return _php_error_log_ex(opt_err, message, (opt_err == 3) ? strlen(message) : 0, opt, headers, TSRMLS_C);
}
/* }}} */
...
PHPAPI int _php_error_log_ex(int opt_err, char *message, int message_len, char *opt, char *headers, TSRMLS_D) /* {{{ */
{
	...
	switch (opt_err)
	{
		case 1:		/*send an email */
			if (!php_mail(opt, "PHP error_log message", message, headers, NULL, TSRMLS_C)) {
				return FAILURE;
			}
			break;

		case 2:		/*send to an address */
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "TCP/IP option not available!");
			return FAILURE;
			break;
		...
		case 4: /* send to SAPI */
			if (sapi_module.log_message) {
				sapi_module.log_message(message, TSRMLS_C);
			} else {
				return FAILURE;
			}
			break;
...
PHP_FUNCTION(call_user_func)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "f*", &fci, &fci_cache, &fci.params, &fci.param_count) == FAILURE) {
		return;
	}

	fci.retval_ptr_ptr = &retval_ptr;

	if (zend_call_function(&fci, &fci_cache, TSRMLS_C) == SUCCESS && fci.retval_ptr_ptr && *fci.retval_ptr_ptr) {
...
PHP_FUNCTION(call_user_func_array)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "fa/", &fci, &fci_cache, &params) == FAILURE) {
		return;
	}

	zend_fcall_info_args(&fci, params, TSRMLS_C);
	fci.retval_ptr_ptr = &retval_ptr;
...
PHP_FUNCTION(call_user_method)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "z/z*", &callback, &object, &params, &n_params) == FAILURE) {
		return;
	}
	...
	if (Z_TYPE_P(object) != IS_OBJECT &&
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Second argument is not an object or class name");
		if (params) {
			efree(params);
		}
		RETURN_FALSE;
	...
	if (call_user_function_ex(EG(function_table), &object, callback, &retval_ptr, n_params, params, 0, NULL, TSRMLS_C) == SUCCESS) {
		if (retval_ptr) {
			COPY_PZVAL_TO_ZVAL(*return_value, retval_ptr);
		}
	} else {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to call %s()", Z_STRVAL_P(callback));
	}
	if (n_params) {
		efree(params);
	}
...
PHP_FUNCTION(call_user_method_array)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "z/zA/", &callback, &object, &params) == FAILURE) {
		return;
	}
	...
	if (call_user_function_ex(EG(function_table), &object, callback, &retval_ptr, num_elems, method_args, 0, NULL, TSRMLS_C) == SUCCESS) {
		if (retval_ptr) {
			COPY_PZVAL_TO_ZVAL(*return_value, retval_ptr);
		}
	} else {
...
PHP_FUNCTION(forward_static_call)
{
	...
	if (EG(called_scope) &&
		instanceof_function(EG(called_scope), fci_cache.calling_scope, TSRMLS_C)) {
...
static int user_shutdown_function_call(php_shutdown_function_entry *shutdown_function_entry, TSRMLS_D) /* {{{ */
{
	zval retval;
	char *function_name;

	if (!zend_is_callable(shutdown_function_entry->arguments[0], 0, &function_name, TSRMLS_C)) {
...
static void user_tick_function_call(user_tick_function_entry *tick_fe, TSRMLS_D) /* {{{ */
{
	zval retval;
	...
	if (! tick_fe->calling) {
		...
		if (call_user_function(	EG(function_table), NULL,
			...
			if (Z_TYPE_P(function) == IS_STRING) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to call %s() - function does not exist", Z_STRVAL_P(function));
				...
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to call %s::%s() - function does not exist", Z_OBJCE_PP(obj)->name, Z_STRVAL_PP(method));
			} else {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to call tick function");
			}
		}
...
static void run_user_tick_functions(int tick_count) /* {{{ */
{
	...
	zend_llist_apply(BG(user_tick_functions), (llist_apply_func_t) user_tick_function_call, TSRMLS_C);
}
/* }}} */
...
static int user_tick_function_compare(user_tick_function_entry * tick_fe1, user_tick_function_entry * tick_fe2) /* {{{ */
{
	...
	if (Z_TYPE_P(func1) == IS_STRING && Z_TYPE_P(func2) == IS_STRING) {
		...
		zval result;
		zend_compare_arrays(&result, func1, func2, TSRMLS_C);
		ret = (Z_LVAL(result) == 0);
	} else if (Z_TYPE_P(func1) == IS_OBJECT && Z_TYPE_P(func2) == IS_OBJECT) {
		zval result;
		zend_compare_objects(&result, func1, func2, TSRMLS_C);
		ret = (Z_LVAL(result) == 0);
	} else {
		ret = 0;
	}
	...
	if (ret && tick_fe1->calling) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to delete tick function executed at the moment");
		return 0;
	}
	return ret;
}
...
void php_call_shutdown_functions(TSRMLS_D) /* {{{ */
{
	if (BG(user_shutdown_function_names)) {
		zend_try {
			zend_hash_apply(BG(user_shutdown_function_names), (apply_func_t) user_shutdown_function_call, TSRMLS_C);
		}
		zend_end_try();
...
PHP_FUNCTION(register_shutdown_function)
{
	...
	if (!zend_is_callable(shutdown_function_entry.arguments[0], 0, &callback_name, TSRMLS_C)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid shutdown callback '%s' passed", callback_name);
		efree(shutdown_function_entry.arguments);
		RETVAL_FALSE;
	} else {
		if (!BG(user_shutdown_function_names)) {
...
PHPAPI zend_bool register_user_shutdown_function(char *function_name, size_t function_len, php_shutdown_function_entry *shutdown_function_entry, TSRMLS_D) /* {{{ */
{
	if (!BG(user_shutdown_function_names)) {
		ALLOC_HASHTABLE(BG(user_shutdown_function_names));
		zend_hash_init(BG(user_shutdown_function_names), 0, NULL, (void (*)(void *)) user_shutdown_function_dtor, 0);
...
PHPAPI zend_bool remove_user_shutdown_function(char *function_name, size_t function_len, TSRMLS_D) /* {{{ */
{
	if (BG(user_shutdown_function_names)) {
...
PHPAPI zend_bool append_user_shutdown_function(php_shutdown_function_entry shutdown_function_entry, TSRMLS_D) /* {{{ */
{
	if (!BG(user_shutdown_function_names)) {
		ALLOC_HASHTABLE(BG(user_shutdown_function_names));
		zend_hash_init(BG(user_shutdown_function_names), 0, NULL, (void (*)(void *)) user_shutdown_function_dtor, 0);
...
PHP_FUNCTION(highlight_file)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "p|b", &filename, &filename_len, &i) == FAILURE) {
		RETURN_FALSE;
	}
	...
	ret = highlight_file(filename, &syntax_highlighter_ini, TSRMLS_C);
	...
	if (i) {
		php_output_get_contents(return_value, TSRMLS_C);
...
PHP_FUNCTION(php_strip_whitespace)
{
	...
	if (open_file_for_scanning(&file_handle, TSRMLS_C) == FAILURE) {
...
PHP_FUNCTION(highlight_string)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "Z|b", &expr, &i) == FAILURE) {
		RETURN_FALSE;
	}
	...
	hicompiled_string_description = zend_make_compiled_string_description("highlighted code", TSRMLS_C);

	if (highlight_string(*expr, &syntax_highlighter_ini, hicompiled_string_description, TSRMLS_C) == FAILURE) {
...
static int php_ini_get_option(zend_ini_entry *ini_entry, TSRMLS_D, int num_args, va_list args, zend_hash_key *hash_key) /* {{{ */
{
...
PHP_FUNCTION(ini_get_all)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "|s!b", &extname, &extname_len, &details) == FAILURE) {
		return;
	}
	...
	if (extname) {
		if (zend_hash_find(&module_registry, extname, extname_len+1, (void **) &module) == FAILURE) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to find extension '%s'", extname);
			RETURN_FALSE;
		}
	...
	array_init(return_value);
	zend_hash_apply_with_arguments(EG(ini_directives), TSRMLS_C, (apply_func_args_t) php_ini_get_option, 2, return_value, extnumber, details);
}
/* }}} */
...
PHP_FUNCTION(ini_set)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ss", &varname, &varname_len, &new_value, &new_value_len) == FAILURE) {
		return;
	}
...
#define _CHECK_PATH(var, var_len, ini) php_ini_check_path(var, var_len, ini, sizeof(ini))
	...
	if (PG(open_basedir)) {
		if (_CHECK_PATH(varname, varname_len, "error_log") ||
			...
			if (php_check_open_basedir(new_value, TSRMLS_C)) {
	...
	if (zend_alter_ini_entry_ex(varname, varname_len + 1, new_value, new_value_len, PHP_INI_USER, PHP_INI_STAGE_RUNTIME, 0, TSRMLS_C) == FAILURE) {
		zval_dtor(return_value);
		RETURN_FALSE;
	}
}
...
PHP_FUNCTION(set_include_path)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s", &new_value, &new_value_len) == FAILURE) {
		return;
	}
	...
	if (zend_alter_ini_entry_ex("include_path", sizeof("include_path"), new_value, new_value_len, PHP_INI_USER, PHP_INI_STAGE_RUNTIME, 0, TSRMLS_C) == FAILURE) {
		zval_dtor(return_value);
		RETURN_FALSE;
	}
}
...
PHP_FUNCTION(print_r)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "z|b", &var, &do_return) == FAILURE) {
		RETURN_FALSE;
	}
	...
	zend_print_zval_r(var, 0, TSRMLS_C);
...
PHP_FUNCTION(ignore_user_abort)
{
	...
	if (arg) {
		zend_alter_ini_entry_ex("ignore_user_abort", sizeof("ignore_user_abort"), arg, arg_len, PHP_INI_USER, PHP_INI_STAGE_RUNTIME, 0, TSRMLS_C);
	}
...
PHP_FUNCTION(getservbyname)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ss", &name, &name_len, &proto, &proto_len) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(getservbyport)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ls", &port, &proto, &proto_len) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(getprotobynumber)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l", &proto) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(register_tick_function)
{
	...
	if (!zend_is_callable(tick_fe.arguments[0], 0, &function_name, TSRMLS_C)) {
		efree(tick_fe.arguments);
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid tick callback '%s' passed", function_name);
		efree(function_name);
		RETURN_FALSE;
...
PHP_FUNCTION(unregister_tick_function)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "z/", &function) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(is_uploaded_file)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s", &path, &path_len) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(move_uploaded_file)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ss", &path, &path_len, &new_path, &new_path_len) == FAILURE) {
		return;
	}
	...
	if (php_check_open_basedir(new_path, TSRMLS_C)) {
		RETURN_FALSE;
	}
...
#endif
	} else if (php_copy_file_ex(path, new_path, STREAM_DISABLE_OPEN_BASEDIR, TSRMLS_C) == SUCCESS) {
		VCWD_UNLINK(path);
		successful = 1;
	}
	...
	if (successful) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to move '%s' to '%s'", path, new_path);
	}
 ...
 */
static void php_simple_ini_parser_cb(zval *arg1, zval *arg2, zval *arg3, int callback_type, zval *arr, TSRMLS_D)
{
	zval *element;
 ...
 */
static void php_ini_parser_cb_with_sections(zval *arg1, zval *arg2, zval *arg3, int callback_type, zval *arr, TSRMLS_D)
{
	if (callback_type == ZEND_INI_PARSER_SECTION) {
		...
		php_simple_ini_parser_cb(arg1, arg2, arg3, callback_type, active_arr, TSRMLS_C);
	}
}
/* }}} */
...
PHP_FUNCTION(parse_ini_file)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "p|bl", &filename, &filename_len, &process_sections, &scanner_mode) == FAILURE) {
		RETURN_FALSE;
	}

	if (filename_len == 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Filename cannot be empty!");
		RETURN_FALSE;
	}
	...
	array_init(return_value);
	if (zend_parse_ini_file(&fh, 0, scanner_mode, ini_parser_cb, return_value, TSRMLS_C) == FAILURE) {
...
PHP_FUNCTION(parse_ini_string)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|bl", &str, &str_len, &process_sections, &scanner_mode) == FAILURE) {
		RETURN_FALSE;
	}
	...
	array_init(return_value);
	if (zend_parse_ini_string(string, 0, scanner_mode, ini_parser_cb, return_value, TSRMLS_C) == FAILURE) {
		zend_hash_destroy(Z_ARRVAL_P(return_value));
		efree(Z_ARRVAL_P(return_value));
		RETVAL_FALSE;
	}
...
PHP_FUNCTION(config_get_hash) /* {{{ */
{
	...
	array_init(return_value);
	zend_hash_apply_with_arguments(hash, TSRMLS_C, (apply_func_args_t) add_config_entry_cb, 1, return_value);
}
/* }}} */
#endif
