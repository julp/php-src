...
static int php_disk_total_space(char *path, double *space, TSRMLS_D) /* {{{ */
#if defined(WINDOWS) /* {{{ */
{
	double bytestotal = 0;
	HINSTANCE kernel32;
	...
	if (kernel32) {
		...
		if (gdfse) {
			...
			if (func(path,
				...
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "%s", php_win_err());
				return FAILURE;
			}
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to load kernel32.dll");
		return FAILURE;
	}
...
PHP_FUNCTION(disk_total_space)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "p", &path, &path_len) == FAILURE) {
		return;
	}

	if (php_check_open_basedir(path, TSRMLS_C)) {
		RETURN_FALSE;
	}

	if (php_disk_total_space(path, &bytestotal, TSRMLS_C) == SUCCESS) {
...
static int php_disk_free_space(char *path, double *space, TSRMLS_D) /* {{{ */
#if defined(WINDOWS) /* {{{ */
{
	double bytesfree = 0;
...
PHP_FUNCTION(disk_free_space)
{
	...
	if (php_disk_free_space(path, &bytesfree, TSRMLS_C) == SUCCESS) {
...
PHPAPI int php_get_gid_by_name(const char *name, gid_t *gid, TSRMLS_D)
{
...
static void php_do_chgrp(INTERNAL_FUNCTION_PARAMETERS, int do_lchgrp) /* {{{ */
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "pz/", &filename, &filename_len, &group) == FAILURE) {
		RETURN_FALSE;
	}
	...
	if(wrapper != &php_plain_files_wrapper || strncasecmp("file://", filename, 7) == 0) {
		if(wrapper && wrapper->wops->stream_metadata) {
			...
			if (Z_TYPE_P(group) == IS_LONG) {
				...
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "parameter 2 should be string or integer, %s given", zend_zval_type_name(group));
				RETURN_FALSE;
			}
			if(wrapper->wops->stream_metadata(wrapper, filename, option, value, NULL, TSRMLS_C)) {
				RETURN_TRUE;
			} else {
				RETURN_FALSE;
			}
...
/* On Windows, we expect regular chgrp to fail silently by default */
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Can not call chgrp() for a non-standard stream");
#endif
			RETURN_FALSE;
		}
	}
	...
	if (Z_TYPE_P(group) == IS_LONG) {
		...
		if(php_get_gid_by_name(Z_STRVAL_P(group), &gid, TSRMLS_C) != SUCCESS) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to find gid for %s", Z_STRVAL_P(group));
			RETURN_FALSE;
		}
	} else {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "parameter 2 should be string or integer, %s given", zend_zval_type_name(group));
...
#if !defined(WINDOWS) && !defined(NETWARE)
PHPAPI uid_t php_get_uid_by_name(const char *name, uid_t *uid, TSRMLS_D)
{
...
static void php_do_chown(INTERNAL_FUNCTION_PARAMETERS, int do_lchown) /* {{{ */
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "pz/", &filename, &filename_len, &user) == FAILURE) {
		return;
	}
	...
	if(wrapper != &php_plain_files_wrapper || strncasecmp("file://", filename, 7) == 0) {
		if(wrapper && wrapper->wops->stream_metadata) {
			...
			if (Z_TYPE_P(user) == IS_LONG) {
				...
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "parameter 2 should be string or integer, %s given", zend_zval_type_name(user));
				RETURN_FALSE;
			}
			if(wrapper->wops->stream_metadata(wrapper, filename, option, value, NULL, TSRMLS_C)) {
				RETURN_TRUE;
...
/* On Windows, we expect regular chown to fail silently by default */
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Can not call chown() for a non-standard stream");
#endif
			RETURN_FALSE;
		}
	}
	...
	if (Z_TYPE_P(user) == IS_LONG) {
		...
		if(php_get_uid_by_name(Z_STRVAL_P(user), &uid, TSRMLS_C) != SUCCESS) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to find uid for %s", Z_STRVAL_P(user));
			RETURN_FALSE;
		}
	} else {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "parameter 2 should be string or integer, %s given", zend_zval_type_name(user));
...
PHP_FUNCTION(chmod)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "pl", &filename, &filename_len, &mode) == FAILURE) {
		return;
	}
	...
	if(wrapper != &php_plain_files_wrapper || strncasecmp("file://", filename, 7) == 0) {
		if(wrapper && wrapper->wops->stream_metadata) {
			if(wrapper->wops->stream_metadata(wrapper, filename, PHP_STREAM_META_ACCESS, &mode, NULL, TSRMLS_C)) {
				RETURN_TRUE;
			} else {
				RETURN_FALSE;
			}
		} else {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Can not call chmod() for a non-standard stream");
			RETURN_FALSE;
		}
	}
...
PHP_FUNCTION(touch)
{
	...
	if (zend_parse_parameters(argc, TSRMLS_C, "p|ll", &filename, &filename_len, &filetime, &fileatime) == FAILURE) {
		return;
	}
	...
	if(wrapper != &php_plain_files_wrapper || strncasecmp("file://", filename, 7) == 0) {
		if(wrapper && wrapper->wops->stream_metadata) {
			if(wrapper->wops->stream_metadata(wrapper, filename, PHP_STREAM_META_TOUCH, newtime, NULL, TSRMLS_C)) {
				RETURN_TRUE;
			} else {
				RETURN_FALSE;
			}
			...
			php_stream *stream;
			if(argc > 1) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Can not call touch() for a non-standard stream");
				RETURN_FALSE;
			}
	...
	if (VCWD_ACCESS(filename, F_OK) != 0) {
		...
		if (file == NULL) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to create file %s because %s", filename, strerror(errno));
			RETURN_FALSE;
		}
		fclose(file);
	}
	...
	if (ret == -1) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Utime failed: %s", strerror(errno));
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
*/
PHPAPI void php_clear_stat_cache(zend_bool clear_realpath_cache, const char *filename, int filename_len, TSRMLS_D)
{
	...
	if (clear_realpath_cache) {
		if (filename != NULL) {
			realpath_cache_del(filename, filename_len, TSRMLS_C);
		} else {
...
PHP_FUNCTION(clearstatcache)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "|bp", &clear_realpath_cache, &filename, &filename_len) == FAILURE) {
		return;
	}

	php_clear_stat_cache(clear_realpath_cache, filename, filename_len, TSRMLS_C);
}
/* }}} */
...
/* {{{ php_stat
 */
PHPAPI void php_stat(const char *filename, php_stat_len filename_length, int type, zval *return_value, TSRMLS_D)
{
	...
	if ((wrapper = php_stream_locate_url_wrapper(filename, &local, 0, TSRMLS_C)) == &php_plain_files_wrapper && php_check_open_basedir(local, TSRMLS_C)) {
		RETURN_FALSE;
	}
	...
	if (php_stream_stat_path_ex((char *)filename, flags, &ssb, NULL)) {
		/* Error Occurred */
		if (!IS_EXISTS_CHECK(type)) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "%sstat failed for %s", IS_LINK_OPERATION(type) ? "L" : "", filename);
		}
		RETURN_FALSE;
	}
	...
	case FS_TYPE:
		...
		php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Unknown file type (%d)", ssb.sb.st_mode&S_IFMT);
	...
	php_error_docref(NULL, TSRMLS_C, E_WARNING, "Didn't understand stat call");
	RETURN_FALSE;
}
/* }}} */
...
void name(INTERNAL_FUNCTION_PARAMETERS) { \
	...
	\
	php_stat(filename, (php_stat_len) filename_len, funcnum, return_value, TSRMLS_C); \
}
/* }}} */
