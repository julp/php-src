...
php_stream *php_stream_url_wrap_http_ex(php_stream_wrapper *wrapper, char *path, char *mode, int options, char **opened_path, php_stream_context *context, int redirect_max, int flags STREAMS_DC, TSRMLS_D) /* {{{ */
{
	...
	if (redirect_max < 1) {
		php_stream_wrapper_log_error(wrapper, options, TSRMLS_C, "Redirection limit reached, aborting");
		return NULL;
	}
	...
	if (strncasecmp(resource->scheme, "http", sizeof("http")) && strncasecmp(resource->scheme, "https", sizeof("https"))) {
		...
		if (strpbrk(mode, "awx+")) {
			php_stream_wrapper_log_error(wrapper, options, TSRMLS_C, "HTTP wrapper does not support writeable connections");
			php_url_free(resource);
			return NULL;
		}
	...
	if (errstr) {
		php_stream_wrapper_log_error(wrapper, options, TSRMLS_C, "%s", errstr);
		efree(errstr);
		errstr = NULL;
	}
...
finish:
		...
		if (php_stream_write(stream, header.c, header.len) != header.len) {
			php_stream_wrapper_log_error(wrapper, options, TSRMLS_C, "Cannot connect to HTTPS server through proxy");
		...
		if (stream) {
			if (php_stream_xport_crypto_setup(stream, STREAM_CRYPTO_METHOD_SSLv23_CLIENT, NULL, TSRMLS_C) < 0 ||
	...
	if (context && php_stream_context_get_option(context, "http", "header", &tmpzval) == SUCCESS) {
		...
		if (Z_TYPE_PP(tmpzval) == IS_ARRAY) {
			...
			if (tmpstr.c) {
				tmp = php_trim(tmpstr.c, strlen(tmpstr.c), NULL, 0, NULL, 3, TSRMLS_C);
				smart_str_free(&tmpstr);
			}
		}
		if (Z_TYPE_PP(tmpzval) == IS_STRING && Z_STRLEN_PP(tmpzval)) {
			/* Remove newlines and spaces from start and end php_trim will estrndup() */
			tmp = php_trim(Z_STRVAL_PP(tmpzval), Z_STRLEN_PP(tmpzval), NULL, 0, NULL, 3, TSRMLS_C);
		}
		if (tmp && strlen(tmp) > 0) {
			char *s;
			...
			if (!header_init) { /* Remove post headers for redirects */
				...
				efree(tmp_c);
				tmp_c = php_trim(tmp, strlen(tmp), NULL, 0, NULL, 3, TSRMLS_C);
				efree(tmp);
				tmp = tmp_c;
			}
...
#define _UA_HEADER "User-Agent: %s\r\n"
		...
		if (ua_len > sizeof(_UA_HEADER)) {
			...
			if ((ua_len = slprintf(ua, ua_len, _UA_HEADER, ua_str)) > 0) {
				...
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Cannot construct User-agent header");
			}
	...
	if (header_init && context &&
		...
		if (!(have_header & HTTP_HEADER_TYPE)) {
			...
			php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Content-type not specified assuming application/x-www-form-urlencoded");
		}
		php_stream_write(stream, "\r\n", sizeof("\r\n")-1);
	...
	if (!php_stream_eof(stream)) {
		...
		php_stream_wrapper_log_error(wrapper, options, TSRMLS_C, "HTTP request failed, unexpected end of socket!");
		goto out;
	}
		...
		if (php_stream_get_line(stream, http_header_line, HTTP_HEADER_BLOCK_SIZE, &http_header_line_length) && *http_header_line != '\n' && *http_header_line != '\r') {
			...
			if (*e != '\n') {
				do { /* partial header */
					if (php_stream_get_line(stream, http_header_line, HTTP_HEADER_BLOCK_SIZE, &http_header_line_length) == NULL) {
						php_stream_wrapper_log_error(wrapper, options, TSRMLS_C, "Failed to read HTTP headers");
						goto out;
					}
			...
			if (!strncasecmp(http_header_line, "Location: ", 10)) {
				...
				if (!(options & STREAM_ONLY_GET_HEADERS)) {
					...
					if (decode) {
						transfer_encoding = php_stream_filter_create("dechunk", NULL, php_stream_is_persistent(stream), TSRMLS_C);
		...
		if (location[0] != '\0') {
			...
			*new_path='\0';
			...
			if ((resource = php_url_parse(new_path)) == NULL) {
				php_stream_wrapper_log_error(wrapper, options, TSRMLS_C, "Invalid redirect URL! %s", new_path);
				goto out;
			}
...
#define CHECK_FOR_CNTRL_CHARS(val) { \
	if (val) { \
		...
		while (s < e) { \
			...
			stream = php_stream_url_wrap_http_ex(wrapper, new_path, mode, options, opened_path, context, --redirect_max, HTTP_WRAPPER_REDIRECTED STREAMS_CC, TSRMLS_C);
		} else {
			php_stream_wrapper_log_error(wrapper, options, TSRMLS_C, "HTTP request failed! %s", tmp_line);
		}
	}
out:
	if (protocol_version) {
	...
	if (stream) {
		...
		php_stream_filter_free(transfer_encoding, TSRMLS_C);
	}
...
php_stream *php_stream_url_wrap_http(php_stream_wrapper *wrapper, char *path, char *mode, int options, char **opened_path, php_stream_context *context STREAMS_DC, TSRMLS_D) /* {{{ */
{
	return php_stream_url_wrap_http_ex(wrapper, path, mode, options, opened_path, context, PHP_URL_REDIRECT_MAX, HTTP_WRAPPER_HEADER_INIT STREAMS_CC, TSRMLS_C);
}
/* }}} */

static int php_stream_http_stream_stat(php_stream_wrapper *wrapper, php_stream *stream, php_stream_statbuf *ssb, TSRMLS_D) /* {{{ */
{
