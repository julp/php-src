...
PHP_FUNCTION(readlink)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "p", &link, &link_len) == FAILURE) {
		return;
	}
	...
	if (php_sys_readlink(link, target, MAXPATHLEN) == -1) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "readlink failed to read the symbolic link (%s), error %d)", link, GetLastError());
		RETURN_FALSE;
	}
...
PHP_FUNCTION(symlink)
{
	...
	if (kernel32) {
		...
		if (pCreateSymbolicLinkA == NULL) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Can't call CreateSymbolicLinkA");
			RETURN_FALSE;
		}
	} else {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Can't call get a handle on kernel32.dll");
		RETURN_FALSE;
	}

	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "pp", &topath, &topath_len, &frompath, &frompath_len) == FAILURE) {
		return;
	}
	
	if (!expand_filepath(frompath, source_p, TSRMLS_C)) {
	...
	if (!expand_filepath_ex(topath, dest_p, dirname, len, TSRMLS_C)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "No such file or directory");
		RETURN_FALSE;
	}

	if (php_stream_locate_url_wrapper(source_p, NULL, STREAM_LOCATE_WRAPPERS_ONLY, TSRMLS_C) ||
		php_stream_locate_url_wrapper(dest_p, NULL, STREAM_LOCATE_WRAPPERS_ONLY, TSRMLS_C) ) 
	{
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to symlink to a URL");
		RETURN_FALSE;
	}
	...
	if ((attr = GetFileAttributes(topath)) == INVALID_FILE_ATTRIBUTES) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Could not fetch file information(error %d)", GetLastError());
			RETURN_FALSE;
	}
	...
	if (!ret) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Cannot create symlink, error code(%d)", GetLastError());
		RETURN_FALSE;
	}
...
PHP_FUNCTION(link)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ss", &frompath, &frompath_len, &topath, &topath_len) == FAILURE) {
		return;
	}

	if (!expand_filepath(frompath, source_p, TSRMLS_C) || !expand_filepath(topath, dest_p, TSRMLS_C)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "No such file or directory");
		RETURN_FALSE;
	}
	...
	if (php_stream_locate_url_wrapper(source_p, NULL, STREAM_LOCATE_WRAPPERS_ONLY, TSRMLS_C) ||
		php_stream_locate_url_wrapper(dest_p, NULL, STREAM_LOCATE_WRAPPERS_ONLY, TSRMLS_C) ) 
	{
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to link to a URL");
		RETURN_FALSE;
	}
