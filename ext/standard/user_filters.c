...
#define GET_FILTER_FROM_OBJ()	{ \
	...
	if (FAILURE == zend_hash_index_find(Z_OBJPROP_P(this_ptr), 0, (void**)&tmp)) { \
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "filter property vanished"); \
		RETURN_FALSE; \
	} \
...
PHP_MINIT_FUNCTION(user_filters)
{
	...
	if ((php_user_filter = zend_register_internal_class(&user_filter_class_entry, TSRMLS_C)) == NULL) {
		return FAILURE;
	}
	zend_declare_property_string(php_user_filter, "filtername", sizeof("filtername")-1, "", ZEND_ACC_PUBLIC, TSRMLS_C);
	zend_declare_property_string(php_user_filter, "params", sizeof("params")-1, "", ZEND_ACC_PUBLIC, TSRMLS_C);
...
static void userfilter_dtor(php_stream_filter *thisfilter, TSRMLS_D)
{
			...
			,TSRMLS_D)
{
	...
	if (call_result == SUCCESS && retval != NULL) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "failed to call filter function");
	}
	...
	if (buckets_in->head) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unprocessed filter buckets remaining on input brigade");
	...
	Z_OBJ_HANDLER_P(obj, unset_property)(obj, &zpropname, 0, TSRMLS_C);
...
static php_stream_filter *user_filter_factory_create(const char *filtername,
		zval *filterparams, int persistent, TSRMLS_D)
{
	...
	if (fdat->ce == NULL) {
		if (FAILURE == zend_lookup_class(fdat->classname, strlen(fdat->classname),
					(zend_class_entry ***)&fdat->ce, TSRMLS_C)) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING,
...
PHP_FUNCTION(stream_bucket_make_writeable)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "z", &zbrigade) == FAILURE) {
		RETURN_FALSE;
	}
	...
	if (brigade->head && (bucket = php_stream_bucket_make_writeable(brigade->head, TSRMLS_C))) {
...
static void php_stream_bucket_attach(int append, INTERNAL_FUNCTION_PARAMETERS)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "zo", &zbrigade, &zobject) == FAILURE) {
		RETURN_FALSE;
	}

	if (FAILURE == zend_hash_find(Z_OBJPROP_P(zobject), "bucket", 7, (void**)&pzbucket)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Object has no bucket property");
		RETURN_FALSE;
	}
	...
	if (SUCCESS == zend_hash_find(Z_OBJPROP_P(zobject), "data", 5, (void**)&pzdata) && (*pzdata)->type == IS_STRING) {
		if (!bucket->own_buf) {
			bucket = php_stream_bucket_make_writeable(bucket, TSRMLS_C);
		}
	...
	if (append) {
		php_stream_bucket_append(brigade, bucket, TSRMLS_C);
	} else {
		php_stream_bucket_prepend(brigade, bucket, TSRMLS_C);
	}
...
PHP_FUNCTION(stream_bucket_new)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "zs", &zstream, &buffer, &buffer_len) == FAILURE) {
		RETURN_FALSE;
	}
	...
	bucket = php_stream_bucket_new(stream, pbuffer, buffer_len, 1, php_stream_is_persistent(stream), TSRMLS_C);
...
PHP_FUNCTION(stream_filter_register)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ss", &filtername, &filtername_len,
	...
	if (!filtername_len) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Filter name cannot be empty");
		return;
	}

	if (!classname_len) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Class name cannot be empty");
		return;
	}
	...
	if (zend_hash_add(BG(user_filter_map), filtername, filtername_len + 1, (void*)fdat,
			...
			php_stream_filter_register_factory_volatile(filtername, &user_filter_factory, TSRMLS_C) == SUCCESS) {
		RETVAL_TRUE;
	}
