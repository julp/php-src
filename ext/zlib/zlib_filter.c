	...
	while (buckets_in->head) {
		...
		while (bin < (unsigned int) bucket->buflen) {
			...
			if (data->strm.avail_out < data->outbuf_len) {
				...
				out_bucket = php_stream_bucket_new(stream, estrndup(data->outbuf, bucketlen), bucketlen, 1, 0, TSRMLS_C);
				php_stream_bucket_append(buckets_out, out_bucket, TSRMLS_C);
	...
	if (!data->finished && flags & PSFS_FLAG_FLUSH_CLOSE) {
		...
		while (status == Z_OK) {
			...
			if (data->strm.avail_out < data->outbuf_len) {
				...
				bucket = php_stream_bucket_new(stream, estrndup(data->outbuf, bucketlen), bucketlen, 1, 0, TSRMLS_C);
...
static void php_zlib_inflate_dtor(php_stream_filter *thisfilter, TSRMLS_D)
{
	if (thisfilter && thisfilter->abstract) {
...
static void php_zlib_deflate_dtor(php_stream_filter *thisfilter, TSRMLS_D)
{
	if (thisfilter && thisfilter->abstract) {
		php_zlib_filter_data *data = thisfilter->abstract;
...
static php_stream_filter *php_zlib_filter_create(const char *filtername, zval *filterparams, int persistent, TSRMLS_D)
{
	...
	if (!data) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Failed allocating %zd bytes", sizeof(php_zlib_filter_data));
		return NULL;
	}
	...
	if (!data->inbuf) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Failed allocating %zd bytes", data->inbuf_len);
	...
	if (!data->outbuf) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Failed allocating %zd bytes", data->outbuf_len);
		...
		if (filterparams) {
			...
			if ((Z_TYPE_P(filterparams) == IS_ARRAY || Z_TYPE_P(filterparams) == IS_OBJECT) &&
				...
				if (Z_LVAL(tmp) < -MAX_WBITS || Z_LVAL(tmp) > MAX_WBITS + 32) {
					php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid parameter give for window size. (%ld)", Z_LVAL(tmp));
				} else {
				...
				case IS_OBJECT:
					if (zend_hash_find(HASH_OF(filterparams), "memory", sizeof("memory"), (void**) &tmpzval) == SUCCESS) {
						...
						if (Z_LVAL(tmp) < 1 || Z_LVAL(tmp) > MAX_MEM_LEVEL) {
							php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid parameter give for memory level. (%ld)", Z_LVAL(tmp));
						} else {
							memLevel = Z_LVAL(tmp);
						}
					}
...
factory_setlevel:
					...
					if (Z_LVAL(tmp) < -1 || Z_LVAL(tmp) > 9) {
						php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid compression level specified. (%ld)", Z_LVAL(tmp));
					} else {
						level = Z_LVAL(tmp);
					}
					break;
				default:
					php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid filter parameter, ignored");
			}
		}
