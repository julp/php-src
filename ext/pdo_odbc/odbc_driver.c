...
static int pdo_odbc_fetch_error_func(pdo_dbh_t *dbh, pdo_stmt_t *stmt, zval *info, TSRMLS_D)
{
...
/* printf("@@ SQLSTATE[%s] %s\n", *pdo_err, einfo->last_err_msg); */
	if (!dbh->methods) {
		zend_throw_exception_ex(php_pdo_get_exception(), einfo->last_error, TSRMLS_C, "SQLSTATE[%s] %s: %d %s",
...
static int odbc_handle_closer(pdo_dbh_t *dbh, TSRMLS_D)
{
	pdo_odbc_db_handle *H = (pdo_odbc_db_handle*)dbh->driver_data;
...
static int odbc_handle_preparer(pdo_dbh_t *dbh, const char *sql, long sql_len, pdo_stmt_t *stmt, zval *driver_options, TSRMLS_D)
{
	RETCODE rc;
	pdo_odbc_db_handle *H = (pdo_odbc_db_handle *)dbh->driver_data;
	...
	cursor_type = pdo_attr_lval(driver_options, PDO_ATTR_CURSOR, PDO_CURSOR_FWDONLY, TSRMLS_C);
...
static long odbc_handle_doer(pdo_dbh_t *dbh, const char *sql, long sql_len, TSRMLS_D)
{
	pdo_odbc_db_handle *H = (pdo_odbc_db_handle *)dbh->driver_data;
	RETCODE rc;
	SQLLEN row_count = -1;
...
static int odbc_handle_quoter(pdo_dbh_t *dbh, const char *unquoted, int unquotedlen, char **quoted, int *quotedlen, enum pdo_param_type param_type,  TSRMLS_D)
{
	pdo_odbc_db_handle *H = (pdo_odbc_db_handle *)dbh->driver_data;
	/* TODO: figure it out */
	return 0;
...
static int odbc_handle_begin(pdo_dbh_t *dbh, TSRMLS_D)
{
	if (dbh->auto_commit) {
...
static int odbc_handle_commit(pdo_dbh_t *dbh, TSRMLS_D)
{
	pdo_odbc_db_handle *H = (pdo_odbc_db_handle *)dbh->driver_data;
	RETCODE rc;
...
static int odbc_handle_rollback(pdo_dbh_t *dbh, TSRMLS_D)
{
	pdo_odbc_db_handle *H = (pdo_odbc_db_handle *)dbh->driver_data;
	RETCODE rc;
...
static int odbc_handle_set_attr(pdo_dbh_t *dbh, long attr, zval *val, TSRMLS_D)
{
	pdo_odbc_db_handle *H = (pdo_odbc_db_handle *)dbh->driver_data;
	switch (attr) {
...
static int odbc_handle_get_attr(pdo_dbh_t *dbh, long attr, zval *val, TSRMLS_D)
{
	pdo_odbc_db_handle *H = (pdo_odbc_db_handle *)dbh->driver_data;
	switch (attr) {
...
static int pdo_odbc_handle_factory(pdo_dbh_t *dbh, zval *driver_options, TSRMLS_D) /* {{{ */
{
	...
	cursor_lib = pdo_attr_lval(driver_options, PDO_ODBC_ATTR_USE_CURSOR_LIBRARY, SQL_CUR_USE_IF_NEEDED, TSRMLS_C);
