...
PHP_FUNCTION(curl_multi_add_handle)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rr", &z_mh, &z_ch) == FAILURE) {
		return;
	}
...
void _php_curl_multi_cleanup_list(void *data) /* {{{ */
{
	...
	ch = (php_curl *) zend_fetch_resource(&z_ch, TSRMLS_C, -1, le_curl_name, NULL, 1, le_curl);
	if (!ch) {
		return;
	}
...
PHP_FUNCTION(curl_multi_select)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r|d", &z_mh, &timeout) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(curl_multi_exec)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rz", &z_mh, &z_still_running) == FAILURE) {
		return;
	}
...

	{
		...
		for(pz_ch = (zval *)zend_llist_get_first_ex(&mh->easyh, &pos); pz_ch;
			...
			_php_curl_verify_handlers(ch, 1, TSRMLS_C);
		}
	}
...
PHP_FUNCTION(curl_multi_getcontent)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r", &z_ch) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(curl_multi_info_read)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r|z", &z_mh, &zmsgs_in_queue) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(curl_multi_close)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r", &z_mh) == FAILURE) {
		return;
	}
...
void _php_curl_multi_close(zend_rsrc_list_entry *rsrc, TSRMLS_D) /* {{{ */
{
	...
	if (mh) {
		...
		for(pz_ch = (zval *)zend_llist_get_first_ex(&mh->easyh, &pos); pz_ch;
			...
			ch = (php_curl *) zend_fetch_resource(&pz_ch, TSRMLS_C, -1, le_curl_name, NULL, 1, le_curl);
			_php_curl_verify_handlers(ch, 0, TSRMLS_C);
		}
...
static int _php_curl_multi_setopt(php_curlm *mh, long option, zval **zvalue, zval *return_value, TSRMLS_D) /* {{{ */
{ 
	...
	switch (option) {
		...
		default:
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid curl multi configuration option");
...
PHP_FUNCTION(curl_multi_setopt)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rlZ", &z_mh, &options, &zvalue) == FAILURE) {
		return;
	}

	ZEND_FETCH_RESOURCE(mh, php_curlm *, &z_mh, -1, le_curl_multi_handle_name, le_curl_multi_handle);

	if (!_php_curl_multi_setopt(mh, options, zvalue, return_value, TSRMLS_C)) {
		RETURN_TRUE;
	} else {
		RETURN_FALSE;
	}
