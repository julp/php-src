...
static int pdo_oci_fetch_error_func(pdo_dbh_t *dbh, pdo_stmt_t *stmt, zval *info, TSRMLS_D) /* {{{ */
{
...
ub4 _oci_error(OCIError *err, pdo_dbh_t *dbh, pdo_stmt_t *stmt, char *what, sword status, int isinit, const char *file, int line, TSRMLS_D) /* {{{ */
{
	...
	if (!dbh->methods) {
		zend_throw_exception_ex(php_pdo_get_exception(), einfo->errcode, TSRMLS_C, "SQLSTATE[%s]: %s", *pdo_err, einfo->errmsg);
	}
...
static int oci_handle_closer(pdo_dbh_t *dbh, TSRMLS_D) /* {{{ */
{
	pdo_oci_db_handle *H = (pdo_oci_db_handle *)dbh->driver_data;
...
static int oci_handle_preparer(pdo_dbh_t *dbh, const char *sql, long sql_len, pdo_stmt_t *stmt, zval *driver_options, TSRMLS_D) /* {{{ */
{
	pdo_oci_db_handle *H = (pdo_oci_db_handle *)dbh->driver_data;
	...
	prefetch = pdo_oci_sanitize_prefetch(pdo_attr_lval(driver_options, PDO_ATTR_PREFETCH, PDO_OCI_PREFETCH_DEFAULT, TSRMLS_C));
	if (prefetch) {
...
static long oci_handle_doer(pdo_dbh_t *dbh, const char *sql, long sql_len, TSRMLS_D) /* {{{ */
{
	pdo_oci_db_handle *H = (pdo_oci_db_handle *)dbh->driver_data;
	OCIStmt		*stmt;
	ub2 stmt_type;
	...
	if (stmt_type == OCI_STMT_SELECT) {
		...
		OCIHandleFree(stmt, OCI_HTYPE_STMT);
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "issuing a SELECT query here is invalid");
		return -1;
	}
...
static int oci_handle_quoter(pdo_dbh_t *dbh, const char *unquoted, int unquotedlen, char **quoted, int *quotedlen, enum pdo_param_type paramtype,  TSRMLS_D) /* {{{ */
{
	int qcount = 0;
	char const *cu, *l, *r;
	char *c;
...
static int oci_handle_begin(pdo_dbh_t *dbh, TSRMLS_D) /* {{{ */
{
...
static int oci_handle_commit(pdo_dbh_t *dbh, TSRMLS_D) /* {{{ */
{
	pdo_oci_db_handle *H = (pdo_oci_db_handle *)dbh->driver_data;
...
static int oci_handle_rollback(pdo_dbh_t *dbh, TSRMLS_D) /* {{{ */
{
	pdo_oci_db_handle *H = (pdo_oci_db_handle *)dbh->driver_data;
...
static int oci_handle_set_attribute(pdo_dbh_t *dbh, long attr, zval *val, TSRMLS_D) /* {{{ */
{
	pdo_oci_db_handle *H = (pdo_oci_db_handle *)dbh->driver_data;
...
static int oci_handle_get_attribute(pdo_dbh_t *dbh, long attr, zval *return_value, TSRMLS_D)  /* {{{ */
{
	pdo_oci_db_handle *H = (pdo_oci_db_handle *)dbh->driver_data;
...
static int pdo_oci_check_liveness(pdo_dbh_t *dbh, TSRMLS_D) /* {{{ */
{
	pdo_oci_db_handle *H = (pdo_oci_db_handle *)dbh->driver_data;
	sb4 error_code = 0;
	char version[256];
...
static int pdo_oci_handle_factory(pdo_dbh_t *dbh, zval *driver_options, TSRMLS_D) /* {{{ */
{
	pdo_oci_db_handle *H;
	int i, ret = 0;
...
cleanup:
	...
	if (!ret) {
		oci_handle_closer(dbh, TSRMLS_C);
	}
