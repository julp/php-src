 ...
 */
static void php_mysqli_set_error(long mysql_errno, char *mysql_err, TSRMLS_D)
{
...
void mysqli_common_connect(INTERNAL_FUNCTION_PARAMETERS, zend_bool is_real_connect, zend_bool in_ctor)
{
	...
	if (!is_real_connect) {
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "|ssssls", &hostname, &hostname_len, &username, &username_len,
									&passwd, &passwd_len, &dbname, &dbname_len, &port, &socket, &socket_len) == FAILURE) {
			return;
		}

		if (object && instanceof_function(Z_OBJCE_P(object), mysqli_link_class_entry, TSRMLS_C)) {
			mysqli_resource = ((mysqli_object *) zend_object_store_get_object(object, TSRMLS_C))->ptr;
		...
		/* We have flags too */
		if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "O|sssslsl", &object, mysqli_link_class_entry,
		...
		(mysqli_resource->status > MYSQLI_STATUS_INITIALIZED))
	{
		...
		php_mysqli_close(mysql, MYSQLI_CLOSE_IMPLICIT, mysqli_resource->status, TSRMLS_C);
	}

	if (strlen(SAFE_STR(hostname)) > 2 && !strncasecmp(hostname, "p:", 2)) {
		hostname += 2;
		if (!MyG(allow_persistent)) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Persistent connections are disabled. Downgrading to normal");
		} else {
	...
	if (MyG(max_links) != -1 && MyG(num_links) >= MyG(max_links)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Too many open links (%ld)", MyG(num_links));
		goto err;
	}
	...
	{
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Too many open persistent links (%ld)",
	...
	if (mysqlnd_connect(mysql->mysql, hostname, username, passwd, passwd_len, dbname, dbname_len,
						port, socket, flags, TSRMLS_C) == NULL)
#endif
	{
		/* Save error messages - for mysqli_connect_error() & mysqli_connect_errno() */
		php_mysqli_set_error(mysql_errno(mysql->mysql), (char *) mysql_error(mysql->mysql), TSRMLS_C);
		php_mysqli_throw_sql_exception((char *)mysql_sqlstate(mysql->mysql), mysql_errno(mysql->mysql), TSRMLS_C,
	...
	if (!object || !instanceof_function(Z_OBJCE_P(object), mysqli_link_class_entry, TSRMLS_C)) {
		...
		((mysqli_object *) zend_object_store_get_object(object, TSRMLS_C))->ptr = mysqli_resource;
	}
	if (!is_real_connect) {
		return;
	} else {
...
PHP_FUNCTION(mysqli_fetch_all)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "O|l", &mysql_result, mysqli_result_class_entry, &mode) == FAILURE) {
		return;
	}
	MYSQLI_FETCH_RESOURCE(result, MYSQL_RES *, &mysql_result, "mysqli_result", MYSQLI_STATUS_VALID);

	if (!mode || (mode & ~MYSQLND_FETCH_BOTH)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Mode can be only MYSQLI_FETCH_NUM, "
...
PHP_FUNCTION(mysqli_query)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Os|l", &mysql_link, mysqli_link_class_entry, &query, &query_len, &resultmode) == FAILURE) {
		return;
	}

	if (!query_len) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Empty query");
		RETURN_FALSE;
	}
...
static int mysqlnd_zval_array_to_mysqlnd_array(zval *in_array, MYSQLND ***out_array, TSRMLS_D)
{
	zval **elem;
	int i = 0, current = 0;
	...
	*out_array = ecalloc(zend_hash_num_elements(Z_ARRVAL_P(in_array)) + 1, sizeof(MYSQLND *));
	for (zend_hash_internal_pointer_reset(Z_ARRVAL_P(in_array));
		...
		if (Z_TYPE_PP(elem) != IS_OBJECT ||
			!instanceof_function(Z_OBJCE_PP(elem), mysqli_link_class_entry, TSRMLS_C)) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Parameter %d not a mysqli object", i);
		} else {
			MY_MYSQL *mysql;
			MYSQLI_RESOURCE *my_res;
			mysqli_object *intern = (mysqli_object *)zend_object_store_get_object(*elem, TSRMLS_C);
			if (!(my_res = (MYSQLI_RESOURCE *)intern->ptr)) {
		  		php_error_docref(NULL, TSRMLS_C, E_WARNING, "[%d] Couldn't fetch %s", i, intern->zo.ce->name);
				continue;
		  	}
			mysql = (MY_MYSQL*) my_res->ptr;
			if (MYSQLI_STATUS_VALID && my_res->status < MYSQLI_STATUS_VALID) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid object %d or resource %s", i, intern->zo.ce->name);
				continue;
			}
...
static int mysqlnd_zval_array_from_mysqlnd_array(MYSQLND **in_array, zval *out_array, TSRMLS_D)
{
	MYSQLND **p = in_array;
	HashTable *new_hash;
	zval **elem, **dest_elem;
	...
	{
		i++;
		if (Z_TYPE_PP(elem) != IS_OBJECT || !instanceof_function(Z_OBJCE_PP(elem), mysqli_link_class_entry, TSRMLS_C)) {
			continue;
		}
		{
			MY_MYSQL *mysql;
...
static int mysqlnd_dont_poll_zval_array_from_mysqlnd_array(MYSQLND **in_array, zval *in_zval_array, zval *out_array, TSRMLS_D)
{
	MYSQLND **p = in_array;
	HashTable *new_hash;
	zval **elem, **dest_elem;
...
PHP_FUNCTION(mysqli_poll)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "a!a!al|l", &r_array, &e_array, &dont_poll_array, &sec, &usec) == FAILURE) {
		return;
	}
	if (sec < 0 || usec < 0) {
	...
	if (r_array != NULL) {
		mysqlnd_zval_array_to_mysqlnd_array(r_array, &new_r_array, TSRMLS_C);
	}
	if (e_array != NULL) {
		mysqlnd_zval_array_to_mysqlnd_array(e_array, &new_e_array, TSRMLS_C);
	}

	ret = mysqlnd_poll(new_r_array, new_e_array, &new_dont_poll_array, sec, usec, &desc_num);

	mysqlnd_dont_poll_zval_array_from_mysqlnd_array(r_array != NULL ? new_dont_poll_array:NULL, r_array, dont_poll_array, TSRMLS_C);

	if (r_array != NULL) {
		mysqlnd_zval_array_from_mysqlnd_array(new_r_array, r_array, TSRMLS_C);
	}
	if (e_array != NULL) {
		mysqlnd_zval_array_from_mysqlnd_array(new_e_array, e_array, TSRMLS_C);
	}
...
PHP_FUNCTION(mysqli_reap_async_query)
{
	...
	if (!mysql_field_count(mysql->mysql)) {
		/* no result set - not a SELECT */
		if (MyG(report_mode) & MYSQLI_REPORT_INDEX) {
/*			php_mysqli_report_index("n/a", mysqli_server_status(mysql->mysql), TSRMLS_C); */
		}
		RETURN_TRUE;
	}
...
#ifdef MYSQLI_USE_MYSQLND
		w = php_get_warnings(mysql->mysql->data, TSRMLS_C);
#else
		w = php_get_warnings(mysql->mysql, TSRMLS_C);
#endif
	} else {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(mysqli_stmt_get_warnings)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "O", &stmt_link, mysqli_stmt_class_entry) == FAILURE) {
		return;
	}
	MYSQLI_FETCH_RESOURCE_STMT(stmt, &stmt_link, MYSQLI_STATUS_VALID);

	if (mysqli_stmt_warning_count(stmt->stmt)) {
		w = php_get_warnings(mysqli_stmt_get_connection(stmt->stmt), TSRMLS_C);
	} else {
		RETURN_FALSE;
	}
	mysqli_resource = (MYSQLI_RESOURCE *)ecalloc (1, sizeof(MYSQLI_RESOURCE));
...
PHP_FUNCTION(mysqli_set_charset)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Os", &mysql_link, mysqli_link_class_entry, &cs_name, &csname_len) == FAILURE) {
		return;
	}
	MYSQLI_FETCH_RESOURCE_CONN(mysql, &mysql_link, MYSQLI_STATUS_VALID);
...
PHP_FUNCTION(mysqli_get_charset)
{
	...
	if (!cs) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "The connection has no charset associated");
		RETURN_NULL();
	}
	name = cs->name;
...
PHP_FUNCTION(mysqli_begin_transaction)
{
	...
	MYSQLI_FETCH_RESOURCE_CONN(mysql, &mysql_link, MYSQLI_STATUS_VALID);
	if (flags < 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid value for parameter flags (%ld)", flags);
		err = TRUE;
	}
	if (!name_len) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Savepoint name cannot be empty");
		err = TRUE;
	}
	if (TRUE == err) {
		RETURN_FALSE;			
...
PHP_FUNCTION(mysqli_savepoint)
{
	...
	if (zend_parse_method_parameters(ZEND_NUM_ARGS(), TSRMLS_C, getThis(), "Os", &mysql_link, mysqli_link_class_entry, &name, &name_len) == FAILURE) {
		return;
	}
	MYSQLI_FETCH_RESOURCE_CONN(mysql, &mysql_link, MYSQLI_STATUS_VALID);
	if (!name || !name_len) {
