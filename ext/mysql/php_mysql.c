		...
		do {					\
			...
			if (_mysql_result && type==le_result) {						\
				if (mysql_result_is_unbuffered(_mysql_result) && !mysql_eof(_mysql_result)) { \
					php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Function called without first fetching all rows from a previous unbuffered query");	\
				}						\
 ...
 */
static void _free_mysql_result(zend_rsrc_list_entry *rsrc, TSRMLS_D)
{
 ...
 */
static void php_mysql_set_default_link(int id, TSRMLS_D)
{
...
/* {{{ php_mysql_select_db
*/
static int php_mysql_select_db(php_mysql_conn *mysql, char *db, TSRMLS_D)
{
...
/* {{{ _close_mysql_link
 */
static void _close_mysql_link(zend_rsrc_list_entry *rsrc, TSRMLS_D)
{
...
/* {{{ _close_mysql_plink
 */
static void _close_mysql_plink(zend_rsrc_list_entry *rsrc, TSRMLS_D)
{
	php_mysql_conn *link = (php_mysql_conn *)rsrc->ptr;
	void (*handler) (int);
...
static MYSQLND * mysql_convert_zv_to_mysqlnd(zval * zv, TSRMLS_D)
{
	...
	mysql = (php_mysql_conn *)zend_fetch_resource(&zv, TSRMLS_C, -1, "MySQL-Link", NULL, 2, le_link, le_plink);
...
#ifdef MYSQL_USE_MYSQLND
	mysqlnd_reverse_api_register_api(&mysql_reverse_api, TSRMLS_C);
#endif
...
static int php_mysql_persistent_helper(zend_rsrc_list_entry *le, TSRMLS_D)
{
	if (le->type == le_plink) {
...
PHP_RSHUTDOWN_FUNCTION(mysql)
{
	...
	if (MySG(trace_mode)) {
		if (MySG(result_allocated)){
			php_error_docref("function.mysql-free-result", TSRMLS_C, E_WARNING, "%lu result set(s) not freed. Use mysql_free_result to free result sets which were requested using mysql_query()", MySG(result_allocated));
		}
	}
...
#if defined(A0) && defined(MYSQL_USE_MYSQLND)
	zend_hash_apply(&EG(persistent_list), (apply_func_t) php_mysql_persistent_helper, TSRMLS_C);
#endif
...
static void php_mysql_do_connect(INTERNAL_FUNCTION_PARAMETERS, int persistent)
{
    ...
    php_error_docref(NULL, TSRMLS_C,
                     E_DEPRECATED,
	...
	if (PG(sql_safe_mode)) {
		if (ZEND_NUM_ARGS()>0) {
			php_error_docref(NULL, TSRMLS_C, E_NOTICE, "SQL safe mode in effect - ignoring host/user/password information");
		}
		...
		if (persistent) {
			if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "|s!s!s!l", &host_and_port, &host_len,
									&user, &user_len, &passwd, &passwd_len,
									&client_flags)==FAILURE) {
				return;
        	}
		} else {
			if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "|s!s!s!bl", &host_and_port, &host_len,
										&user, &user_len, &passwd, &passwd_len,
	...
	if (persistent) {
		...
		if (zend_hash_find(&EG(persistent_list), hashed_details, hashed_details_length+1, (void **) &le)==FAILURE) {  /* we don't */
			...
			if (MySG(max_links) != -1 && MySG(num_links) >= MySG(max_links)) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Too many open links (%ld)", MySG(num_links));
				efree(hashed_details);
				MYSQL_DO_CONNECT_RETURN_FALSE();
			}
			if (MySG(max_persistent) != -1 && MySG(num_persistent) >= MySG(max_persistent)) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Too many open persistent links (%ld)", MySG(num_persistent));
				efree(hashed_details);
				MYSQL_DO_CONNECT_RETURN_FALSE();
			}
			/* create the link */
			...
			if (!mysql) {
				php_error_docref(NULL, TSRMLS_C, E_ERROR, "Out of memory while allocating memory for a persistent link");
			}
			mysql->active_result_id = 0;
#ifdef CLIENT_MULTI_STATEMENTS
...
#else
			if (mysqlnd_connect(mysql->conn, host, user, passwd, passwd_len, NULL, 0, port, socket, client_flags, TSRMLS_C) == NULL)
#endif
			{
				...
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "%s", MySG(connect_error));
...
#endif
					{
						php_error_docref(NULL, TSRMLS_C, E_WARNING, "Link to server lost, unable to reconnect");
		...
		if (!new_link && zend_hash_find(&EG(regular_list), hashed_details, hashed_details_length+1,(void **) &index_ptr)==SUCCESS) {
			...
			if (ptr && (type==le_link || type==le_plink)) {
				...
				php_mysql_set_default_link(link, TSRMLS_C);
		...
		if (!mysql->conn) {
			...
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "OOM");
			efree(hashed_details);
			efree(mysql);
			MYSQL_DO_CONNECT_RETURN_FALSE();
		}
	...
	efree(hashed_details);
	php_mysql_set_default_link(Z_LVAL_P(return_value), TSRMLS_C);
	MYSQL_DO_CONNECT_CLEANUP();
}
/* }}} */
...
PHP_FUNCTION(mysql_close)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "|r", &mysql_link) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(mysql_select_db)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|r", &db, &db_len, &mysql_link) == FAILURE) {
		return;
	}
	...
	if (php_mysql_select_db(mysql, db, TSRMLS_C)) {
		RETURN_TRUE;
	} else {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(mysql_set_charset)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|r", &csname, &csname_len, &mysql_link) == FAILURE) {
		return;
	}
 ...
 */
static void php_mysql_do_query_general(char *query, int query_len, zval *mysql_link, int link_id, char *db, int use_store, zval *return_value, TSRMLS_D)
{
	php_mysql_conn *mysql;
	MYSQL_RES *mysql_result;
	...
	if (MySG(trace_mode)) {
		if (!strncasecmp("select", query, 6)){
			...
			if (mysql_errno(mysql->conn)) {
				php_error_docref("http://www.mysql.com/doc", TSRMLS_C, E_WARNING, "%s", mysql_error(mysql->conn));
				RETURN_FALSE;
			}
			else {
				...
				while ((row = mysql_fetch_row(mysql_result))) {
					if (!strcmp("ALL", row[1])) {
						php_error_docref("http://www.mysql.com/doc", TSRMLS_C, E_WARNING, "Your query requires a full tablescan (table %s, %s rows affected). Use EXPLAIN to optimize your query.", row[0], row[6]);
					} else if (!strcmp("INDEX", row[1])) {
						php_error_docref("http://www.mysql.com/doc", TSRMLS_C, E_WARNING, "Your query requires a full indexscan (table %s, %s rows affected). Use EXPLAIN to optimize your query.", row[0], row[6]);
					}
				}
				mysql_free_result(mysql_result);
			}
	...
	if (!mysql_result) {
		if (PHP_MYSQL_VALID_RESULT(mysql->conn)) { /* query should have returned rows */
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to save result set");
			RETURN_FALSE;
		} else {
			RETURN_TRUE;
		}
...
static void php_mysql_do_query(INTERNAL_FUNCTION_PARAMETERS, int use_store)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|r", &query, &query_len, &mysql_link) == FAILURE) {
		return;
	}
	...
	php_mysql_do_query_general(query, query_len, mysql_link, id, NULL, use_store, return_value, TSRMLS_C);
}
/* }}} */
...
PHP_FUNCTION(mysql_db_query)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ss|r", &db, &db_len, &query, &query_len, &mysql_link) == FAILURE) {
		return;
	}
	...
	php_error_docref(NULL, TSRMLS_C, E_DEPRECATED, "This function is deprecated; use mysql_query() instead");

	php_mysql_do_query_general(query, query_len, mysql_link, id, db, MYSQL_STORE_RESULT, return_value, TSRMLS_C);
}
/* }}} */
...
PHP_FUNCTION(mysql_list_dbs)
{
	...
	php_error_docref(NULL, TSRMLS_C, E_DEPRECATED, "This function is deprecated; use mysql_query() with SHOW DATABASES instead");
	...
	if ((mysql_result=mysql_list_dbs(mysql->conn, NULL))==NULL) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to save MySQL query result");
		RETURN_FALSE;
	}
	MySG(result_allocated)++;
	ZEND_REGISTER_RESOURCE(return_value, mysql_result, le_result);
...
PHP_FUNCTION(mysql_list_fields)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ss|r", &db, &db_len, &table, &table_len, &mysql_link) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(mysql_escape_string)
{
	...
	php_error_docref("function.mysql-real-escape-string", TSRMLS_C, E_DEPRECATED, "This function is deprecated; use mysql_real_escape_string() instead.");
}
/* }}} */
...
PHP_FUNCTION(mysql_real_escape_string)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|r", &str, &str_len, &mysql_link) == FAILURE) {
		return;
	}
...
*/
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rl|z", &result, &row, &field) == FAILURE) {
		return;
	}
	...
	if (row<0 || row>=(int)mysql_num_rows(mysql_result)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to jump to row %ld on MySQL result index %ld", row, Z_LVAL_P(result));
		RETURN_FALSE;
	}
		...
		switch(Z_TYPE_P(field)) {
			case IS_STRING: {
					...
					if (!tmp_field) { /* no match found */
						php_error_docref(NULL, TSRMLS_C, E_WARNING, "%s%s%s not found in MySQL result index %ld",
	...
	if (sql_row[field_offset]) {
		...
		if (PG(magic_quotes_runtime)) {
			Z_STRVAL_P(return_value) = php_addslashes(sql_row[field_offset], sql_row_lengths[field_offset],&Z_STRLEN_P(return_value), 0, TSRMLS_C);
		} else {
#endif
...
static void php_mysql_fetch_hash(INTERNAL_FUNCTION_PARAMETERS, long result_type, int expected_args, int into_object)
{
	...
	if (into_object) {
		...
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "z|sz", &res, &class_name, &class_name_len, &ctor_params) == FAILURE) {
			return;
		}
...
#endif
	{
		if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r|l", &res, &result_type) == FAILURE) {
			return;
		}
		if (!result_type) {
	...
	if (result_type & ~MYSQL_BOTH) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "The result type should be either MYSQL_NUM, MYSQL_ASSOC or MYSQL_BOTH");
		result_type = MYSQL_BOTH;
	}
		 ...
		 mysql_field = mysql_fetch_field(mysql_result), i++)
	{
		if (mysql_row[i]) {
			...
			if (PG(magic_quotes_runtime)) {
				...
				Z_STRVAL_P(data) = php_addslashes(mysql_row[i], mysql_row_lengths[i], &Z_STRLEN_P(data), 0, TSRMLS_C);
			} else {
#endif
...
PHP_FUNCTION(mysql_data_seek)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rl", &result, &offset)) {
		return;
	}
	...
	if (offset<0 || offset>=(int)mysql_num_rows(mysql_result)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Offset %ld is invalid for MySQL result index %ld (or the query data is unbuffered)", offset, Z_LVAL_P(result));
		RETURN_FALSE;
	}
...
PHP_FUNCTION(mysql_fetch_field)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r|l", &result, &field) == FAILURE) {
		return;
	}
	...
	if (ZEND_NUM_ARGS() > 1) {
		if (field<0 || field>=(int)mysql_num_fields(mysql_result)) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Bad field offset");
			RETURN_FALSE;
		}
...
PHP_FUNCTION(mysql_field_seek)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rl", &result, &offset) == FAILURE) {
		return;
	}
	ZEND_FETCH_RESOURCE(mysql_result, MYSQL_RES *, &result, -1, "MySQL result", le_result);

	if (offset<0 || offset>=(int)mysql_num_fields(mysql_result)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Field %ld is invalid for MySQL result index %ld", offset, Z_LVAL_P(result));
		RETURN_FALSE;
	}
...
static void php_mysql_field_info(INTERNAL_FUNCTION_PARAMETERS, int entry_type)
{
	...
	if (field<0 || field>=(int)mysql_num_fields(mysql_result)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Field %ld is invalid for MySQL result index %ld", field, Z_LVAL_P(result));
		RETURN_FALSE;
	}
	mysql_field_seek(mysql_result, field);
	if ((mysql_field=mysql_fetch_field(mysql_result))==NULL) {
...
PHP_FUNCTION(mysql_ping)
{
	...
	if (0 == ZEND_NUM_ARGS()) {
		id = php_mysql_get_default_link(INTERNAL_FUNCTION_PARAM_PASSTHRU);
		CHECK_LINK(id);
	} else if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r", &mysql_link)==FAILURE) {
		return;
	}
