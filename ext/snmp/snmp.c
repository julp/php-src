...
{ \
	...
	while (b[i].name != NULL) { \
		php_snmp_add_property((a), (b)[i].name, (b)[i].name_length, \
							(php_snmp_read_t)(b)[i].read_func, (php_snmp_write_t)(b)[i].write_func, TSRMLS_C); \
		i++; \
	} \
}
...
static void php_snmp_session_destructor(zend_rsrc_list_entry *rsrc, TSRMLS_D)
{
	php_snmp_session *session = (php_snmp_session *)rsrc->ptr;
	netsnmp_session_free(&session);
}

static void php_snmp_object_free_storage(void *object, TSRMLS_D)
{
	...
	zend_object_std_dtor(&intern->zo, TSRMLS_C);
	
	efree(intern);
}

static zend_object_value php_snmp_object_new(zend_class_entry *class_type, TSRMLS_D) /* {{{ */
{
	...
	retval.handle = zend_objects_store_put(intern, (zend_objects_store_dtor_t)zend_objects_destroy_object, (zend_objects_free_object_storage_t) php_snmp_object_free_storage, NULL, TSRMLS_C);
 ...
 *
 */
static void php_snmp_error(zval *object, const char *docref, TSRMLS_D, int type, const char *format, ...)
{
	va_list args;
	php_snmp_object *snmp_object;

	if (object) {
		snmp_object = (php_snmp_object *)zend_object_store_get_object(object, TSRMLS_C);
	...
	if (object && (snmp_object->exceptions_enabled & type)) {
		zend_throw_exception_ex(php_snmp_exception_ce, type, snmp_object->snmp_errstr, TSRMLS_C);
	} else {
		va_start(args, format);
		php_verror(docref, "", E_WARNING, format, args, TSRMLS_C);
		va_end(args);
	}
}
...
*
*/
static void php_snmp_getvalue(struct variable_list *vars, zval *snmpval, TSRMLS_D, int valueretrieval)
{
	zval *val;
	char sbuf[512];
	char *buf = &(sbuf[0]);
	...
	while ((valueretrieval & SNMP_VALUE_PLAIN) == 0) {
		*buf = '\0';
		if (snprint_value(buf, buflen, vars->name, vars->name_length, vars) == -1) {
			if (val_len > 512*1024) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "snprint_value() asks for a buffer more than 512k, Net-SNMP bug?");
				break;
			}
		...
		if (!dbuf) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "emalloc() failed: %s, fallback to static buffer", strerror(errno));
			buf = &(sbuf[0]);
	...
	if (valueretrieval & SNMP_VALUE_PLAIN) {
		*buf = 0;
		...
		default:
			...
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unknown value type: %u", vars->type);
			break;
		}
							...
							struct objid_query *objid_query)
{
	...
	php_snmp_error(getThis(), NULL, TSRMLS_C, PHP_SNMP_ERRNO_NOERROR, "");
	...
	if ((ss = snmp_open(session)) == NULL) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Could not open snmp connection: %s", err);
		free(err);
		RETVAL_FALSE;
		return;
	}
	...
	if ((st & SNMP_CMD_SET) && objid_query->count > objid_query->step) {
		php_snmp_error(getThis(), NULL, TSRMLS_C, PHP_SNMP_ERRNO_MULTIPLE_SET_QUERIES, "Can not fit all OIDs for SET query into one packet, using multiple queries");
	}
	...
	while (keepwalking) {
		...
		if (st & SNMP_CMD_WALK) {
			...
			if (st & SNMP_CMD_GET) {
				...
				snmp_close(ss);
				php_error_docref(NULL, TSRMLS_C, E_ERROR, "Unknown SNMP command (internals)");
				RETVAL_FALSE;
				return;
			}
			for (count = 0; objid_query->offset < objid_query->count && count < objid_query->step; objid_query->offset++, count++){
				if (st & SNMP_CMD_SET) {
					if ((snmp_errno = snmp_add_var(pdu, objid_query->vars[objid_query->offset].name, objid_query->vars[objid_query->offset].name_length, objid_query->vars[objid_query->offset].type, objid_query->vars[objid_query->offset].value))) {
						...
						php_snmp_error(getThis(), NULL, TSRMLS_C, PHP_SNMP_ERRNO_OID_PARSING_ERROR, "Could not add variable: OID='%s' type='%c' value='%s': %s", buf, objid_query->vars[objid_query->offset].type, objid_query->vars[objid_query->offset].value, snmp_api_errstring(snmp_errno));
						snmp_free_pdu(pdu);
						snmp_close(ss);
						RETVAL_FALSE;
						return;
...
retry:
		...
		if (status == STAT_SUCCESS) {
			if (response->errstat == SNMP_ERR_NOERROR) {
				...
				for (vars = response->variables; vars; vars = vars->next_variable) {
					...
					if ( 	vars->type == SNMP_ENDOFMIBVIEW || 
						...
						php_snmp_error(getThis(), NULL, TSRMLS_C, PHP_SNMP_ERRNO_ERROR_IN_REPLY, "Error in packet at '%s': %s", buf, buf2);
						continue;
					}
					...
					php_snmp_getvalue(vars, snmpval, TSRMLS_C, objid_query->valueretrieval);
					...
					if (objid_query->array_output) {
						...
						if (st & SNMP_NUMERIC_KEYS) {
							...
							if (found) {
								...
								php_error_docref(NULL, TSRMLS_C, E_WARNING, "Could not find original OID name for '%s'", buf2);
							}
					...
					if (st & SNMP_CMD_WALK) {
						if (objid_query->oid_increasing_check == TRUE && snmp_oid_compare(objid_query->vars[0].name, objid_query->vars[0].name_length, vars->name, vars->name_length) >= 0) {
							snprint_objid(buf2, sizeof(buf2), vars->name, vars->name_length);
							php_snmp_error(getThis(), NULL, TSRMLS_C, PHP_SNMP_ERRNO_OID_NOT_INCREASING, "Error: OID not increasing: %s", buf2);
							keepwalking = 0;
						} else {
				...
				if (!(st & SNMP_CMD_WALK) || response->errstat != SNMP_ERR_NOSUCHNAME || Z_TYPE_P(return_value) == IS_BOOL) {
					...
					if (vars) {
						snprint_objid(buf, sizeof(buf), vars->name, vars->name_length);
						php_snmp_error(getThis(), NULL, TSRMLS_C, PHP_SNMP_ERRNO_ERROR_IN_REPLY, "Error in packet at '%s': %s", buf, snmp_errstring(response->errstat));
					} else {
						php_snmp_error(getThis(), NULL, TSRMLS_C, PHP_SNMP_ERRNO_ERROR_IN_REPLY, "Error in packet at %u object_id: %s", response->errindex, snmp_errstring(response->errstat));
					}
			...
			php_snmp_error(getThis(), NULL, TSRMLS_C, PHP_SNMP_ERRNO_TIMEOUT, "No response from %s", session->peername);
			if (objid_query->array_output) {
				zval_dtor(return_value);
			}
			snmp_close(ss);
			...
			php_snmp_error(getThis(), NULL, TSRMLS_C, PHP_SNMP_ERRNO_GENERIC, "Fatal error: %s", err);
			free(err);
			if (objid_query->array_output) {
				zval_dtor(return_value);
			}
...
static int php_snmp_parse_oid(zval *object, int st, struct objid_query *objid_query, zval **oid, zval **type, zval **value, TSRMLS_D)
{
	char *pptr;
	...
	if (Z_TYPE_PP(oid) == IS_STRING) {
		...
		if (objid_query->vars == NULL) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "emalloc() failed while parsing oid: %s", strerror(errno));
		...
		if (st & SNMP_CMD_SET) {
			if (Z_TYPE_PP(type) == IS_STRING && Z_TYPE_PP(value) == IS_STRING) {
				if (Z_STRLEN_PP(type) != 1) {
					php_error_docref(NULL, TSRMLS_C, E_WARNING, "Bogus type '%s', should be single char, got %u", Z_STRVAL_PP(type), Z_STRLEN_PP(type));
					efree(objid_query->vars);
					return FALSE;
				}
				...
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Single objid and multiple type or values are not supported");
				efree(objid_query->vars);
				return FALSE;
			}
		}
		...
		if (zend_hash_num_elements(Z_ARRVAL_PP(oid)) == 0) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Got empty OID array");
			return FALSE;
		}
		objid_query->vars = (snmpobjarg *)emalloc(sizeof(snmpobjarg) * zend_hash_num_elements(Z_ARRVAL_PP(oid)));
		if (objid_query->vars == NULL) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "emalloc() failed while parsing oid array: %s", strerror(errno));
			efree(objid_query->vars);
			return FALSE;
		}
			...
			if (st & SNMP_CMD_SET) {
				if (Z_TYPE_PP(type) == IS_STRING) {
					...
					if (SUCCESS == zend_hash_get_current_data_ex(Z_ARRVAL_PP(type), (void **) &tmp_type, &pos_type)) {
						...
						if (Z_STRLEN_PP(tmp_type) != 1) {
							php_error_docref(NULL, TSRMLS_C, E_WARNING, "'%s': bogus type '%s', should be single char, got %u", Z_STRVAL_PP(tmp_oid), Z_STRVAL_PP(tmp_type), Z_STRLEN_PP(tmp_type));
							efree(objid_query->vars);
							return FALSE;
						}
						...
						php_error_docref(NULL, TSRMLS_C, E_WARNING, "'%s': no type set", Z_STRVAL_PP(tmp_oid));
						efree(objid_query->vars);
						return FALSE;
					}
				}
				...
				if (Z_TYPE_PP(value) == IS_STRING) {
					...
					if (SUCCESS == zend_hash_get_current_data_ex(Z_ARRVAL_PP(value), (void **) &tmp_value, &pos_value)) {
						...
						php_error_docref(NULL, TSRMLS_C, E_WARNING, "'%s': no value set", Z_STRVAL_PP(tmp_oid));
						efree(objid_query->vars);
						return FALSE;
					}
				}
	...
	if (st & SNMP_CMD_WALK) {
		if (objid_query->count > 1) {
			php_snmp_error(object, NULL, TSRMLS_C, PHP_SNMP_ERRNO_OID_PARSING_ERROR, "Multi OID walks are not supported!");
			efree(objid_query->vars);
			return FALSE;
		}
		...
		if (strlen(objid_query->vars[0].oid)) { /* on a walk, an empty string means top of tree - no error */
			if (!snmp_parse_oid(objid_query->vars[0].oid, objid_query->vars[0].name, &(objid_query->vars[0].name_length))) {
				php_snmp_error(object, NULL, TSRMLS_C, PHP_SNMP_ERRNO_OID_PARSING_ERROR, "Invalid object identifier: %s", objid_query->vars[0].oid);
				efree(objid_query->vars);
				return FALSE;
			}
		} else { 
		...
		for (objid_query->offset = 0; objid_query->offset < objid_query->count; objid_query->offset++) {
			...
			if (!snmp_parse_oid(objid_query->vars[objid_query->offset].oid, objid_query->vars[objid_query->offset].name, &(objid_query->vars[objid_query->offset].name_length))) {
				php_snmp_error(object, NULL, TSRMLS_C, PHP_SNMP_ERRNO_OID_PARSING_ERROR, "Invalid object identifier: %s", objid_query->vars[objid_query->offset].oid);
				efree(objid_query->vars);
				return FALSE;
			}
		}
...
*/
static int netsnmp_session_init(php_snmp_session **session_p, int version, char *hostname, char *community, int timeout, int retries, TSRMLS_D)
{
	...
	*session_p = (php_snmp_session *)emalloc(sizeof(php_snmp_session));
	session = *session_p;
	if (session == NULL) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "emalloc() failed allocating session");
		return (-1);
	}
	...
	if (session->peername == NULL) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "emalloc() failed while copying hostname");
		return (-1);
	}
	...
	if (*host_ptr == '[') { /* IPv6 address */
		...
		if ((pptr = strchr(host_ptr, ']'))) {
			...
			*pptr = '\0';
			...
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "malformed IPv6 address, closing square bracket missing");
			return (-1);
		}
	} else { /* IPv4 address */
		if ((pptr = strchr(host_ptr, ':'))) {
	...
	if ((n = php_network_getaddresses(host_ptr, SOCK_DGRAM, &psal, NULL, TSRMLS_C)) == 0) { /* some resolver error */
	...
	*(session->peername) = '\0';
	...
	if (strlen(session->peername) == 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unknown failure while resolving '%s'", hostname);
		return (-1);
	}
	/* XXX FIXME
...
static int netsnmp_session_set_auth_protocol(struct snmp_session *s, char *prot, TSRMLS_D)
{
	if (!strcasecmp(prot, "MD5")) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unknown authentication protocol '%s'", prot);
		return (-1);
	}
	return (0);
}
...
static int netsnmp_session_set_sec_protocol(struct snmp_session *s, char *prot, TSRMLS_D)
{
	if (!strcasecmp(prot, "DES")) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unknown security protocol '%s'", prot);
		return (-1);
	}
	return (0);
}
...
static int netsnmp_session_gen_auth_key(struct snmp_session *s, char *pass, TSRMLS_D)
{
	int snmp_errno;
	...
	if ((snmp_errno = generate_Ku(s->securityAuthProto, s->securityAuthProtoLen,
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Error generating a key for authentication pass phrase '%s': %s", pass, snmp_api_errstring(snmp_errno));
		return (-1);
	}
	return (0);
}
...
/* {{{ int netsnmp_session_gen_sec_key(struct snmp_session *s, u_char *pass)
   Make key from pass phrase in the snmpv3 session */
static int netsnmp_session_gen_sec_key(struct snmp_session *s, char *pass, TSRMLS_D)
{
	int snmp_errno;
	...
	if ((snmp_errno = generate_Ku(s->securityAuthProto, s->securityAuthProtoLen,
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Error generating a key for privacy pass phrase '%s': %s", pass, snmp_api_errstring(snmp_errno));
		return (-2);
	}
	return (0);
}
...
static int netsnmp_session_set_contextEngineID(struct snmp_session *s, char * contextEngineID, TSRMLS_D)
{
	size_t	ebuf_len = 32, eout_len = 0;
	u_char	*ebuf = (u_char *) emalloc(ebuf_len);

	if (ebuf == NULL) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "malloc failure setting contextEngineID");
		return (-1);
	}
	if (!snmp_hex_to_binary(&ebuf, &ebuf_len, &eout_len, 1, contextEngineID)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Bad engine ID value '%s'", contextEngineID);
		efree(ebuf);
		return (-1);
	}
...
static int netsnmp_session_set_security(struct snmp_session *session, char *sec_level, char *auth_protocol, char *auth_passphrase, char *priv_protocol, char *priv_passphrase, char *contextName, char *contextEngineID, TSRMLS_D)
{

	/* Setting the security level. */
	if (netsnmp_session_set_sec_level(session, sec_level)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid security level '%s'", sec_level);
		return (-1);
	}
	...
	if (session->securityLevel == SNMP_SEC_LEVEL_AUTHNOPRIV || session->securityLevel == SNMP_SEC_LEVEL_AUTHPRIV) {
		...
		if (netsnmp_session_set_auth_protocol(session, auth_protocol, TSRMLS_C)) {
			/* Warning message sent already, just bail out */
			return (-1);
		}

		/* Setting the authentication passphrase. */
		if (netsnmp_session_gen_auth_key(session, auth_passphrase, TSRMLS_C)) {
			/* Warning message sent already, just bail out */
			return (-1);
		}
		...
		if (session->securityLevel == SNMP_SEC_LEVEL_AUTHPRIV) {
			...
			if (netsnmp_session_set_sec_protocol(session, priv_protocol, TSRMLS_C)) {
				/* Warning message sent already, just bail out */
				return (-1);
			}

			/* Setting the security protocol passphrase. */
			if (netsnmp_session_gen_sec_key(session, priv_passphrase, TSRMLS_C)) {
				/* Warning message sent already, just bail out */
				return (-1);
			}
		}
	...
	if (contextEngineID && strlen(contextEngineID) && netsnmp_session_set_contextEngineID(session, contextEngineID, TSRMLS_C)) {
		/* Warning message sent already, just bail out */
		return (-1);
	}
...
static void php_snmp(INTERNAL_FUNCTION_PARAMETERS, int st, int version)
{
	...
	if (session_less_mode) {
		if (version == SNMP_VERSION_3) {
			if (st & SNMP_CMD_SET) {
				if (zend_parse_parameters(argc, TSRMLS_C, "sssssssZZZ|ll", &a1, &a1_len, &a2, &a2_len, &a3, &a3_len,
				 ...
				 * SNMP_CMD_WALK
				 */
				if (zend_parse_parameters(argc, TSRMLS_C, "sssssssZ|ll", &a1, &a1_len, &a2, &a2_len, &a3, &a3_len,
			...
			if (st & SNMP_CMD_SET) {
				if (zend_parse_parameters(argc, TSRMLS_C, "ssZZZ|ll", &a1, &a1_len, &a2, &a2_len, &oid, &type, &value, &timeout, &retries) == FAILURE) {
					RETURN_FALSE;
				}
			} else {
				/* SNMP_CMD_GET
				 ...
				 * SNMP_CMD_WALK
				 */
				if (zend_parse_parameters(argc, TSRMLS_C, "ssZ|ll", &a1, &a1_len, &a2, &a2_len, &oid, &timeout, &retries) == FAILURE) {
					RETURN_FALSE;
				}
			}
		}
		...
		if (st & SNMP_CMD_SET) {
			if (zend_parse_parameters(argc, TSRMLS_C, "ZZZ", &oid, &type, &value) == FAILURE) {
				RETURN_FALSE;
			}
		} else if (st & SNMP_CMD_WALK) {
			if (zend_parse_parameters(argc, TSRMLS_C, "Z|bll", &oid, &suffix_keys, &(objid_query.max_repetitions), &(objid_query.non_repeaters)) == FAILURE) {
				RETURN_FALSE;
			}
			if (suffix_keys) {
			...
			if (zend_parse_parameters(argc, TSRMLS_C, "Z|b", &oid, &use_orignames) == FAILURE) {
				RETURN_FALSE;
			}
			if (use_orignames) {
			...
			/* SNMP_CMD_GETNEXT
			 */
			if (zend_parse_parameters(argc, TSRMLS_C, "Z", &oid) == FAILURE) {
				RETURN_FALSE;
			}
		}
	}

	if (!php_snmp_parse_oid(getThis(), st, &objid_query, oid, type, value, TSRMLS_C)) {
		RETURN_FALSE;
	}

	if (session_less_mode) {
		if (netsnmp_session_init(&session, version, a1, a2, timeout, retries, TSRMLS_C)) {
			efree(objid_query.vars);
			netsnmp_session_free(&session);
			RETURN_FALSE;
		}
		if (version == SNMP_VERSION_3 && netsnmp_session_set_security(session, a3, a4, a5, a6, a7, NULL, NULL, TSRMLS_C)) {
			efree(objid_query.vars);
			netsnmp_session_free(&session);
			/* Warning message sent already, just bail out */
			RETURN_FALSE;
		...
		if (!session) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid or uninitialized SNMP object");
			efree(objid_query.vars);
			RETURN_FALSE;
		}
...
PHP_FUNCTION(snmp_set_quick_print)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l", &a1) == FAILURE) {
		RETURN_FALSE;
	}
...
PHP_FUNCTION(snmp_set_oid_output_format)
{
	...
	switch((int) a1) {
		...
		case NETSNMP_OID_OUTPUT_NONE:
			...
			break;
		default:
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unknown SNMP output print format '%d'", (int) a1);
			RETURN_FALSE;
			break;
	}
} 
...
PHP_FUNCTION(snmp_set_valueretrieval)
{
	...
	if (method >= 0 && method <= (SNMP_VALUE_LIBRARY|SNMP_VALUE_PLAIN|SNMP_VALUE_OBJECT)) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unknown SNMP value retrieval method '%ld'", method);
		RETURN_FALSE;
	}
}
/* }}} */
...
PHP_FUNCTION(snmp_read_mib)
{
	...
	if (!read_mib(filename)) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Error while reading MIB file '%s': %s", filename, error);
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_METHOD(snmp, __construct)
{
	...
	if (zend_parse_parameters(argc, TSRMLS_C, "lss|ll", &version, &a1, &a1_len, &a2, &a2_len, &timeout, &retries) == FAILURE) {
	...
	switch(version) {
		...
		case SNMP_VERSION_3:
			break;
		default:
			zend_throw_exception(zend_exception_get_default(TSRMLS_C), "Unknown SNMP protocol version", 0, TSRMLS_C);
			return;
	}
	...
	if (netsnmp_session_init(&(snmp_object->session), version, a1, a2, timeout, retries, TSRMLS_C)) {
		return;
	}
...
PHP_METHOD(snmp, setSecurity)
{
	...
	if (zend_parse_parameters(argc, TSRMLS_C, "s|ssssss", &a1, &a1_len, &a2, &a2_len, &a3, &a3_len,
		&a4, &a4_len, &a5, &a5_len, &a6, &a6_len, &a7, &a7_len) == FAILURE) {
		RETURN_FALSE;
	}

	if (netsnmp_session_set_security(snmp_object->session, a1, a2, a3, a4, a5, a6, a7, TSRMLS_C)) {
		/* Warning message sent already, just bail out */
		RETURN_FALSE;
	}
	RETURN_TRUE;
...
/* {{{ */
void php_snmp_add_property(HashTable *h, const char *name, size_t name_length, php_snmp_read_t read_func, php_snmp_write_t write_func, TSRMLS_D)
{
...
zval *php_snmp_read_property(zval *object, zval *member, int type, const zend_literal *key, TSRMLS_D)
{
	zval tmp_member;
	zval *retval;
	php_snmp_object *obj;
	...
	ret = FAILURE;
	obj = (php_snmp_object *)zend_objects_get_address(object, TSRMLS_C);
...
void php_snmp_write_property(zval *object, zval *member, zval *value, const zend_literal *key, TSRMLS_D)
{
	zval tmp_member;
	php_snmp_object *obj;
	php_snmp_prop_handler *hnd;
...
static int php_snmp_has_property(zval *object, zval *member, int has_set_exists, const zend_literal *key, TSRMLS_D)
{
	php_snmp_prop_handler *hnd;
	int ret = 0;
	...
	if (zend_hash_find(&php_snmp_properties, Z_STRVAL_P(member), Z_STRLEN_P(member) + 1, (void **)&hnd) == SUCCESS) {
		switch (has_set_exists) {
			case 2:
				...
				break;
			case 0: {
				zval *value = php_snmp_read_property(object, member, BP_VAR_IS, key, TSRMLS_C);
		...
		zend_object_handlers * std_hnd = zend_get_std_object_handlers();
		ret = std_hnd->has_property(object, member, has_set_exists, key, TSRMLS_C);
	}
	return ret;
}
/* }}} */
...
static HashTable *php_snmp_get_properties(zval *object, TSRMLS_D)
{
	php_snmp_object *obj;
	php_snmp_prop_handler *hnd;
	HashTable *props;
	...
	while (zend_hash_get_current_data_ex(&php_snmp_properties, (void**)&hnd, &pos) == SUCCESS) {
		...
		if (!hnd->read_func || hnd->read_func(obj, &val, TSRMLS_C) != SUCCESS) {
...
/* {{{ */
static int php_snmp_read_info(php_snmp_object *snmp_object, zval **retval, TSRMLS_D)
{
	zval *val;
...
/* {{{ */
static int php_snmp_read_max_oids(php_snmp_object *snmp_object, zval **retval, TSRMLS_D)
{
	MAKE_STD_ZVAL(*retval);
	if (snmp_object->max_oids > 0) {
...
#define PHP_SNMP_BOOL_PROPERTY_READER_FUNCTION(name) \
	static int php_snmp_read_##name(php_snmp_object *snmp_object, zval **retval, TSRMLS_D) \
	{ \
		MAKE_STD_ZVAL(*retval); \
...
/* {{{ */
static int php_snmp_write_info(php_snmp_object *snmp_object, zval *newval, TSRMLS_D)
{
	php_error_docref(NULL, TSRMLS_C, E_WARNING, "info property is read-only");
	return FAILURE;
}
/* }}} */

/* {{{ */
static int php_snmp_write_max_oids(php_snmp_object *snmp_object, zval *newval, TSRMLS_D)
{
	zval ztmp;
	int ret = SUCCESS;
	...
	if (Z_LVAL_P(newval) > 0) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "max_oids should be positive integer or NULL, got %ld", Z_LVAL_P(newval));
	}
...
/* {{{ */
static int php_snmp_write_valueretrieval(php_snmp_object *snmp_object, zval *newval, TSRMLS_D)
{
	zval ztmp;
	int ret = SUCCESS;
	...
	if (Z_LVAL_P(newval) >= 0 && Z_LVAL_P(newval) <= (SNMP_VALUE_LIBRARY|SNMP_VALUE_PLAIN|SNMP_VALUE_OBJECT)) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unknown SNMP value retrieval method '%ld'", Z_LVAL_P(newval));
		ret = FAILURE;
	}
...
static int php_snmp_write_##name(php_snmp_object *snmp_object, zval *newval, TSRMLS_D) \
{ \
	zval ztmp; \
...
/* {{{ */
static int php_snmp_write_oid_output_format(php_snmp_object *snmp_object, zval *newval, TSRMLS_D)
{
	zval ztmp;
	int ret = SUCCESS;
	if (Z_TYPE_P(newval) != IS_LONG) {
	...
	switch(Z_LVAL_P(newval)) {
		...
		case NETSNMP_OID_OUTPUT_NONE:
			...
			break;
		default:
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unknown SNMP output print format '%ld'", Z_LVAL_P(newval));
			ret = FAILURE;
			break;
	}
...
/* {{{ */
static int php_snmp_write_exceptions_enabled(php_snmp_object *snmp_object, zval *newval, TSRMLS_D)
{
	zval ztmp;
	int ret = SUCCESS;
	if (Z_TYPE_P(newval) != IS_LONG) {
...
PHP_MINIT_FUNCTION(snmp)
{
	...
	php_snmp_ce = zend_register_internal_class(&ce, TSRMLS_C);
...
#ifdef HAVE_SPL
	php_snmp_exception_ce = zend_register_internal_class_ex(&cex, spl_ce_RuntimeException, NULL, TSRMLS_C);
#else
	php_snmp_exception_ce = zend_register_internal_class_ex(&cex, zend_exception_get_default(TSRMLS_C), NULL, TSRMLS_C);
#endif
