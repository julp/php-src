...
#ifdef EXIF_DEBUG
#define EXIFERR_DC , const char *_file, size_t _line, TSRMLS_D
#define EXIFERR_CC , __FILE__, __LINE__, TSRMLS_C
#else
#define EXIFERR_DC, TSRMLS_D
#define EXIFERR_CC, TSRMLS_C
#endif
...
ZEND_INI_MH(OnUpdateEncode)
{
	...
	&return_list, &return_size, 0, TSRMLS_C)) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Illegal encoding ignored: '%s'", new_value);
			return FAILURE;
		}
		efree(return_list);
	}
	return OnUpdateString(entry, new_value, new_value_length, mh_arg1, mh_arg2, mh_arg3, stage, TSRMLS_C);
}
...
static char * exif_get_tagname(int tag_num, char *ret, int len, tag_table_type tag_table, TSRMLS_D)
{
	int i, t;
	char tmp[32];
...
#ifdef EXIF_DEBUG
char * exif_dump_data(int *dump_free, int format, int components, int length, int motorola_intel, char *value_ptr, TSRMLS_D) /* {{{ */
{
	char *dump;
	int len;
...
static double exif_convert_any_format(void *value, int format, int motorola_intel, TSRMLS_D)
{
	int 		s_den;
	unsigned 	u_den;
	...
	switch(format) {
		...
		case TAG_FMT_SINGLE:
#ifdef EXIF_DEBUG
			php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Found value of type single");
#endif
			return (double)*(float *)value;
		case TAG_FMT_DOUBLE:
#ifdef EXIF_DEBUG
			php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Found value of type double");
#endif
			return *(double *)value;
	}
	return 0;
 ...
 * Evaluate number, be it int, rational, or float from directory. */
static size_t exif_convert_any_to_int(void *value, int format, int motorola_intel, TSRMLS_D)
{
	int 		s_den;
	unsigned 	u_den;
...
*/
static char *exif_get_sectionlist(int sectionlist, TSRMLS_D)
{
	int i, len, ml = 0;
	char *sections;
...
static void exif_error_docref(const char *docref EXIFERR_DC, const image_info_type *ImageInfo, int type, const char *format, ...)
{
...
#ifdef EXIF_DEBUG
	{
		...
		php_verror(docref, ImageInfo->FileName?ImageInfo->FileName:"", type, buf, args, TSRMLS_C);
		efree(buf);
	}
#else
	php_verror(docref, ImageInfo->FileName?ImageInfo->FileName:"", type, format, args, TSRMLS_C);
#endif
	va_end(args);
}
/* }}} */
...
*/
static int exif_file_sections_realloc(image_info_type *ImageInfo, int section_index, size_t size, TSRMLS_D)
{
	void *tmp;
...
*/
static void exif_iif_add_value(image_info_type *image_info, int section_index, char *name, int tag, int format, int length, void* value, int motorola_intel, TSRMLS_D)
{
	size_t idex;
	void *vptr;
		...
		case TAG_FMT_DOUBLE:
			...
			for (idex=0,vptr=value; idex<(size_t)length; idex++,vptr=(char *) vptr + php_tiff_bytes_per_format[format]) {
				...
				switch (format) {
					...
					case TAG_FMT_SINGLE:
#ifdef EXIF_DEBUG
						php_error_docref(NULL, TSRMLS_C, E_WARNING, "Found value of type single");
#endif
						info_value->f = *(float *)value;

					case TAG_FMT_DOUBLE:
#ifdef EXIF_DEBUG
						php_error_docref(NULL, TSRMLS_C, E_WARNING, "Found value of type double");
#endif
...
*/
static void exif_iif_add_tag(image_info_type *image_info, int section_index, char *name, int tag, int format, size_t length, void* value, TSRMLS_D)
{
	exif_iif_add_value(image_info, section_index, name, tag, format, (int)length, value, image_info->motorola_intel, TSRMLS_C);
}
/* }}} */
...
*/
static void exif_iif_add_int(image_info_type *image_info, int section_index, char *name, int value, TSRMLS_D)
{
	image_info_data  *info_data;
	image_info_data  *list;
...
*/
static void exif_iif_add_str(image_info_type *image_info, int section_index, char *name, char *value, TSRMLS_D)
{
	image_info_data  *info_data;
	image_info_data  *list;
...
*/
static void exif_iif_add_fmt(image_info_type *image_info, int section_index, char *name, TSRMLS_D, char *value, ...)
{
	char             *tmp;
	va_list 		 arglist;
	...
	if (value) {
		...
		exif_iif_add_str(image_info, section_index, name, tmp, TSRMLS_C);
		efree(tmp);
	}
	va_end(arglist);
}
...
/* {{{ exif_iif_add_str
 Add a string value to image_info MUST BE NUL TERMINATED
*/
static void exif_iif_add_buffer(image_info_type *image_info, int section_index, char *name, int length, char *value, TSRMLS_D)
{
	image_info_data  *info_data;
	image_info_data  *list;
...
static void add_assoc_image_info(zval *value, int sub_array, image_info_type *image_info, int section_index, TSRMLS_D)
{
...
#ifdef EXIF_DEBUG
/*		php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Adding %d infos from section %s", image_info->info_list[section_index].count, exif_get_sectionname(section_index));*/
#endif
	if (image_info->info_list[section_index].count) {
		if (sub_array) {
			MAKE_STD_ZVAL(tmpi);
...
#ifdef EXIF_DEBUG
/*		php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Adding infos: tag(0x%04X,%12s,L=0x%04X): %s", info_tag, exif_get_tagname(info_tag, buffer, -12, exif_get_tag_table(section_index), TSRMLS_C), info_data->length, info_data->format==TAG_FMT_STRING?(info_value&&info_value->s?info_value->s:"<no data>"):exif_get_tagformat(info_data->format));*/
#endif
...
*/
static void exif_process_COM (image_info_type *image_info, char *value, size_t length, TSRMLS_D)
{
	exif_iif_add_tag(image_info, SECTION_COMMENT, "Comment", TAG_COMPUTED_VALUE, TAG_FMT_STRING, length-2, value+2, TSRMLS_C);
}
/* }}} */
...
*/
#ifdef EXIF_JPEG2000
static void exif_process_CME (image_info_type *image_info, char *value, size_t length, TSRMLS_D)
{
	if (length>3) {
		switch(value[2]) {
			case 0:
				exif_iif_add_tag(image_info, SECTION_COMMENT, "Comment", TAG_COMPUTED_VALUE, TAG_FMT_UNDEFINED, length, value, TSRMLS_C);
				break;
			case 1:
				exif_iif_add_tag(image_info, SECTION_COMMENT, "Comment", TAG_COMPUTED_VALUE, TAG_FMT_STRING, length, value);
				break;
			default:
				php_error_docref(NULL, TSRMLS_C, E_NOTICE, "Undefined JPEG2000 comment encoding");
				break;
		}
	} else {
		exif_iif_add_tag(image_info, SECTION_COMMENT, "Comment", TAG_COMPUTED_VALUE, TAG_FMT_UNDEFINED, 0, NULL);
		php_error_docref(NULL, TSRMLS_C, E_NOTICE, "JPEG2000 comment section too small");
	}
}
#endif
/* }}} */
...
static int exif_process_IFD_in_JPEG(image_info_type *ImageInfo, char *dir_start, char *offset_base, size_t IFDlength, size_t displacement, int section_index, TSRMLS_D);
static int exif_process_IFD_TAG(    image_info_type *ImageInfo, char *dir_entry, char *offset_base, size_t IFDlength, size_t displacement, int section_index, int ReadNextIFD, tag_table_type tag_table, TSRMLS_D);
...
PHP_FUNCTION(exif_tagname)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l", &tag) == FAILURE) {
		return;
	}

	szTemp = exif_get_tagname(tag, NULL, 0, tag_table_IFD, TSRMLS_C);
...
static void* exif_ifd_make_value(image_info_data *info_data, int motorola_intel, TSRMLS_D) {
	size_t  byte_count;
...
static void exif_thumbnail_build(image_info_type *ImageInfo, TSRMLS_D) {
	...
	switch(ImageInfo->Thumbnail.filetype) {
		...
		case IMAGE_FILETYPE_TIFF_MM:
			...
			for (i=0; i<info_list->count; i++) {
				...
				byte_count = php_tiff_bytes_per_format[info_data->format] * info_data->length;
#ifdef EXIF_DEBUG
				exif_error_docref(NULL EXIFERR_CC, ImageInfo, E_NOTICE, "Thumbnail: process tag(x%04X=%s): %s%s (%d bytes)", info_data->tag, exif_get_tagname(info_data->tag, tagname, -12, tag_table_IFD, TSRMLS_C), (info_data->length>1)&&info_data->format!=TAG_FMT_UNDEFINED&&info_data->format!=TAG_FMT_STRING?"ARRAY OF ":"", exif_get_tagformat(info_data->format), byte_count);
#endif
				if (info_data->tag==TAG_STRIP_OFFSETS || info_data->tag==TAG_JPEG_INTERCHANGE_FORMAT) {
					...
					value_ptr  = exif_ifd_make_value(info_data, ImageInfo->motorola_intel, TSRMLS_C);
					if (byte_count <= 4) {
...
static void exif_thumbnail_extract(image_info_type *ImageInfo, char *offset, size_t length, TSRMLS_D) {
	...
	exif_thumbnail_build(ImageInfo, TSRMLS_C);
}
/* }}} */
...
static int exif_process_undefined(char **result, char *value, size_t byte_count, TSRMLS_D) {
...
static int exif_process_string(char **result, char *value, size_t byte_count, TSRMLS_D) {
	...
	if ((byte_count=php_strnlen(value, byte_count)) > 0) {
		return exif_process_undefined(result, value, byte_count, TSRMLS_C);
	}
...
static int exif_process_user_comment(image_info_type *ImageInfo, char **pszInfoPtr, char **pszEncoding, char *szValuePtr, int ByteCount, TSRMLS_D)
{
	int   a;
	char  *decode;
	size_t len;;
	...
	*pszEncoding = NULL;
		...
		if (!memcmp(szValuePtr, "UNICODE\0", 8)) {
			*pszEncoding = estrdup((const char*)szValuePtr);
			...
			if (zend_multibyte_encoding_converter(
					...
					ByteCount,
					zend_multibyte_fetch_encoding(ImageInfo->encode_unicode, TSRMLS_C),
					zend_multibyte_fetch_encoding(decode, TSRMLS_C)
					,TSRMLS_C) < 0) {
			...
			*pszEncoding = estrdup((const char*)szValuePtr);
			...
			if (zend_multibyte_encoding_converter(
					...
					(unsigned char*)szValuePtr,
					ByteCount,
					zend_multibyte_fetch_encoding(ImageInfo->encode_jis, TSRMLS_C),
					zend_multibyte_fetch_encoding(ImageInfo->motorola_intel ? ImageInfo->decode_jis_be : ImageInfo->decode_jis_le, TSRMLS_C)
					,TSRMLS_C) < 0) {
				len = exif_process_string_raw(pszInfoPtr, szValuePtr, ByteCount);
			}
			return len;
	...
	exif_process_string(pszInfoPtr, szValuePtr, ByteCount, TSRMLS_C);
...
static int exif_process_unicode(image_info_type *ImageInfo, xp_field_type *xp_field, int tag, char *szValuePtr, int ByteCount, TSRMLS_D)
{
	xp_field->tag = tag;	
	...
	if (zend_multibyte_encoding_converter(
			...
			ByteCount,
			zend_multibyte_fetch_encoding(ImageInfo->encode_unicode, TSRMLS_C),
			zend_multibyte_fetch_encoding(ImageInfo->motorola_intel ? ImageInfo->decode_unicode_be : ImageInfo->decode_unicode_le, TSRMLS_C)
			,TSRMLS_C) < 0) {
...
static int exif_process_IFD_in_MAKERNOTE(image_info_type *ImageInfo, char * value_ptr, int value_len, char *offset_base, size_t IFDlength, size_t displacement, TSRMLS_D)
{
	...
	for (de=0;de<NumDirEntries;de++) {
		if (!exif_process_IFD_TAG(ImageInfo, dir_start + 2 + 12 * de,
								  offset_base, IFDlength, displacement, section_index, 0, maker_note->tag_table, TSRMLS_C)) {
			return FALSE;
		}
	}
...
static int exif_process_IFD_TAG(image_info_type *ImageInfo, char *dir_entry, char *offset_base, size_t IFDlength, size_t displacement, int section_index, int ReadNextIFD, tag_table_type tag_table, TSRMLS_D)
{
	...
	if (!format || format > NUM_FORMATS) {
		...
		exif_error_docref("exif_read_data#error_ifd" EXIFERR_CC, ImageInfo, E_WARNING, "Process tag(x%04X=%s): Illegal format code 0x%04X, suppose BYTE", tag, exif_get_tagname(tag, tagname, -12, tag_table, TSRMLS_C), format);
		format = TAG_FMT_BYTE;
		/*return TRUE;*/
	}

	if (components < 0) {
		exif_error_docref("exif_read_data#error_ifd" EXIFERR_CC, ImageInfo, E_WARNING, "Process tag(x%04X=%s): Illegal components(%ld)", tag, exif_get_tagname(tag, tagname, -12, tag_table, TSRMLS_C), components);
		return FALSE;
	}
	...
	if (byte_count_signed < 0 || (byte_count_signed > INT32_MAX)) {
		exif_error_docref("exif_read_data#error_ifd" EXIFERR_CC, ImageInfo, E_WARNING, "Process tag(x%04X=%s): Illegal byte_count", tag, exif_get_tagname(tag, tagname, -12, tag_table, TSRMLS_C));
		return FALSE;
	}
		...
		if (byte_count > IFDlength || offset_val > IFDlength-byte_count || value_ptr < dir_entry) {
			...
			if (byte_count > ImageInfo->FileSize || offset_val>ImageInfo->FileSize-byte_count || (ImageInfo->FileType!=IMAGE_FILETYPE_TIFF_II && ImageInfo->FileType!=IMAGE_FILETYPE_TIFF_MM && ImageInfo->FileType!=IMAGE_FILETYPE_JPEG)) {
				if (value_ptr < dir_entry) {
					...
					exif_error_docref("exif_read_data#error_ifd" EXIFERR_CC, ImageInfo, E_WARNING, "Process tag(x%04X=%s): Illegal pointer offset(x%04X < x%04X)", tag, exif_get_tagname(tag, tagname, -12, tag_table, TSRMLS_C), offset_val, dir_entry);
				} else {
					/* this is for sure not allowed */
					/* exception are IFD pointers */
					exif_error_docref("exif_read_data#error_ifd" EXIFERR_CC, ImageInfo, E_WARNING, "Process tag(x%04X=%s): Illegal pointer offset(x%04X + x%04X = x%04X > x%04X)", tag, exif_get_tagname(tag, tagname, -12, tag_table, TSRMLS_C), offset_val, byte_count, offset_val+byte_count, IFDlength);
				}
				return FALSE;
			}
...
#ifdef EXIF_DEBUG
	dump_data = exif_dump_data(&dump_free, format, components, length, ImageInfo->motorola_intel, value_ptr, TSRMLS_C);
	exif_error_docref(NULL EXIFERR_CC, ImageInfo, E_NOTICE, "Process tag(x%04X=%s,@x%04X + x%04X(=%d)): %s%s %s", tag, exif_get_tagname(tag, tagname, -12, tag_table, TSRMLS_C), offset_val+displacement, byte_count, byte_count, (components>1)&&format!=TAG_FMT_UNDEFINED&&format!=TAG_FMT_STRING?"ARRAY OF ":"", exif_get_tagformat(format), dump_data);
	if (dump_free) {
		efree(dump_data);
	}
#endif
	...
	if (section_index==SECTION_THUMBNAIL) {
		if (!ImageInfo->Thumbnail.data) {
			switch(tag) {
				...
				case TAG_COMP_IMAGE_WIDTH:
					ImageInfo->Thumbnail.width = exif_convert_any_to_int(value_ptr, format, ImageInfo->motorola_intel, TSRMLS_C);
					break;
	
				case TAG_IMAGEHEIGHT:
				case TAG_COMP_IMAGE_HEIGHT:
					ImageInfo->Thumbnail.height = exif_convert_any_to_int(value_ptr, format, ImageInfo->motorola_intel, TSRMLS_C);
					break;
	
				case TAG_STRIP_OFFSETS:
				case TAG_JPEG_INTERCHANGE_FORMAT:
					/* accept both formats */
					ImageInfo->Thumbnail.offset = exif_convert_any_to_int(value_ptr, format, ImageInfo->motorola_intel, TSRMLS_C);
					break;
				...
				case TAG_STRIP_BYTE_COUNTS:
					...
					ImageInfo->Thumbnail.size = exif_convert_any_to_int(value_ptr, format, ImageInfo->motorola_intel, TSRMLS_C);
					break;
		...
		switch(tag) {
			...
			case TAG_USERCOMMENT:
				ImageInfo->UserCommentLength = exif_process_user_comment(ImageInfo, &(ImageInfo->UserComment), &(ImageInfo->UserCommentEncoding), value_ptr, byte_count, TSRMLS_C);
				break;
			...
			case TAG_XP_SUBJECT:
				...
				exif_process_unicode(ImageInfo, &(ImageInfo->xp_fields.list[ImageInfo->xp_fields.count-1]), tag, value_ptr, byte_count, TSRMLS_C);
				break;

			case TAG_FNUMBER:
				/* Simplest way of expressing aperture, so I trust it the most.
				   (overwrite previously computed value if there is one) */
				ImageInfo->ApertureFNumber = (float)exif_convert_any_format(value_ptr, format, ImageInfo->motorola_intel, TSRMLS_C);
				break;
			...
			case TAG_MAX_APERTURE:
				...
				if (ImageInfo->ApertureFNumber == 0) {
					ImageInfo->ApertureFNumber
						= (float)exp(exif_convert_any_format(value_ptr, format, ImageInfo->motorola_intel, TSRMLS_C)*log(2)*0.5);
				}
				break;
			...
			case TAG_SHUTTERSPEED:
				...
				if (ImageInfo->ExposureTime == 0) {
					ImageInfo->ExposureTime
						= (float)(1/exp(exif_convert_any_format(value_ptr, format, ImageInfo->motorola_intel, TSRMLS_C)*log(2)));
				}
				break;
			...
			case TAG_COMP_IMAGE_WIDTH:
				ImageInfo->ExifImageWidth = exif_convert_any_to_int(value_ptr, format, ImageInfo->motorola_intel, TSRMLS_C);
				break;

			case TAG_FOCALPLANE_X_RES:
				ImageInfo->FocalplaneXRes = exif_convert_any_format(value_ptr, format, ImageInfo->motorola_intel, TSRMLS_C);
				break;

			case TAG_SUBJECT_DISTANCE:
				/* Inidcates the distacne the autofocus camera is focused to.
				   Tends to be less accurate as distance increases. */
				ImageInfo->Distance = (float)exif_convert_any_format(value_ptr, format, ImageInfo->motorola_intel, TSRMLS_C);
				break;

			case TAG_FOCALPLANE_RESOLUTION_UNIT:
				switch((int)exif_convert_any_format(value_ptr, format, ImageInfo->motorola_intel, TSRMLS_C)) {
			...
			case TAG_MAKER_NOTE:
				exif_process_IFD_in_MAKERNOTE(ImageInfo, value_ptr, byte_count, offset_base, IFDlength, displacement, TSRMLS_C);
				break;
			...
			case TAG_INTEROP_IFD_POINTER:
				if (ReadNextIFD) {
					...
					if (!exif_process_IFD_in_JPEG(ImageInfo, Subdir_start, offset_base, IFDlength, displacement, sub_section_index, TSRMLS_C)) {
						return FALSE;
					}
#ifdef EXIF_DEBUG
	...
	exif_iif_add_tag(ImageInfo, section_index, exif_get_tagname(tag, tagname, sizeof(tagname), tag_table, TSRMLS_C), tag, format, components, value_ptr, TSRMLS_C);
	EFREE_IF(outside);
	return TRUE;
}
/* }}} */
...
static int exif_process_IFD_in_JPEG(image_info_type *ImageInfo, char *dir_start, char *offset_base, size_t IFDlength, size_t displacement, int section_index, TSRMLS_D)
{
	...
	for (de=0;de<NumDirEntries;de++) {
		if (!exif_process_IFD_TAG(ImageInfo, dir_start + 2 + 12 * de,
								  offset_base, IFDlength, displacement, section_index, 1, exif_get_tag_table(section_index), TSRMLS_C)) {
			return FALSE;
		}
	}
	/*
...
#endif
		if (exif_process_IFD_in_JPEG(ImageInfo, offset_base + NextDirOffset, offset_base, IFDlength, displacement, SECTION_THUMBNAIL, TSRMLS_C)) {
#ifdef EXIF_DEBUG
			...
			&&  ImageInfo->read_thumbnail
				...
				exif_thumbnail_extract(ImageInfo, offset_base, IFDlength, TSRMLS_C);
			}
			return TRUE;
		} else {
			return FALSE;
...
*/
static void exif_process_TIFF_in_JPEG(image_info_type *ImageInfo, char *CharBuf, size_t length, size_t displacement, TSRMLS_D)
{
	...
	exif_process_IFD_in_JPEG(ImageInfo, CharBuf+offset_of_ifd, CharBuf, length/*-14*/, displacement, SECTION_IFD0, TSRMLS_C);
...
*/
static void exif_process_APP1(image_info_type *ImageInfo, char *CharBuf, size_t length, size_t displacement, TSRMLS_D)
{
	...
	exif_process_TIFF_in_JPEG(ImageInfo, CharBuf + 8, length - 8, displacement+8, TSRMLS_C);
#ifdef EXIF_DEBUG
...
*/
static void exif_process_APP12(image_info_type *ImageInfo, char *buffer, size_t length, TSRMLS_D)
{
	size_t l1, l2=0;

	if ((l1 = php_strnlen(buffer+2, length-2)) > 0) {
		exif_iif_add_tag(ImageInfo, SECTION_APP12, "Company", TAG_NONE, TAG_FMT_STRING, l1, buffer+2, TSRMLS_C);
		if (length > 2+l1+1) {
			l2 = php_strnlen(buffer+2+l1+1, length-2-l1-1);
			exif_iif_add_tag(ImageInfo, SECTION_APP12, "Info", TAG_NONE, TAG_FMT_STRING, l2, buffer+2+l1+1, TSRMLS_C);
		}
	}
#ifdef EXIF_DEBUG
...
static int exif_scan_JPEG_header(image_info_type *ImageInfo, TSRMLS_D)
{
	int section, sn;
	...
	for(section=0;;section++) {
		...
		switch(marker) {
			...
			case M_COM: /* Comment section */
				exif_process_COM(ImageInfo, (char *)Data, itemlen, TSRMLS_C);
				break;
			...
			case M_EXIF:
				if (!(ImageInfo->sections_found&FOUND_IFD0)) {
					...
					exif_process_APP1(ImageInfo, (char *)Data, itemlen, fpos, TSRMLS_C);
				}
				break;

			case M_APP12:
				exif_process_APP12(ImageInfo, (char *)Data, itemlen, TSRMLS_C);
				break;
...
static int exif_scan_thumbnail(image_info_type *ImageInfo, TSRMLS_D)
{
...
static int exif_process_IFD_in_TIFF(image_info_type *ImageInfo, size_t dir_offset, int section_index, TSRMLS_D)
{
		...
		if (ImageInfo->FileSize >= dir_offset+dir_size) {
...
#endif
			if (exif_file_sections_realloc(ImageInfo, sn, dir_size, TSRMLS_C)) {
				return FALSE;
			}
			...
			for(i=0;i<num_entries;i++) {
				...
				if (entry_type > NUM_FORMATS) {
					exif_error_docref(NULL EXIFERR_CC, ImageInfo, E_NOTICE, "Read from TIFF: tag(0x%04X,%12s): Illegal format code 0x%04X, switching to BYTE", entry_tag, exif_get_tagname(entry_tag, tagname, -12, tag_table, TSRMLS_C), entry_type);
			...
			if (ImageInfo->FileSize >= dir_offset + ImageInfo->file.list[sn].size) {
				if (ifd_size > dir_size) {
					...
					if (exif_file_sections_realloc(ImageInfo, sn, ifd_size, TSRMLS_C)) {
						return FALSE;
					}
				...
				for(i=0;i<num_entries;i++) {
					...
					if (entry_tag == TAG_EXIF_IFD_POINTER ||
...
#endif
						ImageInfo->ifd_nesting_level++;
						exif_process_IFD_in_TIFF(ImageInfo, entry_offset, sub_section_index, TSRMLS_C);
						...
						if (!exif_process_IFD_TAG(ImageInfo, (char*)dir_entry,
												  ...
												  ifd_size, 0, section_index, 0, tag_table, TSRMLS_C)) {
							return FALSE;
						}
					}
				}
...
#endif
					ImageInfo->ifd_nesting_level++;
					exif_process_IFD_in_TIFF(ImageInfo, next_offset, SECTION_THUMBNAIL, TSRMLS_C);
#ifdef EXIF_DEBUG
					exif_error_docref(NULL EXIFERR_CC, ImageInfo, E_NOTICE, "%s THUMBNAIL @0x%04X + 0x%04X", ImageInfo->Thumbnail.data ? "Ignore" : "Read", ImageInfo->Thumbnail.offset, ImageInfo->Thumbnail.size);
#endif
 ...
 * Parse the marker stream until SOS or EOI is seen; */
static int exif_scan_FILE_header(image_info_type *ImageInfo, TSRMLS_D)
{
	...
	if (ImageInfo->FileSize >= 2) {
		...
		if ((file_header[0]==0xff) && (file_header[1]==M_SOI)) {
			...
			if (exif_scan_JPEG_header(ImageInfo, TSRMLS_C)) {
				ret = TRUE;
			} else {
...
/* {{{ exif_read_file
 */
static int exif_read_file(image_info_type *ImageInfo, char *FileName, int read_thumbnail, int read_all, TSRMLS_D)
{
	int ret;
	struct stat st;
	...
	php_basename(FileName, strlen(FileName), NULL, 0, &(ImageInfo->FileName), NULL, TSRMLS_C);
	...
	ret = exif_scan_FILE_header(ImageInfo, TSRMLS_C);
...
PHP_FUNCTION(exif_read_data)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "p|sbb", &p_name, &p_name_len, &p_sections_needed, &p_sections_needed_len, &sub_arrays, &read_thumbnail) == FAILURE) {
		return;
	}
...
#ifdef EXIF_DEBUG
		sections_str = exif_get_sectionlist(sections_needed, TSRMLS_C);
		if (!sections_str) {
			RETURN_FALSE;
		}
	...
	ret = exif_read_file(&ImageInfo, p_name, read_thumbnail, read_all, TSRMLS_C);
	sections_str = exif_get_sectionlist(ImageInfo.sections_found, TSRMLS_C);
	...
	exif_iif_add_str(&ImageInfo, SECTION_FILE, "FileName",      ImageInfo.FileName, TSRMLS_C);
	exif_iif_add_int(&ImageInfo, SECTION_FILE, "FileDateTime",  ImageInfo.FileDateTime, TSRMLS_C);
	exif_iif_add_int(&ImageInfo, SECTION_FILE, "FileSize",      ImageInfo.FileSize, TSRMLS_C);
	exif_iif_add_int(&ImageInfo, SECTION_FILE, "FileType",      ImageInfo.FileType, TSRMLS_C);
	exif_iif_add_str(&ImageInfo, SECTION_FILE, "MimeType",      (char*)php_image_type_to_mime_type(ImageInfo.FileType), TSRMLS_C);
	exif_iif_add_str(&ImageInfo, SECTION_FILE, "SectionsFound", sections_str ? sections_str : "NONE", TSRMLS_C);
	...
	if (ImageInfo.Width>0 &&  ImageInfo.Height>0) {
		exif_iif_add_fmt(&ImageInfo, SECTION_COMPUTED, "html",    TSRMLS_C, "width=\"%d\" height=\"%d\"", ImageInfo.Width, ImageInfo.Height);
		exif_iif_add_int(&ImageInfo, SECTION_COMPUTED, "Height", ImageInfo.Height, TSRMLS_C);
		exif_iif_add_int(&ImageInfo, SECTION_COMPUTED, "Width",  ImageInfo.Width, TSRMLS_C);
	}
	exif_iif_add_int(&ImageInfo, SECTION_COMPUTED, "IsColor", ImageInfo.IsColor, TSRMLS_C);
	if (ImageInfo.motorola_intel != -1) {
		exif_iif_add_int(&ImageInfo, SECTION_COMPUTED, "ByteOrderMotorola", ImageInfo.motorola_intel, TSRMLS_C);
	}
	if (ImageInfo.FocalLength) {
		exif_iif_add_fmt(&ImageInfo, SECTION_COMPUTED, "FocalLength", TSRMLS_C, "%4.1Fmm", ImageInfo.FocalLength);
		if(ImageInfo.CCDWidth) {
			exif_iif_add_fmt(&ImageInfo, SECTION_COMPUTED, "35mmFocalLength", TSRMLS_C, "%dmm", (int)(ImageInfo.FocalLength/ImageInfo.CCDWidth*35+0.5));
		}
	}
	if(ImageInfo.CCDWidth) {
		exif_iif_add_fmt(&ImageInfo, SECTION_COMPUTED, "CCDWidth", TSRMLS_C, "%dmm", (int)ImageInfo.CCDWidth);
	}
	if(ImageInfo.ExposureTime>0) {
		if(ImageInfo.ExposureTime <= 0.5) {
			exif_iif_add_fmt(&ImageInfo, SECTION_COMPUTED, "ExposureTime", TSRMLS_C, "%0.3F s (1/%d)", ImageInfo.ExposureTime, (int)(0.5 + 1/ImageInfo.ExposureTime));
		} else {
			exif_iif_add_fmt(&ImageInfo, SECTION_COMPUTED, "ExposureTime", TSRMLS_C, "%0.3F s", ImageInfo.ExposureTime);
		}
	}
	if(ImageInfo.ApertureFNumber) {
		exif_iif_add_fmt(&ImageInfo, SECTION_COMPUTED, "ApertureFNumber", TSRMLS_C, "f/%.1F", ImageInfo.ApertureFNumber);
	}
	if(ImageInfo.Distance) {
		if(ImageInfo.Distance<0) {
			exif_iif_add_str(&ImageInfo, SECTION_COMPUTED, "FocusDistance", "Infinite", TSRMLS_C);
		} else {
			exif_iif_add_fmt(&ImageInfo, SECTION_COMPUTED, "FocusDistance", TSRMLS_C, "%0.2Fm", ImageInfo.Distance);
		}
	}
	if (ImageInfo.UserComment) {
		exif_iif_add_buffer(&ImageInfo, SECTION_COMPUTED, "UserComment", ImageInfo.UserCommentLength, ImageInfo.UserComment, TSRMLS_C);
		if (ImageInfo.UserCommentEncoding && strlen(ImageInfo.UserCommentEncoding)) {
			exif_iif_add_str(&ImageInfo, SECTION_COMPUTED, "UserCommentEncoding", ImageInfo.UserCommentEncoding, TSRMLS_C);
		}
	}

	exif_iif_add_str(&ImageInfo, SECTION_COMPUTED, "Copyright",              ImageInfo.Copyright, TSRMLS_C);
	exif_iif_add_str(&ImageInfo, SECTION_COMPUTED, "Copyright.Photographer", ImageInfo.CopyrightPhotographer, TSRMLS_C);
	exif_iif_add_str(&ImageInfo, SECTION_COMPUTED, "Copyright.Editor",       ImageInfo.CopyrightEditor, TSRMLS_C);

	for (i=0; i<ImageInfo.xp_fields.count; i++) {
		exif_iif_add_str(&ImageInfo, SECTION_WINXP, exif_get_tagname(ImageInfo.xp_fields.list[i].tag, NULL, 0, exif_get_tag_table(SECTION_WINXP), TSRMLS_C), ImageInfo.xp_fields.list[i].value, TSRMLS_C);
	}
	if (ImageInfo.Thumbnail.size) {
		if (read_thumbnail) {
			/* not exif_iif_add_str : this is a buffer */
			exif_iif_add_tag(&ImageInfo, SECTION_THUMBNAIL, "THUMBNAIL", TAG_NONE, TAG_FMT_UNDEFINED, ImageInfo.Thumbnail.size, ImageInfo.Thumbnail.data, TSRMLS_C);
		}
		if (!ImageInfo.Thumbnail.width || !ImageInfo.Thumbnail.height) {
			/* try to evaluate if thumbnail data is present */
			exif_scan_thumbnail(&ImageInfo, TSRMLS_C);
		}
		exif_iif_add_int(&ImageInfo, SECTION_COMPUTED, "Thumbnail.FileType", ImageInfo.Thumbnail.filetype, TSRMLS_C);
		exif_iif_add_str(&ImageInfo, SECTION_COMPUTED, "Thumbnail.MimeType", (char*)php_image_type_to_mime_type(ImageInfo.Thumbnail.filetype), TSRMLS_C);
	}
	if (ImageInfo.Thumbnail.width && ImageInfo.Thumbnail.height) {
		exif_iif_add_int(&ImageInfo, SECTION_COMPUTED, "Thumbnail.Height", ImageInfo.Thumbnail.height, TSRMLS_C);
		exif_iif_add_int(&ImageInfo, SECTION_COMPUTED, "Thumbnail.Width",  ImageInfo.Thumbnail.width, TSRMLS_C);
	}
   	EFREE_IF(sections_str);
	...
	add_assoc_image_info(return_value, sub_arrays, &ImageInfo, SECTION_FILE,       TSRMLS_C);
	add_assoc_image_info(return_value, 1,          &ImageInfo, SECTION_COMPUTED,   TSRMLS_C);
	add_assoc_image_info(return_value, sub_arrays, &ImageInfo, SECTION_ANY_TAG,    TSRMLS_C);
	add_assoc_image_info(return_value, sub_arrays, &ImageInfo, SECTION_IFD0,       TSRMLS_C);
	add_assoc_image_info(return_value, 1,          &ImageInfo, SECTION_THUMBNAIL,  TSRMLS_C);
	add_assoc_image_info(return_value, 1,          &ImageInfo, SECTION_COMMENT,    TSRMLS_C);
	add_assoc_image_info(return_value, sub_arrays, &ImageInfo, SECTION_EXIF,       TSRMLS_C);
	add_assoc_image_info(return_value, sub_arrays, &ImageInfo, SECTION_GPS,        TSRMLS_C);
	add_assoc_image_info(return_value, sub_arrays, &ImageInfo, SECTION_INTEROP,    TSRMLS_C);
	add_assoc_image_info(return_value, sub_arrays, &ImageInfo, SECTION_FPIX,       TSRMLS_C);
	add_assoc_image_info(return_value, sub_arrays, &ImageInfo, SECTION_APP12,      TSRMLS_C);
	add_assoc_image_info(return_value, sub_arrays, &ImageInfo, SECTION_WINXP,      TSRMLS_C);
	add_assoc_image_info(return_value, sub_arrays, &ImageInfo, SECTION_MAKERNOTE,  TSRMLS_C);
...
#ifdef EXIF_DEBUG
	php_error_docref1(NULL, TSRMLS_C, Z_STRVAL_PP(p_name), E_NOTICE, "done");
#endif
}
/* }}} */
...
PHP_FUNCTION(exif_thumbnail)
{
	...
	if (zend_parse_parameters(arg_c, TSRMLS_C, "p|z/z/z/", &p_name, &p_name_len, &p_width, &p_height, &p_imagetype) == FAILURE) {
		return;
	}

	ret = exif_read_file(&ImageInfo, p_name, 1, 0, TSRMLS_C);
	if (ret==FALSE) {
		exif_discard_imageinfo(&ImageInfo);
		RETURN_FALSE;
	}
...
#ifdef EXIF_DEBUG
	php_error_docref1(NULL, TSRMLS_C, p_name, E_NOTICE, "Done");
#endif
}
/* }}} */
...
PHP_FUNCTION(exif_imagetype)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s", &imagefile, &imagefile_len) == FAILURE) {
		return;
	}
	...
	itype = php_getimagetype(stream, NULL, TSRMLS_C);
