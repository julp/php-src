...
PHP_FUNCTION(recode_string)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ss", &req, &req_len, &str, &str_len) == FAILURE) {
		return;
	}
	...
	if (request == NULL) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Cannot allocate request structure");
		RETURN_FALSE;
	}

	if (!recode_scan_request(request, req)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Illegal recode request '%s'", req);
		goto error_exit;
	}
	
	recode_buffer_to_buffer(request, str, str_len, &r, &r_len, &r_alen);
	if (!r) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Recoding failed.");
error_exit:
		RETVAL_FALSE;
	} else {
...
PHP_FUNCTION(recode_file)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "srr", &req, &req_len, &input, &output) == FAILURE) {
	 	return;
	}
