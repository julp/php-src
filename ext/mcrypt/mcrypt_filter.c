	...
	int flags, TSRMLS_D)
{
	...
	while(buckets_in->head) {
		...
		if (data->blocksize) {
			...
			newbucket = php_stream_bucket_new(stream, outchunk, n, 1, data->persistent, TSRMLS_C);
			php_stream_bucket_append(buckets_out, newbucket, TSRMLS_C);
			...
			/* Stream cipher */
			php_stream_bucket_make_writeable(bucket, TSRMLS_C);
			if (data->encrypt) {
	...
	if ((flags & PSFS_FLAG_FLUSH_CLOSE) && data->blocksize && data->block_used) {
		...
		newbucket = php_stream_bucket_new(stream, data->block_buffer, data->blocksize, 0, data->persistent, TSRMLS_C);
		php_stream_bucket_append(buckets_out, newbucket, TSRMLS_C);
...
static void php_mcrypt_filter_dtor(php_stream_filter *thisfilter, TSRMLS_D)
{
	if (thisfilter && thisfilter->abstract) {
 ...
 */
static php_stream_filter *php_mcrypt_filter_create(const char *filtername, zval *filterparams, int persistent, TSRMLS_D)
{
	...
	if (!filterparams || Z_TYPE_P(filterparams) != IS_ARRAY) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Filter parameters for %s must be an array", filtername);
		return NULL;
	}
	...
	if (zend_hash_find(HASH_OF(filterparams), "mode", sizeof("mode"), (void**)&tmpzval) == SUCCESS) {
		if (Z_TYPE_PP(tmpzval) == IS_STRING) {
			...
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "mode is not a string, ignoring");
		}
	}
	...
	if (zend_hash_find(HASH_OF(filterparams), "algorithms_dir", sizeof("algorithms_dir"), (void**)&tmpzval) == SUCCESS) {
		if (Z_TYPE_PP(tmpzval) == IS_STRING) {
			...
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "algorithms_dir is not a string, ignoring");
		}
	}
	...
	if (zend_hash_find(HASH_OF(filterparams), "modes_dir", sizeof("modes_dir"), (void**)&tmpzval) == SUCCESS) {
		if (Z_TYPE_PP(tmpzval) == IS_STRING) {
			...
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "modes_dir is not a string, ignoring");
		}
	}
	...
	if (zend_hash_find(HASH_OF(filterparams), "key", sizeof("key"), (void**)&tmpzval) == SUCCESS &&
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "key not specified or is not a string");
		return NULL;
	}
	...
	if (zend_hash_find(HASH_OF(filterparams), "iv", sizeof("iv"), (void**) &tmpzval) == FAILURE ||
		Z_TYPE_PP(tmpzval) != IS_STRING) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Filter parameter[iv] not provided or not of type: string");
