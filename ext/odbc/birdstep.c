...
#define PHP_GET_BIRDSTEP_RES_IDX(id) if (!(res = birdstep_find_result(list, id))) { php_error_docref(NULL, TSRMLS_C, E_WARNING, "Birdstep: Not result index (%ld)", id); RETURN_FALSE; } 
#define PHP_BIRDSTEP_CHK_LNK(id) if (!(conn = birdstep_find_conn(list, id))) { php_error_docref(NULL, TSRMLS_C, E_WARNING, "Birdstep: Not connection index (%ld)", id); RETURN_FALSE; }
                                                        

static void _close_birdstep_link(zend_rsrc_list_entry *rsrc, TSRMLS_D)
{
...
static void _free_birdstep_result(zend_rsrc_list_entry *rsrc, TSRMLS_D)
{
...
static int birdstep_add_conn(HashTable *list,VConn *conn,HDBC hdbc, TSRMLS_D)
{
	int ind;

	ind = zend_list_insert(conn,php_birdstep_module.le_link, TSRMLS_C);
	conn->hdbc = hdbc;
	conn->index = ind;
...
PHP_FUNCTION(birdstep_connect)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "sss", &serv, &serv_len, &user, &user_len, &pass, &pass_len) == FAILURE) {
		return;
	}
	
	if ( php_birdstep_module.max_links != -1 && php_birdstep_module.num_links == php_birdstep_module.max_links ) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Birdstep: Too many open connections (%d)",php_birdstep_module.num_links);
		RETURN_FALSE;
	}

	stat = SQLAllocConnect(henv,&hdbc);
	if ( stat != SQL_SUCCESS ) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Birdstep: Could not allocate connection handle");
		RETURN_FALSE;
	}
	stat = SQLConnect(hdbc, serv, SQL_NTS, user, SQL_NTS, pass, SQL_NTS);
	if ( stat != SQL_SUCCESS && stat != SQL_SUCCESS_WITH_INFO ) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Birdstep: Could not connect to server \"%s\" for %s", serv, user);
		SQLFreeConnect(hdbc);
		RETURN_FALSE;
	}
	new = (VConn *)emalloc(sizeof(VConn));
	ind = birdstep_add_conn(list,new,hdbc, TSRMLS_C);
...
PHP_FUNCTION(birdstep_close)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l", &id) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(birdstep_exec)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ls", &ind, &query, &query_len) == FAILURE) {
		return;
	}
	...
	if ( stat != SQL_SUCCESS && stat != SQL_SUCCESS_WITH_INFO ) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Birdstep: SQLAllocStmt return %d",stat);
		efree(res);
		RETURN_FALSE;
	}
	stat = SQLExecDirect(res->hstmt,query,SQL_NTS);
	if ( stat != SQL_SUCCESS && stat != SQL_SUCCESS_WITH_INFO ) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Birdstep: Cannot execute \"%s\" query",query);
	...
	if ( stat != SQL_SUCCESS ) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Birdstep: SQLNumResultCols return %d",stat);
		SQLFreeStmt(res->hstmt,SQL_DROP);
		efree(res);
		RETURN_FALSE;
	}
...
PHP_FUNCTION(birdstep_fetch)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l", &ind) == FAILURE) {
		return;
	}
	...
	if ( stat != SQL_SUCCESS && stat != SQL_SUCCESS_WITH_INFO ) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Birdstep: SQLFetch return error");
		SQLFreeStmt(res->hstmt,SQL_DROP);
		birdstep_del_result(list,Z_LVAL_PP(ind));
		RETURN_FALSE;
	}
...
PHP_FUNCTION(birdstep_result)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "lZ", &ind, &col) == FAILURE) {
		return;
	}
	...
	if ( field ) {
		...
		if ( indx < 0 || indx >= res->numcols ) {
			php_error_docref(NULL, TSRMLS_C, E_WARNING, "Birdstep: Field index not in range");
			RETURN_FALSE;
		}
	}
	if ( !res->fetched ) {
...
l1:
			...
			if ( stat != SQL_SUCCESS && stat != SQL_SUCCESS_WITH_INFO ) {
				php_error_docref(NULL, TSRMLS_C, E_WARNING, "Birdstep: SQLGetData return error");
				SQLFreeStmt(res->hstmt,SQL_DROP);
				birdstep_del_result(list,Z_LVAL_PP(ind));
				RETURN_FALSE;
			}
...
PHP_FUNCTION(birdstep_autocommit)
{
	...
	if ( stat != SQL_SUCCESS && stat != SQL_SUCCESS_WITH_INFO ) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Birdstep: Set autocommit_on option failure");
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(birdstep_off_autocommit)
{
	...
	if ( stat != SQL_SUCCESS && stat != SQL_SUCCESS_WITH_INFO ) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Birdstep: Set autocommit_off option failure");
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(birdstep_commit)
{
	...
	if ( stat != SQL_SUCCESS ) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Birdstep: Commit failure");
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(birdstep_rollback)
{
	...
	if ( stat != SQL_SUCCESS ) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Birdstep: Rollback failure");
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
PHP_FUNCTION(birdstep_fieldname)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ll", &ind, &col) == FAILURE) {
		return;
	}
