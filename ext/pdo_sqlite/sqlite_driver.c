...
int _pdo_sqlite_error(pdo_dbh_t *dbh, pdo_stmt_t *stmt, const char *file, int line, TSRMLS_D) /* {{{ */
{
...
static int pdo_sqlite_fetch_error_func(pdo_dbh_t *dbh, pdo_stmt_t *stmt, zval *info, TSRMLS_D)
{
	pdo_sqlite_db_handle *H = (pdo_sqlite_db_handle *)dbh->driver_data;
	pdo_sqlite_error_info *einfo = &H->einfo;
...
static void pdo_sqlite_cleanup_callbacks(pdo_sqlite_db_handle *H, TSRMLS_D)
{
...
static int sqlite_handle_closer(pdo_dbh_t *dbh, TSRMLS_D) /* {{{ */
{
	pdo_sqlite_db_handle *H = (pdo_sqlite_db_handle *)dbh->driver_data;
	...
	if (H) {
		...
		pdo_sqlite_cleanup_callbacks(H, TSRMLS_C);
		if (H->db) {
			sqlite3_close(H->db);
			H->db = NULL;
		}
...
static int sqlite_handle_preparer(pdo_dbh_t *dbh, const char *sql, long sql_len, pdo_stmt_t *stmt, zval *driver_options, TSRMLS_D)
{
	pdo_sqlite_db_handle *H = (pdo_sqlite_db_handle *)dbh->driver_data;
	...
	if (PDO_CURSOR_FWDONLY != pdo_attr_lval(driver_options, PDO_ATTR_CURSOR, PDO_CURSOR_FWDONLY, TSRMLS_C)) {
...
static long sqlite_handle_doer(pdo_dbh_t *dbh, const char *sql, long sql_len, TSRMLS_D)
{
	pdo_sqlite_db_handle *H = (pdo_sqlite_db_handle *)dbh->driver_data;
	char *errmsg = NULL;
...
static char *pdo_sqlite_last_insert_id(pdo_dbh_t *dbh, const char *name, unsigned int *len, TSRMLS_D)
{
	pdo_sqlite_db_handle *H = (pdo_sqlite_db_handle *)dbh->driver_data;
	char *id;
	
	id = php_pdo_int64_to_str(sqlite3_last_insert_rowid(H->db), TSRMLS_C);
	*len = strlen(id);
	return id;
}

/* NB: doesn't handle binary strings... use prepared stmts for that */
static int sqlite_handle_quoter(pdo_dbh_t *dbh, const char *unquoted, int unquotedlen, char **quoted, int *quotedlen, enum pdo_param_type paramtype,  TSRMLS_D)
{
...
static int sqlite_handle_begin(pdo_dbh_t *dbh, TSRMLS_D)
{
	pdo_sqlite_db_handle *H = (pdo_sqlite_db_handle *)dbh->driver_data;
	char *errmsg = NULL;
...
static int sqlite_handle_commit(pdo_dbh_t *dbh, TSRMLS_D)
{
	pdo_sqlite_db_handle *H = (pdo_sqlite_db_handle *)dbh->driver_data;
	char *errmsg = NULL;
...
static int sqlite_handle_rollback(pdo_dbh_t *dbh, TSRMLS_D)
{
	pdo_sqlite_db_handle *H = (pdo_sqlite_db_handle *)dbh->driver_data;
	char *errmsg = NULL;
...
static int pdo_sqlite_get_attribute(pdo_dbh_t *dbh, long attr, zval *return_value, TSRMLS_D)
{
	switch (attr) {
...
static int pdo_sqlite_set_attr(pdo_dbh_t *dbh, long attr, zval *val, TSRMLS_D)
{
	pdo_sqlite_db_handle *H = (pdo_sqlite_db_handle *)dbh->driver_data;
	...
	sqlite3_value **argv)
{
	...
	do_callback(&func->afunc, func->func, argc, argv, context, 0, TSRMLS_C);
}
	...
	sqlite3_value **argv)
{
	...
	do_callback(&func->astep, func->step, argc, argv, context, 1, TSRMLS_C);
}
...
static void php_sqlite3_func_final_callback(sqlite3_context *context)
{
	...
	do_callback(&func->afini, func->fini, 0, NULL, context, 1, TSRMLS_C);
}
	...
	int string2_len, const void *string2)
{
	...
	if ((ret = zend_call_function(&collation->fc.fci, &collation->fc.fcc, TSRMLS_C)) == FAILURE) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "An error occurred while invoking the callback");
	}
	else if (retval) {
...
static PHP_METHOD(SQLite, sqliteCreateFunction)
{
	...
	if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "sz|l",
	...
	if (!zend_is_callable(callback, 0, &cbname, TSRMLS_C)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "function '%s' is not callable", cbname);
		efree(cbname);
		RETURN_FALSE;
	}
	efree(cbname);
...
static PHP_METHOD(SQLite, sqliteCreateAggregate)
{
	...
	if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "szz|l",
	...
	if (!zend_is_callable(step_callback, 0, &cbname, TSRMLS_C)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "function '%s' is not callable", cbname);
		efree(cbname);
		RETURN_FALSE;
	}
	efree(cbname);
	if (!zend_is_callable(fini_callback, 0, &cbname, TSRMLS_C)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "function '%s' is not callable", cbname);
		efree(cbname);
		RETURN_FALSE;
	}
...
static PHP_METHOD(SQLite, sqliteCreateCollation)
{
	...
	if (FAILURE == zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "sz",
...
static const zend_function_entry *get_driver_methods(pdo_dbh_t *dbh, int kind, TSRMLS_D)
{
	switch (kind) {
...
static void pdo_sqlite_request_shutdown(pdo_dbh_t *dbh, TSRMLS_D)
{
	pdo_sqlite_db_handle *H = (pdo_sqlite_db_handle *)dbh->driver_data;
...
static char *make_filename_safe(const char *filename, TSRMLS_D)
{
	if (*filename && memcmp(filename, ":memory:", sizeof(":memory:"))) {
		char *fullpath = expand_filepath(filename, NULL, TSRMLS_C);
		...
		const char *arg5, const char *arg6)
{
	...
	switch (access_type) {
		case SQLITE_COPY: {
			TSRMLS_FETCH();
			filename = make_filename_safe(arg4, TSRMLS_C);
			if (!filename) {
				return SQLITE_DENY;
			}
			efree(filename);
		...
		case SQLITE_ATTACH: {
			TSRMLS_FETCH();
			filename = make_filename_safe(arg3, TSRMLS_C);
			if (!filename) {
				return SQLITE_DENY;
			}
			efree(filename);
...
static int pdo_sqlite_handle_factory(pdo_dbh_t *dbh, zval *driver_options, TSRMLS_D) /* {{{ */
{
	pdo_sqlite_db_handle *H;
	int i, ret = 0;
	long timeout = 60;
	...
	filename = make_filename_safe(dbh->data_source, TSRMLS_C);

	if (!filename) {
		zend_throw_exception_ex(php_pdo_get_exception(), 0, TSRMLS_C,
	...
	if (driver_options) {
		timeout = pdo_attr_lval(driver_options, PDO_ATTR_TIMEOUT, timeout, TSRMLS_C);
	}
