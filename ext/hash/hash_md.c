...
PHP_NAMED_FUNCTION(php_if_md5)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|b", &arg, &arg_len, &raw_output) == FAILURE) {
		return;
	}
...
PHP_NAMED_FUNCTION(php_if_md5_file)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "p|b", &arg, &arg_len, &raw_output) == FAILURE) {
		return;
	}
