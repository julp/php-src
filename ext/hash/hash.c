...
static void php_hash_do_hash(INTERNAL_FUNCTION_PARAMETERS, int isfilename, zend_bool raw_output_default) /* {{{ */
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ss|b", &algo, &algo_len, &data, &data_len, &raw_output) == FAILURE) {
		return;
	}

	ops = php_hash_fetch_ops(algo, algo_len);
	if (!ops) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unknown hashing algorithm: %s", algo);
		RETURN_FALSE;
	}
	if (isfilename) {
...
static void php_hash_do_hash_hmac(INTERNAL_FUNCTION_PARAMETERS, int isfilename, zend_bool raw_output_default) /* {{{ */
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "sss|b", &algo, &algo_len, &data, &data_len, 
...
PHP_FUNCTION(hash_init)
{
	...
	if (zend_parse_parameters(argc, TSRMLS_C, "s|ls", &algo, &algo_len, &options, &key, &key_len) == FAILURE) {
		return;
	}
	...
	if (options & PHP_HASH_HMAC &&
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "HMAC requested without a key");
		RETURN_FALSE;
	}
...
PHP_FUNCTION(hash_update)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rs", &zhash, &data, &data_len) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(hash_update_stream)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rr|l", &zhash, &zstream, &length) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(hash_update_file)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "rs|r", &zhash, &filename, &filename_len, &zcontext) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(hash_final)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r|b", &zhash, &raw_output) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(hash_copy)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "r", &zhash) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(hash_pbkdf2)
{
	...
	argc = ZEND_NUM_ARGS();
	if (zend_parse_parameters(argc, TSRMLS_C, "sssl|lb", &algo, &algo_len, &pass, &pass_len, &salt, &salt_len, &iterations, &length, &raw_output) == FAILURE) {
		return;
	}
	...
	if (iterations <= 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Iterations must be a positive integer: %ld", iterations);
		RETURN_FALSE;
	}

	if (length < 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Length must be greater than or equal to 0: %ld", length);
		RETURN_FALSE;
	}

	if (salt_len > INT_MAX - 4) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Supplied salt is too long, max of INT_MAX - 4 bytes: %d supplied", salt_len);
		RETURN_FALSE;
	}
...
static void php_hash_dtor(zend_rsrc_list_entry *rsrc, TSRMLS_D) /* {{{ */
{
...
static void mhash_init(INIT_FUNC_ARGS)
{
	...
	for (algo_number = 0; algo_number < MHASH_NUM_ALGOS; algo_number++) {
		...
		zend_register_long_constant(buf, len + 1, algorithm.value, CONST_CS | CONST_PERSISTENT, module_number, TSRMLS_C);
	}
	zend_register_internal_module(&mhash_module_entry, TSRMLS_C);
}
...
PHP_FUNCTION(mhash)
{
	...
	if (zend_parse_parameters(1, TSRMLS_C, "Z", &z_algorithm) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(mhash_get_hash_name)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l", &algorithm) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(mhash_keygen_s2k)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "lssl", &algorithm, &password, &password_len, &salt, &salt_len, &l_bytes) == FAILURE) {
		return;
	}

	bytes = (int)l_bytes;
	if (bytes <= 0){
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "the byte parameter must be greater than 0");
		RETURN_FALSE;
	}
