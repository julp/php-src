...
static void php_bz2_decompress_dtor(php_stream_filter *thisfilter, TSRMLS_D)
{
	if (thisfilter && thisfilter->abstract) {
...
static void php_bz2_compress_dtor(php_stream_filter *thisfilter, TSRMLS_D)
{
	if (thisfilter && thisfilter->abstract) {
		php_bz2_filter_data *data = thisfilter->abstract;
...
static php_stream_filter *php_bz2_filter_create(const char *filtername, zval *filterparams, int persistent, TSRMLS_D)
{
	...
	if (!data) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Failed allocating %zu bytes", sizeof(php_bz2_filter_data));
		return NULL;
	}
	...
	if (!data->inbuf) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Failed allocating %zu bytes", data->inbuf_len);
	...
	if (!data->outbuf) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Failed allocating %zu bytes", data->outbuf_len);
			...
			if (Z_TYPE_P(filterparams) == IS_ARRAY || Z_TYPE_P(filterparams) == IS_OBJECT) {
				if (zend_hash_find(HASH_OF(filterparams), "blocks", sizeof("blocks"), (void**) &tmpzval) == SUCCESS) {
					...
					if (Z_LVAL(tmp) < 1 || Z_LVAL(tmp) > 9) {
						php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid parameter given for number of blocks to allocate. (%ld)", Z_LVAL_PP(tmpzval));
					} else {
				...
				if (zend_hash_find(HASH_OF(filterparams), "work", sizeof("work"), (void**) &tmpzval) == SUCCESS) {
					...
					if (Z_LVAL(tmp) < 0 || Z_LVAL(tmp) > 250) {
						php_error_docref(NULL, TSRMLS_C, E_WARNING, "Invalid parameter given for work factor. (%ld)", Z_LVAL(tmp));
					} else {
