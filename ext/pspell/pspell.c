...
static void php_pspell_close(zend_rsrc_list_entry *rsrc, TSRMLS_D)
{
...
static void php_pspell_close_config(zend_rsrc_list_entry *rsrc, TSRMLS_D)
{
...
#define PSPELL_FETCH_CONFIG \
	...
	if (config == NULL || type != le_pspell_config) {	\
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "%ld is not a PSPELL config index", conf);	\
		RETURN_FALSE;	\
	}	\
...
#define PSPELL_FETCH_MANAGER \
	...
	if (!manager || type != le_pspell) {	\
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "%ld is not a PSPELL result index", scin);	\
		RETURN_FALSE;	\
	}	\
...
static PHP_FUNCTION(pspell_new)
{
	...
	if (zend_parse_parameters(argc, TSRMLS_C, "s|sssl", &language, &language_len, &spelling, &spelling_len,
	...
	if (pspell_error_number(ret) != 0) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "PSPELL couldn't open the dictionary. reason: %s", pspell_error_message(ret));
		delete_pspell_can_have_error(ret);
		RETURN_FALSE;
	}
	
	manager = to_pspell_manager(ret);
	ind = zend_list_insert(manager, le_pspell, TSRMLS_C);
	RETURN_LONG(ind);
}
/* }}} */
...
static PHP_FUNCTION(pspell_new_personal)
{
	...
	if (zend_parse_parameters(argc, TSRMLS_C, "ps|sssl", &personal, &personal_len, &language, &language_len, 
	...
	if (php_check_open_basedir(personal, TSRMLS_C)) {
		delete_pspell_config(config);
		RETURN_FALSE;
	}
...
static PHP_FUNCTION(pspell_new_config)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l", &conf) == FAILURE) {
		return;
	}
...
static PHP_FUNCTION(pspell_check)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ls", &scin, &word, &word_len) == FAILURE) {
		return;
	}
...
static PHP_FUNCTION(pspell_suggest)
{
	...
	if (wl) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "PSPELL had a problem. details: %s", pspell_manager_error_message(manager));
		RETURN_FALSE;
	}
}
/* }}} */
...
static PHP_FUNCTION(pspell_store_replacement)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "lss", &scin, &miss, &miss_len, &corr, &corr_len) == FAILURE) {
		return;
	}
	...
	if (pspell_manager_error_number(manager) == 0) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "pspell_store_replacement() gave error: %s", pspell_manager_error_message(manager));
		RETURN_FALSE;
	}
}
/* }}} */
...
static PHP_FUNCTION(pspell_add_to_personal)
{
	...
	if (pspell_manager_error_number(manager) == 0) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "pspell_add_to_personal() gave error: %s", pspell_manager_error_message(manager));
		RETURN_FALSE;
	}
}
/* }}} */
...
static PHP_FUNCTION(pspell_add_to_session)
{
	...
	if (pspell_manager_error_number(manager) == 0) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "pspell_add_to_session() gave error: %s", pspell_manager_error_message(manager));
		RETURN_FALSE;
	}
}
/* }}} */
...
static PHP_FUNCTION(pspell_clear_session)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "l", &scin) == FAILURE) {
		return;
	}
	...
	if (pspell_manager_error_number(manager) == 0) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "pspell_clear_session() gave error: %s", pspell_manager_error_message(manager));
		RETURN_FALSE;
	}
}
/* }}} */
...
static PHP_FUNCTION(pspell_save_wordlist)
{
	...
	if (pspell_manager_error_number(manager) == 0) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "pspell_save_wordlist() gave error: %s", pspell_manager_error_message(manager));
		RETURN_FALSE;
	}
...
static PHP_FUNCTION(pspell_config_create)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|sss", &language, &language_len, &spelling, &spelling_len, 
	...
	ind = zend_list_insert(config, le_pspell_config, TSRMLS_C);
	RETURN_LONG(ind);
}
/* }}} */
...
static PHP_FUNCTION(pspell_config_runtogether)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "lb", &conf, &runtogether) == FAILURE) {
		return;
	}
...
static PHP_FUNCTION(pspell_config_mode)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ll", &conf, &mode) == FAILURE) {
		return;
	}
...
static PHP_FUNCTION(pspell_config_ignore)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "ll", &conf, &ignore) == FAILURE) {
		return;
	}
...
static void pspell_config_path(INTERNAL_FUNCTION_PARAMETERS, char *option)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "lp", &conf, &value, &value_len) == FAILURE) {
		return;
	}

	PSPELL_FETCH_CONFIG;

	if (php_check_open_basedir(value, TSRMLS_C)) {
		RETURN_FALSE;
	}
...
static PHP_FUNCTION(pspell_config_repl)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "lp", &conf, &repl, &repl_len) == FAILURE) {
		return;
	}
	...
	if (php_check_open_basedir(repl, TSRMLS_C)) {
		RETURN_FALSE;
	}
...
static PHP_FUNCTION(pspell_config_save_repl)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "lb", &conf, &save) == FAILURE) {
		return;
	}
