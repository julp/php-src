...
static void cwd_globals_ctor(virtual_cwd_globals *cwd_g, TSRMLS_D) /* {{{ */
{
...
static void cwd_globals_dtor(virtual_cwd_globals *cwd_g, TSRMLS_D) /* {{{ */
{
...
#else
	cwd_globals_ctor(&cwd_globals, TSRMLS_C);
#endif
...
{
#ifndef ZTS
	cwd_globals_dtor(&cwd_globals, TSRMLS_C);
#endif
#if (defined(TSRM_WIN32) || defined(NETWARE)) && defined(ZTS)
...
CWD_API char *virtual_getcwd_ex(size_t *length, TSRMLS_D) /* {{{ */
{
	cwd_state *state;
...
CWD_API char *virtual_getcwd(char *buf, size_t size, TSRMLS_D) /* {{{ */
{
	size_t length;
	char *cwd;

	cwd = virtual_getcwd_ex(&length, TSRMLS_C);
...
#ifdef PHP_WIN32
static inline unsigned long realpath_cache_key(const char *path, int path_len, TSRMLS_D) /* {{{ */
{
	register unsigned long h;
	char *bucket_key_start = tsrm_win32_get_path_sid_key(path, TSRMLS_C);
...
CWD_API void realpath_cache_del(const char *path, int path_len, TSRMLS_D) /* {{{ */
{
#ifdef PHP_WIN32
	unsigned long key = realpath_cache_key(path, path_len, TSRMLS_C);
#else
...
static inline void realpath_cache_add(const char *path, int path_len, const char *realpath, int realpath_len, int is_dir, time_t t, TSRMLS_D) /* {{{ */
{
...
#ifdef PHP_WIN32
		bucket->key = realpath_cache_key(path, path_len, TSRMLS_C);
#else
...
static inline realpath_cache_bucket* realpath_cache_find(const char *path, int path_len, time_t t, TSRMLS_D) /* {{{ */
{
#ifdef PHP_WIN32
	unsigned long key = realpath_cache_key(path, path_len, TSRMLS_C);
#else
...
CWD_API realpath_cache_bucket* realpath_cache_lookup(const char *path, int path_len, time_t t, TSRMLS_D) /* {{{ */
{
	return realpath_cache_find(path, path_len, t, TSRMLS_C);
}
/* }}} */
...
static int tsrm_realpath_r(char *path, int start, int len, int *ll, time_t *t, int use_realpath, int is_dir, int *link_is_dir, TSRMLS_D) /* {{{ */
{
	int i, j, save;
	int directory = 0;
#ifdef TSRM_WIN32
	...
	while (1) {
		...
		if (i == len ||
			...
			j = tsrm_realpath_r(path, start, i-1, ll, t, use_realpath, 1, NULL, TSRMLS_C);
			if (j > start) {
				j--;
		...
		if (start && save && CWDG(realpath_cache_size_limit)) {
			...
			if ((bucket = realpath_cache_find(path, len, *t, TSRMLS_C)) != NULL) {
		...
		if(save &&
			...
			if(isabsolute == 1) {
				if (!((j == 3) && (path[1] == ':') && (path[2] == '\\'))) {
					...
					j = tsrm_realpath_r(path, 0, j, ll, t, 0, is_dir, &directory, TSRMLS_C);
					if(j < 0) {
				...
				j  = tsrm_realpath_r(path, start, i + j, ll, t, use_realpath, is_dir, &directory, TSRMLS_C);
				if(j < 0) {
					tsrm_free_alloca(tmp, use_heap);
					return -1;
				}
		...
		if (save && S_ISLNK(st.st_mode)) {
			...
			if (IS_ABSOLUTE_PATH(path, j)) {
				j = tsrm_realpath_r(path, 1, j, ll, t, use_realpath, is_dir, &directory, TSRMLS_C);
				if (j < 0) {
					tsrm_free_alloca(tmp, use_heap);
					return -1;
				}
			...
			if (i - 1 <= start) {
				...
				j = tsrm_realpath_r(path, start, i-1, ll, t, save ? CWD_FILEPATH : use_realpath, 1, NULL, TSRMLS_C);
				if (j > start) {
					path[j++] = DEFAULT_SLASH;
				}
			}
		...
		if (save && start && CWDG(realpath_cache_size_limit)) {
			...
			realpath_cache_add(tmp, len, path, j, directory, *t, TSRMLS_C);
		}
...
CWD_API int virtual_file_ex(cwd_state *state, const char *path, verify_path_func verify_path, int use_realpath, TSRMLS_D) /* {{{ */
{
...
# endif
	...
	path_length = tsrm_realpath_r(resolved_path, start, path_length, &ll, &t, use_realpath, 0, NULL, TSRMLS_C);
...
CWD_API int virtual_chdir(const char *path, TSRMLS_D) /* {{{ */
{
	return virtual_file_ex(&CWDG(cwd), path, php_is_dir_ok, CWD_REALPATH, TSRMLS_C)?-1:0;
}
/* }}} */

CWD_API int virtual_chdir_file(const char *path, int (*p_chdir)(const char *path, TSRMLS_D), TSRMLS_D) /* {{{ */
{
...
#endif
	retval = p_chdir(temp, TSRMLS_C);
	tsrm_free_alloca(temp, use_heap);
	return retval;
}
/* }}} */

CWD_API char *virtual_realpath(const char *path, char *real_path, TSRMLS_D) /* {{{ */
{
	cwd_state new_state;
	char *retval;
	char cwd[MAXPATHLEN];
	...
	if (virtual_file_ex(&new_state, path, NULL, CWD_REALPATH, TSRMLS_C)==0) {
...
CWD_API int virtual_filepath_ex(const char *path, char **filepath, verify_path_func verify_path, TSRMLS_D) /* {{{ */
{
	cwd_state new_state;
	int retval;

	CWD_STATE_COPY(&new_state, &CWDG(cwd));
	retval = virtual_file_ex(&new_state, path, verify_path, CWD_FILEPATH, TSRMLS_C);
...
CWD_API int virtual_filepath(const char *path, char **filepath, TSRMLS_D) /* {{{ */
{
	return virtual_filepath_ex(path, filepath, php_is_file_ok, TSRMLS_C);
}
/* }}} */

CWD_API FILE *virtual_fopen(const char *path, const char *mode, TSRMLS_D) /* {{{ */
{
	cwd_state new_state;
	FILE *f;
	...
	CWD_STATE_COPY(&new_state, &CWDG(cwd));
	if (virtual_file_ex(&new_state, path, NULL, CWD_EXPAND, TSRMLS_C)) {
		CWD_STATE_FREE(&new_state);
		return NULL;
	}
...
CWD_API int virtual_access(const char *pathname, int mode, TSRMLS_D) /* {{{ */
{
	cwd_state new_state;
	int ret;

	CWD_STATE_COPY(&new_state, &CWDG(cwd));
	if (virtual_file_ex(&new_state, pathname, NULL, CWD_REALPATH, TSRMLS_C)) {
		CWD_STATE_FREE(&new_state);
		return -1;
	}

#if defined(TSRM_WIN32)
	ret = tsrm_win32_access(new_state.cwd, mode, TSRMLS_C);
#else
...
CWD_API int virtual_utime(const char *filename, struct utimbuf *buf, TSRMLS_D) /* {{{ */
{
	cwd_state new_state;
	int ret;

	CWD_STATE_COPY(&new_state, &CWDG(cwd));
	if (virtual_file_ex(&new_state, filename, NULL, CWD_REALPATH, TSRMLS_C)) {
		CWD_STATE_FREE(&new_state);
		return -1;
	}
...
CWD_API int virtual_chmod(const char *filename, mode_t mode, TSRMLS_D) /* {{{ */
{
	cwd_state new_state;
	int ret;
...
CWD_API int virtual_chown(const char *filename, uid_t owner, gid_t group, int link, TSRMLS_D) /* {{{ */
{
	cwd_state new_state;
	int ret;
...
CWD_API int virtual_open(const char *path, TSRMLS_D, int flags, ...) /* {{{ */
{
	cwd_state new_state;
	int f;

	CWD_STATE_COPY(&new_state, &CWDG(cwd));
	if (virtual_file_ex(&new_state, path, NULL, CWD_FILEPATH, TSRMLS_C)) {
		CWD_STATE_FREE(&new_state);
		return -1;
	}
...
CWD_API int virtual_creat(const char *path, mode_t mode, TSRMLS_D) /* {{{ */
{
	cwd_state new_state;
	int f;
...
CWD_API int virtual_rename(char *oldname, char *newname, TSRMLS_D) /* {{{ */
{
	cwd_state old_state;
	cwd_state new_state;
	int retval;
	...
	if (virtual_file_ex(&old_state, oldname, NULL, CWD_EXPAND, TSRMLS_C)) {
		CWD_STATE_FREE(&old_state);
		return -1;
	}
	...
	CWD_STATE_COPY(&new_state, &CWDG(cwd));
	if (virtual_file_ex(&new_state, newname, NULL, CWD_EXPAND, TSRMLS_C)) {
		CWD_STATE_FREE(&old_state);
		CWD_STATE_FREE(&new_state);
		return -1;
	}
...
CWD_API int virtual_stat(const char *path, struct stat *buf, TSRMLS_D) /* {{{ */
{
	cwd_state new_state;
	int retval;

	CWD_STATE_COPY(&new_state, &CWDG(cwd));
	if (virtual_file_ex(&new_state, path, NULL, CWD_REALPATH, TSRMLS_C)) {
		CWD_STATE_FREE(&new_state);
		return -1;
	}
...
CWD_API int virtual_lstat(const char *path, struct stat *buf, TSRMLS_D) /* {{{ */
{
	cwd_state new_state;
	int retval;
...
CWD_API int virtual_unlink(const char *path, TSRMLS_D) /* {{{ */
{
	cwd_state new_state;
	int retval;
...
CWD_API int virtual_mkdir(const char *pathname, mode_t mode, TSRMLS_D) /* {{{ */
{
	cwd_state new_state;
	int retval;

	CWD_STATE_COPY(&new_state, &CWDG(cwd));
	if (virtual_file_ex(&new_state, pathname, NULL, CWD_FILEPATH, TSRMLS_C)) {
		CWD_STATE_FREE(&new_state);
		return -1;
	}
...
CWD_API int virtual_rmdir(const char *pathname, TSRMLS_D) /* {{{ */
{
	cwd_state new_state;
	int retval;

	CWD_STATE_COPY(&new_state, &CWDG(cwd));
	if (virtual_file_ex(&new_state, pathname, NULL, CWD_EXPAND, TSRMLS_C)) {
		CWD_STATE_FREE(&new_state);
		return -1;
	}
...
CWD_API DIR *virtual_opendir(const char *pathname, TSRMLS_D) /* {{{ */
{
	cwd_state new_state;
	DIR *retval;
...
#ifdef TSRM_WIN32
CWD_API FILE *virtual_popen(const char *command, const char *type, TSRMLS_D) /* {{{ */
{
	return popen_ex(command, type, CWDG(cwd).cwd, NULL, TSRMLS_C);
}
/* }}} */
#elif defined(NETWARE)
...
CWD_API char *tsrm_realpath(const char *path, char *real_path, TSRMLS_D) /* {{{ */
{
	cwd_state new_state;
	char cwd[MAXPATHLEN];
