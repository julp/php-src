...
static void php_glob_stream_path_split(glob_s_t *pglob, char *path, int get_path, char **p_file, TSRMLS_D) /* {{{ */
{
	char *pos, *gpath = path;
...
static size_t php_glob_stream_read(php_stream *stream, char *buf, size_t count, TSRMLS_D) /* {{{ */
{
	glob_s_t *pglob = (glob_s_t *)stream->abstract;
	...
	if (count == sizeof(php_stream_dirent) && pglob) {
		if (pglob->index < (size_t)pglob->glob.gl_pathc) {
			php_glob_stream_path_split(pglob, pglob->glob.gl_pathv[pglob->index++], pglob->flags & GLOB_APPEND, &path, TSRMLS_C);
...
static int php_glob_stream_close(php_stream *stream, int close_handle, TSRMLS_D)  /* {{{ */
{
	glob_s_t *pglob = (glob_s_t *)stream->abstract;
...
static int php_glob_stream_rewind(php_stream *stream, off_t offset, int whence, off_t *newoffs, TSRMLS_D) /* {{{ */
{
	glob_s_t *pglob = (glob_s_t *)stream->abstract;
		...
		int options, char **opened_path, php_stream_context *context STREAMS_DC, TSRMLS_D)
{
	...
	if (pglob->glob.gl_pathc) {
		php_glob_stream_path_split(pglob, pglob->glob.gl_pathv[0], 1, &tmp, TSRMLS_C);
	} else {
		php_glob_stream_path_split(pglob, path, 1, &tmp, TSRMLS_C);
	}
