...
#define php_stream_mmap_supported(stream)	(_php_stream_set_option((stream), PHP_STREAM_OPTION_MMAP_API, PHP_STREAM_MMAP_SUPPORTED, NULL, TSRMLS_C) == 0 ? 1 : 0)
...
BEGIN_EXTERN_C()
PHPAPI char *_php_stream_mmap_range(php_stream *stream, size_t offset, size_t length, php_stream_mmap_operation_t mode, size_t *mapped_len, TSRMLS_D);
#define php_stream_mmap_range(stream, offset, length, mode, mapped_len)	_php_stream_mmap_range((stream), (offset), (length), (mode), (mapped_len), TSRMLS_C)

/* un-maps the last mapped range */
PHPAPI int _php_stream_mmap_unmap(php_stream *stream, TSRMLS_D);
#define php_stream_mmap_unmap(stream)				_php_stream_mmap_unmap((stream), TSRMLS_C)

PHPAPI int _php_stream_mmap_unmap_ex(php_stream *stream, off_t readden, TSRMLS_D);
#define php_stream_mmap_unmap_ex(stream, readden)			_php_stream_mmap_unmap_ex((stream), (readden), TSRMLS_C)
END_EXTERN_C()
