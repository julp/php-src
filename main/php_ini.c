 ...
 */
static void php_ini_displayer_cb(zend_ini_entry *ini_entry, int type, TSRMLS_D)
{
	if (ini_entry->displayer) {
		...
		if (esc_html) {
			php_html_puts(display_string, display_string_length, TSRMLS_C);
		} else {
...
/* {{{ php_ini_displayer
 */
static int php_ini_displayer(zend_ini_entry *ini_entry, int module_number, TSRMLS_D)
{
	...
	if (!sapi_module.phpinfo_as_text) {
		...
		PUTS("</td><td class=\"v\">");
		php_ini_displayer_cb(ini_entry, ZEND_INI_DISPLAY_ACTIVE, TSRMLS_C);
		PUTS("</td><td class=\"v\">");
		php_ini_displayer_cb(ini_entry, ZEND_INI_DISPLAY_ORIG, TSRMLS_C);
		PUTS("</td></tr>\n");
	} else {
		PHPWRITE(ini_entry->name, ini_entry->name_length - 1);
		PUTS(" => ");
...
/* {{{ php_ini_available
 */
static int php_ini_available(zend_ini_entry *ini_entry, int *module_number_available, TSRMLS_D)
{
...
PHPAPI void display_ini_entries(zend_module_entry *module)
{
	...
	zend_hash_apply_with_argument(EG(ini_directives), (apply_func_arg_t) php_ini_available, &module_number_available, TSRMLS_C);
	if (module_number_available == -1) {
		php_info_print_table_start();
		php_info_print_table_header(3, "Directive", "Local Value", "Master Value");
		zend_hash_apply_with_argument(EG(ini_directives), (apply_func_arg_t) php_ini_displayer, (void *) (zend_intptr_t) module_number, TSRMLS_C);
 ...
 */
static void php_load_php_extension_cb(void *arg, TSRMLS_D)
{
#ifdef HAVE_LIBDL
	php_load_extension(*((char **) arg), MODULE_PERSISTENT, 0, TSRMLS_C);
#endif
}
/* }}} */
 ...
 */
static void php_load_zend_extension_cb(void *arg, TSRMLS_D)
{
...
int php_init_config(TSRMLS_D)
{
	...
	if (!sapi_module.php_ini_ignore || sapi_module.php_ini_path_override) {
		...
		if (php_ini_file_name && php_ini_file_name[0]) {
			...
			if (!VCWD_STAT(php_ini_file_name, &statbuf)) {
				if (!((statbuf.st_mode & S_IFMT) == S_IFDIR)) {
					...
					if (fh.handle.fp) {
						fh.filename = php_ini_opened_path = expand_filepath(php_ini_file_name, NULL, TSRMLS_C);
					}
				}
			}
		}
		...
		if (!fh.handle.fp) {
			...
			fh.handle.fp = php_fopen_with_path(ini_fname, "r", php_ini_search_path, &php_ini_opened_path, TSRMLS_C);
			efree(ini_fname);
			if (fh.handle.fp) {
		...
		if (!fh.handle.fp) {
			fh.handle.fp = php_fopen_with_path("php.ini", "r", php_ini_search_path, &php_ini_opened_path, TSRMLS_C);
			if (fh.handle.fp) {
				fh.filename = php_ini_opened_path;
			}
		}
	...
	if (fh.handle.fp) {
		...
		zend_parse_ini_file(&fh, 1, ZEND_INI_SCANNER_NORMAL, (zend_ini_parser_cb_t) php_ini_parser_cb, &configuration_hash, TSRMLS_C);
				...
				if (VCWD_STAT(ini_file, &sb) == 0) {
					if (S_ISREG(sb.st_mode)) {
						if ((fh2.handle.fp = VCWD_FOPEN(ini_file, "r"))) {
							...
							if (zend_parse_ini_file(&fh2, 1, ZEND_INI_SCANNER_NORMAL, (zend_ini_parser_cb_t) php_ini_parser_cb, &configuration_hash, TSRMLS_C) == SUCCESS) {
	...
	if (sapi_module.ini_entries) {
		/* Reset active ini section */
		RESET_ACTIVE_INI_HASH();
		zend_parse_ini_string(sapi_module.ini_entries, 1, ZEND_INI_SCANNER_NORMAL, (zend_ini_parser_cb_t) php_ini_parser_cb, &configuration_hash, TSRMLS_C);
	}
...
{
	zend_llist_apply(&extension_lists.engine, php_load_zend_extension_cb, TSRMLS_C);
	zend_llist_apply(&extension_lists.functions, php_load_php_extension_cb, TSRMLS_C);
 ...
 */
PHPAPI int php_parse_user_ini_file(const char *dirname, char *ini_filename, HashTable *target_hash, TSRMLS_D)
{
	struct stat sb;
	char ini_file[MAXPATHLEN];
	zend_file_handle fh;
	...
	if (VCWD_STAT(ini_file, &sb) == 0) {
		if (S_ISREG(sb.st_mode)) {
			...
			if ((fh.handle.fp = VCWD_FOPEN(ini_file, "r"))) {
				...
				if (zend_parse_ini_file(&fh, 1, ZEND_INI_SCANNER_NORMAL, (zend_ini_parser_cb_t) php_ini_parser_cb, target_hash, TSRMLS_C) == SUCCESS) {
 ...
 */
PHPAPI void php_ini_activate_config(HashTable *source_hash, int modify_type, int stage, TSRMLS_D)
{
	char *str;
	zval *data;
	uint str_len;
	...
	for (zend_hash_internal_pointer_reset(source_hash);
		...
		zend_alter_ini_entry_ex(str, str_len, Z_STRVAL_P(data), Z_STRLEN_P(data), modify_type, stage, 0, TSRMLS_C);
	}
}
/* }}} */
 ...
 */
PHPAPI void php_ini_activate_per_dir_config(char *path, uint path_len, TSRMLS_D)
{
	zval *tmp2;
	char *ptr;
		...
		while ((ptr = strchr(ptr, '/')) != NULL) {
			*ptr = 0;
			...
			if (zend_hash_find(&configuration_hash, path, strlen(path) + 1, (void **) &tmp2) == SUCCESS) {
				php_ini_activate_config(Z_ARRVAL_P(tmp2), PHP_INI_SYSTEM, PHP_INI_STAGE_ACTIVATE, TSRMLS_C);
			}
			*ptr = '/';
			ptr++;
		}
 ...
 */
PHPAPI void php_ini_activate_per_host_config(const char *host, uint host_len, TSRMLS_D)
{
	zval *tmp;
	...
	if (has_per_host_config && host && host_len) {
		...
		if (zend_hash_find(&configuration_hash, host, host_len, (void **) &tmp) == SUCCESS) {
			php_ini_activate_config(Z_ARRVAL_P(tmp), PHP_INI_SYSTEM, PHP_INI_STAGE_ACTIVATE, TSRMLS_C);
		}
	}
}
/* }}} */
