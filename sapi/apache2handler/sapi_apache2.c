...
static int
php_apache_sapi_ub_write(const char *str, uint str_length, TSRMLS_D)
{
	request_rec *r;
	php_struct *ctx;
...
static int
php_apache_sapi_header_handler(sapi_header_struct *sapi_header, sapi_header_op_enum op, sapi_headers_struct *sapi_headers, TSRMLS_D)
{
	php_struct *ctx;
	char *val, *ptr;
...
static int
php_apache_sapi_send_headers(sapi_headers_struct *sapi_headers, TSRMLS_D)
{
...
static int
php_apache_sapi_read_post(char *buf, uint count_bytes, TSRMLS_D)
{
...
static char *
php_apache_sapi_getenv(char *name, size_t name_len, TSRMLS_D)
{
	php_struct *ctx = SG(server_context);
	const char *env_var;
...
static void
php_apache_sapi_register_variables(zval *track_vars_array, TSRMLS_D)
{
	php_struct *ctx = SG(server_context);
	...
	APR_ARRAY_FOREACH_OPEN(arr, key, val)
		...
		if (sapi_module.input_filter(PARSE_SERVER, key, &val, strlen(val), &new_val_len, TSRMLS_C)) {
			php_register_variable_safe(key, val, new_val_len, track_vars_array, TSRMLS_C);
		}
	APR_ARRAY_FOREACH_CLOSE()

	if (sapi_module.input_filter(PARSE_SERVER, "PHP_SELF", &ctx->r->uri, strlen(ctx->r->uri), &new_val_len, TSRMLS_C)) {
		php_register_variable_safe("PHP_SELF", ctx->r->uri, new_val_len, track_vars_array, TSRMLS_C);
	}
}
...
static void php_apache_sapi_log_message(char *msg, TSRMLS_D)
{
	php_struct *ctx;
...
static void php_apache_sapi_log_message_ex(char *msg, request_rec *r, TSRMLS_D)
{
	if (r) {
		ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, r, msg, r->filename);
	} else {
		php_apache_sapi_log_message(msg, TSRMLS_C);
	}
}
...
static int php_apache_request_ctor(request_rec *r, php_struct *ctx, TSRMLS_D)
{
	char *content_length;
	const char *auth;
...
static void php_apache_request_dtor(request_rec *r, TSRMLS_D)
{
	php_request_shutdown(NULL);
}

static void php_apache_ini_dtor(request_rec *r, request_rec *p, TSRMLS_D)
{
...
#define PHPAP_INI_OFF php_apache_ini_dtor(r, parent_req, TSRMLS_C);
...
normal:
	...
	if (r->finfo.filetype == 0) {
		php_apache_sapi_log_message_ex("script '%s' not found or unable to stat", r, TSRMLS_C);
		PHPAP_INI_OFF;
		return HTTP_NOT_FOUND;
	}
	if (r->finfo.filetype == APR_DIR) {
		php_apache_sapi_log_message_ex("attempt to invoke directory '%s' as script", r, TSRMLS_C);
		PHPAP_INI_OFF;
...
zend_first_try {
	...
	if (ctx == NULL) {
		...
		if (php_apache_request_ctor(r, ctx, TSRMLS_C)!=SUCCESS) {
			zend_bailout();
		}
	} else {
		if (!parent_req) {
	...
	if (strncmp(r->handler, PHP_SOURCE_MAGIC_TYPE, sizeof(PHP_SOURCE_MAGIC_TYPE) - 1) == 0) {
		...
		highlight_file((char *)r->filename, &syntax_highlighter_ini, TSRMLS_C);
	} else {
		...
		if (!parent_req) {
			...
			zend_execute_scripts(ZEND_INCLUDE, TSRMLS_C, NULL, 1, &zfd);
		}

		apr_table_set(r->notes, "mod_php_memory_usage",
			apr_psprintf(ctx->r->pool, "%zu", zend_memory_peak_usage(1, TSRMLS_C)));
	}

} zend_end_try();

	if (!parent_req) {
		php_apache_request_dtor(r, TSRMLS_C);
