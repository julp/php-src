...
static void fcgi_hash_apply(fcgi_hash *h, fcgi_apply_func func, void *arg, TSRMLS_D)
{
	fcgi_hash_bucket *p	= h->list;

	while (p) {
		if (EXPECTED(p->val != NULL)) {
			func(p->var, p->var_len, p->val, p->val_len, arg, TSRMLS_C);
		}
		p = p->list_next;
	}
}
...
void fcgi_loadenv(fcgi_request *req, fcgi_apply_func func, zval *array, TSRMLS_D)
{
	fcgi_hash_apply(&req->env, func, array, TSRMLS_C);
}
