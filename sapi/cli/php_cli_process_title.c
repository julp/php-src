...
PHP_FUNCTION(cli_set_process_title)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s", &title, &title_len) == FAILURE) {
        return;
    }
    ...
    php_error_docref(NULL, TSRMLS_C, E_WARNING, "cli_set_process_title had an error: %s", ps_title_errno(rc));
    RETURN_FALSE;
}
/* }}} */
...
PHP_FUNCTION(cli_get_process_title)
{
        ...
        if (rc != PS_TITLE_SUCCESS) {
                php_error_docref(NULL, TSRMLS_C, E_WARNING, "cli_get_process_title had an error: %s", ps_title_errno(rc));
                RETURN_NULL();
        }
