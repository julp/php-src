 ...
 */
static int sapi_capi_ub_write(const char *str, unsigned int str_length, TSRMLS_D)
{
   int retval;
 ...
 * by the PHP engine.
 */
static int sapi_capi_header_handler(sapi_header_struct * sapi_header, sapi_headers_struct * sapi_headers, TSRMLS_D)
{
 ...
 */
static int sapi_capi_send_headers(sapi_headers_struct * sapi_headers, TSRMLS_D)
{
   int retval;
   capi_request_context *rc = (capi_request_context *) SG(server_context);
...
static int sapi_capi_read_post(char *buffer, uint count_bytes, TSRMLS_D)
{
...
static void sapi_capi_register_server_variables(zval * track_vars_array, TSRMLS_D)
{
   capi_request_context *rc = (capi_request_context *) SG(server_context);
   size_t i;
   char *value;
   ...
   if (value != NULL) {
      php_register_variable("PHP_SELF", value, track_vars_array, TSRMLS_C);
   ...
   if(value!=NULL)
     php_register_variable("COUNTRY_CODE", value, track_vars_array, TSRMLS_C);

   /* argv */
   value = lstFset_get(rc->t->vars, "query");
   if (value != NULL)
      php_register_variable("argv", value, track_vars_array, TSRMLS_C);
   ...
   if (value != NULL) {
      php_register_variable("HTTP_HOST", value, track_vars_array, TSRMLS_C);
      /* TODO: This should probably scrub the port value if one is present. */
      php_register_variable("SERVER_NAME", value, track_vars_array, TSRMLS_C);
   }
   /* SERVER_SOFTWARE */
   value = lstFset_get(rc->t->res_hdrs, "Server");
   if (value != NULL)
      php_register_variable("SERVER_SOFTWARE", value, track_vars_array, TSRMLS_C);

   /* SERVER_PROTOCOL */
   value = lstFset_get(rc->t->vars, "protocol");
   if (value != NULL)
      php_register_variable("SERVER_PROTOCOL", value, track_vars_array, TSRMLS_C);

   /* REQUEST_METHOD */
   value = lstFset_get(rc->t->vars, "method");
   if (value != NULL)
      php_register_variable("REQUEST_METHOD", value, track_vars_array, TSRMLS_C);

   /* QUERY_STRING */
   value = lstFset_get(rc->t->vars, "query");
   if (value != NULL)
      php_register_variable("QUERY_STRING", value, track_vars_array, TSRMLS_C);

   /* DOCUMENT_ROOT */
   value = lstFset_get(rc->t->vars, "docroot");
   if (value != NULL)
      php_register_variable("DOCUMENT_ROOT", value, track_vars_array, TSRMLS_C);

   /* HTTP_ACCEPT */
   value = lstFset_get(rc->t->req_hdrs, "accept");
   if (value != NULL)
      php_register_variable("HTTP_ACCEPT", value, track_vars_array, TSRMLS_C);

   /* HTTP_ACCEPT_CHARSET */
   value = lstFset_get(rc->t->req_hdrs, "accept-charset");
   if (value != NULL)
      php_register_variable("HTTP_ACCEPT_CHARSET", value, track_vars_array, TSRMLS_C);

   /* HTTP_ACCEPT_ENCODING */
   value = lstFset_get(rc->t->req_hdrs, "accept-encoding");
   if (value != NULL)
      php_register_variable("HTTP_ACCEPT_ENCODING", value, track_vars_array, TSRMLS_C);

   /* HTTP_ACCEPT_LANGUAGE */
   value = lstFset_get(rc->t->req_hdrs, "accept-language");
   if (value != NULL)
      php_register_variable("HTTP_ACCEPT_LANGUAGE", value, track_vars_array, TSRMLS_C);

   /* HTTP_CONNECTION */
   value = lstFset_get(rc->t->req_hdrs, "connection");
   if (value != NULL)
      php_register_variable("HTTP_CONNECTION", value, track_vars_array, TSRMLS_C);

   /* HTTP_REFERER */
   value = lstFset_get(rc->t->req_hdrs, "referer");
   if (value != NULL)
      php_register_variable("HTTP_REFERER", value, track_vars_array, TSRMLS_C);

   /* HTTP_USER_AGENT */
   value = lstFset_get(rc->t->req_hdrs, "user-agent");
   if (value != NULL)
      php_register_variable("HTTP_USER_AGENT", value, track_vars_array, TSRMLS_C);

   /* REMOTE_ADDR */
   utlFip_to_str(rc->t->cli_ipv4_addr, buf, sizeof(buf));
   php_register_variable("REMOTE_ADDR", buf, track_vars_array, TSRMLS_C);
   ...
   if (value != NULL) {
      php_register_variable("SCRIPT_FILENAME", value, track_vars_array, TSRMLS_C);
...
static void capi_log_message(char *message, TSRMLS_D)
{
   capi_request_context *rc = (capi_request_context *) SG(server_context);
...
static void capi_request_ctor(NSLS_D, TSRMLS_D)
{
...
static void capi_request_dtor(NSLS_D, TSRMLS_D)
{
...
int capi_module_main(NSLS_D, TSRMLS_D)
{
...
int phpFservice(httpTtrans * t, lstTset * opts)
{
   ...
   capi_request_ctor(NSLS_C, TSRMLS_C);
   retval = capi_module_main(NSLS_C, TSRMLS_C);
   capi_request_dtor(NSLS_C, TSRMLS_C);
