...
PHP_FUNCTION(apache_child_terminate)
{
	...
	if (AP(terminate_child)) {
		...
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "This function is disabled");
		RETURN_FALSE;
	}
#else
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "This function is not supported in this build");
		RETURN_FALSE;
#endif
}
/* }}} */
...
PHP_FUNCTION(apache_note)
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), TSRMLS_C, "s|s", &note_name, &note_name_len, &note_val, &note_val_len) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(virtual)
{
	...
	if (!(rr = sub_req_lookup_uri (filename, ((request_rec *) SG(server_context))))) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to include '%s' - URI lookup failed", filename);
		if (rr)
			destroy_sub_req (rr);
		RETURN_FALSE;
	}
	...
	if (rr->status != 200) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to include '%s' - error finding URI", filename);
		if (rr)
			destroy_sub_req (rr);
		RETURN_FALSE;
	}
	...
	if (run_sub_req(rr)) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "Unable to include '%s' - request execution failed", filename);
		if (rr)
			destroy_sub_req (rr);
		RETURN_FALSE;
	}
...
PHP_FUNCTION(apache_lookup_uri)
{
	...
	if (!(rr = sub_req_lookup_uri(filename, ((request_rec *) SG(server_context))))) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "URI lookup failed '%s'", filename);
		RETURN_FALSE;
	}
...
PHP_FUNCTION(apache_exec_uri)
{
	...
	if(!(rr = ap_sub_req_lookup_uri(filename, ((request_rec *) SG(server_context))))) {
		php_error_docref(NULL, TSRMLS_C, E_WARNING, "URI lookup failed", filename);
		RETURN_FALSE;
	}
