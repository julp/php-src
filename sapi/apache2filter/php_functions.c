...
static request_rec *php_apache_lookup_uri(char *filename, TSRMLS_D)
{
	php_struct *ctx;
...
PHP_FUNCTION(virtual)
{
	...
	if (!(rr = php_apache_lookup_uri(filename, TSRMLS_C))) {
...
PHP_FUNCTION(apache_setenv)
{
	...
	if (zend_parse_parameters(arg_count, TSRMLS_C, "ss|b", &variable, &variable_len, &string_val, &string_val_len, &walk_to_top) == FAILURE) {
		return;
	}
...
PHP_FUNCTION(apache_getenv)
{
	...
	if (zend_parse_parameters(arg_count, TSRMLS_C, "s|b", &variable, &variable_len, &walk_to_top) == FAILURE) {
		return;
	}
