...
static int sapi_isapi_ub_write(const char *str, uint str_length, TSRMLS_D)
{
...
static int sapi_isapi_header_handler(sapi_header_struct *sapi_header, sapi_header_op_enum op, sapi_headers_struct *sapi_headers, TSRMLS_D)
{
...
static void accumulate_header_length(sapi_header_struct *sapi_header, uint *total_length, TSRMLS_D)
{
	*total_length += sapi_header->header_len+2;
}


static void concat_header(sapi_header_struct *sapi_header, char **combined_headers_ptr, TSRMLS_D)
{
...
static int sapi_isapi_send_headers(sapi_headers_struct *sapi_headers, TSRMLS_D)
{
	...
	if (SG(sapi_headers).send_default_content_type) {
		sapi_get_default_content_type_header(&default_content_type, TSRMLS_C);
		accumulate_header_length(&default_content_type, (void *) &total_length, TSRMLS_C);
	}
	zend_llist_apply_with_argument(&SG(sapi_headers).headers, (llist_apply_with_arg_func_t) accumulate_header_length, (void *) &total_length, TSRMLS_C);

	/* Generate headers */
	combined_headers = (char *) emalloc(total_length+1);
	combined_headers_ptr = combined_headers;
	if (SG(sapi_headers).send_default_content_type) {
		concat_header(&default_content_type, (void *) &combined_headers_ptr, TSRMLS_C);
		sapi_free_header(&default_content_type); /* we no longer need it */
	}
	zend_llist_apply_with_argument(&SG(sapi_headers).headers, (llist_apply_with_arg_func_t) concat_header, (void *) &combined_headers_ptr, TSRMLS_C);
	*combined_headers_ptr++ = '\r';
	*combined_headers_ptr++ = '\n';
...
static int sapi_isapi_read_post(char *buffer, uint count_bytes, TSRMLS_D)
{
	LPEXTENSION_CONTROL_BLOCK lpECB = (LPEXTENSION_CONTROL_BLOCK) SG(server_context);
...
static void sapi_isapi_register_zeus_ssl_variables(LPEXTENSION_CONTROL_BLOCK lpECB, zval *track_vars_array, TSRMLS_D)
{
	...
	php_register_variable( "SSL_CLIENT_DN", static_cons_buf, track_vars_array, TSRMLS_C );
	...
	php_register_variable( "SSL_CLIENT_I_DN", static_cons_buf, track_vars_array, TSRMLS_C );	
}

static void sapi_isapi_register_zeus_variables(LPEXTENSION_CONTROL_BLOCK lpECB, zval *track_vars_array, TSRMLS_D)
{
	char static_variable_buf[ISAPI_SERVER_VAR_BUF_SIZE];
	DWORD variable_len = ISAPI_SERVER_VAR_BUF_SIZE;
	...
	if ( lpECB->GetServerVariable(lpECB->ConnID, "PATH_INFO", static_variable_buf, &variable_len) && static_variable_buf[0] ) {
		...
		php_register_variable( "PHP_SELF", static_variable_buf, track_vars_array, TSRMLS_C );

		/* Chop off filename to get just the 'real' PATH_INFO' */
		pathinfo_len = variable_len - scriptname_len;
		php_register_variable( "PATH_INFO", static_variable_buf + scriptname_len - 1, track_vars_array, TSRMLS_C );
		...
		if ( strlen(lpECB->lpszQueryString) + variable_len + 1 < ISAPI_SERVER_VAR_BUF_SIZE ) {
			...
			php_register_variable( "URL", static_variable_buf, track_vars_array, TSRMLS_C );
			php_register_variable( "REQUEST_URI", static_variable_buf, track_vars_array, TSRMLS_C );
		}
	}
	...
	if ( lpECB->GetServerVariable(lpECB->ConnID, "PATH_TRANSLATED", static_variable_buf, &variable_len) && static_variable_buf[0] ) {
		...
		php_register_variable( "PATH_TRANSLATED", static_variable_buf, track_vars_array, TSRMLS_C );
	}
	...
	if ( lpECB->GetServerVariable(lpECB->ConnID, "AUTH_USER", static_variable_buf, &variable_len) && static_variable_buf[0] )  {
		php_register_variable( "PHP_AUTH_USER", static_variable_buf, track_vars_array, TSRMLS_C );
	}
	variable_len = ISAPI_SERVER_VAR_BUF_SIZE;
	if ( lpECB->GetServerVariable(lpECB->ConnID, "AUTH_PASSWORD", static_variable_buf, &variable_len) && static_variable_buf[0] )  {
		php_register_variable( "PHP_AUTH_PW", static_variable_buf, track_vars_array, TSRMLS_C );
	}
	variable_len = ISAPI_SERVER_VAR_BUF_SIZE;
	if ( lpECB->GetServerVariable(lpECB->ConnID, "AUTH_TYPE", static_variable_buf, &variable_len) && static_variable_buf[0] )  {
		php_register_variable( "AUTH_TYPE", static_variable_buf, track_vars_array, TSRMLS_C );
	}
	...
	if ( lpECB->GetServerVariable(lpECB->ConnID, "CERT_COOKIE", static_variable_buf, &variable_len) && static_variable_buf[0] ) {
		sapi_isapi_register_zeus_ssl_variables( lpECB, track_vars_array, TSRMLS_C );
	}
	...
	if ( lpECB->GetServerVariable(lpECB->ConnID, "SERVER_SOFTWARE", static_variable_buf, &variable_len) && static_variable_buf[0] )  {
		php_register_variable( "SERVER_SIGNATURE", static_variable_buf, track_vars_array, TSRMLS_C );
	}
}
#else

static void sapi_isapi_register_iis_variables(LPEXTENSION_CONTROL_BLOCK lpECB, zval *track_vars_array, TSRMLS_D)
{
	char static_variable_buf[ISAPI_SERVER_VAR_BUF_SIZE];
	...
	if ( lpECB->GetServerVariable(lpECB->ConnID, "PATH_INFO", static_variable_buf, &variable_len) && static_variable_buf[0] ) {
		...
		/* Chop off filename to get just the 'real' PATH_INFO' */
		php_register_variable( "ORIG_PATH_INFO", static_variable_buf, track_vars_array, TSRMLS_C );
		pathinfo_len = variable_len - scriptname_len;
		strncpy(path_info_buf, static_variable_buf + scriptname_len - 1, sizeof(path_info_buf)-1);
		php_register_variable( "PATH_INFO", path_info_buf, track_vars_array, TSRMLS_C );
		/* append query string to give url... extra byte for '?' */
		if ( strlen(lpECB->lpszQueryString) + variable_len + 1 < ISAPI_SERVER_VAR_BUF_SIZE ) {
			/* append query string only if it is present... */
			if ( strlen(lpECB->lpszQueryString) ) {
		...
		variable_len = ISAPI_SERVER_VAR_BUF_SIZE;
		if ( lpECB->GetServerVariable(lpECB->ConnID, "PATH_TRANSLATED", static_variable_buf, &variable_len) && static_variable_buf[0] ) {
			php_register_variable( "ORIG_PATH_TRANSLATED", static_variable_buf, track_vars_array, TSRMLS_C );
		}
		if (lpECB->ServerSupportFunction(lpECB->ConnID, HSE_REQ_MAP_URL_TO_PATH_EX, path_info_buf, &pathinfo_len, (LPDWORD) &humi)) {
			...
			php_register_variable("PATH_TRANSLATED", humi.lpszPath, track_vars_array, TSRMLS_C);
		}
	}
	...
	if (lpECB->ServerSupportFunction(lpECB->ConnID, HSE_REQ_MAP_URL_TO_PATH_EX, static_variable_buf, &variable_len, (LPDWORD) &humi)) {
		...
		php_register_variable("DOCUMENT_ROOT", humi.lpszPath, track_vars_array, TSRMLS_C);
	}
	...
	if (!SG(request_info).auth_user || !SG(request_info).auth_password || 
		...
		if (lpECB->GetServerVariable(lpECB->ConnID, "HTTP_AUTHORIZATION", static_variable_buf, &variable_len)
			&& static_variable_buf[0]) {
			php_handle_auth_data(static_variable_buf, TSRMLS_C);
		}
	}

	if (SG(request_info).auth_user)  {
		php_register_variable("PHP_AUTH_USER", SG(request_info).auth_user, track_vars_array, TSRMLS_C );
	}
	if (SG(request_info).auth_password) {
		php_register_variable("PHP_AUTH_PW", SG(request_info).auth_password, track_vars_array, TSRMLS_C );
	}
}
#endif

static void sapi_isapi_register_server_variables2(char **server_variables, LPEXTENSION_CONTROL_BLOCK lpECB, zval *track_vars_array, char **recorded_values, TSRMLS_D)
{
	...
	while (*p) {
		...
		if (lpECB->GetServerVariable(lpECB->ConnID, *p, static_variable_buf, &variable_len)
			&& static_variable_buf[0]) {
			php_register_variable(*p, static_variable_buf, track_vars_array, TSRMLS_C);
			if (recorded_values) {
			...
			if (lpECB->GetServerVariable(lpECB->ConnID, *p, variable_buf, &variable_len)
				&& variable_buf[0]) {
				php_register_variable(*p, variable_buf, track_vars_array, TSRMLS_C);
			}
			if (recorded_values) {
			...
			php_register_variable(*p, "", track_vars_array, TSRMLS_C);
		}
		p++;
	}
}
...
static void sapi_isapi_register_server_variables(zval *track_vars_array, TSRMLS_D)
{
	DWORD variable_len = ISAPI_SERVER_VAR_BUF_SIZE;
	char *variable;
	char *strtok_buf = NULL;
	...
	sapi_isapi_register_server_variables2(isapi_special_server_variable_names, lpECB, track_vars_array, isapi_special_server_variables, TSRMLS_C);
	if (SG(request_info).cookie_data) {
		php_register_variable("HTTP_COOKIE", SG(request_info).cookie_data, track_vars_array, TSRMLS_C);
	}

	/* Register the standard ISAPI variables */
	sapi_isapi_register_server_variables2(isapi_server_variable_names, lpECB, track_vars_array, NULL, TSRMLS_C);
	...
	if (isapi_special_server_variables[SPECIAL_VAR_HTTPS]
		...
		sapi_isapi_register_server_variables2(isapi_secure_server_variable_names, lpECB, track_vars_array, NULL, TSRMLS_C);
	}
...
#ifdef WITH_ZEUS
	sapi_isapi_register_zeus_variables(lpECB, track_vars_array, TSRMLS_C);
#else
	sapi_isapi_register_iis_variables(lpECB, track_vars_array, TSRMLS_C);
#endif

	/* PHP_SELF support */
	if (isapi_special_server_variables[SPECIAL_VAR_PHP_SELF]) {
		php_register_variable("PHP_SELF", isapi_special_server_variables[SPECIAL_VAR_PHP_SELF], track_vars_array, TSRMLS_C);
	...
	if (isapi_special_server_variables[SPECIAL_VAR_ALL_HTTP]) {
		...
		while (variable) {
			...
			if (colon) {
				...
				*colon = 0;
				php_register_variable(variable, value, track_vars_array, TSRMLS_C);
				*colon = ':';
			}
...
static void init_request_info(LPEXTENSION_CONTROL_BLOCK lpECB, TSRMLS_D)
{
	DWORD variable_len = ISAPI_SERVER_VAR_BUF_SIZE;
	char static_variable_buf[ISAPI_SERVER_VAR_BUF_SIZE];
#ifndef WITH_ZEUS
...
static void php_isapi_report_exception(char *message, int message_len, TSRMLS_D)
{
	if (!SG(headers_sent)) {
		HSE_SEND_HEADER_EX_INFO header_info;
		LPEXTENSION_CONTROL_BLOCK lpECB = (LPEXTENSION_CONTROL_BLOCK) SG(server_context);
	...
	sapi_isapi_ub_write(message, message_len, TSRMLS_C);
}
...
DWORD WINAPI HttpExtensionProc(LPEXTENSION_CONTROL_BLOCK lpECB)
{
	...
	zend_first_try {
		...
		__try {
#endif
			init_request_info(lpECB, TSRMLS_C);
			...
			if (file_handle.filename)
				retval = php_fopen_primary_script(&file_handle, TSRMLS_C);
			...
			if (_exception_code()==EXCEPTION_STACK_OVERFLOW) {
				...
				php_isapi_report_exception(buf, strlen(buf), TSRMLS_C);
