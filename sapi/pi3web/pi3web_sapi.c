...
static int zend_pi3web_ub_write(const char *str, uint str_length, TSRMLS_D)
{
...
static int sapi_pi3web_header_handler(sapi_header_struct *sapi_header, sapi_headers_struct *sapi_headers, TSRMLS_D)
{
...
static int sapi_pi3web_send_headers(sapi_headers_struct *sapi_headers, TSRMLS_D)
{
...
static int sapi_pi3web_read_post(char *buffer, uint count_bytes, TSRMLS_D)
{
	LPCONTROL_BLOCK lpCB = (LPCONTROL_BLOCK) SG(server_context);
...
static void init_request_info(LPCONTROL_BLOCK lpCB, TSRMLS_D)
{
...
static void sapi_pi3web_register_variables(zval *track_vars_array, TSRMLS_D)
{
	...
	{	
		...
		if (lpCB->GetServerVariable(lpCB->ConnID, pKey, static_variable_buf, &variable_len)
			&& (variable_len > 1)) {
			php_register_variable(pKey, static_variable_buf, track_vars_array, TSRMLS_C);
		} else if (PIPlatform_getLastError()==PIAPI_EINVAL) {
			variable_buf = (char *) emalloc(variable_len);
			if (lpCB->GetServerVariable(lpCB->ConnID, pKey, variable_buf, &variable_len)) {
				php_register_variable(pKey, variable_buf, track_vars_array, TSRMLS_C);
			}
			efree(variable_buf);
		}
...
MODULE_API DWORD PHP5_wrapper(LPCONTROL_BLOCK lpCB)
{
	...
	zend_first_try {
		...
		init_request_info(lpCB, TSRMLS_C);
		php_request_startup(TSRMLS_C);

		switch ( lpCB->dwBehavior ) {
			case PHP_MODE_STANDARD:
				iRet = ( php_execute_script( &file_handle, TSRMLS_C ) ) ?
			...
			case PHP_MODE_INDENT: {
				...
				break;
			case PHP_MODE_LINT:
				iRet = (php_lint_script(&file_handle, TSRMLS_C) == SUCCESS) ?
					PIAPI_COMPLETED : PIAPI_ERROR;
				break;
			default:
				iRet = PIAPI_ERROR;;
